    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div 
          ...
          className="fixed inset-0 z-[1000] bg-[#050505] flex flex-col font-sans overflow-hidden text-[var(--text)] outline-none"
        >
          {/* ── HEADER OVERLAY ── */}
          <div className="absolute top-0 w-full p-6 flex justify-between items-start bg-gradient-to-b from-black/80 to-transparent z-50 pointer-events-none">
            <div className="flex flex-col gap-1 pointer-events-auto outline-none">
            ...
            <div className="flex gap-3 pointer-events-auto outline-none">
              <button onClick={() => setShowDashboard(!showDashboard)} className={cn("p-3 rounded-2xl border transition-all outline-none [-webkit-tap-highlight-color:transparent]", showDashboard ? "bg-white text-black border-white" : "bg-black/50 border-white/10 text-white hover:bg-white/10 backdrop-blur-md")}>
                <LayoutDashboard size={18} />
              </button>
              <button onClick={onClose} className="p-3 bg-black/50 border border-white/10 rounded-2xl text-white hover:bg-white/10 backdrop-blur-md transition-all outline-none [-webkit-tap-highlight-color:transparent]">
                <Maximize2 size={18} />
              </button>
            </div>
          </div>
          ...
          {/* ── MAIN VIDEO GRID ── */}
          <div className="flex-1 relative w-full h-full p-4 md:p-8 pt-24 pb-32 z-10 flex items-center justify-center outline-none">
            <div className={cn("w-full h-full max-w-6xl gap-4 grid transition-all duration-500", gridClass)}>
              {activeParticipants.map((p, idx) => (
                <div key={p.id} className="relative w-full h-full bg-[#111] rounded-[32px] border border-white/5 overflow-hidden shadow-2xl group outline-none">
          ...
          <AnimatePresence>
            {showDashboard && (
              <motion.div initial={{ x: '100%', opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: '100%', opacity: 0 }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="absolute top-24 right-6 w-80 bottom-32 bg-black/80 backdrop-blur-2xl border border-white/10 rounded-[32px] p-6 z-40 flex flex-col gap-6 shadow-2xl outline-none">
                ...
                <div className="flex-1 flex flex-col gap-4">
                  <div className="p-4 bg-white/5 rounded-2xl border border-white/5"><div className="text-[10px] text-white/40 font-mono mb-1">NETWORK LATENCY</div><div className="text-xl font-bold font-mono">12ms <span className="text-white text-sm">●</span></div></div>
          ...
          <AnimatePresence>
            {showInviteMenu && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="absolute bottom-36 left-1/2 -translate-x-1/2 bg-[#111] border border-white/10 rounded-[28px] p-2 flex gap-2 z-50 shadow-2xl backdrop-blur-xl outline-none">
                <button onClick={() => { setShowInviteMenu(false); }} className="flex items-center gap-3 px-5 py-3.5 hover:bg-white/10 rounded-[20px] transition-colors outline-none [-webkit-tap-highlight-color:transparent]">
          ...
          {/* ── BOTTOM CONTROLS ── */}
          <div className="absolute bottom-10 w-full flex justify-center z-50 outline-none">
            <div className="flex items-center gap-3 p-3 bg-black/60 backdrop-blur-3xl rounded-[40px] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
              ...
              <div className="relative">
                <button onClick={() => setShowInviteMenu(!showInviteMenu)} className={cn("w-14 h-14 rounded-full flex items-center justify-center transition-all outline-none [-webkit-tap-highlight-color:transparent]", showInviteMenu ? "bg-white text-black" : "bg-white/5 text-white hover:bg-white/15")}>
              ...
              <button onClick={() => setIsMicOn(!isMicOn)} className={cn("w-14 h-14 rounded-full flex items-center justify-center transition-all outline-none [-webkit-tap-highlight-color:transparent]", isMicOn ? "bg-white/10 text-white hover:bg-white/20" : "bg-white text-black")}>
                {isMicOn ? <Mic size={22} /> : <MicOff size={22} />}
              </button>

              <button onClick={() => setIsVideoOn(!isVideoOn)} className={cn("w-14 h-14 rounded-full flex items-center justify-center transition-all outline-none [-webkit-tap-highlight-color:transparent]", isVideoOn ? "bg-white/10 text-white hover:bg-white/20" : "bg-white text-black")}>
                {isVideoOn ? <Video size={22} /> : <VideoOff size={22} />}
              </button>

              <button className="w-14 h-14 rounded-full bg-white/5 text-white hover:bg-white/15 transition-all flex items-center justify-center outline-none [-webkit-tap-highlight-color:transparent]">
                <MessageSquare size={22} />
              </button>

              <div className="w-[1px] h-8 bg-white/10 mx-1" />

              <button onClick={onClose} className="w-16 h-16 rounded-[28px] bg-white text-black flex items-center justify-center shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:scale-105 active:scale-95 transition-all outline-none [-webkit-tap-highlight-color:transparent]">
