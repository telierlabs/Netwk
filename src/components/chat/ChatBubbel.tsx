  ...
  return (
    <>
      <div className={cn("w-full mb-2 mt-1 flex", isUser ? "justify-end" : "justify-start")}>
        <div className="w-[85vw] sm:w-[500px] max-w-full flex flex-col gap-1.5">
          {/* 1 GAMBAR */}
          {images.length === 1 && (
...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({ msg, msgIndex, isLast, onResend, onEdit, onSuggest, onTogglePin, onSaveItem, onRegenerate, onSwipeToReply, suggestions }) => {
  ...
  const openUserMenu = (e: React.MouseEvent) => {
    ...
    if (bubbleRef.current) {
        ...
        // Fix pop up berantakan. Sekarang tetep nempel di bawah user chat
  ...
  return (
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start")}>
      ...
      {/* RENDER PDF/FILE DOKUMEN JIKA ADA */}
        ...
        <div ref={bubbleRef} className={cn("msg-bubble rounded-[22px] transform-gpu relative z-10", isUser ? "bg-[var(--sf)] border border-[var(--bd)] text-[var(--text)] rounded-tr-[6px] max-w-[85%] shadow-sm" : "bg-transparent text-[var(--text)] px-0 w-full")}>
          {isUser ? (
            <>
              ...
              <AnimatePresence>
                {showUserMenu && (
                  <motion.div onClick={(e) => e.stopPropagation()} initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -10 }} transition={{ duration: 0.15 }} className="fixed z-[99999] w-[160px] bg-[var(--bg)] backdrop-blur-xl border border-[var(--bd)] rounded-[16px] shadow-xl flex flex-col p-1.5" style={{ top: userMenuPos.top, right: userMenuPos.right }}>
