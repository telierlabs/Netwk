...
// transition dipakai bareng di semua elemen yang ikut animasi pil <-> kotak,
// biar semuanya gerak kompak dalam durasi yang sama (gak ada yang lebih
// cepat/lambat dari yang lain dan bikin keliatan patah-patah)
const LAYOUT_TRANSITION = { duration: 0.22, ease: [0.4, 0, 0.2, 1] as const };
        ...
        <div className="max-w-4xl mx-auto w-full">
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              ...
              {/* FIX (animasi mulus pill->box): pakai `layout` dari Framer Motion,
                  BUKAN CSS transition di height. Framer Motion nge-animate
                  pakai transform (teknik FLIP) di frame abis render — gak
                  pernah nyentuh properti `height` asli, jadi gak akan
                  rebutan lagi sama logic scrollHeight di useLayoutEffect
                  (itu tetep instan seperti biasa). Efeknya: box keliatan
                  ngalir mulus dari pill ke box, cepet, tanpa bug oscillasi
                  atau macet yang kemarin muncul waktu dicoba pakai
                  transition-[height] manual. */}
              {/* FIX (glassmorphic bersih): shadow luar `shadow-[0_4px_24px_
                  rgba(0,0,0,0.25)]` dihapus. Sebelumnya box-shadow gelap ini
                  nyembul keluar radius pil ke background, jadi kelihatan
                  ada "bayangan" abu-abu/item di sekeliling pil walaupun
                  fill-nya sendiri sudah transparan — merusak efek glassmor.
                  Border tipis + backdrop-blur tetap dipertahankan biar efek
                  kaca-nya jalan tanpa bayangan gelap di luar bentuknya. */}
              <motion.div
                layout
                transition={LAYOUT_TRANSITION}
                className="rounded-[26px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.06] border border-[var(--text)]/10 backdrop-blur-md"
              ...
              >
                ...
                {/*
                  FIX (layout dari pill -> box beneran mulus): sebelumnya cuma
                  motion.div PALING LUAR (yang diatas) yang punya `layout`.
                  Waktu `isMultiLine` berubah, class Tailwind di bawah
                  (order-1/order-2/order-3, flex-wrap) berubah instan lewat
                  CSS biasa — Framer Motion cuma tau container luarnya
                  resize, tapi gak tau kalau 3 anak di dalamnya baru aja
                  pindah posisi (paperclip pindah baris, textarea jadi full
                  width, tombol kirim pindah ke pojok kanan bawah). Makanya
                  keliatan "loncat" instan padahal container-nya animasi
                  mulus — pill jadi kotak dengan isi yang nyentak.
                  Sekarang baris ini + 3 anak langsungnya (paperclip,
                  textarea wrapper, tombol aksi) semua dikasih `layout`
                  dengan transition yang SAMA (LAYOUT_TRANSITION), jadi
                  Framer Motion nge-FLIP posisi ketiganya bareng dengan si
                  container — paperclip diam di kiri-bawah, textarea naik
                  ke atasnya, semua dalam satu gerakan yang kompak.
                */}
                <motion.div
                  layout
                  transition={LAYOUT_TRANSITION}
                ...
                >
                  {/* paperclip — layout supaya perpindahannya (row awal -> baris bawah) dianimasikan, bukan loncat */}
                  <motion.button
                    layout
                    transition={LAYOUT_TRANSITION}
                  ...
                  >
                    {/* FIX (teks "meleleh"/naik-turun pas pil <-> kotak): textarea ini
                        tadinya elemen <textarea> polos di dalam motion.div yang punya
                        `layout`. Framer Motion menganimasikan perubahan ukuran pakai
                        scale transform (gepeng/regang tiap frame), dan itu otomatis
                        DIKOREKSI untuk elemen yang juga motion-tracked — tapi textarea
                        polos bukan motion component, jadi dia cuma ikut ke-scale tanpa
                        koreksi. Efeknya: isi teks kelihatan diregangkan/ditarik naik-
                        turun selama transisi (bukan animasi asli, cuma distorsi visual),
                        dan karena box-nya "goyang" tiap frame, cursor handle bawaan
                        Android juga ikut salah posisi (itu titik/garis aneh yang muncul
                        di screenshot). Solusinya: jadikan textarea ini `motion.textarea`
                        dengan `layout` + transition yang SAMA seperti parent-nya, supaya
                        Framer Motion ikut mengoreksi elemen ini juga — hasilnya teks
                        tetap presisi/tidak berubah bentuk sepanjang animasi. */}
                    <motion.textarea
                      layout
                      transition={LAYOUT_TRANSITION}
                      ref={textareaRef}
                  ...
                  {/* clear (kalau ada teks) / mic+volume (kalau kosong) / kirim — layout supaya pindah ke pojok kanan-bawah mulus */}
                  <motion.div
                    layout
                    transition={LAYOUT_TRANSITION}
