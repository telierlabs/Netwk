...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    // FIX (anti-flicker pil<->kotak): sebelumnya ini pakai satu ambang batas
    // tunggal (`scrollHeight > baseline + 4`) buat DUA arah — masuk mode
    // stacked DAN balik ke pill. Kalau tinggi teks pas-pasan di sekitar
    // ambang itu (misal karena font-rendering/sub-pixel beda dikit antar
    // render), `isMultiLine` bisa gonta-ganti true/false di render yang
    // beruntun — itu yang keliatan sebagai "pil jadi kotak" yang nyentak
    // tiba-tiba pas ngetik.
    // Sekarang pakai HYSTERESIS: ambang buat *masuk* mode stacked lebih
    // tinggi dari ambang buat *keluar* dari situ. Jadi begitu sudah di
    // mode kotak, dia gak gampang balik ke pill kecuali tingginya beneran
    // udah turun jelas (mis. user hapus teks). Ini juga collapse ke satu
    // sumber kebenaran (state React), jadi transisinya bisa dianimasikan
    // dengan mulus lewat prop `layout` di elemen-elemen di bawah.
    const baseline = baselineHeight.current;
    if (baseline !== null) {
      const enterStackedAt = baseline + 4;
      const exitStackedAt = baseline + 1;
      setIsMultiLine(prev => {
        if (!prev && el.scrollHeight > enterStackedAt) return true;
        if (prev && el.scrollHeight <= exitStackedAt) return false;
        return prev;
      });
    }
  }, [inputText]);
          ...
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              ...
              {/* FIX (buang animasi pil<->kotak sepenuhnya): dulu di sini + 5
                  ...
                  yang kelihatan sebagai "meleleh"/naik-turun. Sudah dicoba
                  dikoreksi (motion.textarea + layout) tapi masih kerasa
                  goyang di device asli. Daripada terus tambal-sulam animasi
                  yang rewel, animasinya DIBUANG SELURUHNYA: sekarang div
                  biasa (gak ada `layout`/`transition`), jadi pas
                  `isMultiLine` berubah, ukuran & posisi elemen di dalamnya
                  cuma pindah INSTAN lewat class Tailwind biasa — gak ada
                  distorsi, gak ada lompat aneh, karena gak ada animasi sama
                  sekali yang bisa mendistorsi. */}
              {/* FIX (glassmor ikut tema): sebelumnya fill pill pakai
                  `bg-[var(--text)]/[0.06]` — tint dari warna TEKS (bukan
                  warna latar halaman), yang efeknya keliatan abu-abu netral
                  di kedua tema, ga jelas nurut putih (light) atau hitam
                  (dark). Diganti ke `bg-[var(--bg)]` (warna latar HALAMAN
                  itu sendiri) dengan opacity lebih tinggi + tetap
                  backdrop-blur — sama pola-nya kayak AttachPopup yang udah
                  bener duluan (`bg-[var(--bg)]/90`). Hasilnya: di light
                  theme pill jadi kaca putih, di dark theme jadi kaca hitam,
                  ngikutin tema halaman chat yang aktif. */}
              <div
              ...
              >
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 items-center" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => {
                        if (file.type === 'pdf' || file.type === 'text') {
                          ...
                          // FIX (warna chip): sebelumnya `bg-[var(--text)]/7 border
                          // border-[var(--text)]/12` — abu-abu glassmor dengan garis
                          // tebal di pinggiran, gak ikut warna semantik file-nya sama
                          // sekali. Border dihapus total, dan background sekarang
                          // pakai tint warna sesuai tipe file (merah tipis buat PDF,
                          // biru tipis buat dokumen teks) — jadi keliatan lebih jelas
                          // ini jenis file apa, tanpa garis pinggiran yang mengganggu.
                          return (
                        ...
                        // FIX: sama, border abu dihapus dari chip gambar/video —
                        // shadow-sm tetap dipertahankan biar thumbnail tetap
                        // kelihatan "ngambang" dikit, tanpa garis pinggiran tebal.
                        return (
                ...
                {/*
                  FIX UTAMA (akar masalah sebenernya): sebelumnya pas
                  `isMultiLine` true, wrapper textarea berubah dari `flex-1`
                  (berbagi lebar row sama paperclip+tombol → SEMPIT) jadi
                  `w-full` (lebar PENUH, karena paperclip+tombol pindah ke
                  baris bawah). Artinya LEBAR textarea berubah drastis
                  (~100px+) setiap kali mode ganti. Karena jumlah baris teks
                  itu tergantung lebar (teks yang wrap 2 baris di lebar
                  sempit bisa muat 1 baris di lebar penuh), ini bikin LOOP:
                  teks wrap 2 baris di mode pil (sempit) → masuk mode kotak
                  → lebar jadi penuh → teks yang SAMA sekarang muat 1 baris
                  → keluar lagi dari mode kotak → lebar balik sempit → wrap
                  2 baris lagi → masuk mode kotak lagi → begitu terus tanpa
                  henti selama panjang teks pas-pasan di ambang itu. Ini
                  gak bisa ditambal cuma soal padding atau animasi — akarnya
                  di LEBAR yang ikut goyang tiap ganti mode.

                  FIX: paperclip & tombol aksi TIDAK LAGI ikut berbagi lebar
                  row sama textarea. Keduanya sekarang `absolute`, nempel di
                  pojok kiri-bawah & kanan-bawah, lepas dari flex layout.
                  Textarea SELALU `w-full` dengan padding kiri/kanan yang
                  TETAP SAMA — gak pernah berubah ikut `isMultiLine`. Karena
                  lebarnya sekarang konstan, jumlah baris cuma bisa berubah
                  kalau ISI TEKS-nya beneran berubah, bukan gara-gara layout
                  ikut geser. Teks juga gak akan pernah kegambar di bawah
                  ikon: padding kiri/kanan itu ngecualiin kolom horizontal
                  tempat ikon berada di SEMUA baris, bukan cuma baris
                  pertama, jadi ikon di pojok bawah gak pernah numpuk sama
                  teks walau kotaknya udah tinggi banyak baris.
                */}
                <div className="relative pt-1 pb-1">
                  {/* paperclip — pinned pojok kiri-bawah */}
                  <button
                    onClick={() => !isFull && setShowAttachPopup(p => !p)}
                    className={cn(
                      "absolute left-2 bottom-1 w-10 h-10 rounded-full flex items-center justify-center transition-colors z-10 [-webkit-tap-highlight-color:transparent]",
                  ...
                  {/* textarea — selalu full width, padding tetap konstan gak peduli mode */}
                  <textarea
                    ref={textareaRef}
                    value={inputText}
                    ...
                    rows={1}
                    className="w-full bg-transparent border-none outline-none resize-none break-words text-[15.5px] leading-relaxed placeholder:text-[var(--mu)] text-[var(--text)] min-h-[24px] max-h-40 relative z-0 focus:ring-0 focus:outline-none py-[9px] pl-[52px] pr-[100px]"
                    style={{ outline: 'none', border: 'none', boxShadow: 'none', WebkitAppearance: 'none' } as any}
                  />

                  {/* tombol maximize — cuma toggle tampil/hilang, gak ngubah lebar apapun */}
                  <AnimatePresence>
                    {isMultiLine && (
                      <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} onClick={() => setIsFullscreenEditor(true)} className="absolute top-1 right-1 w-7 h-7 flex items-center justify-center bg-[#141414]/70 backdrop-blur-sm border border-[var(--text)]/12 rounded-full text-[var(--text)] shadow-sm active:scale-95 z-10 [-webkit-tap-highlight-color:transparent]"><Maximize2 size={14} strokeWidth={2.5} /></motion.button>
                    )}
                  </AnimatePresence>

                  {/* clear (kalau ada teks) / mic+volume (kalau kosong) / kirim — pinned pojok kanan-bawah */}
                  <div className="absolute right-2 bottom-1 flex items-center gap-1.5 z-10">
                    {hasContent ? (
                      <>
