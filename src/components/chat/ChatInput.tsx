...
export type ChatMode = 'auto' | 'spark' | 'think';
...
const MODES: { id: ChatMode; label: string; desc: string; Icon: React.FC }[] = [
  ...
  { id: 'spark', label: 'Spark', desc: 'Respons cepat & ringan',           Icon: IconSpark },
  { id: 'think', label: 'Think', desc: 'Analisis mendalam & teliti',       Icon: IconThink },
];

// ── Mode Selector Popup (Desain Grok Premium) ──
const ModeSelectorPopup: React.FC<{
  ...
  above?: boolean; 
  align?: 'left' | 'right'; 
}> = ({ current, onSelect, onClose, above = true, align = 'left' }) => (
  <>
    {/* backdrop */}
    <div className="fixed inset-0 z-[110]" onClick={onClose} />

    <div
      className="absolute z-[120] bg-[var(--bg)] rounded-[28px] p-2 flex flex-col gap-1"
      style={{
        width: 270, // Lebih proporsional
        ...(align === 'right' ? { right: 0 } : { left: 0 }),
        ...(above ? { bottom: 'calc(100% + 14px)' } : { top: 'calc(100% + 14px)' }),
        border: '1px solid var(--bd)',
        boxShadow: '0 12px 48px -12px rgba(0,0,0,0.25)', // Shadow lebih soft dan elegan
      }}
    >
      {MODES.map((m) => {
        ...
        return (
          <button
            ...
            className="w-full flex items-center gap-4 px-3 py-3 rounded-[20px] transition-all text-left"
            style={{
          ...
          >
            {/* Icon - Tidak pakai kotak background lagi, langsung icon clean */}
            <div
              className="flex-shrink-0 flex items-center justify-center"
              style={{ color: active ? 'var(--text)' : 'var(--mu)' }}
            >
              <m.Icon />
            </div>

            {/* Text layout dirapihkan */}
            <div className="flex-1 min-w-0">
              <div
                className="text-[15px] font-bold leading-tight mb-0.5"
...
// ── Mode Button (pill di dalam input) ──
const ModeButton: React.FC<{
  mode: ChatMode;
  onClick: () => void;
  size?: 'sm' | 'md';
}> = ({ mode, onClick, size = 'md' }) => {
  ...
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1.5 rounded-full font-semibold transition-all hover:opacity-80 relative z-[105]"
      style={{
        ...
        <div style={{ width: '100%', position: 'relative' }}>
          ...
          >
            <button
              ...
              className={cn('flex-shrink-0 transition-all relative z-[105]', isListening ? 'text-red-400' : 'text-[var(--mu)] hover:text-[var(--text)]')}
            >
              <Mic size={17} />
            </button>
            ...
            <div className="flex items-center gap-2 flex-shrink-0" style={{ position: 'relative' }}>
              ...
              <button
                ...
                className="text-[var(--mu)] hover:text-[var(--text)] transition-colors flex-shrink-0 relative z-[105]"
              >
              ...
              {hasContent && !isSending && (
                <button
                  onClick={handleSend}
                  className="w-8 h-8 rounded-full flex items-center justify-center bg-[var(--text)] text-[var(--bg)] transition-all hover:opacity-85 flex-shrink-0 relative z-[105]"
                >
                  <svg width="15" height="15" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.3}>
          ...
          {/* Picker Desain Premium Grok */}
          {showPicker && (
            <div
              className="absolute bottom-full mb-3 right-0 bg-[var(--bg)] border border-[var(--bd)] rounded-[28px] p-2 flex gap-1 z-[100]"
              style={{ boxShadow: '0 12px 48px -12px rgba(0,0,0,0.25)' }}
            >
              {[
                { label: 'Kamera', ref: cameraInputRef, icon: <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}><path strokeLinecap="round" d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg> },
                { label: 'Galeri',  ref: galleryInputRef, icon: <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path strokeLinecap="round" d="M21 15l-5-5L5 21"/></svg> },
                { label: 'PDF',    ref: pdfInputRef,     icon: <FileText size={20} color="currentColor" /> },
              ].map((item, i) => (
                <button key={i}
                  onClick={() => { setShowPicker(false); !isFull && item.ref.current?.click(); }}
                  className="flex flex-col items-center gap-1.5 px-4 py-3 rounded-[20px] hover:bg-[var(--sf)] transition-colors text-[var(--text)]"
                >
                  <div className="w-10 h-10 rounded-2xl flex items-center justify-center">{item.icon}</div>
  ...
  return (
    <>
      ...
      <footer className="px-4 pb-4 pt-2 bg-[var(--bg)] relative">
        ...
        {/* Picker Normal Mode Premium Grok */}
        {showPicker && (
          <div className="absolute bottom-full mb-3 left-4 bg-[var(--bg)] border border-[var(--bd)] rounded-[28px] p-2 flex gap-1 z-[100]"
            style={{ boxShadow: '0 12px 48px -12px rgba(0,0,0,0.25)' }}>
            {[
              { label: 'Kamera', ref: cameraInputRef, icon: <svg width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}><path strokeLinecap="round" d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg> },
              { label: 'Galeri',  ref: galleryInputRef, icon: <svg width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path strokeLinecap="round" d="M21 15l-5-5L5 21"/></svg> },
              { label: 'PDF',    ref: pdfInputRef,     icon: <FileText size={24} color="currentColor" /> },
              { label: 'File',   ref: fileInputRef,    icon: <svg width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}><path strokeLinecap="round" d="M13 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V9z"/><path strokeLinecap="round" d="M13 2v7h7"/></svg> },
            ].map((item, i) => (
              <button key={i} onClick={() => { setShowPicker(false); !isFull && item.ref.current?.click(); }}
                className="flex flex-col items-center gap-2 px-5 py-3.5 rounded-[20px] hover:bg-[var(--sf)] transition-colors text-[var(--text)]">
                ...
                <div className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
                  {attachedFiles.map(file => (
                    <div key={file.id} className="relative flex-shrink-0 rounded-[20px] overflow-hidden bg-[var(--sf)]" style={{ width: 80, height: 80 }}>
                      ...
                      <button onClick={() => removeFile(file.id)} className="absolute top-2 right-2 w-6 h-6 bg-black/60 text-white rounded-full flex items-center justify-center hover:scale-110 transition-transform">
                        <X size={12} />
                      </button>
                    </div>
                  ))}
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    showPicker ? "bg-[var(--sf)] text-[var(--text)]" :
                    isFull ? "opacity-20 cursor-not-allowed text-[var(--text)]" :
                    "text-[var(--text)] opacity-60 hover:opacity-100 hover:bg-[var(--sf)]"
                  )}
                >
                  <svg width="22" height="22" viewBox="0 0 20 20" fill="none">
                    <path d="M1 5.5L10 1L19 5.5L10 10L1 5.5Z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/>
                    <path d="M1 10L10 14.5L19 10" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" opacity="0.5"/>
                    <path d="M1 14.5L10 19L19 14.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" opacity="0.2"/>
                  </svg>
                </button>

                {/* Mode selector — normal mode */}
                <div style={{ position: 'relative' }}>
              ...
              <div className="flex items-center gap-2 relative z-[105]">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    hasContent && !isSending ? "bg-[var(--text)] text-[var(--bg)] scale-105 shadow-md" : "text-[var(--text)] opacity-30 cursor-not-allowed"
                  )}
                >
