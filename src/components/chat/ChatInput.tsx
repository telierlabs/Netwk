...
interface ChatInputProps {
  ...
  onOpenCanvas?: () => void;
  onCloseCanvas?: () => void; // 🔥 INI PENTING
...
// ── SVG Icons ──
const IconCylen = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>);
const IconAuto = () => (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 0 0-2.91-.09z"/><path d="m12 15 3.38-3.38a2.25 2.25 0 0 0 0-3.18l-2.82-2.82a2.25 2.25 0 0 0-3.18 0L6 9.01"/><path d="m9 15 6-6"/><path d="M15 3.01C18.31 3.01 21 5.7 21 9c0 3.31-2.69 6-6 6l-6 6-3-3 6-6c0-3.31 2.69-6 6-6z"/></svg>);
const IconSpark = () => (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>);
const IconThink = () => (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.9 1.2 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg>);
const IconCheck = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" strokeWidth="1.5"/><path d="M8 12l3 3 5-5" strokeWidth="2.5"/></svg>);
...
const ModeSelectorPopup: React.FC<{
  ...
  <>
    ...
    <div
      className={cn("absolute z-[120] bg-[var(--bg)] rounded-[28px] p-1.5 flex flex-col", overrideClass ? overrideClass : (align === 'right' ? "right-0" : "left-0"))}
      style={overrideStyle ? overrideStyle : { width: 250, ...(above ? { bottom: 'calc(100% + 12px)' } : { top: 'calc(100% + 12px)' }), border: '1px solid var(--bd)', boxShadow: '0 12px 40px -12px rgba(0,0,0,0.25)' }}
    >
      <div className="px-3.5 pt-3 pb-3 mb-1 flex items-center justify-between border-b border-[var(--bd)] border-opacity-60">
        <div className="flex flex-col">
          <div className="flex items-center gap-1.5 mb-0.5"><IconCylen /><span className="text-[15px] font-bold text-[var(--text)] tracking-tight">Cylen</span></div>
          <span className="text-[11px] text-[var(--mu)] font-medium leading-tight">Akses intelijen premium</span>
        </div>
        <button className="bg-[var(--text)] text-[var(--bg)] px-3.5 py-1.5 rounded-full text-[11px] font-bold hover:scale-105 transition-transform">Upgrade</button>
      </div>

      <div className="flex flex-col gap-0.5 pt-0.5">
        <button
          onClick={() => { if (onOpenCanvas) onOpenCanvas(); onClose(); }}
          className="w-full flex items-center gap-3.5 px-3.5 py-2.5 rounded-[20px] transition-all text-left mb-1 hover:bg-[var(--sf)] group"
          style={{ background: isCanvasActive ? 'var(--sf)' : 'transparent' }}
        >
          <div className="flex-shrink-0 mt-0.5 text-[var(--ac)] opacity-90 group-hover:opacity-100 group-hover:scale-110 transition-all"><LayoutDashboard size={20} strokeWidth={2.2} /></div>
          <div className="flex-1 min-w-0">
            <div className="text-[14px] font-bold leading-tight mb-0.5 text-[var(--text)]">Cylen Canvas</div>
            <div className="text-[12px] leading-tight text-[var(--mu)]">Infinite spatial workspace</div>
          </div>
          {isCanvasActive ? <div className="flex-shrink-0 mt-0.5 text-[var(--text)]"><IconCheck /></div> : <ChevronRight size={14} className="text-[var(--mu)] opacity-50 group-hover:opacity-100 transition-opacity" />}
        </button>
        
        <div className="mx-3 my-0.5 h-[1px] bg-[var(--bd)] opacity-40"></div>
        ...
        {MODES.map((m) => {
          ...
          return (
            <button
              ...
              onClick={() => { 
                onSelect(m.id); 
                if (onCloseCanvas) onCloseCanvas(); // 🔥 MATIIN CANVAS PAS MILIH MODE NORMAL!
                onClose(); 
              }}
            ...
            >
              <div className="flex-shrink-0 mt-0.5" style={{ color: active ? 'var(--text)' : 'var(--mu)', opacity: active ? 1 : 0.8 }}><m.Icon /></div>
              <div className="flex-1 min-w-0">
...
const ModeButton: React.FC<{ mode: ChatMode; onClick: () => void; size?: 'sm' | 'md'; isCanvasActive?: boolean; }> = ({ mode, onClick, size = 'md', isCanvasActive }) => {
  ...
  return (
    <button onClick={onClick} className={cn("flex items-center gap-1.5 rounded-full font-bold transition-all hover:opacity-80 relative z-[105]", isCanvasActive ? "bg-[var(--text)] text-[var(--bg)] border-transparent shadow-md" : "bg-[var(--bg)] text-[var(--text)] border-[var(--bd)]")} style={{ padding: size === 'sm' ? '6px 14px' : '8px 16px', fontSize: size === 'sm' ? 13 : 14, borderWidth: isCanvasActive ? 0 : 1 }}>
      <span style={{ width: iconSize, height: iconSize, display: 'flex', alignItems: 'center', justifyItems: 'center' }}><Icon size={iconSize} strokeWidth={2.5} /></span>
      <span>{label}</span><ChevronRight size={iconSize - 2} className="rotate-90 opacity-50" />
    </button>
  );
};
    ...
    <>
      ...
      {compact ? (
        ...
        <footer className="px-4 pb-4 pt-2 bg-[var(--bg)] relative z-50">
          {showModeSelector && (
            <ModeSelectorPopup current={mode} onSelect={m => onModeChange?.(m)} onClose={() => setShowModeSelector(false)} onOpenCanvas={onOpenCanvas} onCloseCanvas={onCloseCanvas} isCanvasActive={isCanvasActive} overrideClass="absolute bottom-full mb-3 left-4" overrideStyle={{ width: 250, border: '1px solid var(--bd)', boxShadow: '0 12px 40px -12px rgba(0,0,0,0.25)' }} />
          )}
