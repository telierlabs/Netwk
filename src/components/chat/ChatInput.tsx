          ...
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              ...
              >
                ...
                {/*
                  FIX (balik ke flex biasa, posisi ikon dibenerin): percobaan
                  sebelumnya pakai `absolute` buat naronya paperclip/tombol
                  di pojok — itu emang ngilangin bug naik-turun, TAPI bikin
                  posisi ikon jadi gak center kayak desain aslinya (teks gak
                  vertically-center bareng ikon di mode pil). Sekarang balik
                  ke FLEX ROW BIASA, cuma bedanya sama versi paling awal:
                  paperclip, wrapper-textarea, dan tombol SEKARANG GAK PUNYA
                  class yang beda-beda tergantung `isMultiLine` lagi (gak
                  ada order-1/2/3, gak ada flex-wrap, gak ada w-full vs
                  flex-1 gonta-ganti). Ketiganya SELALU: paperclip lebar
                  tetap (w-10), textarea `flex-1` (lebar SISA yang gak
                  pernah berubah walau `isMultiLine` ganti), tombol lebar
                  tetap. Karena lebar textarea permanen konstan, teks bakal
                  selalu wrap ke jumlah baris yang sama buat isi yang sama —
                  gak ada lagi loop naik-turun.
                  `items-end` (bukan flex-wrap/order) yang bikin paperclip +
                  tombol nempel ke bagian BAWAH row: pas 1 baris, tinggi
                  textarea (~42px) deket banget sama tinggi ikon (40px), jadi
                  bedanya items-end vs items-center nyaris gak kelihatan —
                  tetep keliatan center kayak desain awal. Begitu teks jadi
                  banyak baris, textarea makin tinggi, dan karena items-end,
                  paperclip+tombol otomatis "diem" di bawah sementara bagian
                  atas kotak yang nambah tinggi ke atas — persis kayak yang
                  diminta dari awal.
                */}
                <div className="flex items-end px-2 pt-1 pb-1">
                  {/* paperclip — lebar tetap, SELALU di baris yang sama, gak pernah pindah */}
                  <button
                    onClick={() => !isFull && setShowAttachPopup(p => !p)}
                    className={cn(
                      "w-10 h-10 rounded-full flex items-center justify-center transition-colors z-10 flex-shrink-0 [-webkit-tap-highlight-color:transparent]",
                  ...
                  {/* textarea — flex-1, lebar SELALU sama gak peduli isMultiLine */}
                  <div className="relative flex-1">
                    <textarea
                      ...
                      rows={1}
                      className="w-full bg-transparent border-none outline-none resize-none break-words text-[15.5px] leading-relaxed placeholder:text-[var(--mu)] text-[var(--text)] min-h-[24px] max-h-40 focus:ring-0 focus:outline-none py-[9px] pl-3 pr-7"
                  ...
                  {/* clear (kalau ada teks) / mic+volume (kalau kosong) / kirim — lebar tetap, SELALU di baris yang sama */}
