...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  // ✅ KUNCI: Pakai pattern LAMA yang terbukti jalan di mobile
  // Satu ref per input, input pakai className="hidden" (Tailwind), trigger via .click()
  ...
  // ✅ Handler untuk tiap picker item — langsung .click() seperti versi lama
  const handlePickCamera  = () => { setShowPicker(false); cameraRef.current?.click();  };
  const handlePickGallery = () => { setShowPicker(false); galleryRef.current?.click(); };
  const handlePickDoc     = () => { setShowPicker(false); docRef.current?.click();     };
  ...
  return (
    <>
      ...
      {/* ✅ PATTERN LAMA: className="hidden" + .click() via ref
          Input langsung di sini, dekat dengan tombol yang trigger,
          BUKAN dipindah jauh ke luar footer atau pakai display:none inline.
          Ini yang terbukti jalan di mobile Chrome Android. */}
      ...
      <footer className={cn("px-2 pb-6 relative z-50 pointer-events-auto w-full", compact ? "pt-0" : "pt-2")}>
        ...
        {showPicker && (
          <div
            ...
            style={{ width: 190 }}
          >
            {/* ✅ onClick langsung — sama persis pattern versi lama */}
            <button
