...
}> = ({ current, onSelect, onClose, onOpenCanvas, onCloseCanvas, isCanvasActive, onUpgradeClick, above = true, align = 'left', overrideClass, overrideStyle }) => (
  <>
    ...
    <div
      ...
      style={overrideStyle ? overrideStyle : { width: 320, ...(above ? { bottom: 'calc(100% + 12px)' } : { top: 'calc(100% + 12px)' }), border: '1px solid var(--bd)', boxShadow: '0 16px 48px -12px rgba(0,0,0,0.3)' }}
    >
      {/* 🔥 HEADER POPUP DIPERBESAR & BERSIH */}
      <div className="px-5 pt-5 pb-5 mb-2 flex items-center justify-between border-b border-[var(--bd)] border-opacity-60">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-3">
            {/* 🔥 IKON MURNI TANPA BACKGROUND */}
            <img 
              src="/IMG_20260427_105231.png" 
              alt="Cylen Logo" 
              className="w-7 h-7 object-contain"
            />
            <span className="text-[20px] font-black text-[var(--text)] tracking-tight leading-none">Cylen</span>
          </div>
          <span className="text-[13px] text-[var(--mu)] font-medium">Akses intelijen premium</span>
        </div>
        {/* 🔥 TOMBOL UPGRADE DIBESARKAN */}
        <button 
          onClick={() => { onClose(); if(onUpgradeClick) onUpgradeClick(); }} 
          className="bg-[var(--text)] text-[var(--bg)] px-5 py-2.5 rounded-full text-[14px] font-bold shadow-md hover:scale-105 active:scale-95 transition-all tracking-wide"
        >
          Upgrade
        </button>
      </div>
      ...
      <div className="flex flex-col gap-1 pt-1 pb-1">
        <button onClick={() => { if (onOpenCanvas) onOpenCanvas(); onClose(); }} className="w-full flex items-center gap-4 px-5 py-3.5 rounded-[24px] transition-all text-left mb-1 hover:bg-[var(--sf)] group" style={{ background: isCanvasActive ? 'var(--sf)' : 'transparent' }}>
          <div className="flex-shrink-0 text-[var(--ac)] opacity-90 group-hover:opacity-100 group-hover:scale-110 transition-all"><LayoutDashboard size={24} strokeWidth={2.2} /></div>
          <div className="flex-1 min-w-0"><div className="text-[16px] font-bold leading-tight mb-1 text-[var(--text)]">Cylen Canvas</div><div className="text-[13px] leading-tight text-[var(--mu)]">Infinite spatial workspace</div></div>
          {isCanvasActive ? <div className="flex-shrink-0 text-[var(--text)]"><IconCheck /></div> : <ChevronRight size={18} className="text-[var(--mu)] opacity-50 group-hover:opacity-100 transition-opacity" />}
        </button>

        <div className="mx-5 my-2 h-[1px] bg-[var(--bd)] opacity-40"></div>
      ...
      {compact ? (
        <div style={{ width: '100%', position: 'relative' }}>
          ...
          {isListening && !showLiveVoice ? (
            <div className="flex items-center justify-between w-full h-[54px] rounded-full px-1.5 relative overflow-hidden bg-[var(--sf)] border border-[var(--bd)] shadow-sm">
        ...
        <footer className="px-4 pb-4 pt-2 bg-[var(--bg)] relative z-50">
          {showModeSelector && <ModeSelectorPopup current={mode} onSelect={m => onModeChange?.(m)} onClose={() => setShowModeSelector(false)} onOpenCanvas={onOpenCanvas} onCloseCanvas={onCloseCanvas} isCanvasActive={isCanvasActive} onUpgradeClick={onUpgradeClick} overrideClass="absolute bottom-full mb-3 left-4" overrideStyle={{ width: 280, border: '1px solid var(--bd)', boxShadow: '0 16px 48px -12px rgba(0,0,0,0.3)' }} />}
          ...
          <div className="max-w-3xl mx-auto">
            {isListening && !showLiveVoice ? (
              <div className="flex items-center justify-between w-full h-[64px] rounded-[32px] px-2 relative overflow-hidden bg-[var(--sf)] border border-[var(--bd)] shadow-sm transition-all duration-300">
                ...
                <div className="relative z-10 flex-1 flex justify-center items-center px-4 overflow-hidden">
                  {voiceText ? (
                    ...
                    <div className="flex items-center text-[var(--mu)] text-[14px] font-medium">
