...
interface ChatInputProps {
  ...
  onOpenCanvas?: () => void;
  isCanvasActive?: boolean; // 🔥 STATUS CANVAS
}
...
const IconSpeakWave = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
    ...
    <line x1="7.5" y1="6" x2="7.5" y2="18"></line>
    <line x1="12" y1="2" x2="12" y2="22"></line>
    <line x1="16.5" y1="6" x2="16.5" y2="18"></line>
...
}> = ({ current, onSelect, onClose, onOpenCanvas, above = true, align = 'left', overrideClass, overrideStyle }) => (
  <>
    ...
    >
      ...
      <div className="flex flex-col gap-0.5 pt-0.5">
        ...
        >
          <div className="flex-shrink-0 mt-0.5 text-[var(--ac)] opacity-90 group-hover:opacity-100 group-hover:scale-110 transition-all">
          ...
          <ChevronRight size={14} className="text-[var(--mu)] opacity-50 group-hover:opacity-100 transition-opacity" />
        </button>
...
// 🔥 LOGIKA UBAH TOMBOL PAS CANVAS AKTIF ADA DI SINI
const ModeButton: React.FC<{ mode: ChatMode; onClick: () => void; size?: 'sm' | 'md'; isCanvasActive?: boolean; }> = ({ mode, onClick, size = 'md', isCanvasActive }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex items-center gap-1.5 rounded-full font-bold transition-all hover:opacity-80 relative z-[105]",
        isCanvasActive ? "bg-[var(--text)] text-[var(--bg)] border-transparent shadow-md" : "bg-[var(--bg)] text-[var(--text)] border-[var(--bd)]"
      )}
      style={{
        padding: size === 'sm' ? '6px 14px' : '8px 16px',
        fontSize: size === 'sm' ? 13 : 14,
        borderWidth: isCanvasActive ? 0 : 1,
      }}
    >
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  compact = false, mode = 'auto', onModeChange, onOpenCanvas, isCanvasActive = false
}) => {
            ...
            <div className="flex gap-2 mb-3 overflow-x-auto px-1" style={{ scrollbarWidth: 'none' }}>
              {attachedFiles.map(file => (
                <div key={file.id} className="relative flex-shrink-0 rounded-xl overflow-hidden bg-[var(--sf)]" style={{ width: 52, height: 52 }}>
                  {file.type === 'pdf' ? (
                    <div className="w-full h-full flex flex-col items-center justify-center bg-red-50 px-1"><FileText size={16} className="text-red-500" /></div>
                  ) : (
          ...
          <div className="flex items-center w-full" style={{ height: 54, borderRadius: 999, border: '1px solid var(--bd)', background: 'var(--sf)', paddingLeft: 20, paddingRight: 8, gap: 12 }}>
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0" style={{ position: 'relative' }}>
              ...
              {hasContent ? (
                ...
                <button onClick={() => setShowLiveVoice(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[var(--text)] text-[var(--bg)] font-bold transition-all hover:scale-105 shadow-md flex-shrink-0 relative z-[105]">
              ...
              <div className="flex items-center justify-between px-3 pb-3 pt-2 relative">
                ...
                <div className="flex items-center gap-1.5 relative z-[105]">
                  ...
                  {hasContent ? (
                    ...
                    <button onClick={() => setShowLiveVoice(true)} className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-[var(--text)] text-[var(--bg)] transition-all hover:scale-105 shadow-md flex-shrink-0">
                      <IconSpeakWave /><span className="text-[15px] font-bold tracking-wide">Speak</span>
                    </button>
                  )}
                </div>
              </div>
