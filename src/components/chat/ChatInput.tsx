          ...
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              <div className="rounded-[32px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.04] border border-gray-500/20 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md" style={{ WebkitTapHighlightColor: 'transparent' }}>
                ...
                {/* 🚀 FIX FINAL: PREVIEW MEGAH & PROPORSIONAL 100% */}
                {attachedFiles.length > 0 && (
                  <div className="px-5 pt-5 pb-2 z-[105]">
                    <div className="flex gap-3 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 group animate-in zoom-in duration-200 inline-block">
                          
                          {file.type === 'pdf' || file.type === 'text' ? (
                            <div className="h-[180px] w-[130px] rounded-[20px] bg-[var(--text)]/5 border border-[var(--text)]/20 flex flex-col items-center justify-center px-4 shadow-sm">
                              <FileText size={36} className="text-[var(--text)]/80 mb-3" />
                              <span className="text-[11px] font-bold truncate w-full text-center text-[var(--text)]/80">{file.name}</span>
                            </div>
                          ) : file.type === 'video' ? (
                            <div className="h-[180px] w-[180px] rounded-[20px] bg-[var(--text)]/5 border border-[var(--text)]/20 flex items-center justify-center cursor-pointer shadow-sm relative overflow-hidden" onClick={() => setLightboxUrl(file.url)}>
                              <video src={file.url} className="absolute inset-0 w-full h-full object-cover opacity-50" />
                              <div className="relative z-10 w-12 h-12 bg-[#0a0a0a]/50 backdrop-blur-md rounded-full flex items-center justify-center">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="#ffffff"><path d="M8 5v14l11-7z"/></svg>
                              </div>
                            </div>
                          ) : (
                            /* 🔥 RAHASIA ANTI KEJEPIT: KASIH HEIGHT FIX, WIDTH AUTO. JADI IMAGE MENYESUAIKAN RATIO ASLI */
                            <img 
                              src={file.url} 
                              alt="" 
                              className="h-[180px] sm:h-[220px] w-auto max-w-[280px] object-cover rounded-[20px] border border-[var(--text)]/20 shadow-sm cursor-pointer hover:opacity-90 transition-opacity" 
                          ...
                          <button 
                            ...
                            className="absolute top-2 right-2 w-8 h-8 bg-[#0a0a0a]/80 hover:bg-[#0a0a0a] text-[#ffffff] rounded-full flex items-center justify-center shadow-lg transition-all backdrop-blur-md border border-[#ffffff]/10 [-webkit-tap-highlight-color:transparent]"
                          >
