  ...
  <>
    ...
    <div
      ...
      style={{
        ...
        boxShadow: '0 12px 48px -12px rgba(0,0,0,0.25)',
      }}
    >
      {MODES.map((m) => {
        ...
        return (
          ...
          >
            <div className="flex-shrink-0 flex items-center justify-center" style={{ color: active ? 'var(--text)' : 'var(--mu)' }}>
              <m.Icon />
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-[15px] font-bold leading-tight mb-0.5" style={{ color: 'var(--text)' }}>{m.label}</div>
              <div className="text-[13px]" style={{ color: 'var(--mu)' }}>{m.desc}</div>
            </div>
            ...
            <div className="flex items-center justify-between px-3 pb-3 pt-2 relative">
              <div className="flex items-center gap-1">
                <button onClick={() => !isFull && setShowPicker(p => !p)} className={cn("p-2.5 rounded-full transition-all relative z-[105]", showPicker ? "bg-[var(--sf)] text-[var(--text)]" : isFull ? "opacity-20 cursor-not-allowed text-[var(--text)]" : "text-[var(--text)] opacity-60 hover:opacity-100 hover:bg-[var(--sf)]")}>
                  <svg width="22" height="22" viewBox="0 0 20 20" fill="none"><path d="M1 5.5L10 1L19 5.5L10 10L1 5.5Z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/><path d="M1 10L10 14.5L19 10" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" opacity="0.5"/><path d="M1 14.5L10 19L19 14.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" opacity="0.2"/></svg>
                </button>

                <div style={{ position: 'relative' }}>
                  <ModeButton mode={mode} onClick={() => setShowModeSelector(p => !p)} size="md" />
                  {showModeSelector && <ModeSelectorPopup current={mode} onSelect={m => onModeChange?.(m)} onClose={() => setShowModeSelector(false)} above={true} align="left" />}
                </div>
              </div>

              <div className="flex items-center gap-2 relative z-[105]">
                <button onClick={isListening ? stopListening : startListening} className={cn("p-3 rounded-full transition-all", isListening ? "bg-red-500 text-white scale-110 shadow-lg shadow-red-500/30" : "text-[var(--text)] opacity-60 hover:opacity-100 hover:bg-[var(--sf)]")}><Mic size={20} /></button>
