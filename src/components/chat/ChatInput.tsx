...
import { Mic, ChevronRight, X, Square, FileText, LayoutDashboard, Sparkles, Reply } from 'lucide-react';
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  // ✅ FIX: Fungsi khusus buat nge-resize textarea secara manual
  ...
  // ✅ FIX: Pantau perubahan text terus panggil fungsi resize
  useEffect(() => {
    resizeTextarea();
  ...
  const handleMicConfirm = () => {
    stopListening();
    if (voiceText) { setInputText(voiceText); setTimeout(() => handleSend(), 50); }
    else if (inputText) { handleSend(); }
  };
    ...
    <>
      ...
      {compact ? (
        <div className="pointer-events-auto" style={{ width: '100%', position: 'relative' }}>
          ...
          <div className="flex flex-col w-full relative z-[105]" style={{ borderRadius: 24, background: 'var(--sf)', border: '1px solid var(--bd)' }}>
            <AnimatePresence>
              {replyingTo && (
                <motion.div 
                ...
                >
                  <div className="bg-[var(--bg)] rounded-xl border-l-[4px] border-[var(--ac)] p-2.5 pr-10 relative">
                    <button onClick={onCancelReply} className="absolute right-1.5 top-1.5 p-1 text-[var(--mu)] hover:text-[var(--text)] rounded-full transition-all active:scale-90">
                       <X size={14} strokeWidth={2.5} />
                    </button>
                    <span className="text-[11px] font-bold text-[var(--ac)] mb-0.5 block truncate">
            ...
            {attachedFiles.length > 0 && !isListening && (
              <div className="flex gap-2 mb-3 overflow-x-auto px-4 pt-3" style={{ scrollbarWidth: 'none' }}>
                {attachedFiles.map(file => (
                  <div key={file.id} className="relative flex-shrink-0 rounded-xl overflow-hidden bg-[var(--bd)]" style={{ width: 52, height: 52 }}>
            ...
            {isListening && !showLiveVoice ? (
              <div className="flex items-center justify-between w-full h-[54px] rounded-full px-1.5 relative overflow-hidden">
              ...
              <div className="flex items-center w-full" style={{ height: 54, paddingLeft: 20, paddingRight: 8, gap: 12 }}>
                <textarea
                  ref={textareaRef}
                  value={inputText}
                  ...
                  rows={1}
                  autoFocus={false}
                  className="flex-1 bg-transparent border-none outline-none resize-none text-[14.5px] placeholder:text-[var(--mu)] text-[var(--text)] py-3"
                  style={{ minHeight: 44, maxHeight: 110, lineHeight: '22px', paddingTop: 11, alignSelf: 'center' }}
                />
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <button onClick={startListening} className="p-2.5 rounded-full text-[var(--mu)] hover:text-[var(--text)] hover:bg-[var(--bd)] transition-all flex-shrink-0"><Mic size={18} strokeWidth={2.5} /></button>
                  {hasContent
                    ? <button onClick={handleSend} disabled={isSending} className="w-9 h-9 rounded-full flex items-center justify-center bg-[var(--text)] text-[var(--bg)] transition-all hover:opacity-85 flex-shrink-0"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg></button>
                    : <button onClick={() => setShowLiveVoice(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[var(--text)] text-[var(--bg)] font-bold transition-all hover:scale-105 shadow-md flex-shrink-0"><IconSpeakWave /><span className="text-[13px]">Speak</span></button>
                  }
                </div>
              </div>
            )}
          ...
          <div className="max-w-3xl mx-auto">
            {isListening && !showLiveVoice ? (
              ...
              <div className="relative">
                ...
                <div
                  className="rounded-[32px] flex flex-col relative outline-none ring-0 overflow-hidden transition-all duration-300"
                  style={{
                    background: 'var(--sf)',
                    border: '1px solid var(--bd)',
                ...
                >
                  ...
                  {/* ✅ FIX: Hapus onKeyDown kustom. Biarin HP merespon "Enter" murni buat baris baru, dan box langsung melar otomatis berkat useEffect & onInput */}
                  <div className="px-5 pt-4 pb-1 relative z-[105]">
                    <textarea
                      ref={textareaRef}
                      value={inputText}
                      onChange={e => setInputText(e.target.value)}
                      ...
                      rows={1}
                      autoFocus={false}
                      className="w-full bg-transparent border-none outline-none resize-none text-[16px] leading-relaxed placeholder:text-[var(--mu)] text-[var(--text)] min-h-[28px] max-h-40 relative z-[105] py-1"
                    />
                  </div>
