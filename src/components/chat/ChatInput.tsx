...
const AttachPopup: React.FC<{
  ...
  return (
    <>
      ...
      <div
        className="absolute bottom-full mb-3 left-1.5 z-[100] rounded-[26px] p-2 overflow-hidden bg-[#181818]/75 border border-[var(--text)]/10 backdrop-blur-2xl shadow-[0_16px_48px_-12px_rgba(0,0,0,0.5)]"
          ...
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              ...
              <div className="rounded-[26px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.06] border border-[var(--text)]/10 shadow-[0_4px_24px_rgba(0,0,0,0.25)] backdrop-blur-md" style={{ WebkitTapHighlightColor: 'transparent' }}>
                ...
                <div className="px-4 pt-3.5 pb-1 z-[105] flex">
                  <textarea
                    ref={textareaRef}
                    value={inputText}
                    ...
                    rows={1}
                    className="w-full bg-transparent border-none outline-none resize-none text-[15.5px] leading-relaxed placeholder:text-[var(--mu)] text-[var(--text)] min-h-[24px] max-h-40 z-[105] py-[9px] pl-5 focus:ring-0 focus:outline-none"
                    style={{ fieldSizing: 'content', outline: 'none', border: 'none', boxShadow: 'none', WebkitAppearance: 'none' } as any}
                  />
                  <AnimatePresence>
                    {inputText.length > 0 && (
                      <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} onClick={handleClearText} className="absolute top-2.5 left-2.5 p-1.5 bg-[var(--text)]/5 text-[var(--text)]/60 rounded-full z-[110] [-webkit-tap-highlight-color:transparent]"><Trash2 size={14} /></motion.button>
                    )}
                  </AnimatePresence>
                  <AnimatePresence>
                    {showMaximize && (
                      <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} onClick={() => setIsFullscreenEditor(true)} className="absolute top-2 right-3 w-7 h-7 flex items-center justify-center bg-[#141414]/70 backdrop-blur-sm border border-[var(--text)]/12 rounded-full text-[var(--text)] shadow-sm active:scale-95 z-[110] [-webkit-tap-highlight-color:transparent]"><Maximize2 size={14} strokeWidth={2.5} /></motion.button>
                    )}
                  </AnimatePresence>
                </div>
                ...
                <div className="flex items-center justify-between px-2 pb-1.5 pt-1">
                  <div className="flex items-center gap-1.5 pl-1">
                    <button
                      ...
                      className={cn(
                        "w-10 h-10 rounded-full flex items-center justify-center transition-colors z-[105] [-webkit-tap-highlight-color:transparent]",
