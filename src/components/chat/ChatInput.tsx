...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  // ✅ FIX: Blur textarea on mount supaya cursor tidak muncul otomatis
  useEffect(() => {
        ...
        <footer className="px-4 pb-6 pt-2 relative z-50 pointer-events-auto">
          ...
          <div className="max-w-3xl mx-auto">
            {isListening && !showLiveVoice ? (
              ...
              <div className="bg-[var(--bg)] rounded-[32px] relative transition-all shadow-[0_8px_24px_rgba(0,0,0,0.12)]" style={{ border: '1px solid var(--bd)' }}>
                ...
                <div className="px-5 pt-4 relative z-[105]">
                  {/* ✅ FIX: autoFocus={false} eksplisit di textarea utama */}
                  <textarea ref={textareaRef} value={inputText} onChange={e => setInputText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }} placeholder="Tanya apa saja..." rows={1} autoFocus={false} className="w-full bg-transparent border-none outline-none resize-none text-[16px] leading-relaxed placeholder:text-[var(--mu)] text-[var(--text)] min-h-[28px] max-h-40 relative z-[105]" onInput={e => { const t = e.target as HTMLTextAreaElement; t.style.height = 'auto'; t.style.height = `${Math.min(t.scrollHeight, 160)}px`; }} />
                </div>
