...
const IconPaperclip = () => <Paperclip size={20} strokeWidth={2.2} />;
    ...
    >
      ...
      <div className="flex flex-col gap-1 pt-1 pb-1 outline-none">
        {MODES.map((m) => {
          ...
          return (
            <button key={m.id} onClick={() => { onSelect(m.id); if (onCloseCanvas) onCloseCanvas(); onClose(); }} className="w-full flex items-start gap-4 px-5 py-3.5 rounded-[24px] transition-colors text-left group outline-none [-webkit-tap-highlight-color:transparent]" style={{ background: active ? 'var(--text)' : 'transparent', opacity: active ? 0.05 : 1 }}>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "transition-transform duration-200 active:scale-95 bg-[var(--text)]/5 hover:bg-[var(--text)]/10 text-[var(--text)] border border-[var(--text)]/10 backdrop-blur-md"
      )}
  ...
  const renderFullscreenEditor = () => (
    <AnimatePresence>
      {isFullscreenEditor && (
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--text)]/10 bg-[var(--bg)] shadow-sm relative z-[310] outline-none">
            <button 
  ...
  return (
    <>
      ...
      <footer className={cn("px-2 pb-6 relative z-50 pointer-events-auto w-full outline-none", compact ? "pt-0" : "pt-2")}>
        ...
        {showPicker && (
          <div className="absolute bottom-full mb-3 left-4 bg-[var(--bg)] border border-[var(--text)]/10 rounded-[28px] p-2 flex flex-col gap-1 z-[100] outline-none [-webkit-tap-highlight-color:transparent] backdrop-blur-3xl" style={{ width: 190, boxShadow: '0 12px 48px -12px rgba(0,0,0,0.25)' }}>
            {[
        ...
        <div className="max-w-4xl mx-auto w-full outline-none">
          {isListening && !showLiveVoice ? (
            <div className="flex items-center justify-between w-full h-[64px] rounded-[32px] px-2 relative overflow-hidden bg-[var(--text)]/5 shadow-sm transition-transform duration-300 outline-none [-webkit-tap-highlight-color:transparent] backdrop-blur-md border border-[var(--text)]/10">
            ...
            <div className="relative w-full outline-none">
              ...
              <div
                className="rounded-[32px] flex flex-col relative overflow-hidden transition-[height,opacity,transform] duration-300 transform-gpu w-full focus:outline-none focus:ring-0 bg-[var(--text)]/[0.04] border border-[var(--text)]/10 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md"
                      ...
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 rounded-[20px] overflow-hidden bg-[var(--text)]/10 outline-none [-webkit-tap-highlight-color:transparent]" style={{ width: 80, height: 80 }}>
                          {file.type === 'pdf' || file.type === 'text' 
                            ? <div className="w-full h-full flex flex-col items-center justify-center gap-1 bg-[var(--text)]/5 px-1 border border-[var(--text)]/10 outline-none"><FileText size={24} className={file.type === 'pdf' ? "text-red-500" : "text-blue-500"} /></div>
                  ...
                  <AnimatePresence>
                    {showMaximize && (
                      <motion.button 
                        ...
                        className="absolute top-4 right-4 p-2 bg-[var(--bg)]/80 backdrop-blur-sm border border-[var(--text)]/10 rounded-full text-[var(--text)] shadow-sm hover:scale-105 active:scale-95 transition-transform z-[110]"
                      >
