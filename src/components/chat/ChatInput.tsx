                  ...
                  <div className="px-4 pt-4 pb-2 z-[105]">
                    <div className="flex gap-3 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 h-[140px] w-auto max-w-[260px] rounded-2xl overflow-hidden bg-[var(--text)]/5 border border-[var(--text)]/20 shadow-md group inline-block">
                          {file.type === 'pdf' || file.type === 'text'
                            ? <div className="min-w-[100px] h-full flex flex-col items-center justify-center px-4"><FileText size={32} className="text-[var(--text)]/70 mb-1" /><span className="text-[10px] truncate w-full text-center text-[var(--text)]/70">{file.name}</span></div>
                            : file.type === 'video'
                            ? <div className="min-w-[140px] h-full flex items-center justify-center cursor-pointer" onClick={() => setLightboxUrl(file.url)}><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[var(--text)]"><polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg></div>
                            : <img src={file.url} alt="" className="h-full w-auto object-contain cursor-pointer" onClick={() => setLightboxUrl(file.url)} />
                          }
                          <button onClick={() => removeFile(file.id)} className="absolute top-1.5 right-1.5 w-7 h-7 bg-black/60 hover:bg-black/80 text-white rounded-full flex items-center justify-center shadow-lg transition-all [-webkit-tap-highlight-color:transparent]">
