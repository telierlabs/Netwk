          ...
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              <div className="rounded-[32px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.04] border border-gray-500/20 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md" style={{ WebkitTapHighlightColor: 'transparent' }}>
                ...
                {/* 🚀 100% PLEK GPT: PREVIEW FOTO GEDE & RASIO ASLI */}
                {attachedFiles.length > 0 && (
                  <div className="px-4 pt-4 pb-2 z-[105]">
                    <div className="flex gap-3 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 h-[120px] w-auto max-w-[200px] rounded-2xl overflow-hidden bg-[var(--bg)] border border-[var(--bd)] shadow-md group">
                          {file.type === 'pdf' || file.type === 'text'
                            ? <div className="w-[100px] h-full flex items-center justify-center bg-[var(--sf)]"><FileText size={32} className="text-red-500" /></div>
                            : file.type === 'video'
                            ? <div className="w-[120px] h-full flex items-center justify-center bg-[var(--sf)] cursor-pointer" onClick={() => setLightboxUrl(file.url)}><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-blue-500"><polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg></div>
                            : <img src={file.url} alt="" className="h-full w-auto object-contain cursor-pointer" onClick={() => setLightboxUrl(file.url)} />
                          }
                          {/* Tombol X yang lebih clean */}
                          <button onClick={() => removeFile(file.id)} className="absolute top-1 right-1 w-7 h-7 bg-black/60 hover:bg-black/80 text-white rounded-full flex items-center justify-center shadow-lg transition-all [-webkit-tap-highlight-color:transparent]">
