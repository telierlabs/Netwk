...
import { Mic, ChevronRight, X, Square, FileText, LayoutDashboard, Sparkles, Paperclip } from 'lucide-react';
...
const ModeSelectorPopup: React.FC<{
  ...
  isCanvasActive?: boolean;
  onUpgradeClick?: () => void; 
}> = ({ current, onSelect, onClose, onOpenCanvas, onCloseCanvas, isCanvasActive, onUpgradeClick }) => (
  <>
    <div className="fixed inset-0 z-[110]" onClick={onClose} />
    <div className="absolute z-[120] bg-[var(--bg)] rounded-[32px] p-2 flex flex-col left-4 shadow-2xl border border-[var(--bd)]" style={{ width: 255 }}>
      <div className="px-4 pt-5 pb-5 mb-1 flex items-center justify-between border-b border-[var(--bd)] border-opacity-60">
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 flex items-center justify-center overflow-hidden">
              <img src="/IMG_20260427_105231.png" alt="Cylen" className="w-full h-full object-contain" />
            </div>
            <span className="text-[20px] font-black text-[var(--text)] tracking-tighter">Cylen</span>
          </div>
          <span className="text-[11px] text-[var(--mu)] font-bold">Akses Premium</span>
        </div>
        <button onClick={() => { onClose(); if(onUpgradeClick) onUpgradeClick(); }} className="bg-[var(--text)] text-[var(--bg)] px-4 py-2 rounded-full text-[13px] font-bold active:scale-95 transition-transform">Upgrade</button>
      </div>
      <div className="flex flex-col gap-0.5 p-1">
        <button onClick={() => { if (onOpenCanvas) onOpenCanvas(); onClose(); }} className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-[24px] transition-all text-left hover:bg-[var(--sf)]", isCanvasActive && "bg-[var(--sf)]")}>
          <LayoutDashboard size={18} className="text-[var(--ac)]" /><div className="flex-1"><div className="text-[14px] font-bold text-[var(--text)]">Cylen Canvas</div><div className="text-[11px] text-[var(--mu)]">Spatial workspace</div></div><ChevronRight size={14} className="text-[var(--mu)] opacity-50" />
        </button>
        <div className="mx-4 my-2 h-[1px] bg-[var(--bd)] opacity-30" />
        {MODES.map((m) => {
          const active = current === m.id && !isCanvasActive;
          return (
            <button key={m.id} onClick={() => { onSelect(m.id); if (onCloseCanvas) onCloseCanvas(); onClose(); }} className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-[24px] transition-all text-left hover:bg-[var(--sf)]", active && "bg-[var(--sf)]")}>
              <div className={active ? "text-[var(--text)]" : "text-[var(--mu)]"}><m.Icon /></div><div className="flex-1"><div className="text-[14px] font-bold text-[var(--text)]">{m.label}</div><div className="text-[11px] text-[var(--mu)]">{m.desc}</div></div>{active && <IconCheck />}
            </button>
          );
        })}
      </div>
...
const ModeButton: React.FC<{ mode: ChatMode; onClick: () => void; isCanvasActive?: boolean; }> = ({ mode, onClick, isCanvasActive }) => {
  ...
  return (
    <button onClick={onClick} className={cn("flex items-center gap-2 rounded-full font-bold transition-all px-4 py-2 border", isCanvasActive ? "bg-[var(--text)] text-[var(--bg)] border-transparent" : "bg-[var(--bg)] text-[var(--text)] border-[var(--bd)]")}>
      <Icon size={16} /><span className="text-[13px]">{isCanvasActive ? 'Canvas' : m?.label || 'Auto'}</span><ChevronRight size={14} className="rotate-90 opacity-50" />
    </button>
  );
};
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  mode = 'auto', onModeChange, onOpenCanvas, onCloseCanvas, isCanvasActive = false, onUpgradeClick
}) => {
      ...
      <footer className="px-4 pb-4 pt-2 bg-[var(--bg)] relative z-50">
        ...
        {showPicker && (
          <div className="absolute bottom-full mb-3 left-4 bg-[var(--bg)] border border-[var(--bd)] rounded-[28px] p-2 flex flex-col gap-1 z-[100] shadow-xl w-[190px]">
            {[
              { label: 'Kamera', ref: cameraInputRef, icon: <Mic size={18}/> },
              { label: 'Galeri', ref: galleryInputRef, icon: <Sparkles size={18}/> },
              { label: 'File', ref: fileInputRef, icon: <FileText size={18}/> }
            ].map((item, i) => (
              <button key={i} onClick={() => { setShowPicker(false); item.ref.current?.click(); }} className="w-full flex items-center gap-4 px-4 py-3.5 rounded-[20px] hover:bg-[var(--sf)] transition-colors text-[var(--text)] text-left font-bold text-[14px]">{item.label}</button>
            ))}
          </div>
        )}
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white rounded-[32px] relative border-none shadow-sm">
            {attachedFiles.length > 0 && (
              <div className="px-4 pt-3 flex gap-2 overflow-x-auto">
                {attachedFiles.map(f => (
                  <div key={f.id} className="relative w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 bg-black/5">
                    {f.type === 'image' ? <img src={f.url} className="w-full h-full object-cover" onClick={() => setLightboxUrl(f.url)} /> : <div className="w-full h-full flex items-center justify-center bg-red-50"><FileText className="text-red-500" /></div>}
            ...
            <div className="px-5 pt-4">
              <textarea ref={textareaRef} value={isListening ? (voiceText || 'Bicara...') : inputText} onChange={e => setInputText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }} placeholder="Tanya apa saja..." rows={1} className="w-full bg-transparent border-none outline-none resize-none text-[16px] leading-relaxed text-black min-h-[28px] max-h-40" onInput={e => { const t = e.target as HTMLTextAreaElement; t.style.height = 'auto'; t.style.height = `${Math.min(t.scrollHeight, 160)}px`; }} />
            </div>

            <div className="flex items-center justify-between px-3 pb-3 pt-2">
              <div className="flex items-center gap-1">
                <button onClick={() => setShowPicker(!showPicker)} className="p-2.5 rounded-full text-black opacity-60 hover:opacity-100 hover:bg-gray-100"><Paperclip size={20}/></button>
                <ModeButton mode={mode} onClick={() => setShowModeSelector(!showModeSelector)} isCanvasActive={isCanvasActive} />
              </div>
              <div className="flex items-center gap-1.5">
                <button onClick={startListening} className={cn("p-3 rounded-full transition-all", isListening ? "bg-red-500 text-white animate-pulse" : "text-gray-400 hover:bg-gray-100")}><Mic size={20} /></button>
                {hasContent ? (
