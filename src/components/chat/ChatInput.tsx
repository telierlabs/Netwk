...
const ModeSelectorPopup: React.FC<{
  ...
  <>
    ...
    <div
      className="absolute z-[120] bg-[var(--bg)] rounded-[32px] p-2 flex flex-col gap-1 left-0"
      style={{
        width: 320, 
        ...(above ? { bottom: 'calc(100% + 16px)' } : { top: 'calc(100% + 16px)' }),
        border: '1px solid var(--bd)',
        boxShadow: '0 16px 64px -16px rgba(0,0,0,0.3)',
      }}
    >
      {/* Cylen Header Header */}
      <div className="px-5 py-4 pb-3 flex items-center justify-between border-b border-[var(--bd)] border-opacity-50">
        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <IconCylen />
            <span className="text-[17px] font-extrabold text-[var(--text)] tracking-tight">Cylen AI</span>
          </div>
          <span className="text-[13px] text-[var(--mu)] font-medium mt-1">Akses intelijen premium</span>
        </div>
        <button className="bg-[var(--text)] text-[var(--bg)] px-4 py-2 rounded-full text-[13px] font-bold hover:scale-105 transition-transform">
          Upgrade
        </button>
      </div>
        ...
        {MODES.map((m) => {
          ...
          return (
            <button
              ...
              className="w-full flex items-center gap-4 px-4 py-3.5 rounded-[24px] transition-all text-left"
            ...
            >
              <div className="flex-shrink-0 flex items-center justify-center" style={{ color: active ? 'var(--text)' : 'var(--text)', opacity: active ? 1 : 0.7 }}>
                <m.Icon />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[16px] font-extrabold leading-tight mb-0.5" style={{ color: 'var(--text)' }}>{m.label}</div>
                <div className="text-[14px] font-medium" style={{ color: 'var(--mu)' }}>{m.desc}</div>
              </div>
              {active && <div style={{ color: 'var(--text)', flexShrink: 0 }}><IconCheck /></div>}
            </button>
          );
        })}
      </div>
      ...
      <>
        ...
        <div style={{ width: '100%', position: 'relative' }}>
          ...
          <div className="flex items-center w-full" style={{ height: 54, borderRadius: 999, border: '1px solid var(--bd)', background: 'var(--sf)', paddingLeft: 20, paddingRight: 12, gap: 12 }}>
            ...
            <div className="flex items-center gap-2 flex-shrink-0" style={{ position: 'relative' }}>
              <div style={{ position: 'relative' }}>
                ...
                {showModeSelector && <ModeSelectorPopup current={mode} onSelect={m => onModeChange?.(m)} onClose={() => setShowModeSelector(false)} above={true} />}
              </div>
          ...
          {showPicker && (
            ...
            >
              {[
                { label: 'Kamera', ref: cameraInputRef, icon: <svg width="22" height="22" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg> },
                { label: 'Galeri', ref: galleryInputRef, icon: <svg width="22" height="22" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path strokeLinecap="round" strokeLinejoin="round" d="M21 15l-5-5L5 21"/></svg> },
                { label: 'File',   ref: fileInputRef,    icon: <svg width="22" height="22" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M13 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V9z"/><path strokeLinecap="round" strokeLinejoin="round" d="M13 2v7h7"/></svg> },
              ].map((item, i) => (
                <button key={i}
            ...
            <div className="flex items-center justify-between px-3 pb-3 pt-2 relative">
              <div className="flex items-center gap-1">
                <button onClick={() => !isFull && setShowPicker(p => !p)} className={cn("p-2.5 rounded-full transition-all relative z-[105]", showPicker ? "bg-[var(--sf)] text-[var(--text)]" : isFull ? "opacity-20 cursor-not-allowed text-[var(--text)]" : "text-[var(--text)] opacity-60 hover:opacity-100 hover:bg-[var(--sf)]")}>
                  <svg width="22" height="22" viewBox="0 0 20 20" fill="none"><path d="M1 5.5L10 1L19 5.5L10 10L1 5.5Z" stroke="currentColor" strokeWidth="2.5" strokeLinejoin="round"/><path d="M1 10L10 14.5L19 10" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.5"/><path d="M1 14.5L10 19L19 14.5" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.2"/></svg>
                </button>
              ...
              <div className="flex items-center gap-2 relative z-[105]">
                ...
                <button onClick={handleSend} disabled={!hasContent || isSending} className={cn("p-3 rounded-full transition-all flex items-center justify-center", hasContent && !isSending ? "bg-[var(--text)] text-[var(--bg)] scale-105 shadow-md" : "text-[var(--text)] opacity-30 cursor-not-allowed")}>
                  <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg>
                </button>
              </div>
            </div>
          </div>
