...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const handleSend = () => {
    ...
    const pdfFiles = attachedFiles.filter(f => f.type === 'pdf').map(f => ({
      data: f.url.split(',')[1], // base64 only
      name: f.name,
    }));
    onSend(
            ...
            <div className="flex items-center justify-between px-3 pb-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={cn(
                    "p-2.5 rounded-full transition-all",
                    isListening ? "bg-red-500 text-white scale-110 shadow-lg shadow-red-500/30" : "text-[var(--text)] opacity-60 hover:opacity-100 hover:bg-[var(--bd)]"
                  )}
                >
                  <Mic size={20} />
                </button>
