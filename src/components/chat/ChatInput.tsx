...
import { Mic, ChevronRight, X, Square, FileText, LayoutDashboard, Sparkles, Reply, Maximize2, ArrowLeft } from 'lucide-react';
...
interface AttachedFile {
  id: string;
  url: string;
  type: 'image' | 'video' | 'pdf' | 'text'; // ✅ Tambah tipe text
  name: string;
  mimeType?: string;
}
...
const MAX_FILES = 5;
const MAX_PASTE_LENGTH = 3000; // ✅ Batas maksimal teks sebelum diubah jadi file
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  // ✅ MAGIC TRICK: Auto-convert paste teks super panjang jadi file txt
  const handlePaste = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
    const pastedText = e.clipboardData.getData('text');
    if (pastedText.length > MAX_PASTE_LENGTH) {
      e.preventDefault(); // Stop paste biar input ga meledak
      const blob = new Blob([pastedText], { type: 'text/plain' });
      const file = new File([blob], 'Kode_Panjang.txt', { type: 'text/plain' });
      processFiles([file]);
      // Kasih feedback bentar ke user
  ...
  const handleSend = () => {
    ...
    const pdfFiles = attachedFiles.filter(f => f.type === 'pdf' || f.type === 'text').map(f => ({ data: f.url.split(',')[1], name: f.name })); // Teks dikirim sebagai file
  ...
  const renderFullscreenEditor = () => (
    <AnimatePresence>
      {isFullscreenEditor && (
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--bd)] bg-[var(--bg)]">
            <button 
          ...
          <div className="flex-1 p-5 overflow-hidden">
            <textarea
              ...
              onPaste={handlePaste} // Pasang interceptor disini juga
              className="w-full h-full bg-transparent border-none outline-none resize-none text-[16px] leading-relaxed text-[var(--text)] placeholder:text-[var(--mu)]"
  ...
  return (
    <>
      ...
      <footer className={cn("px-4 pb-6 relative z-50 pointer-events-auto w-full", compact ? "pt-0" : "pt-2")}>
        {showModeSelector && (
          <ModeSelectorPopup
            current={mode}
        ...
        <div className="max-w-3xl mx-auto">
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative">
              ...
              <div
                className="rounded-[32px] flex flex-col relative outline-none ring-0 overflow-hidden transition-all duration-300 transform-gpu"
                style={{
                  background: 'var(--sf)',
                  border: '1px solid var(--bd)',
              ...
              >
                    ...
                    <div className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 rounded-[20px] overflow-hidden bg-[var(--bd)]" style={{ width: 80, height: 80 }}>
                          {file.type === 'pdf' || file.type === 'text' // Tampil icon khusus buat file teks
                ...
                <div className="px-5 pt-4 pb-1 relative z-[105] flex">
                  <textarea
                    ...
                    value={inputText}
                    onChange={e => setInputText(e.target.value)}
                    onPaste={handlePaste} // ✅ PROTEKSI PASTE PANJANG ADA DI SINI
