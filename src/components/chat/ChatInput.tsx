...
const ModeSelectorPopup: React.FC<{
  ...
  <>
    ...
    <div
      className={cn("absolute z-[120] bg-[var(--bg)] rounded-[32px] p-2 flex flex-col left-4 shadow-2xl")}
      style={{ 
        width: 255, 
    ...
    >
      ...
      <div className="px-4 pt-5 pb-5 mb-1 flex items-center justify-between border-b border-[var(--bd)] border-opacity-60">
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-[11px] text-[var(--mu)] font-bold ml-0.5">Akses Premium</span>
        </div>
        <button 
          onClick={() => { onClose(); if(onUpgradeClick) onUpgradeClick(); }} 
          className="bg-[var(--text)] text-[var(--bg)] px-4 py-2 rounded-full text-[13px] font-bold active:scale-95 transition-transform"
        >
          Upgrade
        </button>
      </div>
      ...
      <div className="flex flex-col gap-0.5 p-1">
        <button onClick={() => { if (onOpenCanvas) onOpenCanvas(); onClose(); }} 
          className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-[24px] transition-all text-left hover:bg-[var(--sf)]", isCanvasActive && "bg-[var(--sf)]")}>
          <LayoutDashboard size={18} className="text-[var(--ac)]" />
          <div className="flex-1">
            <div className="text-[14px] font-bold text-[var(--text)]">Cylen Canvas</div>
            <div className="text-[11px] text-[var(--mu)]">Spatial workspace</div>
          </div>
          <ChevronRight size={14} className="text-[var(--mu)] opacity-50" />
        </button>
        ...
        {MODES.map((m) => {
          ...
          return (
            <button key={m.id} onClick={() => { onSelect(m.id); if (onCloseCanvas) onCloseCanvas(); onClose(); }} 
              className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-[24px] transition-all text-left hover:bg-[var(--sf)]", active && "bg-[var(--sf)]")}>
              <div className={active ? "text-[var(--text)]" : "text-[var(--mu)]"}><m.Icon /></div>
              <div className="flex-1">
                <div className="text-[14px] font-bold text-[var(--text)]">{m.label}</div>
                <div className="text-[11px] text-[var(--mu)]">{m.desc}</div>
              </div>
              {active && <IconCheck />}
            </button>
          );
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const processFiles = useCallback((files: File[]) => {
    ...
    Promise.all(toAdd.map(file => new Promise<AttachedFile>(resolve => {
      ...
      reader.onloadend = () => { resolve({ id: `${Date.now()}-${Math.random()}`, url: reader.result as string, type: file.type === 'application/pdf' ? 'pdf' : file.type.startsWith('video') ? 'video' : 'image', name: file.name }); };
  ...
  return (
    <>
      ...
      <footer className="px-4 pb-4 pt-2 bg-[var(--bg)] relative z-50">
        ...
        {showPicker && (
          <div className="absolute bottom-full mb-3 left-4 bg-[var(--bg)] border border-[var(--bd)] rounded-[28px] p-2 flex flex-col gap-1 z-[100] shadow-xl w-[180px]">
            <button onClick={() => { setShowPicker(false); cameraInputRef.current?.click(); }} className="w-full flex items-center gap-3 px-4 py-3 rounded-[20px] hover:bg-[var(--sf)] text-[var(--text)] text-left font-bold text-[14px]">Kamera</button>
            <button onClick={() => { setShowPicker(false); galleryInputRef.current?.click(); }} className="w-full flex items-center gap-3 px-4 py-3 rounded-[20px] hover:bg-[var(--sf)] text-[var(--text)] text-left font-bold text-[14px]">Galeri</button>
          </div>
        )}
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-[var(--sf)] rounded-[32px] relative border border-[var(--bd)] shadow-sm">
            {attachedFiles.length > 0 && (
              <div className="px-4 pt-3 flex gap-2 overflow-x-auto">
                {attachedFiles.map(f => (
                  <div key={f.id} className="relative w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 bg-black/5">
                    {f.type === 'image' && <img src={f.url} className="w-full h-full object-cover" onClick={() => setLightboxUrl(f.url)} />}
            ...
            <div className="px-5 pt-4">
              <textarea 
                ...
                rows={1} 
                className="w-full bg-transparent border-none outline-none resize-none text-[16px] leading-relaxed text-[var(--text)] min-h-[28px] max-h-40" 
            ...
            <div className="flex items-center justify-between px-3 pb-3 pt-2">
              <div className="flex items-center gap-1">
                <button onClick={() => setShowPicker(!showPicker)} className="p-2.5 rounded-full text-[var(--text)] opacity-60 hover:opacity-100 hover:bg-[var(--bd)]"><IconPaperclip /></button>
                <ModeButton mode={mode} onClick={() => setShowModeSelector(!showModeSelector)} isCanvasActive={isCanvasActive} />
              </div>
              <div className="flex items-center gap-1.5">
                <button onClick={startListening} className="p-3 rounded-full text-[var(--mu)] hover:bg-[var(--bd)]"><Mic size={20} /></button>
                {hasContent ? (
                  <button onClick={handleSend} disabled={isSending} className="w-10 h-10 rounded-full bg-[var(--text)] text-[var(--bg)] flex items-center justify-center active:scale-90 transition-transform">
                  ...
                  <button onClick={() => setShowLiveVoice(true)} className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-[var(--text)] text-[var(--bg)] font-bold active:scale-95 transition-transform">
