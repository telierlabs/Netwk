    ...
    >
      ...
      <div className="flex flex-col gap-1 pt-1 pb-1 outline-none">
        {MODES.map((m) => {
          ...
          return (
            <button key={m.id} onClick={() => { onSelect(m.id); if (onCloseCanvas) onCloseCanvas(); onClose(); }} className="w-full flex items-start gap-4 px-5 py-3.5 rounded-[24px] transition-colors text-left group outline-none [-webkit-tap-highlight-color:transparent]" style={{ background: active ? 'var(--sf)' : 'transparent' }}>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        // 🔥 FIX BORDER KEDIP: Bordernya dikasih langsung tanpa animasi tebel/tipis, animasinya cuma buat scale & warna background
        "transition-[transform,background-color,color] duration-200 active:scale-95 bg-[var(--bg)] hover:bg-[var(--sf)] text-[var(--text)] border border-solid border-[var(--bd)]"
      )}
      ...
      <footer className={cn("px-2 pb-6 relative z-50 pointer-events-auto w-full outline-none", compact ? "pt-0" : "pt-2")}>
        ...
        <div className="max-w-4xl mx-auto w-full outline-none">
          {isListening && !showLiveVoice ? (
            <div className="flex items-center justify-between w-full h-[64px] rounded-[32px] px-2 relative overflow-hidden bg-[var(--sf)] shadow-sm transition-transform duration-300 outline-none [-webkit-tap-highlight-color:transparent]">
            ...
            <div className="relative w-full outline-none">
              ...
              <div
                className="rounded-[32px] flex flex-col relative overflow-hidden transition-[height,opacity,transform] duration-300 transform-gpu w-full focus:outline-none focus:ring-0 bg-[var(--sf)] border border-[var(--bd)]/50 shadow-[0_4px_24px_rgba(0,0,0,0.08)]"
                style={{
                  outline: 'none', 
              ...
              >
                  ...
                  <AnimatePresence>
                    {showMaximize && (
                      <motion.button 
                        ...
                        className="absolute top-4 right-4 p-2 bg-[var(--bg)]/80 backdrop-blur-sm border border-[var(--bd)] rounded-full text-[var(--text)] shadow-sm hover:scale-105 active:scale-95 transition-transform z-[110]"
                      >
                ...
                <div className="flex items-center justify-between px-2 pb-2 pt-1 relative outline-none">
                  ...
                  <div className="flex items-center gap-1.5 relative z-[105] pl-2 outline-none">
                    {!hasContent && (
                      <button onClick={startListening} className="p-2.5 rounded-full text-[var(--mu)] hover:text-[var(--text)] hover:bg-[var(--bd)] transition-colors flex-shrink-0 outline-none [-webkit-tap-highlight-color:transparent]">
