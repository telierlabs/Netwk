          ...
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              <div className="rounded-[32px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.04] border border-[var(--text)]/10 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md" style={{ WebkitTapHighlightColor: 'transparent' }}>
                ...
                {/* 🚀 FIX: ATTACHMENT PREVIEW - BEBAS RASIO (GAK KOTAK) KAYA CHATGPT */}
                {attachedFiles.length > 0 && (
                  <div className="px-4 pt-4 pb-1">
                    <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 animate-in zoom-in duration-200 inline-flex">

                          {file.type === 'pdf' || file.type === 'text' ? (
                            <div className="h-[96px] w-[80px] rounded-[14px] bg-[var(--text)]/5 border border-[var(--text)]/15 flex flex-col items-center justify-center px-2 gap-1 shadow-sm overflow-hidden">
                              <FileText size={28} className="text-red-500 flex-shrink-0" />
                            </div>
                          ) : file.type === 'video' ? (
                            <div
                              className="h-[96px] w-[80px] rounded-[14px] bg-[var(--text)]/5 border border-[var(--text)]/15 flex items-center justify-center cursor-pointer shadow-sm relative overflow-hidden"
                            ...
                            /* Gambar: Tinggi fixed 96px, Lebar w-auto object-cover -> Nyesuain Rasio otomatis */
                            <div
                              className="h-[96px] rounded-[14px] overflow-hidden border border-[var(--text)]/15 shadow-sm cursor-pointer relative bg-[var(--text)]/5 flex items-center justify-center"
                              onClick={() => setLightboxUrl(file.url)}
                            >
                              <img
                                src={file.url}
                                alt=""
                                className="h-full w-auto max-w-[160px] object-cover block"
                              />
                            </div>
                          )}
                          ...
                          <button
                            ...
                            className="absolute -top-2 -right-2 w-6 h-6 bg-black/60 hover:bg-black/80 backdrop-blur-md text-white rounded-full flex items-center justify-center shadow-md border-2 border-[var(--bg)] [-webkit-tap-highlight-color:transparent] z-10"
                          >
