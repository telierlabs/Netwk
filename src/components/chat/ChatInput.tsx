...
import { LiveVoiceMode } from './LiveVoiceMode'; // 🔥 IMPORT GOD MODE UI
    ...
    <>
      ...
      {compact ? (
        <div style={{ width: '100%', position: 'relative' }}>
          {/* ... (Attachment Preview Compact sama persis) ... */}
          ...
          <div className="flex items-center w-full" style={{ height: 54, borderRadius: 999, border: '1px solid var(--bd)', background: 'var(--sf)', paddingLeft: 20, paddingRight: 12, gap: 12 }}>
            ...
            <div className="flex items-center gap-2 flex-shrink-0" style={{ position: 'relative' }}>
              ...
              {/* 🔥 LOGIKA TOMBOL COMPACT (KOSONG = MIC, ADA TEKS = KIRIM) */}
              {hasContent ? (
                <button onClick={handleSend} disabled={isSending} className="w-8 h-8 rounded-full flex items-center justify-center bg-[var(--text)] text-[var(--bg)] transition-all hover:opacity-85 flex-shrink-0 relative z-[105]">
                ...
                <button onClick={() => setShowLiveVoice(true)} className="w-8 h-8 rounded-full flex items-center justify-center text-[var(--mu)] hover:text-[var(--text)] hover:bg-[var(--bd)] transition-all flex-shrink-0 relative z-[105]">
                  <Mic size={16} />
                </button>
              )}
            </div>
            ...
            <div className="bg-[var(--bg)] rounded-[32px] relative" style={{ border: '1px solid var(--bd)' }}>
              ...
              <div className="flex items-center justify-between px-3 pb-3 pt-2 relative">
                ...
                <div className="flex items-center gap-2 relative z-[105]">
                  {/* 🔥 LOGIKA TOMBOL NORMAL MODE (KOSONG = MIC, ADA TEKS = KIRIM) */}
                  {hasContent ? (
                    <button onClick={handleSend} disabled={isSending} className="p-3 rounded-full transition-all flex items-center justify-center bg-[var(--text)] text-[var(--bg)] scale-105 shadow-md hover:opacity-85">
                    ...
                    <button onClick={() => setShowLiveVoice(true)} className="p-3 rounded-full transition-all text-[var(--text)] opacity-60 hover:opacity-100 hover:bg-[var(--sf)]">
