...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  // ✅ STATE BARU: Tampilkan icon maximize jika input text nyentuh batas atas, dan modal fullscreen
  const [showMaximize, setShowMaximize] = useState(false);
  const [isFullscreenEditor, setIsFullscreenEditor] = useState(false);

  // ✅ LOGIKA AUTO RESIZE YANG LICIN (Mencegah macet saat paste kode panjang)
  useLayoutEffect(() => {
    ...
    // Kalau udah mentok 160px, tampilin tombol full screen
    if (newHeight >= 150) {
    ...
    <AnimatePresence>
      {isFullscreenEditor && (
        <motion.div 
          ...
          className="fixed inset-0 z-[300] bg-[var(--bg)] flex flex-col"
        >
          {/* Header Fullscreen */}
          ...
          {/* Area Textarea Fullscreen (Lebih ringan karena gak auto-resize) */}
  ...
  return (
    <>
      ...
      {/* RENDER NORMAL (Compact Mode & Standard Mobile Mode) */}
              ...
              >
                ...
                <div className="px-5 pt-4 pb-1 relative z-[105] flex">
                  <textarea
                    ...
                    style={{ fieldSizing: 'content' } as any} // CSS modern buat bikin mulus
                  />
                  
                  {/* ICON MAXIMIZE MUNCUL KALAU INPUT UDAH KEPANJANGAN */}
                  <AnimatePresence>
                    {showMaximize && (
                      <motion.button 
