...
// ── Mode Selector Popup (persis Grok) ──
const ModeSelectorPopup: React.FC<{
  current: ChatMode;
  onSelect: (m: ChatMode) => void;
  onClose: () => void;
  above?: boolean; 
  align?: 'left' | 'right'; // Tambahan agar rapi di versi mobile/compact
  ...
  <>
    ...
    <div
      className="absolute z-[120] bg-[var(--bg)] rounded-2xl overflow-hidden"
      style={{
        width: 260,
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div
                ...
                style={{ color: active ? 'var(--text)' : 'var(--text)', opacity: active ? 1 : 0.85 }}
              >
                {m.label}
              </div>
              <div className="text-[12px] mt-0.5" style={{ color: 'var(--mu)' }}>
                {m.desc}
              </div>
            </div>
            ...
            {active && (
              <div style={{ color: 'var(--ac)', flexShrink: 0 }}>
                <IconCheck />
              </div>
            )}
          </button>
...
const ModeButton: React.FC<{
  ...
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1.5 rounded-full font-medium transition-all hover:opacity-80 relative z-[105]"
      style={{
    ...
    >
      ...
      <span>{m.label}</span>
      <ChevronRight size={iconSize - 1} className="rotate-90 opacity-50" />
    </button>
  );
};
    ...
    return (
      <>
        ...
        <div style={{ width: '100%', position: 'relative' }}>
          ...
          >
            ...
            <div className="flex items-center gap-2 flex-shrink-0" style={{ position: 'relative' }}>
              ...
              {hasContent && !isSending && (
                <button
                  onClick={handleSend}
                  className="w-8 h-8 rounded-full flex items-center justify-center bg-[var(--ac)] text-[var(--at)] transition-all hover:opacity-85 flex-shrink-0 relative z-[105]"
                >
          ...
          {showPicker && (
            <div
              className="absolute bottom-full mb-3 right-0 bg-[var(--bg)] border border-[var(--bd)] rounded-2xl p-2 flex gap-1 z-[100]"
      ...
      <footer className="px-4 pb-4 pt-2 bg-[var(--bg)] relative">
        {showPicker && (
          <div className="absolute bottom-full mb-2 left-4 bg-[var(--bg)] border border-[var(--bd)] rounded-2xl p-2 flex gap-1 z-[100]"
            ...
            {[
              { label: 'Kamera', ref: cameraInputRef, icon: <svg width="22" height="22" fill="none" stroke="var(--at)" viewBox="0 0 24 24" strokeWidth={1.8}><path strokeLinecap="round" d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg> },
              { label: 'Galeri',  ref: galleryInputRef, icon: <svg width="22" height="22" fill="none" stroke="var(--at)" viewBox="0 0 24 24" strokeWidth={1.8}><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path strokeLinecap="round" d="M21 15l-5-5L5 21"/></svg> },
              { label: 'PDF',    ref: pdfInputRef,     icon: <FileText size={22} color="var(--at)" /> },
              { label: 'File',   ref: fileInputRef,    icon: <svg width="22" height="22" fill="none" stroke="var(--at)" viewBox="0 0 24 24" strokeWidth={1.8}><path strokeLinecap="round" d="M13 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V9z"/><path strokeLinecap="round" d="M13 2v7h7"/></svg> },
            ].map((item, i) => (
              <button key={i} onClick={() => { setShowPicker(false); !isFull && item.ref.current?.click(); }}
                ...
                <div className="w-12 h-12 rounded-2xl bg-[var(--ac)] flex items-center justify-center">{item.icon}</div>
                <span className="text-[11px] font-medium text-[var(--mu)]">{item.label}</span>
              </button>
            ))}
          </div>
        )}
        ...
        <div className="max-w-3xl mx-auto">
          {/* HAPUS overflow-hidden DI SINI BIAR POP-UP BISA TEMBUS KE ATAS */}
          <div className="bg-[var(--bg)] rounded-3xl relative" style={{ border: '1px solid var(--bd)' }}>
            {attachedFiles.length > 0 && (
            ...
            <div className="px-5 pt-3 relative z-[105]">
              <textarea
                ref={textareaRef}
                value={inputText}
                onChange={e => setInputText(e.target.value)}
                ...
                placeholder="Tanya apa saja..."
                rows={1}
                className="w-full bg-transparent border-none outline-none resize-none text-[15px] leading-relaxed placeholder:text-[var(--mu)] text-[var(--text)] min-h-[28px] max-h-40 relative z-[105]"
                onInput={e => {
                  const t = e.target as HTMLTextAreaElement;
                  t.style.height = 'auto';
