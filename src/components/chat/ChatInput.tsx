  ...
  <>
    ...
    <div
      className={cn("absolute z-[120] rounded-[32px] p-2 flex flex-col left-4",
        "bg-[var(--bg)]/60 backdrop-blur-3xl border border-[var(--text)]/10 shadow-[0_16px_48px_-12px_rgba(0,0,0,0.3)] supports-[backdrop-filter]:bg-[var(--bg)]/40"
      )}
          ...
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              <div className="rounded-[32px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.04] border border-[var(--text)]/10 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md" style={{ WebkitTapHighlightColor: 'transparent' }}>
                ...
                {/* 🚀 FIX TOTAL RIVALDY: PREVIEW GAMBAR BEBAS SQUISH & ANTI KOTAK KOSONG */}
                      ...
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 group animate-in zoom-in duration-200 inline-block">
                          ...
                          {file.type === 'pdf' || file.type === 'text' ? (
                            <div className="h-[140px] w-[110px] rounded-[18px] bg-[var(--text)]/5 border border-[var(--text)]/20 flex flex-col items-center justify-center px-4 shadow-sm relative overflow-hidden">
                              <FileText size={32} className="text-[var(--text)]/80 mb-2" />
                              <span className="text-[10px] font-bold truncate w-full text-center text-[var(--text)]/80">{file.name}</span>
                            </div>
                          ) : file.type === 'video' ? (
                            <div className="h-[140px] w-[140px] rounded-[18px] bg-[var(--text)]/5 border border-[var(--text)]/20 flex items-center justify-center cursor-pointer shadow-sm relative overflow-hidden" onClick={() => setLightboxUrl(file.url)}>
                              <video src={file.url} className="absolute inset-0 w-full h-full object-cover opacity-50" />
                              <div className="relative z-10 w-10 h-10 bg-[#0a0a0a]/50 backdrop-blur-md rounded-full flex items-center justify-center">
                                <svg width="20" height="20" viewBox="0 0 24 24" fill="#ffffff"><path d="M8 5v14l11-7z"/></svg>
                              </div>
                            </div>
                          ) : (
                            /* 🔥 Triknya disini: class ditaruh lgsg di img, h fix & w-auto. 
                               Biarkan si tag <img> yg ngatur lebarnya mandiri, tanpa div wrapper! */
                            <img 
                              src={file.url} 
                              alt="" 
                              className="block h-[140px] w-auto max-w-[260px] min-w-[70px] object-cover rounded-[18px] border border-[var(--text)]/20 shadow-sm cursor-pointer hover:opacity-80 transition-opacity" 
                              onClick={() => setLightboxUrl(file.url)} 
                            />
                          )}
                          
                          {/* TOMBOL X LEBIH CLEAN & MINIMALIS */}
                          <button 
                            onClick={() => removeFile(file.id)} 
                            className="absolute top-1.5 right-1.5 w-7 h-7 bg-[#0a0a0a]/80 hover:bg-[var(--text)] text-[#ffffff] hover:text-[var(--bg)] rounded-full flex items-center justify-center shadow-md transition-all backdrop-blur-md border border-[var(--text)]/10 [-webkit-tap-highlight-color:transparent]"
                          >
