...
// Tinggi maksimum textarea (px). Harus sama dengan class `max-h-40` (10rem = 160px)
// di textarea utama. Lewat batas ini composer berhenti tumbuh dan textarea
// scroll di dalam.
  ...
  return (
    <>
      ...
      <div
        ...
        // Sekarang `bg-[var(--bg)]/70` + `backdrop-blur-md` + border yang SAMA
        // persis dengan composer, jadi warna glassmor keduanya selaras. Narik warna latar app yang beneran
        // aktif (sama pattern kayak elemen lain di file ini yang udah themed).
        className="absolute bottom-full mb-3 left-1.5 z-[100] rounded-[26px] p-2 overflow-hidden bg-[var(--bg)]/70 border border-[var(--text)]/10 backdrop-blur-md shadow-[0_16px_48px_-12px_rgba(0,0,0,0.5)]"
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  // textarea bayangan (offscreen). Dipakai SATU tujuan: nentuin "apakah teks ini
  // muat 1 baris di lebar layout PIL?" — dengan lebar yang selalu lebar-pil,
  // gak peduli composer lagi di mode pil atau kotak.
  const mirrorRef      = useRef<HTMLTextAreaElement>(null);
  const composerRef    = useRef<HTMLDivElement>(null);
  const recognitionRef = useRef<any>(null);

  // cerminan `isMultiLine` yang dijamin sinkron dengan DOM yang sudah ke-commit
  // (di-update di awal layout effect). Dibaca oleh syncLayout & ResizeObserver.
  const isMultiLineRef = useRef(false);
  // selisih (px) antara lebar composer dan lebar textarea PADA LAYOUT PIL
  // (= paperclip + tombol kanan + padding + border). Nilainya konstan, jadi
  // lebar textarea-di-mode-pil bisa dihitung kapan saja: lebarComposer - chrome.
  const pillChromeRef = useRef<number | null>(null);
  ...
  // composer (pil/kotak) sedang ter-mount? (saat mode dengar suara, dia di-unmount)
  const composerVisible = !(isListening && !showLiveVoice);

  /**
   * FIX (root cause bug pil ↔ kotak ↔ scroll yang berulang).
   *
   * Versi lama nentuin mode (`isMultiLine`) dari `scrollHeight` textarea ASLI,
   * padahal lebar textarea itu sendiri ditentukan oleh mode:
   *   - mode pil   : textarea sempit (diapit paperclip & tombol kanan)
   *   - mode kotak : textarea selebar composer
   * Teks yang butuh 2 baris di lebar pil bisa muat 1 baris di lebar kotak.
   * Jadi: pil → (2 baris) → kotak → (ukur ulang, sekarang 1 baris) → pil →
   * (2 baris) → kotak ... berganti tiap ketikan. Hysteresis gak menolong
   * karena yang berubah adalah LEBAR-nya, bukan noise pembulatan.
   *
   * Selain itu effect lama cuma jalan di [inputText], jadi setelah mode ganti
   * (lebar berubah) tinggi textarea tidak diukur ulang → tinggi basi 1 baris
   * di textarea yang isinya 2 baris → muncul scroll internal kecil.
   *
   * Sekarang:
   *  1) Mode ditentukan dari textarea bayangan (mirror) yang lebarnya SELALU
   *     lebar-pil. Hasilnya cuma bergantung pada teks + lebar composer,
   *     bukan pada mode saat ini → tidak ada feedback loop.
   *  2) Tinggi textarea asli diukur SETELAH mode sudah final & DOM sudah
   *     ke-commit (effect jalan lagi saat isMultiLine berubah), semua
   *     sebelum paint.
   *  3) overflow-y dikontrol eksplisit: hidden selama belum mentok max,
   *     auto hanya kalau konten > MAX_TEXTAREA_HEIGHT.
   */
  const syncLayout = useCallback(() => {
    const el = textareaRef.current;
    const mirror = mirrorRef.current;
    const composer = composerRef.current;
    if (!el || !mirror || !composer) return;

    const composerWidth = composer.offsetWidth;
    if (composerWidth === 0) return; // lagi hidden / belum ke-layout; ResizeObserver akan manggil lagi

    // Selama layout pil aktif, (re)learn selisih lebar composer vs textarea.
    if (!isMultiLineRef.current) {
      pillChromeRef.current = composerWidth - el.offsetWidth;
    }

    // ---- 1) tentukan mode dari ukuran teks di lebar PIL ----
    let needsStack = isMultiLineRef.current;
    const chrome = pillChromeRef.current;
    if (chrome !== null) {
      const value = el.value;
      if (value === '') {
        needsStack = false;
      } else {
        // -2px = margin aman: offsetWidth dibulatkan, jadi mirror sengaja
        // dibikin sedikit lebih sempit dari textarea pil asli. Efeknya kalau
        // ragu, kita masuk mode kotak lebih awal — TIDAK PERNAH teks 2 baris
        // kejepit di pil.
        mirror.style.width = `${Math.max(composerWidth - chrome - 2, 1)}px`;
        mirror.value = value;
        const cs = window.getComputedStyle(mirror);
        const fontSize = parseFloat(cs.fontSize) || 15.5;
        const lineHeight = parseFloat(cs.lineHeight) || fontSize * 1.625;
        const oneLine =
          lineHeight + (parseFloat(cs.paddingTop) || 0) + (parseFloat(cs.paddingBottom) || 0);
        needsStack = mirror.scrollHeight > oneLine + lineHeight / 2;
      }
    }

    if (needsStack !== isMultiLineRef.current) {
      // Layout bakal berubah → effect otomatis jalan lagi setelah commit.
      // Ukur tinggi di sana, bukan di sini (DOM sekarang masih layout lama).
      setIsMultiLine(needsStack);
      return;
    }

    // ---- 2) fit tinggi textarea asli (mode sudah cocok dengan DOM) ----
    const prevScrollTop = el.scrollTop;
    el.style.overflowY = 'hidden'; // cegah scrollbar (desktop) nyempitin lebar pas ngukur
    el.style.height = 'auto';
    const contentHeight = el.scrollHeight;

    if (contentHeight > MAX_TEXTAREA_HEIGHT) {
      // mentok: tinggi ditahan di max, scroll cuma di dalam textarea
      el.style.height = `${MAX_TEXTAREA_HEIGHT}px`;
      el.style.overflowY = 'auto';
      el.scrollTop = prevScrollTop; // jangan lompatin posisi scroll user
    } else {
  ...
  // Jalan sebelum paint: tiap teks berubah, mode berubah, atau composer baru
  // ter-mount lagi (mis. habis mode dengar suara — textarea baru gak punya
  // inline height sama sekali).
  useLayoutEffect(() => {
    isMultiLineRef.current = isMultiLine;
    syncLayout();
  }, [inputText, isMultiLine, composerVisible, syncLayout]);

  // Lebar composer berubah (rotate layar, resize window, dll) → ukur ulang.
  // Tinggi sengaja TIDAK diobservasi — kita sendiri yang mengubahnya, nanti loop.
  useEffect(() => {
  ...
  }, [composerVisible, syncLayout]);

  // Font web baru selesai load → metrik teks bisa berubah tanpa lebar berubah.
  useEffect(() => {
    const fonts = (document as any).fonts;
    if (fonts?.ready) fonts.ready.then(() => syncLayout());
  }, [syncLayout]);
  ...
  // Catatan: handleSend & handleClearText dulu ikut nulis `style.height = 'auto'`
  // langsung ke DOM. Itu dihapus — tinggi sekarang cuma diatur satu tempat
  // (syncLayout), yang otomatis jalan begitu `inputText` jadi kosong.
  const handleSend = () => {
        ...
        <div className="max-w-4xl mx-auto w-full">
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              ...
              {/* FIX (buang animasi pil<->kotak sepenuhnya): dulu di sini + 5
                  elemen lain di bawah dikasih prop `layout` dari Framer Motion
                  biar transisinya animasi. Masalahnya `layout` itu animasi
                  berbasis SCALE TRANSFORM (bukan animasi height beneran) —
                  konten di dalamnya (textarea, teks) ikut ke-gepeng/ke-regang
                  tiap frame kalau ada anak yang bukan motion-tracked, dan itu
                  yang kelihatan sebagai "meleleh"/naik-turun. Animasinya
                  DIBUANG SELURUHNYA: sekarang div biasa (gak ada
                  `layout`/`transition`), jadi pas `isMultiLine` berubah,
                  ukuran & posisi elemen di dalamnya pindah INSTAN lewat class
                  Tailwind biasa. */}
              {/* FIX (glassmor ikut tema): fill pill pakai `bg-[var(--bg)]`
                  (warna latar HALAMAN) + backdrop-blur — sama pola-nya kayak
                  AttachPopup. Di light theme pill jadi kaca putih, di dark
                  theme jadi kaca hitam. */}
              <div
                ref={composerRef}
                className="rounded-[26px] flex flex-col relative overflow-hidden bg-[var(--bg)]/70 border border-[var(--text)]/10 backdrop-blur-md"
              ...
              >
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 items-center" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => {
                        if (file.type === 'pdf' || file.type === 'text') {
                          ...
                          // FIX (warna chip): border dihapus total, dan background
                          // pakai tint warna sesuai tipe file (merah tipis buat PDF,
                          // biru tipis buat dokumen teks).
                          return (
                            <div key={file.id} className={cn("relative flex-shrink-0 animate-in zoom-in duration-200 flex items-center gap-2 h-11 pl-1 pr-2 rounded-full max-w-[190px]", isPdf ? "bg-red-500/10" : "bg-blue-500/10")}>
                        ...
                        // FIX: border abu dihapus dari chip gambar/video —
                        // shadow-sm tetap dipertahankan biar thumbnail tetap
                        // kelihatan "ngambang" dikit.
                        return (
                          <div key={file.id} className="relative flex-shrink-0 animate-in zoom-in duration-200 h-11 w-[104px] rounded-full overflow-hidden bg-[var(--text)]/7 shadow-sm inline-flex">
                            {file.type === 'video' ? (
                ...
                {/* textarea bayangan (offscreen, tidak ikut layout). Padding, font, dan
                    break-words SENGAJA disamakan dengan textarea utama versi PIL
                    (`py-[9px] pl-3`). Lebarnya di-set lewat JS di syncLayout supaya
                    selalu = lebar textarea di layout pil. Dipakai buat nanya:
                    "teks ini muat 1 baris di lebar pil?" */}
                <textarea
                  ref={mirrorRef}
                  rows={1}
                  readOnly
                  aria-hidden="true"
                  tabIndex={-1}
                  className="absolute -z-10 opacity-0 pointer-events-none bg-transparent border-none outline-none resize-none break-words overflow-hidden text-[15.5px] leading-relaxed py-[9px] pl-3"
                  style={{ left: -9999, top: -9999 }}
                />

                {/*
                  Baris utama: susunannya pindah instan lewat class Tailwind
                  (order-1/2/3, flex-wrap) begitu `isMultiLine` berubah — tanpa
                ...
                */}
                ...
                >
                  ...
                  <div
                    className={cn("relative flex", isMultiLine ? "order-1 w-full flex-1 basis-full" : "flex-1")}
                  >
                    {/* `<textarea>` polos tanpa Framer Motion. Tinggi & overflow-y
                        diatur SATU tempat: syncLayout() (inline style). Jangan
                        tambah `transition-[height]` di sini — height diukur ulang
                        tiap ketikan dan transisi bikin ukurannya kejar-kejaran.
                        `break-words` tetap dipertahankan supaya teks tanpa spasi
                        di-wrap konsisten (tidak meluber ke tombol kanan). */}
                    <textarea
                      ref={textareaRef}
                      value={inputText}
                      ...
                      className={cn(
                        ...
                        // Padding vertikal SAMA di kedua mode (py-[9px]); cuma
                        // horizontal yang beda (ruang buat tombol maximize).
                        isMultiLine ? "py-[9px] pl-2 pr-7" : "py-[9px] pl-3"
                      )}
                      // `fieldSizing: 'content'` sengaja tidak dipakai — satu-satunya
                      // sumber tinggi adalah syncLayout(). `height`/`overflowY`
                      // sengaja tidak ada di sini: dikelola imperatif oleh
                      // syncLayout, dan React tidak menimpanya selama tidak ada
                      // di objek style.
                      style={{ outline: 'none', border: 'none', boxShadow: 'none', WebkitAppearance: 'none' } as any}
                    />
                    <AnimatePresence>
                      {isMultiLine && (
                        <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} onClick={() => setIsFullscreenEditor(true)} className="absolute top-1 right-1 w-7 h-7 flex items-center justify-center rounded-full text-[var(--text)]/70 active:scale-95 z-[110] [-webkit-tap-highlight-color:transparent]"><Maximize2 size={14} strokeWidth={2.5} /></motion.button>
                      )}
                    </AnimatePresence>
                  </div>
                  ...
                  <div
                    className={cn("flex items-center gap-1.5 flex-shrink-0", isMultiLine && "order-3 ml-auto")}
                  >
                    {hasContent ? (
                      <>
