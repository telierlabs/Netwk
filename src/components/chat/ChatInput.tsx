...
const ModeSelectorPopup: React.FC<{
  ...
  <>
    ...
    <div
      className={cn(
        "absolute z-[120] rounded-[32px] p-3 flex flex-col left-4",
        "bg-[var(--bg)] border border-[var(--text)]/10 shadow-[0_24px_64px_-12px_rgba(0,0,0,0.28)]"
      )}
      style={{
        width: 310,
    ...
    >
      <div className="flex items-center justify-between px-3 pt-3 pb-4 mb-1 border-b border-[var(--text)]/10">
        <div className="flex items-center gap-2.5">
          <img src="/IMG_20260427_105231.png" alt="Cylen Logo" className="w-7 h-7 object-contain" />
          <div className="flex flex-col">
            <span className="text-[18px] font-bold text-[var(--text)] tracking-tight leading-none">Cylen</span>
            <span className="text-[12px] text-[var(--mu)] font-medium mt-0.5">Akses intelijen premium</span>
          </div>
        </div>
        <button
          onClick={() => { onClose(); if (onUpgradeClick) onUpgradeClick(); }}
          className="flex-shrink-0 ml-3 bg-[var(--text)] text-[var(--bg)] px-5 py-2.5 rounded-full text-[14px] font-bold shadow hover:scale-105 active:scale-95 transition-transform tracking-wide [-webkit-tap-highlight-color:transparent]"
        >
          Upgrade
        </button>
      </div>

      <div className="flex flex-col gap-0.5 pt-2 pb-1.5">
        {MODES.map((m) => {
          const active = current === m.id && !isCanvasActive;
          return (
            <button
              key={m.id}
              onClick={() => { onSelect(m.id); if (onCloseCanvas) onCloseCanvas(); onClose(); }}
              className={cn(
                "w-full flex items-center gap-4 px-4 py-4 rounded-[22px] transition-colors text-left group [-webkit-tap-highlight-color:transparent]",
                active
                  ? "bg-[var(--text)]/10"
                  : "bg-transparent hover:bg-[var(--text)]/5 active:bg-[var(--text)]/8"
              )}
            >
              <div className={cn(
                "flex-shrink-0 transition-transform",
                active ? "text-[var(--text)]" : "text-[var(--mu)] group-hover:scale-110"
              )}>
                <m.Icon />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[16px] font-bold leading-tight mb-0.5" style={{ color: 'var(--text)' }}>
                  {m.label}
                </div>
                <div className="text-[13px] leading-tight" style={{ color: 'var(--mu)' }}>
                  {m.desc}
                </div>
              </div>
              {active && (
                <div className="flex-shrink-0" style={{ color: 'var(--text)' }}>
                  <IconCheck />
                </div>
              )}
            </button>
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    // Toggle class stacked via state biar UI pindah susunan
    setShowMaximize(newHeight > 34); 
  }, [inputText]);
      ...
      <footer className={cn("px-2 pb-6 relative z-50 pointer-events-auto w-full", compact ? "pt-0" : "pt-2")}>
        ...
        <div className="max-w-4xl mx-auto w-full">
          {isListening && !showLiveVoice ? (
            /* DESAIN PIL BLUR BUAT MIC (TIDAK DIHAPUS) - Radius disejajarkan ke 26px */
            <div className="flex items-center justify-between w-full h-[64px] rounded-[26px] px-2 relative overflow-hidden bg-[var(--text)]/[0.06] shadow-[0_4px_24px_rgba(0,0,0,0.25)] backdrop-blur-[10px] border border-[var(--text)]/10">
            ...
            <div className="relative w-full">
              {/* MAIN CONTAINER - Radius fixed 26px sesuai desain lo */}
              <div className="rounded-[26px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.06] border border-[var(--text)]/10 shadow-[0_4px_24px_rgba(0,0,0,0.25)] backdrop-blur-[10px]" style={{ WebkitTapHighlightColor: 'transparent' }}>
                ...
                {/* ATTACHMENT ROW - Desain Kapsul persis HTML */}
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 items-center" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => {
                        if (file.type === 'pdf' || file.type === 'text') {
                          return (
                            <div key={file.id} className="relative flex-shrink-0 animate-in zoom-in duration-200 flex items-center gap-2 max-w-[190px] h-[44px] rounded-full bg-[var(--text)]/[0.07] border border-[var(--text)]/[0.12] pl-1 pr-2.5">
                              <div className="flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center bg-[var(--text)]/5 text-blue-500">
                                <FileText size={16} strokeWidth={2} />
                              </div>
                              <span className="text-[12.5px] font-semibold truncate text-[var(--text)] flex-1 leading-tight">
                                {file.name || 'Dokumen'}
                              </span>
                              <button onClick={() => removeFile(file.id)} className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center bg-[var(--text)]/10 text-[var(--text)] hover:bg-[var(--text)]/20 transition-colors [-webkit-tap-highlight-color:transparent]">
                        ...
                        return (
                          <div key={file.id} className="relative flex-shrink-0 animate-in zoom-in duration-200 h-[44px] w-[104px] rounded-full overflow-hidden bg-[var(--text)]/[0.07] border border-[var(--text)]/[0.12] inline-flex">
                            {file.type === 'video' ? (
                            ...
                            <button onClick={() => removeFile(file.id)} className="absolute top-[3px] right-[3px] w-5 h-5 bg-black/65 backdrop-blur-md text-white rounded-full flex items-center justify-center shadow-md z-10 transition-colors [-webkit-tap-highlight-color:transparent]">
                ...
                {/* INPUT ROW (STACKED LOGIC) - Ngikutin .input-row.stacked */}
                <div className={cn(
                  "flex items-center gap-1 px-2 py-1.5 z-[105]", 
                  showMaximize ? "flex-wrap items-end px-2 pt-2 pb-1.5" : "flex-nowrap"
                )}>
                  {/* Left Controls */}
                  <div className={cn("flex items-center gap-1 shrink-0", showMaximize ? "order-2" : "order-1")}>
                    <button
                      onClick={() => !isFull && setShowPicker(p => !p)}
                      title="Lampirkan file"
                      className={cn(
                        "w-10 h-10 rounded-full flex items-center justify-center transition-colors [-webkit-tap-highlight-color:transparent]",
                        showPicker ? "bg-[var(--text)]/12 text-[var(--text)]"
                        : isFull   ? "opacity-20 cursor-not-allowed text-[var(--text)]"
                                   : "text-[var(--text)] bg-[var(--text)]/[0.06] hover:bg-[var(--text)]/12"
                      )}
                    >
                      <IconPaperclip />
                    </button>
                    {!showMaximize && <ModeButton mode={mode} onClick={() => setShowModeSelector(p => !p)} size="sm" isCanvasActive={isCanvasActive} />}
                  </div>

                  {/* Textarea Middle */}
                  <div className={cn("relative flex-1 flex", showMaximize ? "order-1 w-full basis-full mb-1" : "order-2")}>
                    <textarea
                      ref={textareaRef}
                      value={inputText}
                      ...
                      rows={1}
                      className={cn(
                        "w-full bg-transparent border-none outline-none resize-none text-[15.5px] leading-[1.4] text-[var(--text)] placeholder:text-[var(--mu)] focus:ring-0 focus:outline-none min-h-[24px] py-[9px]",
                        showMaximize ? "px-[8px] pr-[36px]" : "px-[4px]"
                      )}
                      style={{ fieldSizing: 'content', outline: 'none', border: 'none', boxShadow: 'none', WebkitAppearance: 'none' } as any}
                    />
                    <AnimatePresence>
                      {showMaximize && (
                        <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} onClick={() => setIsFullscreenEditor(true)} title="Buka layar penuh" className="absolute top-[8px] right-[10px] w-7 h-7 bg-[#141414]/70 border border-[var(--text)]/[0.12] rounded-full flex items-center justify-center text-[var(--text)] active:scale-95 z-[110] [-webkit-tap-highlight-color:transparent]">
                  ...
                  {/* Right Controls */}
                  <div className={cn("flex items-center gap-1 shrink-0 ml-auto", showMaximize ? "order-3" : "order-3")}>
                    {showMaximize && <div className="mr-auto"><ModeButton mode={mode} onClick={() => setShowModeSelector(p => !p)} size="sm" isCanvasActive={isCanvasActive} /></div>}
                    
                    {hasContent && (
                      <button onClick={handleClearText} className="w-[34px] h-[34px] rounded-full flex items-center justify-center text-[var(--text)]/50 hover:text-[var(--text)] bg-transparent transition-colors [-webkit-tap-highlight-color:transparent]">
                        <Trash2 size={15} strokeWidth={2} />
                      </button>
                    )}
                    
                    {!hasContent && (
                      <button onClick={startListening} title="Voice input" className="w-10 h-10 rounded-full flex items-center justify-center text-[var(--text)] bg-[var(--text)]/[0.06] hover:bg-[var(--text)]/12 transition-colors [-webkit-tap-highlight-color:transparent]">
                        <Mic size={19} strokeWidth={2.5} />
                      </button>
                    )}
                    
                    {hasContent
                      ? <button onClick={handleSend} disabled={isSending} title="Kirim" className="w-[36px] h-[36px] rounded-full flex items-center justify-center bg-[var(--text)] text-[var(--bg)] shadow-[0_2px_10px_rgba(0,0,0,0.3)] active:scale-95 transition-transform flex-shrink-0 [-webkit-tap-highlight-color:transparent]"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="10" x2="3" y2="14"/><line x1="7.5" y1="6" x2="7.5" y2="18"/><line x1="12" y1="2" x2="12" y2="22"/><line x1="16.5" y1="6" x2="16.5" y2="18"/><line x1="21" y1="10" x2="21" y2="14"/></svg></button>
                      : <button onClick={() => setShowLiveVoice(true)} className="flex items-center gap-1.5 px-3 py-[9px] rounded-full bg-[var(--text)] text-[var(--bg)] font-bold shadow-[0_2px_10px_rgba(0,0,0,0.3)] active:scale-95 transition-transform flex-shrink-0 ml-1 [-webkit-tap-highlight-color:transparent]"><IconSpeakWave /><span className="text-[13px]">Speak</span></button>
                    }
                  </div>
                </div>
              </div>
