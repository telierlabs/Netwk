...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  // --- Voice Logic ---
  // Mic (in-place): membuat pill blur & munculin teks live di tengah, TANPA buka LiveVoiceMode.
    ...
    <>
      ...
      <footer className="w-full px-2.5 pb-7 relative z-50 pointer-events-auto">
        <div className="max-w-[460px] mx-auto w-full relative">
          ...
          {/* THE PILL — mode normal ATAU mode listening (blur + teks live di tengah) */}
          {isListening ? (
            <div className="flex items-center justify-between w-full h-[64px] rounded-[26px] px-2 relative overflow-hidden bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.1)] shadow-[0_4px_24px_rgba(0,0,0,0.25)] backdrop-blur-[10px]">
              {/* Glow blur di belakang teks */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[220px] h-[50px] bg-white/10 blur-[20px] rounded-full animate-pulse" style={{ animationDuration: '1.5s' }} />

              {/* Batal */}
              <button onClick={stopListening} className="relative z-10 w-[44px] h-[44px] rounded-full flex items-center justify-center bg-[var(--bg)] text-[var(--text)] active:scale-95 transition-transform shadow-sm [-webkit-tap-highlight-color:transparent]">
              ...
              <div className="relative z-10 flex-1 flex justify-center items-center px-4 overflow-hidden">
                {voiceText
                  ...
                  : <span className="text-[13px] font-medium text-[var(--mu)]">Bicara sekarang...</span>
                }
              </div>
              ...
              <button
                ...
                className="relative z-10 w-[44px] h-[44px] rounded-full flex items-center justify-center bg-[var(--text)] text-[var(--bg)] active:scale-95 transition-transform shadow-md [-webkit-tap-highlight-color:transparent]"
              >
          ...
          <div className="flex flex-col relative overflow-hidden bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.1)] shadow-[0_4px_24px_rgba(0,0,0,0.25)] backdrop-blur-[10px] rounded-[26px]">
            ...
            <div className={cn("flex items-center flex-nowrap px-2 relative", isStacked ? "flex-wrap items-end pt-2 pb-1.5 gap-y-1.5" : "py-1.5 gap-x-1")}>
              ...
              <div className={cn("flex items-center gap-1 shrink-0", isStacked ? "order-3 ml-auto" : "")}>
                ...
                {/* Mic Btn — klik = rekam in-place, pill blur & teks di tengah */}
                {!hasText && (
                  <button onClick={startListening} title="Rekam suara" className="shrink-0 w-10 h-10 rounded-full flex items-center justify-center bg-[rgba(255,255,255,0.06)] text-[var(--text)] hover:bg-[rgba(255,255,255,0.12)] transition-colors active:scale-95 [-webkit-tap-highlight-color:transparent]">
                ...
                {/* Voice Btn — klik = buka LiveVoiceMode (bola partikel) */}
                {!hasText && (
                  <button onClick={() => setShowLiveVoice(true)} title="Mode suara" className="shrink-0 w-10 h-10 rounded-full flex items-center justify-center bg-[rgba(255,255,255,0.06)] text-[var(--text)] hover:bg-[rgba(255,255,255,0.12)] transition-colors active:scale-95 [-webkit-tap-highlight-color:transparent]">
                    <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="3" y1="10" x2="3" y2="14"/><line x1="7.5" y1="6" x2="7.5" y2="18"/><line x1="12" y1="2" x2="12" y2="22"/><line x1="16.5" y1="6" x2="16.5" y2="18"/><line x1="21" y1="10" x2="21" y2="14"/></svg>
                  </button>
                )}
