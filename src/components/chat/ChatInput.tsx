      ...
      <div className="px-4 pt-4 pb-4 mb-2 flex items-center justify-between border-b border-[var(--bd)] border-opacity-60">
        <div className="flex flex-col">
          <div className="flex items-center gap-2 mb-0.5">
            {/* 🔥 IKON DIGANTI PAKE GAMBAR YANG LO MAU */}
            <div className="w-6 h-6 flex items-center justify-center bg-[var(--sf)] rounded-full overflow-hidden border border-[var(--bd)] shadow-sm">
              <img 
                src="/IMG_20260427_105231.png" 
                alt="Cylen Logo" 
                className="w-full h-full object-contain scale-[1.3]" // Di-scale dikit biar ngepas
              />
            </div>
        ...
        <button onClick={() => { onClose(); if(onUpgradeClick) onUpgradeClick(); }} className="bg-[var(--text)] text-[var(--bg)] px-4 py-2 rounded-full text-[12px] font-bold hover:scale-105 active:scale-95 transition-all">
          Upgrade
        </button>
      </div>
      <div className="flex flex-col gap-1 pt-0.5">
        <button onClick={() => { if (onOpenCanvas) onOpenCanvas(); onClose(); }} className="w-full flex items-center gap-4 px-4 py-3 rounded-[24px] transition-all text-left mb-1 hover:bg-[var(--sf)] group" style={{ background: isCanvasActive ? 'var(--sf)' : 'transparent' }}>
          <div className="flex-shrink-0 text-[var(--ac)] opacity-90 group-hover:opacity-100 group-hover:scale-110 transition-all"><LayoutDashboard size={22} strokeWidth={2.4} /></div>
          <div className="flex-1 min-w-0"><div className="text-[16px] font-bold leading-tight mb-0.5 text-[var(--text)]">Cylen Canvas</div><div className="text-[12px] leading-tight text-[var(--mu)]">Infinite spatial workspace</div></div>
        ...
        {MODES.map((m) => {
          ...
          return (
            <button key={m.id} onClick={() => { onSelect(m.id); if (onCloseCanvas) onCloseCanvas(); onClose(); }} className="w-full flex items-start gap-4 px-4 py-3 rounded-[24px] transition-all text-left group" style={{ background: active ? 'var(--sf)' : 'transparent' }}>
              ...
              <div className="flex-1 min-w-0"><div className="text-[16px] font-bold leading-tight mb-0.5" style={{ color: 'var(--text)' }}>{m.label}</div><div className="text-[12px] leading-tight" style={{ color: 'var(--mu)' }}>{m.desc}</div></div>
