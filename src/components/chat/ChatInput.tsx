          ...
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              <div className="rounded-[32px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.04] border border-gray-500/20 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md" style={{ WebkitTapHighlightColor: 'transparent' }}>
                ...
                {/* 🚀 FIX: ATTACHMENT PREVIEW DIBIKIN PROPOSIONAL, GAK KOTAK 80x80 LAGI */}
                {attachedFiles.length > 0 && (
                  <div className="px-4 pt-3 pb-1 z-[105]">
                    <div className="flex gap-2.5 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 h-[72px] min-w-[50px] max-w-[120px] rounded-2xl overflow-hidden bg-[var(--bg)] border border-[var(--bd)]/30 shadow-sm inline-block">
                          {file.type === 'pdf' || file.type === 'text'
                            ? <div className="w-[72px] h-full flex items-center justify-center bg-[var(--text)]/5"><FileText size={24} className="text-red-500" /></div>
                            : file.type === 'video'
                            ? <div className="w-[72px] h-full flex items-center justify-center bg-[var(--text)]/5 cursor-pointer" onClick={() => setLightboxUrl(file.url)}><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-blue-500"><polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg></div>
                            : <img src={file.url} alt="" className="h-full w-auto object-cover cursor-pointer" onClick={() => setLightboxUrl(file.url)} />
                          }
                          <button onClick={() => removeFile(file.id)} className="absolute top-1 right-1 w-6 h-6 bg-black/50 hover:bg-black/70 backdrop-blur-sm text-white rounded-full flex items-center justify-center transition-colors [-webkit-tap-highlight-color:transparent]"><X size={12} strokeWidth={2.5} /></button>
                        </div>
                      ))}
                    </div>
                  </div>
