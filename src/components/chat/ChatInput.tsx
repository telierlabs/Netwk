...
// --- Kunci desain dari mockup: radius TETAP, bukan rounded-full.
// Di tinggi 1 baris (~56px) radius ini otomatis kelihatan seperti pil.
// Di tinggi banyak baris, radius yang sama kelihatan seperti kotak membulat.
// Jangan diubah jadi rounded-full — itu justru menghilangkan efeknya.
const CONTAINER_RADIUS = 'rounded-[28px]';
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  // Tinggi textarea 1 baris kosong, diukur sekali di awal — jadi acuan untuk
  // menentukan kapan layout harus pindah ke mode "stacked" (persis logika
  // baselineHeight di mockup HTML).
  ...
  // Baru: menentukan apakah pill (1 baris) atau kotak membulat + toolbar di bawah.
  ...
  useLayoutEffect(() => {
    ...
    // Ukur baseline tinggi 1 baris kosong sekali saja, saat pertama mount.
    if (baselineHeightRef.current === null) {
      const prevValue = el.style.height;
      el.style.height = 'auto';
      baselineHeightRef.current = el.scrollHeight;
      el.style.height = prevValue;
    }
    ...
    const baseline = baselineHeightRef.current ?? newHeight;
    setIsMultiLine(el.scrollHeight > baseline + 4);
  }, [inputText]);
        ...
        <div className="max-w-4xl mx-auto w-full">
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              {/* Radius TETAP (28px) — bukan rounded-full. Ini yang bikin kolom
                  otomatis terlihat pil saat pendek, dan kotak membulat saat panjang. */}
              <div className={cn(CONTAINER_RADIUS, "flex flex-col relative overflow-hidden bg-[var(--text)]/[0.04] border border-[var(--text)]/10 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md")} style={{ WebkitTapHighlightColor: 'transparent' }}>
                    ...
                    <div className="flex gap-2.5 overflow-x-auto pb-1 items-center" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => {
                        if (file.type === 'pdf' || file.type === 'text') {
                          return (
                            <div key={file.id} className="relative flex-shrink-0 animate-in zoom-in duration-200 flex items-center gap-3 h-[44px] px-3 bg-[var(--text)]/5 border border-[var(--text)]/15 rounded-full max-w-[220px]">
                        ...
                        // --- Gambar sekarang jadi kapsul horizontal (104x44),
                        // sesuai mockup, bukan thumbnail kotak 86x86.
                        return (
                          <div key={file.id} className="relative flex-shrink-0 animate-in zoom-in duration-200 h-[44px] w-[104px] rounded-full overflow-hidden bg-[var(--text)]/5 border border-[var(--text)]/15 shadow-sm inline-flex">
                            {file.type === 'video' ? (
                            ...
                            <button onClick={() => removeFile(file.id)} className="absolute top-1 right-1 w-5 h-5 bg-black/60 hover:bg-black/80 backdrop-blur-md text-white rounded-full flex items-center justify-center shadow-md z-10 transition-colors [-webkit-tap-highlight-color:transparent]">
                ...
                {!isMultiLine ? (
                  // ====== MODE PIL: teks pendek, 1 baris ======
                  // paperclip - textarea - (clear/mic) - send/speak, semua sejajar.
                  <div className="flex items-center gap-1 pl-1.5 pr-1.5 py-1.5">
                    <button
                      onClick={() => !isFull && setShowPicker(p => !p)}
                      className={cn(
                        "p-2.5 rounded-full transition-colors flex-shrink-0 z-[105] [-webkit-tap-highlight-color:transparent]",
                    ...
                    <textarea
                      ...
                      rows={1}
                      className="flex-1 bg-transparent border-none outline-none resize-none text-[16px] leading-relaxed placeholder:text-[var(--mu)] text-[var(--text)] min-h-[24px] py-2 px-1 focus:ring-0 focus:outline-none"
                      style={{ outline: 'none', border: 'none', boxShadow: 'none', WebkitAppearance: 'none' } as any}
                    />

                    {hasContent && (
                      <button onClick={handleClearText} className="p-2 text-[var(--text)]/50 rounded-full flex-shrink-0 [-webkit-tap-highlight-color:transparent]">
                        <Trash2 size={16} />
                      </button>
                    )}
                    {!hasContent && (
                    ...
                    {hasContent
                      ? <button onClick={handleSend} disabled={isSending} className="w-10 h-10 rounded-full flex items-center justify-center bg-[var(--text)] text-[var(--bg)] shadow-md flex-shrink-0 [-webkit-tap-highlight-color:transparent]"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg></button>
                      : <button onClick={() => setShowLiveVoice(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[var(--text)] text-[var(--bg)] font-bold shadow-md flex-shrink-0 ml-1 [-webkit-tap-highlight-color:transparent]"><IconSpeakWave /><span className="text-[13px]">Speak</span></button>
                    }
                  </div>
                ) : (
                  // ====== MODE STACKED: teks panjang / multi-baris ======
                  // textarea di atas (+ tombol expand di pojok kanan atas),
                  // toolbar (paperclip + mode) dan mic/send pindah ke baris bawah.
                  <>
                    <div className="px-5 pt-4 pb-1 z-[105] flex relative">
                      <textarea
                        ref={textareaRef}
                        value={inputText}
                        onChange={e => setInputText(e.target.value)}
