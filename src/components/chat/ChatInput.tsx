...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  // ✅ KUNCI 1: Hapus useRef untuk input file. Kita nggak butuh JS .click() lagi!
      ...
      <footer className={cn("px-2 pb-6 relative z-50 pointer-events-auto w-full", compact ? "pt-0" : "pt-2")}>
        ...
        {showPicker && (
          <div
            ...
            style={{ width: 190 }}
          >
            {/* ✅ KUNCI 2: Ganti <button> jadi <label> dan masukin <input> ke dalamnya */}
            <label
              className={cn("w-full flex items-center gap-4 px-4 py-3.5 rounded-[20px] transition-colors text-[var(--text)] text-left [-webkit-tap-highlight-color:transparent]",
                isFull ? "opacity-40 cursor-not-allowed pointer-events-none" : "cursor-pointer hover:bg-[var(--text)]/5 active:bg-[var(--text)]/10"
              )}
            >
              <input type="file" accept="image/*" capture="environment" onChange={handleFileChange} className="hidden" disabled={isFull} />
            ...
            >
              <input type="file" accept="image/*,video/*" multiple onChange={handleFileChange} className="hidden" disabled={isFull} />
            ...
            >
              <input type="file" accept=".pdf,.txt,.doc,.docx,application/pdf,text/plain" multiple onChange={handleFileChange} className="hidden" disabled={isFull} />
