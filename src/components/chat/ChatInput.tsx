          ...
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              <div className="rounded-[32px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.04] border border-[var(--text)]/10 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md" style={{ WebkitTapHighlightColor: 'transparent' }}>
                ...
                {/* FILE PREVIEW — compact, proportional, ChatGPT-style */}
                {attachedFiles.length > 0 && (
                  <div className="px-4 pt-4 pb-1">
                    <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 animate-in zoom-in duration-200">

                          {file.type === 'pdf' || file.type === 'text' ? (
                            /* Dokumen: kotak kecil fixed */
                            <div className="h-[72px] w-[72px] rounded-[14px] bg-[var(--text)]/5 border border-[var(--text)]/15 flex flex-col items-center justify-center px-2 gap-1 shadow-sm overflow-hidden">
                              <FileText size={22} className="text-[var(--text)]/70 flex-shrink-0" />
                              <span className="text-[9px] font-bold truncate w-full text-center text-[var(--text)]/60 leading-tight">{file.name}</span>
                            </div>

                          ) : file.type === 'video' ? (
                            /* Video: kotak kecil fixed */
                            <div
                              className="h-[72px] w-[72px] rounded-[14px] bg-[var(--text)]/5 border border-[var(--text)]/15 flex items-center justify-center cursor-pointer shadow-sm relative overflow-hidden"
                              onClick={() => setLightboxUrl(file.url)}
                            >
                              <video src={file.url} className="absolute inset-0 w-full h-full object-cover opacity-50" />
                              <div className="relative z-10 w-8 h-8 bg-[#0a0a0a]/60 backdrop-blur-sm rounded-full flex items-center justify-center">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="#ffffff"><path d="M8 5v14l11-7z"/></svg>
                              </div>
                            </div>

                          ) : (
                            /* Gambar: tinggi fixed 72px, lebar ngikut rasio asli — TIDAK SQUISH */
                            <div
                              className="h-[72px] rounded-[14px] overflow-hidden border border-[var(--text)]/15 shadow-sm cursor-pointer relative bg-[var(--text)]/5 flex items-center"
                              style={{ maxWidth: 160, minWidth: 48 }}
                              onClick={() => setLightboxUrl(file.url)}
                            >
                              <img
                                src={file.url}
                          ...
                          <button
                            ...
                            className="absolute -top-1.5 -right-1.5 w-[22px] h-[22px] bg-[var(--text)] text-[var(--bg)] rounded-full flex items-center justify-center shadow-md border-2 border-[var(--bg)] [-webkit-tap-highlight-color:transparent] z-10"
                          >
