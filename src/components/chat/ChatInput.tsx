          ...
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              ...
              >
                ...
                {/*
                  FIX (buang animasi, instan): sebelumnya baris ini + paperclip
                  + wrapper textarea + wrapper tombol aksi semua dikasih
                  `layout` biar perpindahannya (paperclip pindah baris,
                  textarea jadi full width, tombol geser ke kanan-bawah)
                  dianimasikan bareng. Itu yang kepake bikin bug "meleleh"/
                  naik-turun yang gak kelar-kelar ditambal. Sekarang semuanya
                  div/button BIASA (gak ada `layout`) — begitu `isMultiLine`
                  berubah, susunannya langsung pindah instan lewat class
                  Tailwind (order-1/2/3, flex-wrap), gak ada animasi buat
                  didistorsi.
                */}
                <div
                  className={cn(
                ...
                >
                  ...
                  >
                    {/* FIX (buang animasi total): sempat jadi `motion.textarea`
                        dengan `layout` buat ngoreksi distorsi "meleleh", tapi
                        di device asli masih kerasa goyang. Balikin jadi
                        `<textarea>` polos tanpa Framer Motion sama sekali di
                        elemen ini — resize height-nya tetap instan lewat
                        useLayoutEffect (scrollHeight) seperti sebelumnya,
                        cuma sekarang gak ada lapisan animasi tambahan yang
                        bisa mendistorsi tampilannya. */}
                    <textarea
                      ref={textareaRef}
                      value={inputText}
                      ...
                      className={cn(
                        ...
                        // FIX (naik-turun tanpa animasi): sebelumnya mode kotak pakai
                        // padding vertikal LEBIH KECIL (py-6) daripada mode pil (py-9).
                        // Begitu isMultiLine jadi true, padding turun 6px INSTAN —
                        // ini bikin scrollHeight yang diukur di useLayoutEffect ikut
                        // turun murni gara-gara padding, bukan karena baris tekstur
                        // berkurang. Kalau penurunan itu jatuh di bawah ambang exit,
                        // logic ngira teks udah muat 1 baris lagi → balik ke pil →
                        // padding naik lagi ke 9px → tinggi naik lagi → dianggap 2
                        // baris lagi → balik ke kotak. Itu loop yang bikin "naik-
                        // turun ga jelas" walau animasinya udah dihapus. Sekarang
                        // padding vertikal DISAMAKAN (py-9 di kedua mode) — cuma
                        // padding horizontal (pl/pr) yang beda buat kebutuhan layout
                        // (kasih ruang buat tombol maximize). Dengan ini, tinggi yang
                        // diukur cuma berubah kalau JUMLAH BARIS teks beneran berubah.
