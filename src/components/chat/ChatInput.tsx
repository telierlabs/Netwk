...
// 🔥 IKON ATTACHMENT PAPERCLIP PREMIUM
const IconAttachmentPaperclip = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    <path d="m21.44 11.05-9.19 9.19a6 6 0 0 1-8.49-8.49l8.57-8.57A4 4 0 1 1 18 8.84l-8.59 8.57a2 2 0 0 1-2.83-2.83l8.49-8.48"/>
  </svg>
);

// 🔥 IKON GELOMBANG SUARA (GEDEIN DIKIT SEKARANG)
const IconSpeakWave = () => (
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  return (
    <>
      ...
      {/* ========================================== */}
      {/* NORMAL MODE ONLY (COMPACT REMOVED FOR CLEANESS) */}
      {/* ========================================== */}
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-[var(--bg)] rounded-[32px] relative" style={{ border: '1px solid var(--bd)' }}>
            ...
            <div className="flex items-center justify-between px-3 pb-3 pt-2 relative">
              ...
              {/* 🔥 LOGIKA TOMBOL KANAN ALUR GROK-CYLEN */}
              <div className="flex items-center gap-1.5 relative z-[105]">
                
                {/* 1. TOMBOL MIC ( Speech-to-Text) - SELALU ADA */}
                ...
                {/* 2. GANTI-GANTIAN: ATTACHMENT VS SPEAK WAVE */}
                {hasContent ? (
                  /* 🔥 TOMBOL ATTACHMENT JADI PENGIRIM CHAT (Kirim Panah Diganti) */
                  <button onClick={handleSend} disabled={isSending} className="w-10 h-10 rounded-full transition-all flex items-center justify-center bg-[var(--text)] text-[var(--bg)] scale-105 shadow-md hover:opacity-85">
                    <IconAttachmentPaperclip />
                  </button>
                ) : (
                  /* 3. TOMBOL SPEAK MODE (Bola Cylen) - Muncul JIKA Kosong */
                  <button 
                    onClick={() => setShowLiveVoice(true)} 
                    className="flex items-center justify-center gap-2.5 w-24 h-10 rounded-full bg-[var(--text)] text-[var(--bg)] font-bold transition-all hover:scale-105 shadow-md flex-shrink-0"
                  >
                    <IconSpeakWave />
