...
const ModeSelectorPopup: React.FC<{
  ...
  <>
    ...
    {/* 🔥 LEBAR POPUP DIKECILIN JADI 280px BIAR PAS SEBELUM MIKROFON */}
    <div
      className={cn("absolute z-[120] bg-[var(--bg)] rounded-[28px] p-1.5 flex flex-col left-4")}
      style={{ 
        width: 280, // 🔥 Lebar ideal ala Grok
    ...
    >
      <div className="px-4 pt-5 pb-5 mb-1.5 flex items-center justify-between border-b border-[var(--bd)] border-opacity-60">
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-2">
            {/* 🔥 IKON DIBIKIN SUPER GEDE DAN MURNI TANPA BACKGROUND */}
            <div className="flex items-center justify-center -ml-1">
              <img 
                src="/IMG_20260427_105231.png" 
                alt="Cylen Logo" 
                className="w-12 h-12 object-contain scale-125" // 🔥 Lebar, tinggi, dan scale mentok!
              />
            </div>
            {/* 🔥 TEKS CYLEN */}
            <span className="text-[20px] font-black text-[var(--text)] tracking-tight leading-none">Cylen</span>
          </div>
          <span className="text-[12px] text-[var(--mu)] font-medium ml-1">Akses intelijen premium</span>
        </div>
        {/* 🔥 TOMBOL UPGRADE PROPORSIONAL */}
        <button 
          onClick={() => { onClose(); if(onUpgradeClick) onUpgradeClick(); }} 
          className="bg-[var(--text)] text-[var(--bg)] px-5 py-2 rounded-full text-[13px] font-bold shadow-md hover:scale-105 active:scale-95 transition-all flex-shrink-0"
        >
          Upgrade
        </button>
      </div>
      ...
      <div className="flex flex-col pt-1 pb-1">
        <button onClick={() => { if (onOpenCanvas) onOpenCanvas(); onClose(); }} className="w-full flex items-center gap-3 px-4 py-3 rounded-[20px] transition-all text-left hover:bg-[var(--sf)] group" style={{ background: isCanvasActive ? 'var(--sf)' : 'transparent' }}>
          <div className="flex-shrink-0 text-[var(--ac)] opacity-90 group-hover:opacity-100 group-hover:scale-110 transition-all"><LayoutDashboard size={20} strokeWidth={2.2} /></div>
          <div className="flex-1 min-w-0"><div className="text-[15px] font-bold leading-tight mb-0.5 text-[var(--text)]">Cylen Canvas</div><div className="text-[12px] leading-tight text-[var(--mu)]">Infinite spatial workspace</div></div>
        ...
        {MODES.map((m) => {
          ...
          return (
            <button key={m.id} onClick={() => { onSelect(m.id); if (onCloseCanvas) onCloseCanvas(); onClose(); }} className="w-full flex items-start gap-3 px-4 py-3 rounded-[20px] transition-all text-left group" style={{ background: active ? 'var(--sf)' : 'transparent' }}>
              ...
              <div className="flex-1 min-w-0"><div className="text-[15px] font-bold leading-tight mb-0.5" style={{ color: 'var(--text)' }}>{m.label}</div><div className="text-[12px] leading-tight" style={{ color: 'var(--mu)' }}>{m.desc}</div></div>
