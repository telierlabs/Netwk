...
// 🔥 FIX: Paperclip diputar biar BERDIRI TEGAK sesuai perintah CEO
...
const ModeSelectorPopup: React.FC<{
  ...
  <>
    ...
    <div
      // 🔥 FIX POPUP AUTO: Background full kaca blur, border abu tipis, gak ada bubble aneh!
    ...
    >
      ...
      <div className="flex flex-col gap-1 pt-1 pb-1 outline-none">
        {MODES.map((m) => {
          ...
          return (
            // 🔥 FIX FATAL TYPO: Opacity 0.05 dihapus! Sekarang mode Auto, Spark, Think balik normal & nyala kalau aktif!
            <button 
              key={m.id} 
              onClick={() => { onSelect(m.id); if (onCloseCanvas) onCloseCanvas(); onClose(); }} 
              className={cn(
                "w-full flex items-start gap-4 px-5 py-3.5 rounded-[24px] transition-colors text-left group outline-none [-webkit-tap-highlight-color:transparent]",
                active ? "bg-[var(--text)]/10" : "bg-transparent hover:bg-[var(--text)]/5"
              )}
            >
...
const ModeButton: React.FC<{ mode: ChatMode; onClick: () => void; size?: 'sm' | 'md'; isCanvasActive?: boolean; }> = ({ mode, onClick, size = 'md', isCanvasActive }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex items-center gap-2 rounded-full font-bold relative z-[105] outline-none [-webkit-tap-highlight-color:transparent]",
        "transition-transform duration-200 active:scale-95 bg-[var(--text)]/5 hover:bg-[var(--text)]/10 text-[var(--text)] border border-gray-500/20 backdrop-blur-md"
      )}
      style={{ padding: size === 'sm' ? '6px 14px' : '8px 18px', fontSize: size === 'sm' ? 13 : 15 }}
    >
  ...
  return (
    <>
      ...
      <footer className={cn("px-2 pb-6 relative z-50 pointer-events-auto w-full outline-none", compact ? "pt-0" : "pt-2")}>
        ...
        {showPicker && (
          <div 
            // 🔥 FIX POPUP ATTACHMENT: Disamain Kaca Blur juga biar rata dan elegan
          ...
          >
            {[
              ...
              <button key={i} onClick={() => { setShowPicker(false); !isFull && item.ref.current?.click(); }} className="w-full flex items-center gap-4 px-4 py-3.5 rounded-[20px] hover:bg-[var(--text)]/5 transition-colors text-[var(--text)] text-left outline-none [-webkit-tap-highlight-color:transparent]">
        ...
        <div className="max-w-4xl mx-auto w-full outline-none">
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full outline-none">
              ...
              <div
                className="rounded-[32px] flex flex-col relative overflow-hidden transition-[height,opacity,transform] duration-300 transform-gpu w-full focus:outline-none focus:ring-0 bg-[var(--text)]/[0.04] border border-gray-500/20 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md"
                  ...
                  {replyingTo && (
                    ...
                    >
                      <div className="bg-[var(--bg)] rounded-[16px] border-l-[4px] border-[var(--ac)] p-3 pr-10 relative outline-none [-webkit-tap-highlight-color:transparent]">
                        <button onClick={onCancelReply} className="absolute right-2 top-2 p-1.5 text-[var(--mu)] hover:text-[var(--text)] rounded-full transition-transform active:scale-90 outline-none [-webkit-tap-highlight-color:transparent]">
                      ...
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 rounded-[20px] overflow-hidden bg-[var(--text)]/10 outline-none [-webkit-tap-highlight-color:transparent]" style={{ width: 80, height: 80 }}>
                          {file.type === 'pdf' || file.type === 'text' 
                            ? <div className="w-full h-full flex flex-col items-center justify-center gap-1 bg-[var(--text)]/5 px-1 border border-gray-500/20 outline-none"><FileText size={24} className={file.type === 'pdf' ? "text-red-500" : "text-blue-500"} /></div>
                  ...
                  <AnimatePresence>
                    {showMaximize && (
                      <motion.button 
                        ...
                        className="absolute top-4 right-4 p-2 bg-[var(--bg)]/80 backdrop-blur-sm border border-gray-500/20 rounded-full text-[var(--text)] shadow-sm hover:scale-105 active:scale-95 transition-transform z-[110]"
                      >
