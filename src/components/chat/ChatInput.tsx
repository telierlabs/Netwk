...
const ModeSelectorPopup: React.FC<{
  ...
  <>
    ...
    <div
      className={cn("absolute z-[120] rounded-[32px] p-2 flex flex-col left-4",
    ...
    >
      <div className="px-5 pt-6 pb-5 mb-2 flex items-center justify-between border-b border-[var(--text)]/10">
        ...
        <button onClick={() => { onClose(); if(onUpgradeClick) onUpgradeClick(); }} className="bg-[var(--text)] text-[var(--bg)] px-5 py-2.5 rounded-full text-[14px] font-bold shadow-md hover:scale-105 active:scale-95 transition-transform tracking-wide flex-shrink-0 [-webkit-tap-highlight-color:transparent]">
          Upgrade
        </button>
      </div>
      <div className="flex flex-col gap-1 pt-1 pb-1">
        {MODES.map((m) => {
          ...
          return (
            <button key={m.id} onClick={() => { onSelect(m.id); if (onCloseCanvas) onCloseCanvas(); onClose(); }} className={cn("w-full flex items-start gap-4 px-5 py-3.5 rounded-[24px] transition-colors text-left group [-webkit-tap-highlight-color:transparent]", active ? "bg-[var(--text)]/10" : "bg-transparent hover:bg-[var(--text)]/5")}>
    ...
    <AnimatePresence>
      {isFullscreenEditor && (
        <motion.div initial={{ opacity: 0, y: '100%' }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: '100%' }} transition={{ type: 'tween', ease: [0.32, 0.72, 0, 1], duration: 0.3 }} className="fixed inset-0 z-[300] bg-[var(--bg)] flex flex-col pointer-events-auto">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-500/20 bg-[var(--bg)] shadow-sm z-[310]">
            ...
            <div className="flex items-center">
              <button onClick={(e) => { e.stopPropagation(); handleClearText(); setIsFullscreenEditor(false); }} className="p-2 text-red-500 rounded-full active:scale-90 mr-2 [-webkit-tap-highlight-color:transparent]"><Trash2 size={22} strokeWidth={2} /></button>
    ...
    <>
      ...
      <footer className={cn("px-2 pb-6 relative z-50 pointer-events-auto w-full", compact ? "pt-0" : "pt-2")}>
        ...
        {showPicker && (
          <>
            {/*
              ✅ FIX: Backdrop ada di sini tapi z-[90] — DI BAWAH picker (z-[100]).
              Backdrop hanya untuk menutup picker saat klik di luar.
              Input file ada di dalam picker (z-[100]) jadi TIDAK terblokir backdrop.
            */}
            <div className="fixed inset-0 z-[90]" onClick={() => setShowPicker(false)} />

            <div
              className="absolute bottom-full mb-3 left-4 rounded-[28px] p-2 flex flex-col gap-1 z-[100] bg-[var(--bg)]/80 backdrop-blur-3xl border border-gray-500/20 shadow-[0_16px_48px_-12px_rgba(0,0,0,0.3)]"
              style={{ width: 190 }}
            >
              {/*
                ✅ TEKNIK: Input file overlay penuh di atas tiap row.
                User secara fisik menyentuh input aslinya — tidak ada perantara .click().
                Ini cara paling reliable di semua mobile browser.
                Icon dan teks pakai pointer-events-none agar touch langsung ke input.
              */}
              ...
              <div className={cn("w-full flex items-center gap-4 px-4 py-3.5 rounded-[20px] transition-colors text-[var(--text)] relative overflow-hidden", isFull ? "opacity-40" : "hover:bg-[var(--text)]/5 active:bg-[var(--text)]/10")}>
                <input
                  ...
                  onChange={handleFileChange} disabled={isFull}
              ...
              <div className={cn("w-full flex items-center gap-4 px-4 py-3.5 rounded-[20px] transition-colors text-[var(--text)] relative overflow-hidden", isFull ? "opacity-40" : "hover:bg-[var(--text)]/5 active:bg-[var(--text)]/10")}>
                <input
                  type="file" accept=".pdf,.txt,.doc,.docx,application/pdf,text/plain" multiple
                  onChange={handleFileChange} disabled={isFull}
                  className="absolute inset-0 w-full h-full opacity-0 z-10 cursor-pointer"
                  style={{ fontSize: 0 }}
                />
        ...
        <div className="max-w-4xl mx-auto w-full">
          {isListening && !showLiveVoice ? (
            ...
            <div className="relative w-full">
              <div className="rounded-[32px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.04] border border-gray-500/20 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md" style={{ WebkitTapHighlightColor: 'transparent' }}>
                    ...
                    <div className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 rounded-[20px] overflow-hidden bg-[var(--text)]/10" style={{ width: 80, height: 80 }}>
                          ...
                          <button onClick={() => removeFile(file.id)} className="absolute top-2 right-2 w-6 h-6 bg-black/60 text-white rounded-full flex items-center justify-center [-webkit-tap-highlight-color:transparent]"><X size={12} /></button>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="px-5 pt-4 pb-1 z-[105] flex">
                  ...
                  <AnimatePresence>
                    {showMaximize && (
                      <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} onClick={() => setIsFullscreenEditor(true)} className="absolute top-4 right-4 p-2 bg-[var(--bg)]/80 backdrop-blur-sm border border-gray-500/20 rounded-full text-[var(--text)] shadow-sm active:scale-95 z-[110] [-webkit-tap-highlight-color:transparent]"><Maximize2 size={16} strokeWidth={2.5} /></motion.button>
                    )}
                  </AnimatePresence>
                </div>
