...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const removeFile = useCallback((id: string) => {
    setAttachedFiles(prev => { const next = prev.filter(f => f.id !== id); setAttachedImage(next.find(f => f.type === 'image')?.url || null); return next; });
  }, [setAttachedImage]);
  ...
  // ✅ FIX 1: Panggil click() duluan, lalu kasih delay tipis buat nutup UI
  const handlePickCamera  = () => { cameraRef.current?.click();  setTimeout(() => setShowPicker(false), 150); };
  const handlePickGallery = () => { galleryRef.current?.click(); setTimeout(() => setShowPicker(false), 150); };
  const handlePickDoc     = () => { docRef.current?.click();     setTimeout(() => setShowPicker(false), 150); };
    ...
    <AnimatePresence>
      {isFullscreenEditor && (
        <motion.div initial={{ opacity: 0, y: '100%' }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: '100%' }} transition={{ type: 'tween', ease: [0.32, 0.72, 0, 1], duration: 0.3 }} className="fixed inset-0 z-[300] bg-[var(--bg)] flex flex-col pointer-events-auto">
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-500/20 bg-[var(--bg)] shadow-sm relative z-[310]">
          ...
          <div className="flex-1 p-5 overflow-hidden bg-[var(--bg)] relative z-[305]">
  ...
  return (
    <>
      ...
      {/* ✅ FIX 2: Ganti className="hidden" jadi teknik visually hidden */}
      <input ref={cameraRef}  type="file" accept="image/*"                                          capture="environment" onChange={handleFileChange} className="w-[0.1px] h-[0.1px] opacity-0 absolute -z-10 overflow-hidden" />
      <input ref={galleryRef} type="file" accept="image/*,video/*"                                  multiple              onChange={handleFileChange} className="w-[0.1px] h-[0.1px] opacity-0 absolute -z-10 overflow-hidden" />
      <input ref={docRef}     type="file" accept=".pdf,.txt,.doc,.docx,application/pdf,text/plain" multiple              onChange={handleFileChange} className="w-[0.1px] h-[0.1px] opacity-0 absolute -z-10 overflow-hidden" />

      {showPicker && (
        <div className="fixed inset-0 z-[90] pointer-events-auto" onClick={() => setShowPicker(false)} />
      )}
      ...
      <footer className={cn("px-2 pb-6 relative z-50 pointer-events-auto w-full", compact ? "pt-0" : "pt-2")}>
        ...
        {showPicker && (
          <div
            className="absolute bottom-full mb-3 left-4 rounded-[28px] p-2 flex flex-col gap-1 z-[100] bg-[var(--bg)]/60 backdrop-blur-3xl border border-gray-500/20 shadow-[0_16px_48px_-12px_rgba(0,0,0,0.3)] supports-[backdrop-filter]:bg-[var(--bg)]/40"
            style={{ width: 190 }}
          >
            <button
              onClick={() => !isFull && handlePickCamera()}
              className={cn("w-full flex items-center gap-4 px-4 py-3.5 rounded-[20px] transition-colors text-[var(--text)] text-left [-webkit-tap-highlight-color:transparent]",
                isFull ? "opacity-40 cursor-not-allowed" : "hover:bg-[var(--text)]/5 active:bg-[var(--text)]/10"
              )}
            >
              <svg width="22" height="22" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2" className="flex-shrink-0 opacity-80"><path strokeLinecap="round" strokeLinejoin="round" d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg>
              <span className="text-[15px] font-semibold">Kamera</span>
            </button>

            <button
              onClick={() => !isFull && handlePickGallery()}
              className={cn("w-full flex items-center gap-4 px-4 py-3.5 rounded-[20px] transition-colors text-[var(--text)] text-left [-webkit-tap-highlight-color:transparent]",
                isFull ? "opacity-40 cursor-not-allowed" : "hover:bg-[var(--text)]/5 active:bg-[var(--text)]/10"
              )}
            >
              <svg width="22" height="22" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2" className="flex-shrink-0 opacity-80"><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path strokeLinecap="round" strokeLinejoin="round" d="M21 15l-5-5L5 21"/></svg>
              <span className="text-[15px] font-semibold">Galeri</span>
            </button>

            <button
              onClick={() => !isFull && handlePickDoc()}
              className={cn("w-full flex items-center gap-4 px-4 py-3.5 rounded-[20px] transition-colors text-[var(--text)] text-left [-webkit-tap-highlight-color:transparent]",
                isFull ? "opacity-40 cursor-not-allowed" : "hover:bg-[var(--text)]/5 active:bg-[var(--text)]/10"
              )}
            >
              <svg width="22" height="22" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2" className="flex-shrink-0 opacity-80"><path strokeLinecap="round" strokeLinejoin="round" d="M13 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V9z"/><path strokeLinecap="round" strokeLinejoin="round" d="M13 2v7h7"/></svg>
        ...
        <div className="max-w-4xl mx-auto w-full">
          {isListening && !showLiveVoice ? (
            <div className="flex items-center justify-between w-full h-[64px] rounded-[32px] px-2 relative overflow-hidden bg-[var(--text)]/5 shadow-sm backdrop-blur-md border border-gray-500/20">
            ...
            <div className="relative w-full">
              <div className="rounded-[32px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.04] border border-gray-500/20 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md" style={{ outline: 'none', WebkitTapHighlightColor: 'transparent' }}>

                <AnimatePresence>
                  {replyingTo && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="px-4 pt-4">
                      <div className="bg-[var(--bg)] rounded-[16px] border-l-[4px] border-[var(--ac)] p-3 pr-10 relative">
                        ...
                        <div className="flex items-center gap-1.5 mb-1"><span className="text-[12px] font-bold text-[var(--ac)] tracking-wide uppercase">{replyingTo.role === 'user' ? 'Kamu' : 'Cylen AI'}</span></div>
                  ...
                  <div className="px-4 pt-3 pb-1 relative z-[105]">
                    <div className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 rounded-[20px] overflow-hidden bg-[var(--text)]/10" style={{ width: 80, height: 80 }}>
                          {file.type === 'pdf' || file.type === 'text'
                            ? <div className="w-full h-full flex items-center justify-center bg-[var(--text)]/5 border border-gray-500/20"><FileText size={24} className="text-red-500" /></div>
                            : file.type === 'video'
                            ? <div className="w-full h-full flex items-center justify-center bg-[var(--text)]/5 border border-gray-500/20 cursor-pointer" onClick={() => setLightboxUrl(file.url)}><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-blue-500"><polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg></div>
                ...
                <div className="px-5 pt-4 pb-1 relative z-[105] flex">
                  <textarea
                    ref={textareaRef}
                    value={inputText}
                  ...
                  <AnimatePresence>
                    {inputText.length > 0 && (
                      <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} onClick={handleClearText} className="absolute top-3 left-3 p-1.5 bg-[var(--text)]/5 hover:bg-[var(--text)]/10 text-[var(--text)]/60 hover:text-[var(--text)] rounded-full transition-colors z-[110] outline-none [-webkit-tap-highlight-color:transparent]"><Trash2 size={15} /></motion.button>
                    )}
                  </AnimatePresence>
                  <AnimatePresence>
                    {showMaximize && (
                      <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} onClick={() => setIsFullscreenEditor(true)} className="absolute top-4 right-4 p-2 bg-[var(--bg)]/80 backdrop-blur-sm border border-gray-500/20 rounded-full text-[var(--text)] shadow-sm hover:scale-105 active:scale-95 transition-transform z-[110] outline-none [-webkit-tap-highlight-color:transparent]"><Maximize2 size={16} strokeWidth={2.5} /></motion.button>
                    )}
                  </AnimatePresence>
                </div>
                ...
                <div className="flex items-center justify-between px-2 pb-2 pt-1 relative">
                  ...
                  <div className="flex items-center gap-1.5 relative z-[105] pr-1">
                    {!hasContent && (
                      <button onClick={startListening} className="p-2.5 rounded-full text-[var(--text)] bg-[var(--text)]/5 hover:bg-[var(--text)]/10 transition-colors flex-shrink-0 outline-none [-webkit-tap-highlight-color:transparent]">
                        <Mic size={20} strokeWidth={2.5} />
                      </button>
                    )}
                    {hasContent
                      ? <button onClick={handleSend} disabled={isSending} className="w-10 h-10 rounded-full flex items-center justify-center bg-[var(--text)] text-[var(--bg)] scale-105 shadow-md hover:opacity-85 flex-shrink-0 outline-none [-webkit-tap-highlight-color:transparent]"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg></button>
