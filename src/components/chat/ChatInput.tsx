...
const ModeSelectorPopup: React.FC<{
  ...
  <>
    ...
    <div
      ...
      // 🔥 DILEBARIN LAGI JADI 340px biar lega kayak Grok
      style={overrideStyle ? overrideStyle : { width: 340, ...(above ? { bottom: 'calc(100% + 12px)' } : { top: 'calc(100% + 12px)' }), border: '1px solid var(--bd)', boxShadow: '0 16px 48px -12px rgba(0,0,0,0.3)' }}
    >
      {/* 🔥 HEADER POPUP DIPERBESAR & BERSIH (JARAK AMAN) */}
      <div className="px-5 pt-6 pb-6 mb-2 flex items-center justify-between border-b border-[var(--bd)] border-opacity-60">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-3">
            {/* 🔥 IKON MURNI TANPA BACKGROUND ABU, UKURAN GEDE */}
            <img 
              src="/IMG_20260427_105231.png" 
              alt="Cylen Logo" 
              className="w-9 h-9 object-contain scale-110" 
            />
            {/* 🔥 FONT CYLEN DIGEDEIN & DITEBELIN */}
        ...
        {/* 🔥 TOMBOL UPGRADE DIBESARKAN (Padding & Font) */}
        <button 
          onClick={() => { onClose(); if(onUpgradeClick) onUpgradeClick(); }} 
          className="bg-[var(--text)] text-[var(--bg)] px-6 py-2.5 rounded-full text-[15px] font-bold shadow-md hover:scale-105 active:scale-95 transition-all tracking-wide"
        >
          Upgrade
        </button>
      </div>
    ...
    <>
      ...
      {compact ? (
        ...
        <footer className="px-4 pb-4 pt-2 bg-[var(--bg)] relative z-50">
          {showModeSelector && <ModeSelectorPopup current={mode} onSelect={m => onModeChange?.(m)} onClose={() => setShowModeSelector(false)} onOpenCanvas={onOpenCanvas} onCloseCanvas={onCloseCanvas} isCanvasActive={isCanvasActive} onUpgradeClick={onUpgradeClick} overrideClass="absolute bottom-full mb-3 left-4" overrideStyle={{ width: 340, border: '1px solid var(--bd)', boxShadow: '0 16px 48px -12px rgba(0,0,0,0.3)' }} />}
