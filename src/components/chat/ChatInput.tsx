...
interface ChatInputProps {
  ...
  onUpgradeClick?: () => void;
  replyingTo?: Message | null; // ✅ State untuk pesan yg direply
  onCancelReply?: () => void;  // ✅ Fungsi batal reply
}
    ...
    <>
      ...
      {compact ? (
        <div className="pointer-events-auto" style={{ width: '100%', position: 'relative' }}>
          ...
          {/* ✅ PREVIEW REPLY (Kalo compact mode tetep di-handle) */}
          {replyingTo && (
            <motion.div 
              initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
              className="mb-2 mx-1 bg-[var(--text)]/5 backdrop-blur-md border-l-[3px] border-[var(--ac)] rounded-[14px] p-2.5 pr-10 relative shadow-sm"
            >
              <button onClick={onCancelReply} className="absolute right-1.5 top-1.5 p-1 text-[var(--text)]/50 hover:text-[var(--text)] hover:bg-[var(--text)]/10 rounded-full transition-all active:scale-90">
                 <X size={16} strokeWidth={2.5} />
              </button>
              <span className="text-[11px] font-extrabold text-[var(--ac)] mb-0.5 block tracking-wide uppercase">
                {replyingTo.role === 'user' ? 'Kamu' : 'Cylen AI'}
              </span>
              <span className="text-[12px] text-[var(--text)]/80 line-clamp-1 font-medium leading-snug">
                {replyingTo.content}
              </span>
            </motion.div>
          )}
          ...
          <div className="max-w-3xl mx-auto">
            {isListening && !showLiveVoice ? (
              ...
              <div className="relative">
                {/* ✅ KOTAK PREVIEW WHATSAPP STYLE MUNCUL DI ATAS INPUT NORMAL */}
                <AnimatePresence>
                  {replyingTo && (
                    <motion.div 
                      ...
                      className="absolute bottom-full left-0 right-0 mb-3 mx-2 bg-[var(--bg)]/90 backdrop-blur-xl border border-[var(--bd)] rounded-2xl p-3 pr-10 shadow-lg z-[-1]"
                    >
                      <button 
                        onClick={onCancelReply} 
                        className="absolute right-2 top-2 p-1.5 text-[var(--text)]/50 hover:text-[var(--text)] hover:bg-[var(--sf)] rounded-full transition-all active:scale-90"
                      >
                         <X size={18} strokeWidth={2.5} />
                      </button>
                      <div className="flex items-center gap-2 mb-1">
                        <Reply size={14} className="text-[var(--ac)]" />
                        <span className="text-[12px] font-black text-[var(--ac)] tracking-wide uppercase">
                          {replyingTo.role === 'user' ? 'Kamu' : 'Cylen AI'}
                        </span>
                      </div>
                      <span className="text-[13.5px] text-[var(--text)]/80 line-clamp-1 font-medium pl-6 leading-snug">
                        {replyingTo.content}
                      </span>
                    </motion.div>
                  )}
                ...
                <div
                  className="rounded-[32px] relative outline-none ring-0"
                  style={{
                    background: 'var(--sf)',
                    border: '1px solid var(--bd)',
