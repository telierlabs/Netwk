...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  return (
    <>
      ...
      {/*
        FOOTER — bg solid var(--bg) + safe-area-bottom sebagai padding asli
        (bukan cuma pb-6 statis). Ini nutup total celah di bawah tombol kirim
        supaya warna sistem/gesture-bar Android nggak nembus/keliatan sebagai
        strip cream/beda warna. Ditambah backdrop-blur biar transisinya halus
        kalau ada konten yang scroll di baliknya.
      */}
      <footer
        className={cn(
          "px-2 relative z-50 pointer-events-auto w-full bg-[var(--bg)]",
          compact ? "pt-0" : "pt-2"
        )}
        style={{
                ...
                <div className={cn(
                  ...
                  <div className={cn("relative flex", isMultiLine ? "order-1 w-full flex-1 basis-full" : "flex-1")}>
                    <textarea
                      ...
                      rows={1}
                      className={cn(
                        "w-full bg-transparent border-none outline-none resize-none text-[15.5px] leading-relaxed placeholder:text-[var(--mu)] text-[var(--text)] min-h-[24px] max-h-40 z-[105] focus:ring-0 focus:outline-none",
