...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const renderFullscreenEditor = () => (
    <AnimatePresence>
      {isFullscreenEditor && (
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--bd)] bg-[var(--bg)] shadow-sm relative z-[310]">
            <button 
              onClick={(e) => { e.stopPropagation(); setIsFullscreenEditor(false); }} 
              className="p-2 -ml-2 text-[var(--text)]/60 hover:text-[var(--text)] transition-colors rounded-full active:scale-90"
            >
            ...
            <div className="flex items-center">
                <button 
                  ...
                  className="p-2 text-red-500 hover:text-red-600 transition-colors rounded-full active:scale-90 mr-2"
                >
                  <Trash2 size={22} strokeWidth={2} />
                </button>
                <button 
                  onClick={(e) => { e.stopPropagation(); setIsFullscreenEditor(false); }} 
                  className="p-2 -mr-2 text-[var(--text)]/60 hover:text-[var(--text)] transition-colors rounded-full active:scale-90"
                >
          ...
          <div className="flex-1 p-5 overflow-hidden bg-[var(--bg)] relative z-[305]">
            <textarea
              ...
              onPaste={handlePaste} 
              className="w-full h-full bg-transparent border-none outline-none resize-none text-[16px] leading-relaxed text-[var(--text)] placeholder:text-[var(--mu)] focus:ring-0 focus:outline-none focus:border-transparent"
              style={{ outline: 'none' }} // ✅ CSS INLINE
  ...
  return (
    <>
      ...
      {showPicker && <div className="fixed inset-0 z-[90] pointer-events-auto" onClick={() => setShowPicker(false)} />}
        ...
        <div className="max-w-4xl mx-auto w-full">
          {isListening && !showLiveVoice ? (
            <div className="flex items-center justify-between w-full h-[64px] rounded-[32px] px-2 relative overflow-hidden bg-[var(--sf)] shadow-sm transition-all duration-300">
              ...
              <button onClick={() => { stopListening(); if (voiceText) { setInputText(voiceText); setTimeout(() => handleSend(), 50); } else if (inputText) { handleSend(); } }} className="relative z-10 w-[46px] h-[46px] rounded-full flex items-center justify-center bg-[#0a0a0a] text-[#fff] hover:scale-105 active:scale-95 transition-transform shadow-md">
            ...
            <div className="relative w-full">
              ...
              {/* ✅ PERBAIKAN: Outline diilangin paksa secara inline */}
              <div
                ...
                style={{
                  ...
                  outline: 'none', // 🔥 INI KUNCINYA
                  WebkitTapHighlightColor: 'transparent' // 🔥 INI JUGA KUNCI BUAT HP
                }}
                tabIndex={-1} // ✅ Biar gak bisa dapet focus outline
              >
                  ...
                  {replyingTo && (
                    ...
                    >
                      <div className="bg-[var(--bg)] rounded-[16px] border-l-[4px] border-[var(--ac)] p-3 pr-10 relative">
                        <button onClick={onCancelReply} className="absolute right-2 top-2 p-1.5 text-[var(--mu)] hover:text-[var(--text)] rounded-full transition-all active:scale-90">
                           <X size={16} strokeWidth={2.5} />
                        </button>
                        <div className="flex items-center gap-1.5 mb-1">
                          <span className="text-[12px] font-bold text-[var(--ac)] tracking-wide uppercase">
                            {replyingTo.role === 'user' ? 'Kamu' : 'Cylen AI'}
                          </span>
                        </div>
                        <span className="text-[13.5px] text-[var(--text)]/80 line-clamp-2 font-medium leading-snug break-words">
                          {replyingTo.content}
                        </span>
                      </div>
                    </motion.div>
                      ...
                      {attachedFiles.map(file => (
                        <div key={file.id} className="relative flex-shrink-0 rounded-[20px] overflow-hidden bg-[var(--bd)]" style={{ width: 80, height: 80 }}>
                          {file.type === 'pdf' || file.type === 'text' 
                            ? <div className="w-full h-full flex flex-col items-center justify-center gap-1 bg-[var(--sf)] px-1 border border-[var(--bd)]"><FileText size={24} className={file.type === 'pdf' ? "text-red-500" : "text-blue-500"} /></div>
                ...
                <div className="px-5 pt-4 pb-1 relative z-[105] flex">
                  {/* ✅ TEXTAREA BEBAS GARIS ITEM DENGAN INLINE STYLE */}
                  <textarea
                    ref={textareaRef}
                    value={inputText}
                    onChange={e => setInputText(e.target.value)}
                    ...
                    rows={1}
                    className="w-full bg-transparent border-none outline-none resize-none text-[16px] leading-relaxed placeholder:text-[var(--mu)] text-[var(--text)] min-h-[28px] max-h-40 relative z-[105] py-1 focus:ring-0 focus:outline-none focus:border-transparent" 
                    style={{ fieldSizing: 'content', outline: 'none' } as any} // 🔥 KUNCI JUGA
                  />
                </div>
                ...
                <div className="flex items-center justify-between px-2 pb-2 pt-1 relative">
                  ...
                  <div className="flex items-center gap-1.5 relative z-[105] pl-2">
                      ...
                      {hasContent && (
                        ...
                        >
                          <button
                            ...
                            title="Hapus Teks"
                            className="p-2 text-[var(--text)] hover:text-red-500 transition-colors rounded-full flex-shrink-0"
                          >
                          ...
                          {showMaximize && (
                            <button
                              ...
                              title="Perbesar"
                              className="p-2 text-[var(--mu)] hover:text-[var(--text)] hover:bg-[var(--text)]/10 transition-colors rounded-full flex-shrink-0"
                            >
                    ...
                    {!hasContent && (
                      <button onClick={startListening} className="p-2.5 rounded-full text-[var(--mu)] hover:text-[var(--text)] hover:bg-[var(--bd)] transition-all flex-shrink-0">
                        <Mic size={20} strokeWidth={2.5} />
                      </button>
                    )}

                    {hasContent
                      ? <button onClick={handleSend} disabled={isSending} className="w-10 h-10 rounded-full transition-all flex items-center justify-center bg-[var(--text)] text-[var(--bg)] scale-105 shadow-md hover:opacity-85 flex-shrink-0"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg></button>
                      : <button onClick={() => setShowLiveVoice(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[var(--text)] text-[var(--bg)] font-bold transition-all hover:scale-105 shadow-md flex-shrink-0 ml-1"><IconSpeakWave /><span className="text-[13px]">Speak</span></button>
                    }
                  </div>
                </div>
              </div>
