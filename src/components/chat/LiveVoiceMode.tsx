...
export const LiveVoiceMode: React.FC<LiveVoiceModeProps> = ({ onClose }) => {
      ...
      if (currentShape !== stageData) {
        ...
        for (let i = 0; i < particleCount; i++) {
          ...
          if (currentShape === 'dna') {
            ...
            else { p.tx = 0.4*Math.cos(u); p.ty = -0.6 - (Math.random()*0.3); p.tz = 0.4*Math.sin(u); }
          } else if (currentShape === 'car') {
            if (i < 2000) { p.tx = (Math.random()-0.5)*1.2; p.ty = (Math.random()-0.5)*0.3; p.tz = (Math.random()-0.5)*0.6; }
            else if (i < 3000) { p.tx = (Math.random()-0.5)*0.6; p.ty = 0.15 + Math.random()*0.3; p.tz = (Math.random()-0.5)*0.5; }
            else { p.tx = (i%2===0?-0.4:0.4); p.ty = -0.15; p.tz = (i%4<2?-0.3:0.3); }
  ...
  return (
    <div
      className="fixed inset-0 z-[999] flex flex-col bg-[#000000] overflow-hidden font-sans touch-none select-none pointer-events-auto"
      style={{ opacity: isClosing ? 0 : 1, transition: 'opacity 0.2s ease' }}
    >
      {/* ─── STAGE ─── */}
      ...
      {/* ─── STATUS TEXT ─── */}
      <div className="flex flex-col items-center gap-1.5 pb-5 px-6">
        <div className="text-white font-bold text-[20px] tracking-tight text-center">
          Cylen Voice
        </div>
      ...
      {/* ─── BOTTOM BAR: icon toolbar ─── */}
      <div className="px-4 pb-8 flex flex-col gap-3 pointer-events-auto">
        {/* Icon toolbar row */}
        <div className="flex items-center justify-around bg-white/[0.07] rounded-[28px] px-6 py-4 border border-white/10">
          <button
            className="flex flex-col items-center justify-center gap-1.5 text-white/70 active:text-white active:scale-90 transition-all [-webkit-tap-highlight-color:transparent]"
          >
        ...
        <div className="flex items-center justify-between bg-white/[0.07] rounded-[28px] px-5 py-3 border border-white/10">
          <span className="text-white/30 text-[14px] font-medium">Ask anything</span>
          <button
            onClick={handleClose}
