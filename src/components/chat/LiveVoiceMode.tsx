...
type StageType = 'hologram' | 'map' | 'code' | 'simulation';
...
export const LiveVoiceMode: React.FC<LiveVoiceModeProps> = ({ onClose }) => {
  ...
  const [activeSpec, setActiveSpec] = useState<ValidatedSimulationSpec | null>(null);
  ...
  // ── KLASIFIKASI NIAT: ringan & cepat, cuma menentukan "panggung" mana ──
  // Ini SENGAJA dipisah dari pembuatan SimulationSpec (yang berat) supaya
  // obrolan biasa (bukan permintaan simulasi) tetap cepat & hemat kuota API.
...
3. [STAGE: SIMULATION] -> kalau user minta simulasi/visualisasi 3D/2D dari suatu konsep, objek, proses, atau fenomena (apapun topiknya — biologi, fisika, teknik, astronomi, jaringan, matematika, dst), atau minta modifikasi/lanjutan dari simulasi yang sedang ditampilkan.
4. [STAGE: CHAT] -> untuk semua percakapan lain yang tidak butuh visual simulasi.

Untuk tag SIMULATION, penjelasan lisan cukup kalimat pendek yang menandakan kamu akan segera menyiapkannya (mis. "Baik, saya siapkan simulasinya sekarang.") — penjelasan detail akan menyusul setelah simulasi selesai dibuat.`;
      ...
      if (stageType === 'MAP') {
        setActiveStage('map');
        setStageData(stageArg.toLowerCase());
      ...
      if (stageType === 'SIMULATION') {
        // Panggung TETAP di hologram (visual loading lama, tidak diubah)
        // selama proses pembuatan spec berlangsung.
        setUiText('Membuat simulasi...');
        speakText(spokenText || 'Baik, saya siapkan simulasinya sekarang.');

        const result = await generateSimulationSpec(userText, activeSpecRef.current ?? undefined);
        setActiveSpec(result.spec);
        setActiveStage('simulation');

        // Narasi detail menyusul setelah simulasi siap. speakText di sini
        // akan menimpa ucapan "sedang menyiapkan..." sebelumnya kalau masih
        // berjalan, dan melanjutkan ke startListening() setelah selesai.
        speakText(result.spec.explanation.narration);
        return;
      }

      // Default: CHAT — tetap di panggung hologram (idle default lama).
      setActiveStage('hologram');
      speakText(spokenText);
    } catch (error) {
  ...
  // ── HOLOGRAM IDLE — TIDAK DIUBAH SAMA SEKALI dari versi sebelumnya. ──
  // Ini cuma bola partikel default (golden-ratio distribution) yang jadi
  // visual "listening/processing/speaking". Cabang if/else per-bentuk
  // (dna/heart/brain/dst) yang dulu ada di sini SUDAH DIHAPUS karena itu
  // sumber masalah "DNA jadi cacing" — simulasi sungguhan sekarang lewat
  // SimulationCanvas (Three.js), bukan canvas 2D ini.
  useEffect(() => {
    ...
    const particles: { x: number; y: number; z: number; tx: number; ty: number; tz: number; r: number }[] = [];

    for (let i = 0; i < particleCount; i++) {
      particles.push({ x: (Math.random()-0.5)*2, y: (Math.random()-0.5)*2, z: (Math.random()-0.5)*2, tx: 0, ty: 0, tz: 0, r: Math.random() * 1.2 + 0.5 });
    }
    ...
    const render = () => {
      ...
      const glowColor = `rgba(255,255,255,${animState.glowIntensity})`;
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        const depthAlpha = Math.max(0.1, Math.min(1, (z2 + 1.5) / 2.5));
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex items-center justify-center relative w-full h-full">
        ...
        {activeStage === 'simulation' && activeSpec && (
      ...
      {/* ─── BOTTOM BAR: icon toolbar ─── tidak diubah sama sekali ─── */}
      <div className="px-4 pb-8 flex flex-col gap-3 pointer-events-auto max-w-2xl mx-auto w-full">
        <div className="flex items-center justify-around bg-white/[0.07] rounded-[28px] px-6 py-4 border border-white/10">
          ...
          <button
            className={`flex flex-col items-center justify-center gap-1.5 transition-all active:scale-90 [-webkit-tap-highlight-color:transparent] ${
              status === 'listening' ? 'text-white' : 'text-white/70'
            }`}
          >
