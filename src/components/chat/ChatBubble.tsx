...
import {
  ...
  Edit2, Clock, Globe, List, Trash2, Palette // <-- Tambah Palette
} from 'lucide-react';
...
interface ChatBubbleProps {
  ...
  suggestions?: string[];
  isStreaming?: boolean;
  activityStatus?: 'idle' | 'image' | 'pdf'; // <-- PROP BARU TERIMA DARI CHATPAGE
}
...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({
  ...
  isStreaming = false,
  activityStatus = 'idle', // <-- NERIMA PROP STATUS DARI CHATPAGE
}) => {
  const [copied, setCopied] = useState(false);
  ...
  const pdfLinkMatch = displayContent.match(/^\[PDF Terlampir: "(.*?)"\]\((.*?)\)\n\n([\s\S]*)$/);
  
  if (pdfLinkMatch) {
    const pdfTitle = pdfLinkMatch[1];
    const pdfUrl = pdfLinkMatch[2];
    displayContent = pdfLinkMatch[3];
    ...
    pdfLinkElement = (
      <div className={cn("flex flex-col gap-2 mb-3 mt-1 w-full", isUser ? "items-end" : "items-start")}>
        <a 
          ...
          className={cn(
            "flex items-center gap-3 px-4 py-3 bg-[var(--sf)] border border-[var(--bd)] rounded-2xl shadow-sm hover:opacity-80 transition-all w-fit max-w-[85%]", 
            isUser ? "ml-auto" : "mr-auto"
          )}
        >
          <div className="w-10 h-10 rounded-xl bg-[var(--bd)]/50 flex items-center justify-center flex-shrink-0 text-[var(--text)]">
            <FileText size={20} />
          </div>
          <div className="flex flex-col items-start min-w-0 pr-2">
            <span className="text-[14px] font-bold text-[var(--text)] truncate max-w-[180px]">{pdfTitle || 'Document.pdf'}</span>
            <span className="text-[11px] font-medium text-[var(--mu)]">Klik untuk unduh</span>
          </div>
  ...
  return (
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start", showUserMenu ? "z-[100]" : "z-10")}>
      ...
      {/* ── MODIFIKASI UTAMA: HIDE TEXT SAAT BIKIN PDF / GAMBAR ── */}
      {activityStatus !== 'idle' && !isUser ? (
        // Kalau statusnya lagi PDF/Image, HIDE SEMUA TEKS! Tampilkan Animasi Shimmer ini aja
        <div className="flex flex-col items-start gap-2 w-full mt-1 mb-2">
          <style>{`@keyframes shimmerGen { 0% { background-position: -200% 0; } 100% { background-position: 200% 0; } }`}</style>
          <div className="flex items-center gap-3 px-5 py-3.5 bg-[var(--bg)] border border-[var(--bd)] rounded-2xl shadow-sm w-fit max-w-[85%]">
             {activityStatus === 'pdf' ? <FileText size={22} className="text-[var(--text)] animate-pulse" /> : <Palette size={22} className="text-[var(--text)] animate-pulse" />}
             <span style={{
               ...
               {activityStatus === 'pdf' ? 'Sedang menyusun dokumen PDF...' : 'Sedang melukis gambarmu...'}
             </span>
          </div>
        </div>
      ) : displayContent ? (
        // Kalau statusnya 'idle' dan ada teks, baru tampilin teksnya (atau lagi typing biasa)
      ...
      {/* Tombol aksi JANGAN ditampilin selama lagi bikin PDF/Image */}
      {!isUser && activityStatus === 'idle' && (
      ...
      {/* Suggestion juga sembunyi selama proses PDF/Image */}
      {!isUser && isLast && activityStatus === 'idle' && suggestions && suggestions.length > 0 && (
...
// ── WAJIB DITAMBAHIN BIAR RE-RENDER OTOMATIS PAS STATUS BERUBAH ──
