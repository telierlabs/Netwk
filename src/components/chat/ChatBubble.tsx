        ...
        <div className="flex flex-col gap-2.5 mt-2.5">
          {suggestions.map((s, i) => (
            <button key={i} onClick={() => onSuggest?.(s)}
              ...
              {/* panah Grok: pendek turun → belok → panjang ke kanan */}
              <svg width="20" height="14" viewBox="0 0 20 14" fill="none"
                className="flex-shrink-0 text-[var(--mu)] group-hover/sug:text-[var(--text)] transition-colors"
                stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                {/* turun pendek dari atas */}
                <path d="M4 1 L4 5" />
                {/* belok halus lalu panjang ke kanan */}
                <path d="M4 5 Q4 9 8 9 L17 9" />
                {/* kepala panah */}
                <polyline points="14,6 17,9 14,12" />
              </svg>
              <span className="text-[13.5px] text-[var(--mu)] group-hover/sug:text-[var(--text)] transition-colors leading-snug underline underline-offset-2 decoration-[var(--mu)]/30 group-hover/sug:decoration-[var(--text)]/50">
                {s}
              </span>
            </button>
          ))}
