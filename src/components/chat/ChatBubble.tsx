...
export const ChatBubble: React.FC<ChatBubbleProps> = ({ msg, onResend, onEdit, onSuggest, suggestions }) => {
  ...
  return (
    <div className={cn("flex flex-col group w-full", isUser ? "items-end" : "items-start")}>
      ...
      {/* ── AI action bar — mepet ke teks kayak Grok ── */}
      {!isUser && (
        <div className="flex items-center gap-0 -mt-0.5 -ml-2">
          ...
          <button title="Alternatif" className="p-1.5 rounded-lg text-[var(--mu)] hover:text-[var(--text)] transition-colors">
            <ArrowLeftRight size={17} />
          </button>
        </div>
      )}

      {/* ── Suggestions — mepet ke icon, panah melengkung elegan ── */}
        ...
        <div className="flex flex-col gap-0.5 mt-1 -ml-0.5">
          {suggestions.map((s, i) => (
            <button
              key={i}
              onClick={() => onSuggest?.(s)}
              className="flex items-center gap-1.5 text-left group/sug w-fit max-w-full py-0.5"
            >
              {/* panah melengkung elegan kayak Grok ↳ */}
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none"
                className="flex-shrink-0 text-[var(--mu)] group-hover/sug:text-[var(--text)] transition-colors"
                stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                {/* garis turun */}
                <path d="M3 1 L3 7" />
                {/* lengkungan elegan ke kanan */}
                <path d="M3 7 C3 11.5 5 12 8 12 L12 12" strokeWidth="1.5" />
                {/* kepala panah */}
                <polyline points="9.5,9.5 12,12 9.5,14.5" />
              </svg>
