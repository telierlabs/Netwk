...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({ msg, msgIndex, isLast, onResend, onEdit, onSuggest, onTogglePin, onSaveItem, onRegenerate, onSwipeToReply, suggestions }) => {
  ...
  // AUTO CLOSE MENU KALAU KLIK DI LUAR
  useEffect(() => {
  ...
  return (
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start")}>
      ...
      {msg.content && (
        <div className={cn("msg-bubble rounded-[22px] transform-gpu relative", showUserMenu ? "z-[100]" : "z-10", isUser ? "bg-[var(--sf)] border border-[var(--bd)] text-[var(--text)] rounded-tr-[6px] max-w-[85%] shadow-sm" : "bg-transparent text-[var(--text)] px-0 w-full")}>
          {isUser ? (
            <>
              <motion.div
              ...
              <AnimatePresence>
                {showUserMenu && (
                  <motion.div 
                    ...
                    // ABSOLUTE TOP-FULL: Pasti di bawah chat bubble, rata kanan.
                    className="absolute top-full right-0 mt-2 w-[160px] bg-[var(--bg)] backdrop-blur-xl border border-[var(--bd)] rounded-[16px] shadow-xl overflow-hidden flex flex-col p-1.5"
                  >
