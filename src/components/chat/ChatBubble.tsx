...
// ─── INISIALISASI MERMAID (Di luar komponen biar gak ngulang terus) ───
mermaid.initialize({
  startOnLoad: false,
  theme: 'base', 
  themeVariables: {
...
// ─── MERMAID DIAGRAM RENDERER (DENGAN PENANGKAL ERROR & DEBOUNCE) ───
const MermaidDiagram = memo(({ content }: { content: string }) => {
  ...
  useEffect(() => {
    // FITUR DEBOUNCE: Tunggu 1 detik setelah AI berhenti ngetik baru gambar
    // Ini buat mencegah browser crash / layar putih karena kebanyakan nge-render
    const renderTimer = setTimeout(async () => {
      const cleanContent = content.replace(/```mermaid/gi, '').replace(/```/g, '').trim();
      
      // Jangan render kalau isi kode belum lengkap
      ...
      try {
        ...
        setSvgCode(svg);
        setErrorMsg(''); // Reset error jika sukses
      } catch (err: any) {
        // Tangkap error diam-diam
        setErrorMsg(err?.message || 'Menunggu Cylen selesai mengetik diagram...');
      } finally {
        setIsRendering(false);
      }
    }, 1200); // 1.2 detik jeda

    return () => clearTimeout(renderTimer); // Bersihkan antrian kalau AI ngetik lagi
  }, [content]);
  ...
  // Tampilan layar penuh portal
  const portalContent = isFullscreen ? (
    <div className="fixed inset-0 z-[999999] bg-[#0F0F0F] flex flex-col touch-none animate-in fade-in zoom-in-95 duration-200">
      <div className="flex items-center justify-between p-4 border-b border-white/10 shrink-0 bg-[#0F0F0F]">
        <button onClick={() => setIsFullscreen(false)} className="w-12 h-12 flex items-center justify-center rounded-full text-white hover:bg-white/10 active:scale-95 transition-all">
          <ChevronLeft size={30} strokeWidth={2.5} />
        </button>
        <span className="text-white font-bold tracking-widest uppercase text-[12px]">Diagram Visual</span>
        <button onClick={handleDownload} className="w-12 h-12 flex items-center justify-center rounded-full text-white hover:bg-white/10 active:scale-95 transition-all">
          <Download size={24} strokeWidth={2.5} />
        </button>
      </div>
      <div className="flex-1 w-full h-full overflow-auto flex items-center justify-center p-4">
        {/* SVG di-render di tengah layar */}
        <div dangerouslySetInnerHTML={{ __html: svgCode }} className="max-w-full max-h-full [&>svg]:max-w-full [&>svg]:h-auto pointer-events-none" />
      </div>
    </div>
  ) : null;

  // Kalau masih merender atau ada error sementara
  if (!svgCode || errorMsg) {
    return (
      <div className="my-4 w-full bg-[var(--cd)] border border-[var(--bd)] rounded-2xl p-6 shadow-sm flex flex-col items-center justify-center gap-3 transform-gpu animate-pulse min-h-[150px]">
        ...
        <span className="text-[12px] font-medium text-[var(--mu)]">
  ...
  // Tampilan Sukses Render
  return (
    <>
      <div 
        onClick={() => setIsFullscreen(true)}
        className="my-4 w-full bg-[#1A1A1D] border border-[var(--bd)] rounded-2xl p-6 overflow-hidden shadow-sm transform-gpu flex flex-col items-center cursor-pointer hover:border-[var(--text)]/40 transition-all group relative min-h-[200px]"
      >
        <div dangerouslySetInnerHTML={{ __html: svgCode }} className="max-w-full [&>svg]:max-w-full [&>svg]:h-auto pointer-events-none" />
        
        {/* Label kecil di pojok kanan bawah yang muncul pas dihover/diklik */}
        <div className="absolute bottom-3 right-3 px-3 py-2 bg-white/10 backdrop-blur text-white text-[10px] font-bold rounded-xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5 border border-white/20 shadow-sm">
