...
const MermaidDiagram = memo(({ content }: { content: string }) => {
  ...
  useEffect(() => {
    ...
    const renderDiagram = async () => {
      try {
        ...
        setSvgCode(`<div class="p-4 text-red-500 text-[12px] border border-red-500/30 rounded-xl bg-red-500/10">Gagal memuat diagram. Format tidak valid.</div>`);
      }
    };
    renderDiagram();
  }, [content]);
  ...
  return (
    <div className="my-4 w-full bg-[var(--cd)] border border-[var(--bd)] rounded-2xl p-6 overflow-x-auto shadow-sm transform-gpu flex justify-center items-center">
      <div dangerouslySetInnerHTML={{ __html: svgCode }} className="max-w-full" />
    </div>
  );
});
