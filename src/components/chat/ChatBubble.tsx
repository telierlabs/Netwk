    ...
    <div className={cn("flex flex-col group w-full", isUser ? "items-end" : "items-start")}>
      ...
      {!isUser && (
        <div className="flex items-center gap-1 mt-3">
          ...
          <button onClick={handleCopy} title="Salin"
            className={cn(
              ...
              copied
                ...
                : "text-[var(--mu)] hover:text-[var(--text)] hover:bg-[var(--sf)] hover:border-[var(--bd)]"
            )}>
          ...
          {/* Regenerate */}
          <button title="Ulangi respons"
            className="p-2.5 rounded-xl transition-all border border-transparent text-[var(--mu)] hover:text-[var(--text)] hover:bg-[var(--sf)] hover:border-[var(--bd)]">
            <RotateCcw size={17} />
          </button>

          {/* Compare */}
          <button title="Alternatif jawaban"
            className="p-2.5 rounded-xl transition-all border border-transparent text-[var(--mu)] hover:text-[var(--text)] hover:bg-[var(--sf)] hover:border-[var(--bd)]">
        ...
        <div className="flex flex-col gap-2.5 mt-3">
          {suggestions.map((s, i) => (
            <button
              key={i}
              onClick={() => onSuggest?.(s)}
              className="flex items-center gap-2.5 text-left group/sug w-fit max-w-full"
            >
              {/* panah belok ↳ dari atas turun lalu ke kanan — Grok style */}
              <svg
                width="16" height="16" viewBox="0 0 16 16" fill="none"
                className="flex-shrink-0 text-[var(--mu)] group-hover/sug:text-[var(--ac)] transition-colors mt-0.5"
                stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"
              >
                {/* garis turun dari kiri atas ke bawah */}
                <path d="M4 2 L4 10" />
                {/* lalu belok ke kanan dengan ujung panah */}
                <path d="M4 10 L13 10" />
                <path d="M10 7 L13 10 L10 13" />
              </svg>
              <span className="text-[13.5px] text-[var(--mu)] group-hover/sug:text-[var(--ac)] font-medium underline underline-offset-2 decoration-transparent group-hover/sug:decoration-[var(--ac)]/50 transition-all leading-snug">
                {s}
              </span>
            </button>
          ))}
