...
const CollapsibleUserBubble = memo(({ content, onClick }: { content: string; onClick: () => void }) => {
  ...
  return (
    <div className="w-full relative flex flex-col transform-gpu">
      ...
      <div onPointerDown={(e) => { e.stopPropagation(); onClick(); }} className={cn("px-4 cursor-pointer active:opacity-80 transition-opacity pb-3", replyText ? "pt-1" : "pt-3")}>
    ...
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start")}>
      ...
      {msg.content && (
        <motion.div
          ...
          onDragEnd={(e, info) => { if (Math.abs(info.offset.x) > 60) onSwipeToReply?.(msg); }}
          className={cn("msg-bubble rounded-[22px] transform-gpu will-change-transform", isUser ? "bg-[var(--sf)] border border-[var(--bd)] text-[var(--text)] rounded-tr-[6px] max-w-[85%] shadow-sm relative cursor-grab active:cursor-grabbing" : "bg-transparent text-[var(--text)] px-0 w-full cursor-grab active:cursor-grabbing")}>
          {isUser ? (
            <>
              ...
              <AnimatePresence>
                {showUserMenu && (
                  <>
                    <div className="fixed inset-0 z-[100]" onPointerDown={(e) => { e.stopPropagation(); setShowUserMenu(false); }} />
                    <motion.div onPointerDown={(e) => e.stopPropagation()} initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -10 }} transition={{ duration: 0.15, ease: "easeOut" }} className="absolute right-0 top-full mt-2 z-[110] w-[160px] bg-[var(--bg)]/95 backdrop-blur-xl border border-[var(--bd)] rounded-[16px] shadow-xl overflow-hidden flex flex-col p-1.5">
            ...
            <div className="cylen-md w-full">
              ...
              <Markdown remarkPlugins={[remarkGfm]} components={{
                code({ node, inline, className, children, ...props }: any) {
                  ...
                  // ✅ FIX: Kalau lang = 'text' atau kosong dan isinya singkat (simbol/variabel/rumus pendek),
                  // render sebagai teks biasa inline — bukan blok CodeBlock yang jelek
                  ...
                  // Kalau lang='text' dan isinya pendek (simbol/variabel/label), render inline bukan blok
