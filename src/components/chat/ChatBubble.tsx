...
const CodeBlock = ({ lang, content }: { lang: string; content: string }) => {
  ...
  return (
    <div className="my-3 rounded-2xl overflow-hidden" style={{ background: '#0f1117' }}>
      {/* header bar */}
      <div className="flex items-center justify-between px-4 py-2.5"
        style={{ background: '#161b22', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="flex items-center gap-2">
          {/* colored dot based on lang */}
          <div className="w-2 h-2 rounded-full" style={{
            background: lang === 'html' ? '#e34c26'
              : lang === 'css' ? '#264de4'
              : ['javascript','js','nodejs'].includes(lang) ? '#f7df1e'
              : ['typescript','ts','tsx'].includes(lang) ? '#3178c6'
              : lang === 'python' ? '#3776ab'
              : lang === 'json' ? '#40b3a2'
              : '#6e7681'
          }} />
          <span style={{
            ...
            color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', fontFamily: 'monospace'
          }}>{label}</span>
        </div>
  ...
  return (
    <div className="my-6 bg-white border-2 border-black rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
      <div className="p-4 border-b-2 border-black flex items-center justify-between bg-slate-50">
        <div className="flex items-center gap-2"><FileText size={18} /><span className="font-bold text-sm uppercase tracking-tight">Dokumen Cylen</span></div>
        <button onClick={exportPDF} className="p-2 bg-black text-white rounded-lg hover:scale-110 transition-all"><Download size={16} /></button>
      </div>
...
const GraphRenderer = ({ content }: { content: string }) => {
  try {
    ...
    return <div className="my-4 p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-xs">Gagal merender grafik.</div>;
  }
};

/* ─────────────── HTML PREVIEW — renders like foto 4 & 5 ─────────────── */
const HtmlPreview = ({ content }: { content: string }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  return (
    <div className="my-3 border border-[var(--bd)] rounded-2xl overflow-hidden shadow-sm">
      {/* browser chrome bar */}
      <div className="flex items-center justify-between px-4 py-2.5 bg-[var(--sf)] border-b border-[var(--bd)]">
        <div className="flex items-center gap-2">
          ...
          <div className="ml-2 px-3 py-1 bg-[var(--bg)] border border-[var(--bd)] rounded-md flex items-center gap-1.5" style={{ minWidth: 120 }}>
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--mu)" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
            <span style={{ fontSize: 10, color: 'var(--mu)', fontFamily: 'monospace' }}>preview.cylen</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--mu)' }}>Live Preview</span>
          <button onClick={() => setIsExpanded(p => !p)}
            className="p-1 hover:bg-[var(--bd)] rounded transition-colors text-[var(--mu)]">
...
const CopyCard = ({ content }: { content: string }) => {
  ...
  return (
    <div className="my-4 p-5 bg-[var(--cd)] border border-[var(--bd)] rounded-2xl relative shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center gap-1.5 mt-4 text-[10px] font-bold uppercase tracking-widest text-[var(--mu)]"><Zap size={9} /><span>Cylen AI Card</span></div>
    </div>
  );
};
    ...
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full", isUser ? "items-end" : "items-start")}>
      ...
      {msg.content && (
        <div onClick={() => isUser && setShowUserSheet(true)}
          className={cn("msg-bubble rounded-2xl", isUser
          ...
          {isUser ? (
            ...
            <div className="cylen-md w-full">
              <style>{`
                ...
                .cylen-md code { background:var(--sf); color:var(--ac); padding:0.15em 0.45em; border-radius:5px; font-family:'JetBrains Mono',monospace; font-size:0.875em; border:1px solid var(--bd); }
                ...
                .cylen-md td { padding:10px 14px; border-bottom:1px solid var(--bd); }
            ...
            <div className="flex flex-wrap gap-2 mt-5">
              {msg.sources.map((src, si) => (
                <a key={si} href={src.url} target="_blank" rel="noopener noreferrer"
                  ...
                  <img src={`https://www.google.com/s2/favicons?domain=${new URL(src.url).hostname}&sz=32`} alt="" className="w-4 h-4 rounded-sm"
      ...
      {!isUser && (
        <div className="flex items-center gap-[28px] mt-1 relative">
          ...
          <button title="Ulangi" className="text-[var(--mu)] hover:text-[var(--text)] transition-colors"><RotateCcw size={18} /></button>
          ...
          <button
            onClick={() => onSaveItem?.({ type: 'teks', title: 'Pesan AI Cylen', preview: msg.content.substring(0, 50) + '...', content: msg.content })}
            title="Simpan" className="text-[var(--mu)] hover:text-[var(--text)] transition-colors">
            <Bookmark size={18} />
          </button>
          <div className="relative">
            <button onClick={() => setShowExportMenu(p => !p)} title="Ekspor"
              className="text-[var(--mu)] hover:text-[var(--text)] transition-colors">
              <Download size={18} />
            </button>
            {showExportMenu && (
              <>
      ...
      {!isUser && suggestions && suggestions.length > 0 && (
        <div className="flex flex-col gap-[44px] mt-[44px]">
          {suggestions.map((s, i) => (
            <button key={i} onClick={() => onSuggest?.(s)} className="flex items-center gap-2 text-left group/sug w-fit max-w-full">
      ...
      {isUser && showUserSheet && (
        <AnimatePresence>
          <motion.div key="overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-[150] bg-black/30" onClick={() => setShowUserSheet(false)} />
          <motion.div key="sheet" initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }}
            transition={{ type: 'spring', damping: 28, stiffness: 320 }}
            className="fixed bottom-0 left-0 right-0 z-[160] bg-[var(--bg)] border-t border-[var(--bd)] rounded-t-2xl shadow-2xl pb-8">
            <div className="flex justify-center pt-3 pb-4"><div className="w-10 h-1 rounded-full bg-[var(--bd)]" /></div>
            <div className="px-5 pb-4"><p className="text-[13px] text-[var(--mu)] line-clamp-2 leading-relaxed">{msg.content}</p></div>
            <div className="border-t border-[var(--bd)]" />
            {[
              { icon: <RotateCcw size={18} />, label: 'Kirim Ulang', sub: 'Kirim pesan ini lagi ke AI', action: () => { onResend?.(msg.content); setShowUserSheet(false); }, color: 'text-[var(--text)]' },
              { icon: <Share2 size={18} />, label: 'Edit Pesan', sub: 'Ubah teks lalu kirim ulang', action: () => { onEdit?.(msg.content); setShowUserSheet(false); }, color: 'text-[var(--text)]' },
              { icon: userSheetCopied ? <Check size={18} /> : <Copy size={18} />, label: userSheetCopied ? 'Tersalin!' : 'Salin Pesan', sub: 'Salin teks ke clipboard', action: handleUserSheetCopy, color: userSheetCopied ? 'text-green-500' : 'text-[var(--text)]' },
            ].map((item, i) => (
              <button key={i} onClick={item.action} className="w-full flex items-center gap-4 px-5 py-4 hover:bg-[var(--sf)] active:bg-[var(--bd)] transition-colors text-left">
                <div className={cn("w-9 h-9 rounded-xl bg-[var(--sf)] flex items-center justify-center flex-shrink-0", item.color)}>{item.icon}</div>
                <div><div className={cn("text-[15px] font-semibold", item.color)}>{item.label}</div><div className="text-[12px] text-[var(--mu)]">{item.sub}</div></div>
              </button>
            ))}
            <div className="border-t border-[var(--bd)] mt-2" />
            <button onClick={() => setShowUserSheet(false)} className="w-full flex items-center justify-center gap-2 px-5 py-4 text-[var(--mu)] hover:bg-[var(--sf)] transition-colors text-[14px] font-medium">
              <X size={15} /> Batal
            </button>
          </motion.div>
        </AnimatePresence>
