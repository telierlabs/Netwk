...
const MermaidDiagram = memo(({ content }: { content: string }) => {
  ...
  useEffect(() => {
    const isDarkMode = document.documentElement.classList.contains('dark') || 
                       getComputedStyle(document.documentElement).getPropertyValue('--bg').includes('#0F0F0F');
    ...
    const renderTimer = setTimeout(async () => {
      ...
      try {
        ...
        // Hapus hardcode width/height dari Mermaid biar gak kaku
  ...
  // LOGIKA ZOOM LICIN 100% (DOM MANIPULATION)
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (e.touches.length === 1 && lastTouchRef.current.distance === 0) {
      ...
      if (lastTouchRef.current.distance > 0) {
        ...
        newScale = Math.min(Math.max(newScale, 0.3), 5); // Range Zoom Aman
  ...
  if (!svgCode || errorMsg) {
    return (
      <div className="my-4 w-full bg-[var(--cd)] border border-[var(--bd)] rounded-2xl p-6 shadow-sm flex flex-col items-center justify-center gap-3 transform-gpu animate-pulse min-h-[150px]">
        <div className="w-8 h-8 rounded-full border-2 border-[var(--mu)] border-t-[var(--text)] animate-spin" />
        <span className="text-[12px] font-medium text-[var(--mu)] text-center">
          {errorMsg && !isRendering ? 'Sintaks tidak lengkap. Menunggu kode utuh...' : 'Menyiapkan diagram visual...'}
        </span>
      </div>
    );
  }

  // PORTAL FULLSCREEN (BUAT ZOOM LICIN & BACA JELAS)
  const portalContent = isFullscreen ? (
    <div className="fixed inset-0 z-[999999] bg-[var(--bg)] flex flex-col animate-in fade-in zoom-in-95 duration-200">
      ...
      <div className="flex items-center justify-between p-4 border-b border-[var(--bd)] shrink-0 bg-[var(--bg)] relative z-20 shadow-sm">
        ...
        <span className="text-[var(--text)] font-bold tracking-widest uppercase text-[12px]">Diagram Visual</span>
      ...
      {/* Lahan Kerja Jari - Native Touch-None biar gak nyangkut scroll HP */}
      <div 
      ...
      >
        ...
        >
          {/* Lebar dipaksa gede (min-w-[1000px]) biar Mermaid render teksnya besar dan gak tumpuk ke bawah */}
          <div 
            dangerouslySetInnerHTML={{ __html: svgCode }} 
            className="p-8 [&>svg]:!max-w-none [&>svg]:min-w-[1000px] [&>svg]:w-max [&>svg]:h-auto [&_text]:!fill-[var(--text)] [&_text]:!font-bold [&_text]:!font-sans [&_.node_rect]:!fill-[var(--sf)] [&_.node_rect]:!stroke-[var(--bd)] [&_.node_polygon]:!fill-[var(--sf)] [&_.node_polygon]:!stroke-[var(--bd)] [&_.edgePath_.path]:!stroke-[var(--mu)] [&_.edgePath_.path]:!stroke-width-[1.8px] [&_.marker]:!fill-[var(--mu)]" 
          />
        </div>
      </div>
    </div>
  ...
  return (
    <>
      {/* KOTAK DI CHAT: DIEM, GAK BISA GESER, NGIKUTIN UKURAN KOTAK */}
      <div 
        onClick={() => setIsFullscreen(true)}
        className="my-4 w-full bg-[var(--cd)] border border-[var(--bd)] rounded-2xl overflow-hidden shadow-sm transform-gpu flex flex-col items-center justify-center cursor-pointer hover:border-[var(--text)]/40 transition-all group relative min-h-[150px] p-4"
      >
        {/* SVG di-scale 100% biar pas kotak, jadi thumbnail */}
        <div 
          dangerouslySetInnerHTML={{ __html: svgCode }} 
          className="w-full flex justify-center pointer-events-none [&>svg]:w-full [&>svg]:max-w-full [&>svg]:h-auto [&_text]:!fill-[var(--text)] [&_text]:!font-bold [&_text]:!font-sans [&_.node_rect]:!fill-[var(--sf)] [&_.node_rect]:!stroke-[var(--bd)] [&_.node_polygon]:!fill-[var(--sf)] [&_.node_polygon]:!stroke-[var(--bd)] [&_.edgePath_.path]:!stroke-[var(--mu)] [&_.edgePath_.path]:!stroke-width-[1.8px] [&_.marker]:!fill-[var(--mu)]" 
        />
