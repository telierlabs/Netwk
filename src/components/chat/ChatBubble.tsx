    ...
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start")}>
      ...
      {msg.content && (
        <motion.div
          ...
          {/* 🔥 INI DIA RENDERER SUMBER (LOGO) YANG GUA PERBAIKI TOTAL 🔥 */}
          {msg.sources && msg.sources.length > 0 && (
            <div className="flex flex-wrap gap-2.5 mt-5">
              {msg.sources.map((src, si) => {
                ...
                // Nampilin nama medianya, bukan cuma URL kosong
                const displayName = src.title && src.title !== 'Referensi' && src.title !== 'Sumber' 
                  ? src.title 
                  : domain;

                // Logo Fallback UI-Avatars (Buat jaga-jaga kalau Google Favicon nge-bug)
