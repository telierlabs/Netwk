  ...
  return (
    <>
      <div className="w-full mb-2 mt-1">
        ...
        {/* ─── JIKA 1 GAMBAR: Proporsional ─── */}
        {images.length === 1 && (
          <div onClick={() => openViewer(0)} className="w-fit max-w-[85vw] sm:max-w-[500px] rounded-2xl overflow-hidden cursor-pointer shadow-sm border border-[var(--bd)]/20 relative bg-[var(--sf)]/20 inline-block">
            <img src={images[0]} alt="Uploaded" className="w-full h-auto max-h-[500px] object-contain" />
            <div className="absolute inset-0 bg-black/0 hover:bg-black/10 transition-colors" />
          </div>
        )}

        {/* ─── JIKA 2 GAMBAR: Grid 2 Kolom proporsional sama tinggi ─── */}
        {images.length === 2 && (
          <div className="grid grid-cols-2 gap-1.5 rounded-2xl overflow-hidden shadow-sm border border-[var(--bd)]/20 bg-[var(--sf)]/20 w-[85vw] sm:w-[500px] max-w-full h-[250px] sm:h-[300px]">
            {images.map((src, i) => (
        ...
        {/* ─── JIKA 3 GAMBAR: Grid 1 Atas 2 Bawah ─── */}
        {images.length === 3 && (
          <div className="grid grid-cols-2 grid-rows-2 gap-1.5 rounded-2xl overflow-hidden shadow-sm border border-[var(--bd)]/20 bg-[var(--sf)]/20 w-[85vw] sm:w-[500px] max-w-full h-[300px] sm:h-[400px]">
            {images.map((src, i) => (
        ...
        <div ref={bubbleRef} className={cn("msg-bubble rounded-[22px] transform-gpu relative z-10", isUser ? "bg-[var(--sf)] border border-[var(--bd)] text-[var(--text)] rounded-tr-[6px] max-w-[85%] shadow-sm" : "bg-transparent text-[var(--text)] px-0 w-full")}>
          {isUser ? (
            <>
              ...
              <AnimatePresence>
                {showUserMenu && (
                  <motion.div onClick={(e) => e.stopPropagation()} initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -10 }} transition={{ duration: 0.15 }} className="absolute z-[150] w-[160px] bg-[var(--bg)] backdrop-blur-xl border border-[var(--bd)] rounded-[16px] shadow-xl flex flex-col p-1.5" style={{ top: userMenuPos.top, right: userMenuPos.right }}>
