...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({ msg, msgIndex, isLast, onResend, onEdit, onSuggest, onTogglePin, onSaveItem, onRegenerate, onSwipeToReply, suggestions }) => {
  ...
  const { docsToRender } = React.useMemo(() => {
      const docs: { name: string; data?: string; url?: string }[] = [];

      const checkFileItem = (item: any) => {
          if (!item) return;

          let fileDetails: { name: string; data?: string; url?: string } | null = null;

          // Case 1: Item is an object
          if (typeof item === 'object') {
              const name = item.name || item.filename || item.title || 'Dokumen';
              const url = item.url || item.path || (typeof item === 'string' && item);
              const data = item.data || item.base64 || item.base64Data || item.content;

              // Ensure at least one of url or data is present
              if (url || data) {
                  fileDetails = { name, data, url };
              }
          ...
          // Case 2: Item is a string
              ...
              if (item.startsWith('data:application/pdf') || item.startsWith('data:text/') || item.endsWith('.pdf') || item.endsWith('.txt')) {
                  fileDetails = { name: 'Dokumen Terlampir', url: item };
              }
          }

          if (fileDetails) {
              docs.push(fileDetails);
          }
      };

      // 1. Ekstrak dari property asli (pdfs/files/attachments)
      if (msg.pdfs && Array.isArray(msg.pdfs)) {
          msg.pdfs.forEach(checkFileItem);
      } else {
          checkFileItem(msg.pdfs);
      }

      if ((msg as any).files && Array.isArray((msg as any).files)) {
          (msg as any).files.forEach(checkFileItem);
      }

      if ((msg as any).attachments && Array.isArray((msg as any).attachments)) {
          (msg as any).attachments.forEach(checkFileItem);
      }

      return { docsToRender: docs };
  }, [msg]);
