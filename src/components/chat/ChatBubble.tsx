...
interface ChatBubbleProps {
  // Ditambah properti opsional khusus untuk mendeteksi loading
  msg: Message & {
    isLoading?: boolean;
    loadingType?: 'normal' | 'web';
    searchQueries?: string[]; // teks/judul riil yang lagi dicari di web
  };
...
/* ─────────────── LOADING STATES (BARU) ─────────────── */
const NeuralPulseLoading = () => (
  <div className="flex items-center gap-1.5 h-8 px-2 py-1">
    {[1, 2, 3, 4, 5].map((i) => (
      <motion.div
        key={i}
        className="w-1 bg-[var(--mu)] rounded-full"
...
const WebSearchLoading = ({ queries }: { queries?: string[] }) => {
  const displayQueries = queries && queries.length > 0 ? queries : ['Mencari informasi relevan...'];
  
  return (
    <div className="flex flex-col gap-3 py-2 w-full">
      <span className="text-[11px] font-bold text-[var(--mu)] uppercase tracking-widest mb-1">Mencari di Web</span>
      <div className="flex flex-col gap-2.5">
        {displayQueries.map((q, i) => (
          <div key={i} className="flex items-center gap-3 bg-[var(--sf)] border border-[var(--bd)] px-3 py-2.5 rounded-xl w-fit max-w-full shadow-sm">
            <div className="w-5 h-5 flex items-center justify-center flex-shrink-0 text-[var(--mu)]">
              {i % 2 === 0 ? (
            ...
            <span className="text-[13px] text-[var(--text)] font-medium line-clamp-2">{q}</span>
          </div>
        ))}
      </div>
      <div className="flex items-center gap-2 mt-3">
        <motion.div
          ...
          className="w-3.5 h-3.5 border-2 border-[var(--ac)] border-t-transparent rounded-full"
        />
        <motion.span
          animate={{ opacity: [0.4, 1, 0.4] }}
          transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }}
          className="text-[10px] font-bold text-[var(--ac)] uppercase tracking-widest"
        >
          Membaca Sumber...
        </motion.span>
      </div>
  ...
  return (
    <div className={cn("flex flex-col group w-full", isUser ? "items-end" : "items-start")}>
      ...
      {(msg.content || msg.isLoading) && (
        <div
          ...
          {isUser ? (
            ...
            /* ── assistant: loading state (Neural Pulse / Web Search) ── */
            msg.loadingType === 'web' ? (
              <WebSearchLoading queries={msg.searchQueries} />
            ) : (
              <NeuralPulseLoading />
            )
          ) : msg.content ? (
      ...
      {!isUser && !msg.isLoading && msg.content && (
        <div className="flex items-center gap-[28px] mt-1">
          ...
          <button title="Bagikan" className="text-[var(--mu)] hover:text-[var(--text)] transition-colors">
            <Share2 size={18} />
          </button>
          ...
          <button title="Ulangi" className="text-[var(--mu)] hover:text-[var(--text)] transition-colors">
            <RotateCcw size={18} />
          </button>
        </div>
      )}
      ...
      {/* ── Suggestions ── */}
      {!isUser && !msg.isLoading && suggestions && suggestions.length > 0 && (
        <div className="flex flex-col gap-[44px] mt-[44px]">
          {suggestions.map((s, i) => (
            <button key={i} onClick={() => onSuggest?.(s)}
              ...
              <span className="text-[15px] text-[var(--mu)] group-hover/sug:text-[var(--text)] transition-colors leading-snug">
                {s}
              </span>
            </button>
          ))}
      ...
      {isUser && showUserSheet && msg.content && (
        <AnimatePresence>
          <motion.div
            key="overlay"
            initial={{ opacity: 0 }}
