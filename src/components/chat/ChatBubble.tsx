...
// ─── 🚀 SMART IMAGE GALLERY (EXACTLY LIKE CHATGPT) ───
const SmartImageGallery = memo(({ images }: { images: string[] }) => {
  ...
  return (
    <>
      {/* ─── TAMPILAN DI DALAM CHAT (UKURAN DIPERBESAR FULL WIDTH) ─── */}
      <div className="w-full mb-2 mt-1" style={{ maxWidth: '85vw' }}> 
        
        {/* JIKA 1 GAMBAR: Memanjang lebar, ukuran bebas menyesuaikan gambar */}
        {images.length === 1 && (
          <div onClick={() => openViewer(0)} className="w-full rounded-2xl overflow-hidden cursor-pointer shadow-sm border border-[var(--bd)]/20 relative group bg-[var(--sf)]">
            <img src={images[0]} alt="Uploaded" className="w-full h-auto max-h-[600px] object-cover" />
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
          </div>
        )}

        {/* JIKA 2 GAMBAR: Dibagi rata 2 kolom, ditarik full */}
        {images.length === 2 && (
          <div className="flex w-full gap-1.5 rounded-2xl overflow-hidden shadow-sm border border-[var(--bd)]/20 bg-[var(--sf)]">
            {images.map((src, i) => (
              <div key={i} onClick={() => openViewer(i)} className="flex-1 relative cursor-pointer group">
                <img src={src} alt={`Uploaded ${i+1}`} className="w-full h-[250px] object-cover sm:h-[300px]" />
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
              </div>
            ))}
          </div>
        ...
        {/* JIKA 3 GAMBAR: 1 di atas panjang, 2 di bawah */}
        {images.length === 3 && (
          <div className="grid grid-cols-2 gap-1.5 rounded-2xl overflow-hidden shadow-sm border border-[var(--bd)]/20 bg-[var(--sf)] w-full">
            {images.map((src, i) => (
              <div key={i} onClick={() => openViewer(i)} className={cn("relative cursor-pointer group", i === 0 ? "col-span-2 h-[200px] sm:h-[250px]" : "col-span-1 h-[150px] sm:h-[180px]")}>
        ...
        {/* JIKA 4+ GAMBAR: Grid 2x2 yang lega */}
        {images.length >= 4 && (
          <div className="grid grid-cols-2 gap-1.5 rounded-2xl overflow-hidden shadow-sm border border-[var(--bd)]/20 bg-[var(--sf)] w-full">
            {images.slice(0, 4).map((src, i) => (
              <div key={i} onClick={() => openViewer(i)} className="relative cursor-pointer group h-[150px] sm:h-[180px]">
                <img src={src} alt={`Uploaded ${i+1}`} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
                {i === 3 && images.length > 4 && (
