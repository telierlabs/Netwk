...
const SmartImageGallery = memo(({ images }: { images: string[] }) => {
  ...
  return (
    <>
      {/* ─── TAMPILAN DI DALAM CHAT (Gak Kotak Mini, Tapi Gede & Proporsional) ─── */}
      <div className="w-full max-w-[420px] mb-2 mt-1">
        
        {images.length === 1 && (
          <div onClick={() => openViewer(0)} className="w-full rounded-2xl overflow-hidden cursor-pointer shadow-sm border border-[var(--bd)]/20 relative group">
            {/* object-cover dimatikan, dibikin proporsional (height nyesuain gambar aslinya) */}
        ...
        {images.length === 2 && (
          <div className="flex w-full gap-1.5 rounded-2xl overflow-hidden shadow-sm border border-[var(--bd)]/20">
            {images.map((src, i) => (
              <div key={i} onClick={() => openViewer(i)} className="flex-1 relative cursor-pointer group">
                <img src={src} alt={`Uploaded ${i+1}`} className="w-full h-auto min-h-[150px] max-h-[350px] object-cover" />
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
              </div>
            ))}
          </div>
        ...
        {images.length >= 3 && (
          <div className="grid grid-cols-2 gap-1.5 rounded-2xl overflow-hidden shadow-sm border border-[var(--bd)]/20">
            {images.map((src, i) => (
              <div key={i} onClick={() => openViewer(i)} className={cn("relative cursor-pointer group", images.length === 3 && i === 0 ? "col-span-2" : "col-span-1")}>
                <img src={src} alt={`Uploaded ${i+1}`} className="w-full aspect-square object-cover" />
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
              </div>
            ))}
          </div>
      ...
      <AnimatePresence>
        {viewerOpen && (
          <motion.div 
            ...
            // z-[99999] memastikan nutupin SEMUANYA, termasuk header & chat input
            className="fixed inset-0 z-[99999] bg-[#000000] flex flex-col items-center justify-center touch-none"
          >
            {/* TOP BAR ACTIONS (Kayak ChatGPT: X di kiri, Download & Titik Tiga di kanan) */}
            <div className="absolute top-0 left-0 w-full p-4 md:p-6 flex items-center justify-between z-50 bg-gradient-to-b from-black/60 to-transparent">
              ...
              <button 
                ...
                className="w-11 h-11 flex items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20 transition-all backdrop-blur-md"
              >
            ...
            {/* GAMBAR FULLSCREEN (object-contain biar ga kepotong sama sekali) */}
            ...
            {/* DOTS INDICATOR BAWAH (Kayak di ChatGPT) */}
            {images.length > 1 && (
              <div className="absolute bottom-10 left-0 right-0 flex justify-center items-center gap-2 z-50">
                {images.map((_, i) => (
                  <div 
                    key={i} 
                    className={cn("w-2 h-2 rounded-full transition-all duration-300", currentIndex === i ? "bg-white scale-125" : "bg-white/30")}
                  />
                ))}
              </div>
            )}
