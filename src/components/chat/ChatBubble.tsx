...
export const ChatBubble: React.FC<ChatBubbleProps> = ({ msg, onResend, onEdit, onSuggest, suggestions }) => {
  ...
  return (
    <div className={cn("flex flex-col group w-full", isUser ? "items-end" : "items-start")}>
      ...
      {/* ── AI action bar — Grok style, nempel ke teks ── */}
      {!isUser && (
        <div className="flex items-center gap-0 mt-1">
          ...
          <button onClick={handleCopy} title="Salin"
            className={cn(
              ...
              copied ? "text-green-500" : "text-[var(--mu)] hover:text-[var(--text)]"
            )}>
          ...
          <button title="Bagikan"
            className="p-2 rounded-lg text-[var(--mu)] hover:text-[var(--text)] transition-all">
            <Share2 size={18} />
          </button>
          ...
          <button onClick={() => setLiked(l => l === 'up' ? null : 'up')} title="Suka"
            className={cn(
              "p-2 rounded-lg transition-all",
              liked === 'up' ? "text-blue-500" : "text-[var(--mu)] hover:text-[var(--text)]"
            )}>
            <ThumbsUp size={18} />
          </button>
          ...
          <button onClick={() => setLiked(l => l === 'down' ? null : 'down')} title="Tidak Suka"
            className={cn(
              "p-2 rounded-lg transition-all",
              liked === 'down' ? "text-red-400" : "text-[var(--mu)] hover:text-[var(--text)]"
            )}>
            <ThumbsDown size={18} />
          </button>
      ...
      {/* ── Suggested follow-up — Grok style ↳ ── */}
          ...
          {suggestions.map((s, i) => (
            ...
            >
              ...
              >
                ...
                {/* sudut belok lalu lurus ke kanan */}
                <path d="M3 9 Q3 13 7 13 L13 13" />
                {/* kepala panah */}
                <polyline points="10.5,10.5 13,13 10.5,15.5" />
              </svg>
              <span className="text-[13.5px] text-[var(--mu)] group-hover/sug:text-[var(--text)] font-normal transition-colors leading-snug underline underline-offset-2 decoration-[var(--mu)]/30 group-hover/sug:decoration-[var(--text)]/40">
                {s}
              </span>
            </button>
          ))}
