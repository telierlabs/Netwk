...
// ─── MERMAID DIAGRAM RENDERER (PERFECT UX) ───
const MermaidDiagram = memo(({ content }: { content: string }) => {
  ...
  // Reference DOM murni biar zoom licin 60FPS
  const transformRef = useRef({ x: 0, y: 0, scale: 1 });
  const contentRef = useRef<HTMLDivElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  
  // State untuk drag/pan
  const lastTouchRef = useRef<{ distance: number; x: number; y: number } | null>(null);
  const isDraggingRef = useRef(false);

  useEffect(() => {
    // Config Cylen: Clean, Matte Black & White layout logic
    mermaid.initialize({
      startOnLoad: false,
      theme: 'base',
      themeVariables: {
        ...
        primaryBorderColor: 'var(--bd)',
        lineColor: 'var(--text)',
        secondaryColor: 'var(--sf)',
    ...
    const renderTimer = setTimeout(async () => {
      ...
      try {
        ...
        // Membersihkan hardcoded width/height biar natural
        svg = svg.replace(/max-width: ?[\d.]+px;?/, '');
  ...
  // LOGIKA ZOOM & PAN LICIN (DOM MANIPULATION)
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (e.touches.length === 1 && lastTouchRef.current.distance === 0) {
      ...
      if (lastTouchRef.current.distance > 0) {
        ...
        newScale = Math.min(Math.max(newScale, 0.2), 5); // Range Zoom Aman
  ...
  // Attach native wheel event untuk mencegah warning passive event di React
  useEffect(() => {
  ...
  if (!svgCode || errorMsg) {
    return (
      <div className="my-4 w-full bg-[var(--cd)] border border-[var(--bd)] rounded-2xl p-6 shadow-sm flex flex-col items-center justify-center gap-3 transform-gpu animate-pulse min-h-[120px]">
        <div className="w-6 h-6 rounded-full border-2 border-[var(--mu)] border-t-[var(--text)] animate-spin" />
        <span className="text-[12px] font-medium text-[var(--mu)] text-center">
          {errorMsg && !isRendering ? 'Sintaks tidak lengkap. Menunggu kode...' : 'Menyiapkan diagram visual...'}
        </span>
      </div>
    );
  }

  // PORTAL FULLSCREEN (GELAP DI ATAS BAWAH, FOKUS DI TENGAH)
  const portalContent = isFullscreen ? (
    <div className="fixed inset-0 z-[999999] bg-[var(--bg)] flex flex-col animate-in fade-in zoom-in-95 duration-200">
      {/* Header Gelap Kontras */}
      <div className="flex items-center justify-between p-4 border-b border-[var(--bd)] shrink-0 bg-[#050505] relative z-20 shadow-sm">
        <button onClick={handleCloseFullscreen} className="w-10 h-10 flex items-center justify-center rounded-full text-white hover:bg-white/10 active:scale-95 transition-all">
          <ChevronLeft size={28} strokeWidth={2.5} />
        </button>
        <span className="text-white font-bold tracking-widest uppercase text-[12px]">Diagram Visual</span>
        <button onClick={handleDownload} className="w-10 h-10 flex items-center justify-center rounded-full text-white hover:bg-white/10 active:scale-95 transition-all">
          <Download size={22} strokeWidth={2.5} />
        </button>
      </div>
      
      {/* Lahan Kerja Jari & Mouse */}
      <div 
        ref={containerRef}
      ...
      >
        ...
        >
          {/* Tampilan Natural, nggak dipaksa melar */}
          <div 
            dangerouslySetInnerHTML={{ __html: svgCode }} 
            className="p-8 [&>svg]:max-w-none [&>svg]:w-auto [&>svg]:h-auto [&_text]:!fill-[var(--text)] [&_text]:!font-semibold [&_text]:!font-sans [&_.node_rect]:!stroke-[var(--bd)] [&_.node_rect]:!stroke-[1.5px] [&_.node_polygon]:!stroke-[var(--bd)] [&_.edgePath_.path]:!stroke-[var(--mu)] [&_.edgePath_.path]:!stroke-width-[1.8px] [&_.marker]:!fill-[var(--mu)]" 
          />
        </div>
      </div>
      
      {/* Footer Gelap (Opsional buat estetika) */}
      <div className="h-12 bg-[#050505] border-t border-[var(--bd)] shrink-0 flex items-center justify-center pointer-events-none">
        <span className="text-[10px] text-white/50 font-mono tracking-widest">CYLEN INTERACTIVE VIEW</span>
      </div>
    </div>
  ) : null;
  ...
  return (
    <>
      {/* KOTAK DI CHAT: PROPORSI PAS, GAK MELUBER */}
      <div 
        onClick={() => setIsFullscreen(true)}
        className="my-4 w-full bg-[var(--cd)] border border-[var(--bd)] rounded-2xl overflow-hidden shadow-sm transform-gpu flex flex-col items-center justify-center cursor-pointer hover:border-[var(--text)]/40 transition-all group relative p-4 max-h-[300px]"
      >
        {/* SVG Skala Proporsional */}
        <div 
          dangerouslySetInnerHTML={{ __html: svgCode }} 
          className="w-full h-full flex justify-center pointer-events-none overflow-hidden [&>svg]:w-full [&>svg]:max-w-[100%] [&>svg]:h-auto [&>svg]:max-h-[250px] [&_text]:!fill-[var(--text)] [&_text]:!font-semibold [&_text]:!font-sans [&_.node_rect]:!stroke-[var(--bd)] [&_.node_polygon]:!stroke-[var(--bd)] [&_.edgePath_.path]:!stroke-[var(--mu)] [&_.edgePath_.path]:!stroke-width-[1.5px] [&_.marker]:!fill-[var(--mu)]" 
        />
        <div className="absolute bottom-3 right-3 px-3 py-2 bg-[var(--sf)]/90 backdrop-blur text-[var(--text)] text-[10px] font-bold rounded-xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5 border border-[var(--bd)] shadow-sm">
...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({
  ...
  // ── JURUS JITU: POTONG KOMPAS JIKA LAGI LOADING FILE/GAMBAR ──
  ...
  // ─── LOGIKA DETEKSI SEMUA LINK FILE ───
