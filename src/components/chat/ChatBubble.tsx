          ...
          {isUser ? (
            ...
            <motion.div drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.1} onDragEnd={(_e, info) => { if (info.offset.x > 55) onSwipeToReply?.(msg); }} className="w-full">
              <div className="cylen-md w-full" style={{ userSelect: "text", WebkitUserSelect: "text" }}>
                {/* ─── STYLE BLOCK — HANYA TAMBAHAN 6 BARIS TABEL DI BAWAH, SISANYA TIDAK DIUBAH ─── */}
                <style>{`
                  ...
                  .cylen-md table { display:block; overflow-x:auto; -webkit-overflow-scrolling:touch; border-collapse:collapse; width:max-content; max-width:100%; margin:1rem 0; border-radius:12px; }
                  .cylen-md thead tr { background:var(--sf); }
                  .cylen-md th { padding:8px 16px; font-size:11px; font-weight:700; text-align:left; color:var(--mu); text-transform:uppercase; letter-spacing:0.06em; border-bottom:2px solid var(--bd); white-space:nowrap; }
