      ...
      <AnimatePresence>
        {viewerOpen && (
          <motion.div 
            ...
            className="fixed inset-0 z-[99999] bg-[#000000] flex flex-col items-center justify-center touch-none pointer-events-auto"
          >
            <div className="absolute top-0 left-0 w-full p-4 md:p-6 flex items-center justify-between z-50 bg-gradient-to-b from-black/60 to-transparent">
              <button onClick={() => setViewerOpen(false)} className="w-11 h-11 flex items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20 transition-all backdrop-blur-md">
                <X size={24} strokeWidth={2.5} />
              </button>
              <div className="flex items-center gap-3">
                <button onClick={(e) => downloadImage(e, images[currentIndex])} className="w-11 h-11 flex items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20 transition-all backdrop-blur-md">
                  <Download size={22} strokeWidth={2.5} />
                </button>
                <button className="w-11 h-11 flex items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20 transition-all backdrop-blur-md">
            ...
            <div className="w-full h-full flex items-center justify-center p-2 relative">
              <motion.img 
                ...
                src={images[currentIndex]} alt="Fullscreen Preview" className="max-w-full max-h-[95vh] object-contain"
              ...
              {images.length > 1 && currentIndex > 0 && (
                <button onClick={handlePrev} className="hidden md:flex absolute left-6 w-12 h-12 bg-white/10 hover:bg-white/20 rounded-full items-center justify-center text-white backdrop-blur-md transition-colors z-50">
                  <ChevronLeft size={28} />
                </button>
              )}
              {images.length > 1 && currentIndex < images.length - 1 && (
                <button onClick={handleNext} className="hidden md:flex absolute right-6 w-12 h-12 bg-white/10 hover:bg-white/20 rounded-full items-center justify-center text-white backdrop-blur-md transition-colors z-50">
            ...
            {images.length > 1 && (
              <div className="absolute bottom-10 left-0 right-0 flex justify-center items-center gap-2 z-50">
                {images.map((_, i) => (
...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({ msg, msgIndex, isLast, onResend, onEdit, onSuggest, onTogglePin, onSaveItem, onRegenerate, onSwipeToReply, suggestions }) => {
  ...
  const openUserMenu = (e: React.MouseEvent) => {
    ...
    if (bubbleRef.current) {
        ...
        let targetRight = window.innerWidth - rect.right;
        if (targetTop + 150 > window.innerHeight) targetTop = rect.top - 150 - 8;
  ...
  // 🚀 EKSTRAKSI DOKUMEN SUPER AGRESIF
  const { docsToRender, imagesToRender, displayContent } = React.useMemo(() => {
      const docs: { name: string; data?: string; url?: string }[] = [];
      const imgs: string[] = [];
      let text = msg.content || '';

      // 1. Tangkap foto/gambar dari msg.images
      const checkImageItem = (item: any) => {
          if (!item) return;
          if (typeof item === 'string') imgs.push(item);
          else if (typeof item === 'object' && item.url) imgs.push(item.url);
      };
      if (msg.images) Array.isArray(msg.images) ? msg.images.forEach(checkImageItem) : checkImageItem(msg.images);
      if (msg.image) checkImageItem(msg.image);

      // 2. Tangkap dokumen dari properti object msg
      const checkFileItem = (item: any) => {
          if (!item) return;
          if (typeof item === 'object') {
              const name = item.name || item.filename || item.title || 'Dokumen Terlampir';
              const url = item.url || item.path || (typeof item.data === 'string' && item.data.startsWith('http') ? item.data : undefined);
              const data = item.data || item.base64 || item.base64Data || item.content;

              if (url || data) {
                  if (!docs.some(d => d.name === name && d.data === data && d.url === url)) docs.push({ name, data, url });
              }
          } else if (typeof item === 'string') {
              if (item.startsWith('data:application/pdf') || item.startsWith('data:text/') || item.endsWith('.pdf') || item.endsWith('.txt')) {
                  if (!docs.some(d => d.url === item)) docs.push({ name: 'Dokumen Terlampir', url: item });
              }
          }
      };

      const fileKeys = ['pdfs', 'pdf', 'files', 'file', 'attachments', 'attachment', 'docs', 'doc', 'documents', 'document'];
      fileKeys.forEach(key => {
          if ((msg as any)[key]) {
              const val = (msg as any)[key];
              if (Array.isArray(val)) val.forEach(checkFileItem);
              else checkFileItem(val);
          }
      });

      // 3. Tangkap dokumen yang nyelip di teks (Markdown Link)
      const mdLinkRegex = /\[([^\]]+)\]\((.*?)\)/g;
      text = text.replace(mdLinkRegex, (match, name, url) => {
          // Kalau url kosong (yang bikin muncul "()"), atau link base64, kita ekstrak jadi Kapsul PDF
          if (!url || url.trim() === '' || url.startsWith('data:') || name.endsWith('.pdf') || name.endsWith('.txt') || name.includes('CARAM') || name.length > 5) {
              if (!docs.some(d => d.name === name)) docs.push({ name, url });
              return ''; // Hapus link markdown dari teks biar bersih
          }
          return match;
      });

      // Hapus fallback teks dari backend
      if (text.includes('[File kode dilampirkan]')) {
          if (!docs.some(d => d.name === 'Kode_Panjang.txt')) docs.push({ name: 'Kode_Panjang.txt' });
          text = text.replace('[File kode dilampirkan]', '');
      }

      // Bersihkan teks dari sisa "()" yang ditinggalin markdown kosong
      text = text.replace(/^\s*\(\s*\)\s*$/gm, '').trim();

      return { docsToRender: docs, imagesToRender: imgs, displayContent: text };
  }, [msg]);
  ...
  const handleDocClick = (doc: any) => {
    if (doc.url && doc.url.startsWith('http')) {
        ...
        const link = document.createElement('a');
        link.href = doc.data.startsWith('http') || doc.data.startsWith('data:') ? doc.data : `data:application/pdf;base64,${doc.data}`;
        link.download = doc.name || 'document.pdf';
        link.click();
    }
  };
  ...
  return (
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start", showUserMenu ? "z-[100]" : "z-10")}>
      ...
      {/* 🚀 RENDER PDF/FILE SEBAGAI KAPSUL PANJANG (DI ATAS CHAT USER) */}
        ...
        <div className={cn("flex flex-col gap-2 mb-2 w-full", isUser ? "items-end" : "items-start")}>
          {docsToRender.map((doc, i) => (
             <div 
               ...
               className={cn(
                 "flex items-center gap-3.5 px-4 py-2.5 bg-[var(--sf)] border border-[var(--bd)] rounded-[20px] shadow-sm cursor-pointer hover:bg-[var(--bd)]/50 transition-colors max-w-[85%]", 
             ...
             >
               ...
               <div className="flex flex-col min-w-0 flex-1">
                 <span className="text-[14px] font-bold text-[var(--text)] truncate">
                   {doc.name || 'Dokumen Terlampir'}
                 </span>
                 <span className="text-[11px] font-medium text-[var(--mu)] uppercase tracking-wider mt-0.5">
                   File PDF / Teks
                 </span>
               </div>
             </div>
      ...
      {/* RENDER SMART GALLERY JIKA ADA GAMBAR */}
      {imagesToRender.length > 0 && (
        <SmartImageGallery images={imagesToRender} isUser={isUser} />
      )}

      {/* BUBBLE CHAT TEXT (Ditampilkan HANYA kalau teks nggak kosong) */}
      {displayContent && (
        <div ref={bubbleRef} className={cn("msg-bubble rounded-[22px] transform-gpu relative z-10", isUser ? "bg-[var(--sf)] border border-[var(--bd)] text-[var(--text)] rounded-tr-[6px] max-w-[85%] shadow-sm" : "bg-transparent text-[var(--text)] px-0 w-full")}>
          {isUser ? (
            <>
              ...
              <AnimatePresence>
                {showUserMenu && (
                  <motion.div 
                    ...
                    // ABSOLUTE Nempel di sudut bubble, GAK BAKAL KEMANA-MANA WALAU DI-SCROLL
