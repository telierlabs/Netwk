...
const CollapsibleUserBubble = ({ content, onClick }: { content: string; onClick: () => void }) => {
  ...
  return (
    <div className="w-full relative">
      <div onClick={onClick} className="px-4 pt-3 cursor-pointer active:opacity-80 transition-opacity pb-2">
...
export const ChatBubble: React.FC<ChatBubbleProps> = ({
  ...
  const [showUserMenu, setShowUserMenu] = useState(false); // Ganti nama state biar lebih cocok
    ...
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative", isUser ? "items-end" : "items-start")}>
      ...
      {msg.content && (
        <div
          ...
          {isUser ? (
            <>
              {/* KLIK BUBBLE BUAT MUNCULIN MENU */}
              <CollapsibleUserBubble
                content={msg.content}
                onClick={() => setShowUserMenu(true)}
              />
              
              {/* FIX: MENU KECIL MELAYANG DI POJOK KANAN BAWAH BUBBLE */}
              <AnimatePresence>
                {showUserMenu && (
                  <>
                    {/* Layer transparan biar kalau klik di luar menu dia ketutup */}
                    <div 
                    ...
                    <motion.div 
                      ...
                      // Positioning absolute relative ke bubble text
                    ...
                    >
                      ...
                      <button 
                        onClick={(e) => { e.stopPropagation(); onEdit?.(msg.content); setShowUserMenu(false); }}
                        className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl hover:bg-[var(--sf)] transition-colors active:scale-[0.98] text-left"
                      >
