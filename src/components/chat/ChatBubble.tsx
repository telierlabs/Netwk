...
// ─── SMART IMAGE GALLERY — ChatGPT-style grid ───
const SmartImageGallery = memo(({ images }: { images: string[] }) => {
  ...
  const downloadImage = (e: React.MouseEvent, url: string) => {
    ...
    a.href = url; a.download = `cylen_image_${Date.now()}.png`;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
  };

  // ── LAYOUT LOGIC ──────────────────────────────────────────────
  // 1 gambar  → full width, max 320px, proporsional
  // 2 gambar  → 2 kolom sama lebar
  // 3 gambar  → gambar 1 full width baris atas, 2+3 di baris bawah 2 kolom
  // 4+ gambar → grid 2 kolom, semua sama
  const renderGrid = () => {
    if (images.length === 1) {
      return (
        <div
          onClick={() => openViewer(0)}
          className="rounded-[20px] overflow-hidden cursor-pointer border border-[var(--bd)]/20 shadow-sm relative bg-[var(--sf)]/10 inline-block max-w-full"
          style={{ maxWidth: 320 }}
        >
          <img src={images[0]} alt="Uploaded" className="block w-full h-auto max-h-[400px] object-contain" />
    ...
    if (images.length === 2) {
      return (
        <div className="grid grid-cols-2 gap-1.5" style={{ maxWidth: 360 }}>
          {images.map((src, i) => (
            <div key={i} onClick={() => openViewer(i)} className="rounded-[16px] overflow-hidden cursor-pointer border border-[var(--bd)]/20 shadow-sm relative bg-[var(--sf)]/10 aspect-square">
    ...
    if (images.length === 3) {
      return (
        <div className="flex flex-col gap-1.5" style={{ maxWidth: 360 }}>
          {/* Baris atas: gambar pertama full width */}
          <div onClick={() => openViewer(0)} className="rounded-[16px] overflow-hidden cursor-pointer border border-[var(--bd)]/20 shadow-sm relative bg-[var(--sf)]/10" style={{ aspectRatio: '16/9' }}>
            <img src={images[0]} alt="Uploaded 1" className="w-full h-full object-cover" />
            <div className="absolute inset-0 bg-black/0 hover:bg-black/10 transition-colors" />
          </div>
          {/* Baris bawah: 2 gambar grid */}
          <div className="grid grid-cols-2 gap-1.5">
            {[images[1], images[2]].map((src, i) => (
              <div key={i} onClick={() => openViewer(i + 1)} className="rounded-[16px] overflow-hidden cursor-pointer border border-[var(--bd)]/20 shadow-sm relative bg-[var(--sf)]/10 aspect-square">
                <img src={src} alt={`Uploaded ${i+2}`} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black/0 hover:bg-black/10 transition-colors" />
              </div>
            ))}
          </div>
    ...
    // 4+ gambar: grid 2 kolom
    return (
      <div className="grid grid-cols-2 gap-1.5" style={{ maxWidth: 360 }}>
        {images.map((src, i) => (
          <div key={i} onClick={() => openViewer(i)} className="rounded-[16px] overflow-hidden cursor-pointer border border-[var(--bd)]/20 shadow-sm relative bg-[var(--sf)]/10 aspect-square">
            <img src={src} alt={`Uploaded ${i+1}`} className="w-full h-full object-cover" />
            {/* Badge "+N lebih" di sel terakhir kalau > 4 */}
            {i === 3 && images.length > 4 && (
              <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                <span className="text-white text-[18px] font-black">+{images.length - 4}</span>
              </div>
            )}
            <div className="absolute inset-0 bg-black/0 hover:bg-black/10 transition-colors" />
          </div>
          ...
          >
            ...
            <div className="w-full h-full flex items-center justify-center p-2 relative">
              ...
              {images.length > 1 && currentIndex > 0 && (
                <button onClick={handlePrev} className="hidden md:flex absolute left-6 w-12 h-12 bg-white/10 hover:bg-white/20 rounded-full items-center justify-center text-white backdrop-blur-md transition-colors z-50"><ChevronLeft size={28} /></button>
              )}
              {images.length > 1 && currentIndex < images.length - 1 && (
                <button onClick={handleNext} className="hidden md:flex absolute right-6 w-12 h-12 bg-white/10 hover:bg-white/20 rounded-full items-center justify-center text-white backdrop-blur-md transition-colors z-50"><ChevronRight size={28} /></button>
              )}
            </div>
            ...
            {images.length > 1 && (
              <div className="absolute bottom-10 left-0 right-0 flex justify-center items-center gap-2 z-50">
                {images.map((_, i) => (
                  <div key={i} onClick={() => setCurrentIndex(i)} className={cn("w-2 h-2 rounded-full transition-all duration-300 cursor-pointer", currentIndex === i ? "bg-white scale-125" : "bg-white/30")} />
                ))}
              </div>
            )}
          </motion.div>
