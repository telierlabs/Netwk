...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({ msg, msgIndex, isLast, onResend, onEdit, onSuggest, onTogglePin, onSaveItem, onRegenerate, onSwipeToReply, suggestions }) => {
  ...
  // 🚀 EXTRAKSI FILE/PDF SUPER AGRESIF (Biar gak hilang)
  const { imagesToRender, docsToRender } = React.useMemo(() => {
      const imgs: string[] = [];
      ...
      const checkItem = (item: any) => {
          ...
          if (typeof item === 'string') {
              if (item.startsWith('data:application/pdf') || item.startsWith('data:text/') || item.endsWith('.pdf') || item.endsWith('.txt')) {
                  docs.push({ name: 'Dokumen Terlampir', url: item });
              } else {
                  imgs.push(item);
              }
          } else if (typeof item === 'object') {
              if (item.name && (item.data || item.url)) {
                  docs.push(item);
              } else if (item.url && typeof item.url === 'string') {
                  if (item.url.startsWith('data:application/pdf') || item.url.endsWith('.pdf') || item.url.endsWith('.txt')) {
                      docs.push({ name: item.name || 'Dokumen Terlampir', url: item.url });
                  } else {
                      imgs.push(item.url);
                  }
              }
      ...
      if (Array.isArray(msg.images)) msg.images.forEach(checkItem);
      else checkItem(msg.images);

      checkItem(msg.image);

      if (Array.isArray(msg.pdfs)) msg.pdfs.forEach(checkItem);
      else checkItem(msg.pdfs);

      if (Array.isArray((msg as any).files)) (msg as any).files.forEach(checkItem);
      if (Array.isArray((msg as any).attachments)) (msg as any).attachments.forEach(checkItem);

      return { imagesToRender: imgs, docsToRender: docs };
  }, [msg]);
  ...
  const handleDocClick = (doc: any) => {
    if (doc.url) window.open(doc.url, '_blank');
       ...
       link.href = `data:application/pdf;base64,${doc.data}`;
  ...
  return (
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start", showUserMenu ? "z-[100]" : "z-10")}>
      ...
      {/* 🚀 RENDER DOKUMEN / PDF SEBAGAI KAPSUL PANJANG (DI ATAS CHAT USER) */}
        ...
        <div className={cn("flex flex-col gap-2 mb-2 w-full", isUser ? "items-end" : "items-start")}>
          {docsToRender.map((doc, i) => (
             <div 
               ...
               className={cn(
                 "flex items-center gap-3 px-4 py-2.5 bg-[var(--sf)] border border-[var(--bd)] rounded-full shadow-sm w-fit max-w-[85%] cursor-pointer hover:bg-[var(--bd)]/30 transition-colors", 
             ...
             >
               ...
               <span className="text-[14px] font-semibold text-[var(--text)] truncate max-w-[200px]">
