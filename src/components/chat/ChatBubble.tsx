...
const ImageGallery = ({ src }: { src: string }) => {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: 'Cylen AI Image', url: src });
  ...
  return (
    <>
      <div className="mb-2 relative rounded-2xl overflow-hidden cursor-pointer" style={{ width: 240, height: 240 }} onClick={() => setIsFullscreen(true)}>
...
export const ChatBubble: React.FC<ChatBubbleProps> = ({ msg }) => {
  ...
  return (
    <div className={cn("flex flex-col group w-full", msg.role === 'user' ? "items-end" : "items-start")}>
      ...
      {/* GAMBAR — full width, terpisah di atas bubble */}
      {msg.image && (
        <div className={msg.role === 'user' ? "flex justify-end mb-1 w-full" : "mb-1 w-full"}>
          <ImageGallery src={msg.image} />
        </div>
      )}

      {/* BUBBLE — hanya teks, terpisah */}
      {msg.content && (
      <div className={cn(
        "msg-bubble p-3 rounded-2xl text-sm leading-relaxed",
        msg.role === 'user' 
          ? "bg-[var(--ac)] text-[var(--at)] rounded-tr-none max-w-[75%] inline-block" 
