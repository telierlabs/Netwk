...
/* ─────────────── HTML PREVIEW (CLAUDE ARTIFACTS STYLE) ─────────────── */
const HtmlPreview = ({ content }: { content: string }) => {
  ...
  const handleDownload = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'simulasi_cylen.html';
            ...
            {showMenu && (
              <>
                ...
                >
                  ...
                  >
                    <Download size={16} />
                    <span className="font-medium">Unduh HTML</span>
                  </button>
                </motion.div>
              </>
            )}
...
export const ChatBubble: React.FC<ChatBubbleProps> = ({
  ...
  return (
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full", isUser ? "items-end" : "items-start")}>
      ...
      {/* 👇 ── AI ACTION BAR (UDAH GUA KEMBALIIN 100% PERSIS KAYA DULU) ── 👇 */}
      {!isUser && (
      ...
      {/* 👇 ── SUGGESTIONS / PERTANYAAN FOLLOW-UP (UDAH GUA KEMBALIIN JUGA) ── 👇 */}
