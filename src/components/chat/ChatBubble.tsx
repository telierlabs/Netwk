...
const CollapsibleUserBubble = memo(({ content, onClick }: { content: string; onClick: (e: React.MouseEvent) => void }) => {
  ...
  return (
    <div className="w-full relative flex flex-col transform-gpu">
      ...
      <div 
        onClick={onClick}
        className={cn("px-4 cursor-pointer active:opacity-80 transition-opacity pb-3 select-none", replyText ? "pt-1" : "pt-3")}
      >
...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({ msg, msgIndex, isLast, onResend, onEdit, onSuggest, onTogglePin, onSaveItem, onRegenerate, onSwipeToReply, suggestions }) => {
  ...
  const [userMenuPos, setUserMenuPos] = useState({ top: 0, right: 0 }); // Tambahan State untuk koordinat Fixed Popup
  const [userMenuCopied, setUserMenuCopied] = useState(false);
  const [showSourcesModal, setShowSourcesModal] = useState(false);

  const bubbleRef = useRef<HTMLDivElement>(null); // Ref untuk ngukur bubble
  ...
  const openUserMenu = (e: React.MouseEvent) => {
    ...
    // Hitung posisi bubble agar popup ga berantakan
    if (bubbleRef.current) {
        ...
        let targetTop = rect.bottom + 8; // Default muncul di Bawah bubble
        let targetRight = window.innerWidth - rect.right; // Ratakan kanan dengan bubble

        // Kalau nyentuh bawah layar, balik ke ATAS
        if (targetTop + popupHeight > window.innerHeight) {
            targetTop = rect.top - popupHeight - 8;
        }

        // Kalau kurang geser kiri, fix posisi biar gak kepotong layar kanan
        if (targetRight < 10) {
            targetRight = 10;
        }

        setUserMenuPos({ top: targetTop, right: targetRight });
  ...
  const getSourceInfo = (src: { title: string; url: string }) => {
    let cleanTitle = src.title && src.title !== 'Referensi' && src.title !== 'Sumber' ? src.title : 'Artikel Terkait';
    let displayDomain = 'google.com';
    if (src.url.includes('vertexaisearch') || src.url.includes('googleusercontent') || src.url.includes('google.com/search')) {
      const parts = cleanTitle.split(/[-|]/);
      const lastPart = parts[parts.length - 1].trim().toLowerCase();
      let guessDomain = lastPart.replace(/[^a-z0-9.]/g, '');
      if (guessDomain.length > 2 && !guessDomain.includes('.')) guessDomain += '.com';
      if (guessDomain.length > 2) displayDomain = guessDomain;
    } else {
      try { displayDomain = new URL(src.url).hostname.replace(/^www\./, ''); } catch (e) {}
    }
  ...
  return (
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start")}>
      ...
      {msg.content && (
        <div ref={bubbleRef} className={cn("msg-bubble rounded-[22px] transform-gpu relative", isUser ? "bg-[var(--sf)] border border-[var(--bd)] text-[var(--text)] rounded-tr-[6px] max-w-[85%] shadow-sm" : "bg-transparent text-[var(--text)] px-0 w-full")}>
          {isUser ? (
            <>
              <motion.div
                drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.1} dragMomentum={false}
              ...
              {/* FIXED POPUP DI SINI AGAR TIDAK NABRAK OVERFLOW */}
              <AnimatePresence>
                {showUserMenu && (
                  <>
                    {/* Backdrop tidak terlihat agar bisa di-klik di area lain untuk menutup popup */}
                    ...
                    <motion.div 
                      ...
                      className="fixed z-[200] w-[160px] bg-[var(--bg)] backdrop-blur-xl border border-[var(--bd)] rounded-[16px] shadow-xl overflow-hidden flex flex-col p-1.5"
                      style={{ top: userMenuPos.top, right: userMenuPos.right }} // Posisikan berdasarkan letak yang dihitung dari useRef
                    >
            ...
            <div className="cylen-md w-full" style={{ userSelect: "text", WebkitUserSelect: "text" }}>
              <style>{`
                ...
                .cylen-md .math-inline { font-family: 'Georgia', serif; font-style: italic; color: var(--text); }
                ...
                .cylen-md pre { font-family: monospace; white-space: pre; overflow-x: auto; }
                .cylen-md, .cylen-md * { user-select: text !important; -webkit-user-select: text !important; }
                .cylen-md ::selection { background: var(--ac); color: var(--at); }
                .cylen-md ::-moz-selection { background: var(--ac); color: var(--at); }
              `}</style>
              <Markdown remarkPlugins={[remarkGfm]} components={{
                code({ node, inline, className, children, ...props }: any) {
                  ...
                  if ((lang === 'text' || lang === '') && raw.trim().length <= 80 && !raw.includes('\n')) {
            ...
            <div className="mt-6 pt-4 border-t border-[var(--bd)] w-full">
              ...
              <div className="flex overflow-x-auto pb-2 -mx-1 px-1 gap-2.5" style={{ scrollbarWidth: 'none' }}>
                {msg.sources.map((src, si) => {
                  ...
                  return (
                    <button key={si} onClick={() => setShowSourcesModal(true)} className="flex items-center gap-2.5 px-3.5 py-2 bg-[var(--bg)] border border-[var(--bd)] rounded-xl hover:bg-[var(--sf)] transition-all shadow-sm flex-shrink-0 active:scale-[0.98]">
                      <img src={logoUrl} alt="" className="w-5 h-5 rounded-md bg-white object-cover shadow-[0_2px_4px_rgba(0,0,0,0.05)]" onError={(e) => { e.currentTarget.src = fallbackLogo; }} />
          ...
          <AnimatePresence>
            {showSourcesModal && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[500] bg-black/40 flex flex-col justify-end sm:justify-center items-center p-0 sm:p-4">
                <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "tween", duration: 0.25, ease: "easeOut" }} className="bg-[var(--bg)] w-full sm:max-w-lg h-[85vh] sm:h-[600px] rounded-t-[24px] sm:rounded-2xl flex flex-col overflow-hidden shadow-2xl border border-[var(--bd)]">
                  <div className="flex items-center justify-between p-4 border-b border-[var(--bd)] bg-[var(--sf)]">
                    <h3 className="font-bold text-[16px] text-[var(--text)] flex items-center gap-2"><Globe size={18} className="text-[var(--ac)]" /> Daftar Sumber Artikel</h3>
                    <button onClick={() => setShowSourcesModal(false)} className="p-2 hover:bg-[var(--bd)] rounded-full transition-colors text-[var(--mu)] hover:text-[var(--text)]"><X size={20} /></button>
                  </div>
                  <div className="flex-1 overflow-y-auto p-2" style={{ scrollbarWidth: 'thin' }}>
                    {msg.sources?.map((src, i) => {
                      const { cleanTitle, displayDomain, logoUrl, fallbackLogo } = getSourceInfo(src);
                      return (
                        <a key={i} href={src.url} target="_blank" rel="noopener noreferrer" className="flex gap-4 p-4 border-b border-[var(--bd)] hover:bg-[var(--sf)] transition-colors group">
                          <div className="text-[12px] font-black text-[var(--mu)] mt-1 w-4">{i + 1}.</div>
                          <div className="flex-1 min-w-0">
                            <h4 className="font-bold text-[15px] text-[var(--text)] leading-snug group-hover:text-[var(--ac)] transition-colors line-clamp-3">{cleanTitle}</h4>
                            <div className="flex items-center gap-2 mt-3">
                              <img className="w-6 h-6 rounded bg-white shadow-sm border border-[var(--bd)]" src={logoUrl} onError={(e) => { e.currentTarget.src = fallbackLogo; }} alt="" />
                              <span className="text-[13px] font-semibold text-[var(--mu)] truncate">{displayDomain}</span>
                            </div>
                          </div>
                        </a>
                      );
      ...
      {!isUser && (
        <div className="flex items-center gap-[28px] mt-1 relative">
          <button onClick={handleCopy} title="Salin" className={cn("transition-colors", copied ? "text-green-500" : "text-[var(--mu)] hover:text-[var(--text)]")}>{copied ? <Check size={18} /> : <Copy size={18} />}</button>
          ...
          <button onClick={() => onTogglePin?.(msgIndex)} title={msg.pinned ? "Lepas pin" : "Sematkan"} className={cn("transition-colors", msg.pinned ? "text-[var(--ac)]" : "text-[var(--mu)] hover:text-[var(--text)]")}>{msg.pinned ? <PinOff size={18} /> : <Pin size={18} />}</button>
          <button onClick={() => onSaveItem?.({ type: 'teks', title: 'Pesan AI Cylen', preview: msg.content.substring(0, 50) + '...', content: msg.content })} title="Simpan" className="text-[var(--mu)] hover:text-[var(--text)] transition-colors"><Bookmark size={18} /></button>
        </div>
      )}
