...
// ─── MERMAID DIAGRAM RENDERER (FULLSCREEN & CUSTOM COLORS) ───
const MermaidDiagram = memo(({ content }: { content: string }) => {
  ...
  useEffect(() => {
    ...
    mermaid.initialize({
      ...
      themeVariables: {
        ...
        lineColor: 'var(--mu)', 
        secondaryColor: 'var(--bg)',
        tertiaryColor: 'var(--cd)'
      },
      fontFamily: 'inherit',
    });
    ...
    const renderDiagram = async () => {
      try {
        ...
        // Tangkap error dengan santai, jangan langsung nampilin error kalau bisa diabaikan
        setErrorMsg(err?.message || 'Gagal memuat diagram');
      }
    };
    renderDiagram();
  }, [content]);
  ...
  if (errorMsg) {
    return (
      <div className="my-4 w-full bg-[var(--cd)] border border-[var(--bd)] rounded-2xl p-4 shadow-sm flex flex-col gap-2 transform-gpu">
        <div className="text-[12px] text-[var(--text)] font-bold px-1 flex items-center gap-2">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>
          Format Diagram Tidak Dikenali
        </div>
        <div className="mt-2 text-[11px] font-bold text-[var(--mu)] px-1">Kode Sumber (Copy untuk perbaiki):</div>
        <div className="p-3 text-[11px] text-[var(--text)] font-mono bg-[var(--sf)] border border-[var(--bd)] rounded-xl overflow-x-auto whitespace-pre-wrap leading-relaxed relative group">
          {content}
        </div>
      </div>
    );
  ...
  return (
    <>
      <div 
        ...
        className="my-4 w-full bg-[var(--cd)] border border-[var(--bd)] rounded-2xl p-6 overflow-hidden shadow-sm transform-gpu flex flex-col items-center cursor-pointer hover:border-[var(--text)]/30 transition-all group relative"
      >
        ...
        <div className="absolute bottom-3 right-3 px-3 py-2 bg-[var(--bg)]/90 backdrop-blur text-[var(--text)] text-[10px] font-bold rounded-xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1.5 border border-[var(--bd)] shadow-sm">
