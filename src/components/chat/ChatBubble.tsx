...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({ msg, msgIndex, isLast, onResend, onEdit, onSuggest, onTogglePin, onSaveItem, onRegenerate, onSwipeToReply, suggestions }) => {
  ...
  // 🚀 EKSTRAKSI DOKUMEN SUPER AGRESIF (JAMINAN MUNCUL)
  let finalContent = msg.content || '';
  const extractedPdfs: { name?: string; url?: string; data?: string }[] = [];

  // 1. Ekstrak dari property asli (pdfs/files/attachments)
  if (msg.pdfs && Array.isArray(msg.pdfs)) {
    msg.pdfs.forEach((p: any) => {
      if (typeof p === 'string') extractedPdfs.push({ name: 'Dokumen', url: p });
      else extractedPdfs.push(p);
    });
  }
  if ((msg as any).files && Array.isArray((msg as any).files)) extractedPdfs.push(...(msg as any).files);
  if ((msg as any).attachments && Array.isArray((msg as any).attachments)) extractedPdfs.push(...(msg as any).attachments);

  // 2. Ekstrak dari Teks (Jika backend masukin file ke string text)
  const pdfRegex1 = /^\[PDF Terlampir: "(.*?)"\]\((.*?)\)\n\n([\s\S]*)$/;
  const match1 = finalContent.match(pdfRegex1);
  if (match1) {
    extractedPdfs.push({ name: match1[1], url: match1[2] });
    finalContent = match1[3];
  }

  const pdfRegex2 = /^\[File dilampirkan: "(.*?)"\]\n\n([\s\S]*)$/;
  const match2 = finalContent.match(pdfRegex2);
  if (match2) {
    extractedPdfs.push({ name: match2[1] });
    finalContent = match2[2];
  }

  // Fallback terakhir, kalau ternyata ada string "[File kode dilampirkan]" (seperti pas Paste kode)
  if (finalContent.includes('[File kode dilampirkan]')) {
    extractedPdfs.push({ name: 'Kode_Panjang.txt' });
    finalContent = finalContent.replace('[File kode dilampirkan]', '').trim();
  }
  ...
  return (
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start", showUserMenu ? "z-[100]" : "z-10")}>
      ...
      {/* 🚀 RENDER PDF/FILE SEBAGAI KAPSUL (DI ATAS CHAT USER) */}
        ...
        <div className={cn("flex flex-col gap-2 mb-2 w-full", isUser ? "items-end" : "items-start")}>
          {extractedPdfs.map((pdf, i) => (
             <div 
               ...
               onClick={() => {
                 if (pdf.url) window.open(pdf.url, '_blank');
                   ...
                   link.href = pdf.data.startsWith('http') || pdf.data.startsWith('data:') ? pdf.data : `data:application/pdf;base64,${pdf.data}`;
                   link.download = pdf.name || 'document.pdf';
                   link.click();
                 }
               }}
               className={cn(
                 "flex items-center gap-3 px-4 py-2.5 bg-[var(--sf)] border border-[var(--bd)] rounded-full shadow-sm w-fit max-w-[85%] cursor-pointer hover:bg-[var(--bd)] transition-colors", 
             ...
             >
               ...
               <span className="text-[14px] font-bold text-[var(--text)] truncate max-w-[200px]">
      ...
      {/* RENDER TEXT BUBBLE (PAKE FINAL CONTENT YANG UDAH DIBERSIHIN) */}
      {finalContent && (
          ...
          {isUser ? (
            <>
              <motion.div drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.1} onDragEnd={(_e, info) => { if (info.offset.x > 55) onSwipeToReply?.(msg); }}>
                <CollapsibleUserBubble content={finalContent} onClick={openUserMenu} />
              </motion.div>
