...
const ImageGallery = memo(({ src, square }: { src: string; square?: boolean }) => {
  ...
  return (
    <>
      <div className="relative overflow-hidden cursor-pointer border border-[var(--bd)] shadow-sm transform-gpu"
...
const HtmlPreview = memo(({ content }: { content: string }) => {
  ...
  if (!isExpanded) {
    return (
      <div onClick={() => setIsExpanded(true)}
        className="my-3 flex items-center gap-4 p-4 bg-[var(--sf)] border border-[var(--bd)] rounded-2xl cursor-pointer hover:opacity-80 transition-all shadow-sm w-full transform-gpu">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
      ...
      className="fixed inset-0 z-[400] bg-[var(--bg)] flex flex-col transform-gpu">
...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({
  ...
  // 🔥 RENDER KHUSUS TWEET AI (KARTU BAYANGAN)
    ...
    return (
      <div id={`message-${msgIndex}`} className="flex flex-col group w-full relative transform-gpu items-start my-6">
        <motion.div 
          ...
          className="w-[90%] md:max-w-[80%] bg-transparent border border-[var(--text)]/10 shadow-[0_8px_30px_rgba(0,0,0,0.08)] rounded-3xl p-5 relative overflow-hidden"
        >
          {/* Efek Glow Elegan di pojok */}
          ...
          <div className="border-l-[3px] border-[var(--text)]/20 pl-3 mb-4 relative z-10">
            <div className="flex items-center gap-1.5 mb-1.5">
              <Clock size={12} className="text-[var(--text)]/50" />
              <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text)]/50">Cylen Mengingatkan</span>
            </div>
            <p className="text-[13px] italic text-[var(--text)]/70 line-clamp-2 leading-relaxed">"{msg.quotedText}"</p>
          </div>
          
          <p className="text-[15px] font-medium text-[var(--text)] relative z-10 leading-relaxed">
            {msg.content}
          </p>
        </motion.div>
      </div>
  ...
  // RENDER NORMAL (Bukan Auto Reminder)
  return (
                ...
                {showUserMenu && (
                  <>
                    ...
                    >
                      ...
                      <button onClick={(e) => { e.stopPropagation(); let cleanEditText = msg.content; const replyMatch = msg.content.match(/^\[Membalas pesan: "(.*?)"\]\n\n([\s\S]*)$/); if (replyMatch) cleanEditText = replyMatch[2]; onEdit?.(cleanEditText); setShowUserMenu(false); }} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl hover:bg-[var(--sf)] transition-colors active:scale-[0.98] text-left">
                      ...
                      <button onClick={handleUserMenuCopy} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl hover:bg-[var(--sf)] transition-colors active:scale-[0.98] text-left">
                        {userMenuCopied ? <><Check size={16} className="text-green-500" /><span className="text-[13.5px] font-bold text-green-500">Tersalin</span></> : <><Copy size={16} className="text-[var(--text)]" /><span className="text-[13.5px] font-bold text-[var(--text)]">Salin</span></>}
                      </button>
                    </motion.div>
                  </>
                )}
              ...
              {msg.sources.map((src, si) => {
                ...
                return (
                  <a key={si} href={src.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3.5 py-1.5 bg-[var(--bg)] border border-[var(--bd)] rounded-full text-[12px] font-medium hover:bg-[var(--sf)] transition-all shadow-sm">
                    <img src={`https://www.google.com/s2/favicons?domain=${domain}&sz=32`} alt="" className="w-4 h-4 rounded-full bg-white flex-shrink-0" onError={(e) => (e.currentTarget.src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="%23999" stroke-width="2"><circle cx="12" cy="12" r="10"/></svg>')} />
            ...
            {showExportMenu && (
              <>
                ...
                <div className="absolute bottom-full mb-2 left-0 bg-[var(--bg)] border border-[var(--bd)] rounded-2xl shadow-xl z-[60] overflow-hidden min-w-[140px]">
                  <button onClick={() => { exportToPDFFromText(msg.content); setShowExportMenu(false); }} className="w-full flex items-center gap-2.5 px-4 py-3 text-[13px] font-medium hover:bg-[var(--sf)] transition-colors text-[var(--text)]">
                    <FileText size={15} /> Ekspor PDF
                  </button>
                  <div className="border-t border-[var(--bd)]" />
                  <button onClick={() => { exportToDoc(msg.content); setShowExportMenu(false); }} className="w-full flex items-center gap-2.5 px-4 py-3 text-[13px] font-medium hover:bg-[var(--sf)] transition-colors text-[var(--text)]">
