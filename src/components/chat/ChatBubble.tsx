...
// ─── MERMAID DIAGRAM RENDERER (GESTURE-DRIVEN & CUSTOM COLORS) ───
const MermaidDiagram = memo(({ content }: { content: string }) => {
  ...
  // GESTURE STATES (Pan & Pinch to Zoom murni pake tangan)
  ...
  // LOGIKA HAND GESTURES (TOUCH HANDLERS)
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    if (e.touches.length === 1 && lastTouchRef.current.distance === 0) {
      // 1 Jari = Pan / Geser Diagram Gesit
      ...
      // 2 Jari = Cubit Luwes (Pinch to Zoom) + Geser Berbarengan
      const dist = Math.hypot(
        e.touches[0].clientX - e.touches[1].clientX,
        e.touches[0].clientY - e.touches[1].clientY
      );
      const factor = dist / lastTouchRef.current.distance;
      setScale(s => Math.min(Math.max(s * factor, 0.4), 6)); // Batas zoom aman 0.4x hingga 6x
  ...
  // PORTAL HALAMAN KHUSUS DIAGRAM (GESTURE READY)
  const portalContent = isFullscreen ? (
    <div className="fixed inset-0 z-[999999] bg-[var(--bg)] flex flex-col touch-none select-none overflow-hidden animate-in fade-in zoom-in-95 duration-200">
      {/* Header Atas */}
      ...
      {/* Lahan Kerja Jari Tangan */}
      <div 
        onTouchStart={handleTouchStart}
        onTouchMove={handleTouchMove}
        onTouchEnd={handleTouchEnd}
        className="flex-1 w-full h-full overflow-hidden flex items-center justify-center p-4 relative bg-[var(--bg)] cursor-grab active:cursor-grabbing"
      >
        <div 
          style={{ 
  ...
  return (
    <>
      <div 
        ...
        className="my-4 w-full bg-[var(--cd)] border border-[var(--bd)] rounded-2xl overflow-x-auto shadow-sm transform-gpu flex flex-col cursor-pointer hover:border-[var(--text)]/40 transition-all group relative min-h-[200px]"
        style={{ scrollbarWidth: 'none' }}
      >
        {/* Kontainer digedein paksa ke samping biar rapi gak gencet-gencetan */}
        <div 
          dangerouslySetInnerHTML={{ __html: svgCode }} 
          className="min-w-[600px] w-max h-full flex items-center p-6 [&>svg]:w-full [&>svg]:h-auto pointer-events-none [&_text]:!fill-[var(--text)] [&_text]:!font-bold [&_text]:!font-sans [&_.node_rect]:!fill-[var(--sf)] [&_.node_rect]:!stroke-[var(--bd)] [&_.node_polygon]:!fill-[var(--sf)] [&_.node_polygon]:!stroke-[var(--bd)] [&_.edgePath_.path]:!stroke-[var(--mu)] [&_.edgePath_.path]:!stroke-width-[1.8px] [&_.marker]:!fill-[var(--mu)]" 
        />
    ...
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start", showUserMenu ? "z-[100]" : "z-10")}>
      ...
      {displayContent && (
        <div className={cn("msg-bubble rounded-[22px] transform-gpu relative z-10", isUser
          ...
          {isUser ? (
            <>
              {/* CRITICAL BUGS FIX: Tambah dragDirectionLock & dragSnapToOrigin biar bubble gak melenceng aneh ke bawah pas digeser */}
