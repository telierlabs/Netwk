...
// ─── MERMAID DIAGRAM RENDERER (NATIVE PAN/ZOOM & CUSTOM COLORS) ───
const MermaidDiagram = memo(({ content }: { content: string }) => {
  ...
  // Tampilan layar penuh portal (NATIVE ZOOM)
  const portalContent = isFullscreen ? (
    <div className="fixed inset-0 z-[999999] bg-[var(--bg)] flex flex-col animate-in fade-in zoom-in-95 duration-200">
      ...
      <div className="flex items-center justify-between p-4 border-b border-[var(--bd)] shrink-0 bg-[var(--bg)] relative z-20 shadow-sm">
        <button onClick={() => setIsFullscreen(false)} className="w-12 h-12 flex items-center justify-center rounded-full text-[var(--text)] hover:bg-[var(--sf)] active:scale-95 transition-all">
      ...
      {/* Lahan Kerja Jari Tangan (Menggunakan scroll/zoom bawaan browser biar super licin) */}
      <div className="flex-1 w-full h-full overflow-auto bg-[var(--bg)] p-4">
        {/* Kontainer dibikin min-w-max biar SVG-nya bisa melar sebebasnya (teks tetep gede) */}
        <div 
          dangerouslySetInnerHTML={{ __html: svgCode }} 
          className="min-w-max min-h-full flex items-center justify-center p-8 [&>svg]:min-w-[800px] md:[&>svg]:min-w-[1200px] [&>svg]:h-auto [&_text]:!fill-[var(--text)] [&_text]:!font-bold [&_text]:!font-sans [&_.node_rect]:!fill-[var(--sf)] [&_.node_rect]:!stroke-[var(--bd)] [&_.node_polygon]:!fill-[var(--sf)] [&_.node_polygon]:!stroke-[var(--bd)] [&_.edgePath_.path]:!stroke-[var(--mu)] [&_.edgePath_.path]:!stroke-width-[1.8px] [&_.marker]:!fill-[var(--mu)]" 
        />
      </div>
    </div>
  ) : null;
