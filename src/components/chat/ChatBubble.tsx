...
const GraphRenderer = memo(({ content }: { content: string }) => {
  try {
    ...
    return (
      <div className="w-full h-64 my-4 bg-[var(--cd)] p-4 rounded-2xl border border-[var(--bd)] shadow-sm transform-gpu">
  ...
  } catch { return <div className="my-4 p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-xs w-full">Gagal merender grafik.</div>; }
});
...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({ msg, msgIndex, isLast, onResend, onEdit, onSuggest, onTogglePin, onSaveItem, onRegenerate, onSwipeToReply, suggestions }) => {
  ...
  const getSourceInfo = (src: { title: string; url: string }) => {
    let originalDomain = src.url;
    try { originalDomain = new URL(src.url).hostname.replace(/^www\./, ''); } catch (e) {}
    
    const displayName = src.title && src.title !== 'Referensi' && src.title !== 'Sumber' 
      ? src.title 
      : originalDomain;

    let logoDomain = originalDomain;
    if (originalDomain.includes('vertexaisearch') || originalDomain.includes('google') || originalDomain.includes('yahoo')) {
       let guessDomain = displayName.toLowerCase().replace(/[^a-z0-9.]/g, '');
       if (!guessDomain.includes('.')) guessDomain += '.com';
       logoDomain = guessDomain;
    }

    const fallbackLogo = `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName.charAt(0))}&background=random&color=fff&size=32`;
    const logoUrl = `https://www.google.com/s2/favicons?domain=${logoDomain}&sz=64`;

    return { displayName, originalDomain, logoUrl, fallbackLogo };
  };
            ...
            <div className="mt-6 pt-4 border-t border-[var(--bd)] w-full">
              ...
              <div className="flex overflow-x-auto pb-2 -mx-1 px-1 gap-2.5" style={{ scrollbarWidth: 'none' }}>
                {msg.sources.map((src, si) => {
                  const { displayName, logoUrl, fallbackLogo } = getSourceInfo(src);
                  return (
                    <button 
                      key={si} 
                    ...
                    >
                      <img 
                        src={logoUrl} 
                        alt="" 
                        className="w-5 h-5 rounded-md bg-white object-cover shadow-[0_2px_4px_rgba(0,0,0,0.05)]" 
                        onError={(e) => { e.currentTarget.src = fallbackLogo; }} 
                      />
                      <span className="text-[13px] font-bold text-[var(--text)] whitespace-nowrap">{displayName}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <AnimatePresence>
            {showSourcesModal && (
              <motion.div 
                ...
                animate={{ opacity: 1 }} 
                exit={{ opacity: 0 }} 
                className="fixed inset-0 z-[500] bg-black/60 flex flex-col justify-end sm:justify-center items-center p-0 sm:p-4 backdrop-blur-sm"
              >
                <motion.div 
                ...
                >
                  <div className="flex items-center justify-between p-4 border-b border-[var(--bd)] bg-[var(--sf)]">
                    <h3 className="font-bold text-[16px] text-[var(--text)] flex items-center gap-2">
                      <Globe size={18} className="text-[var(--ac)]" /> Daftar Sumber
                    </h3>
                    <button onClick={() => setShowSourcesModal(false)} className="p-2 hover:bg-[var(--bd)] rounded-full transition-colors text-[var(--mu)] hover:text-[var(--text)]">
                      <X size={20} />
                    </button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-2" style={{ scrollbarWidth: 'thin' }}>
                    {msg.sources?.map((src, i) => {
                      const { displayName, originalDomain, logoUrl, fallbackLogo } = getSourceInfo(src);
                      return (
                        <a 
                          key={i} 
                          href={src.url} 
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="font-bold text-[15px] text-[var(--text)] leading-snug group-hover:text-[var(--ac)] transition-colors line-clamp-3">
                              {src.title && src.title !== 'Referensi' ? src.title : `Artikel dari ${displayName}`}
                            </h4>
                            <div className="flex items-center gap-2 mt-3">
                              <img 
                                className="w-6 h-6 rounded bg-white shadow-sm border border-[var(--bd)]" 
                                src={logoUrl} 
                                onError={(e) => { e.currentTarget.src = fallbackLogo; }} 
                                alt=""
                              />
                              <span className="text-[13px] font-semibold text-[var(--mu)] truncate">
                                {originalDomain}
                              </span>
                            </div>
                          </div>
