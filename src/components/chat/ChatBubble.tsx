...
interface ChatBubbleProps {
  ...
  onSwipeToReply?: (msg: Message) => void; // ✅ Prop buat trigger swipe to reply
  suggestions?: string[];
}
...
export const ChatBubble: React.FC<ChatBubbleProps> = ({
  ...
  return (
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative", isUser ? "items-end" : "items-start")}>
      ...
      {msg.content && (
        // ✅ Ganti div jadi motion.div buat drag
        <motion.div
          drag="x"
          dragConstraints={{ left: 0, right: 0 }}
          dragElastic={0.15}
          onDragEnd={(e, info) => {
            // Trigger reply kalau ditarik lebih dari 60px ke kanan
            if (info.offset.x > 60) {
              onSwipeToReply?.(msg);
            }
          }}
