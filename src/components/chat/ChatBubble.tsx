...
interface ChatBubbleProps {
  ...
  onRegenerate?: (index: number) => void; // ✅ Tambahin props ini
  suggestions?: string[];
}
...
export const ChatBubble: React.FC<ChatBubbleProps> = ({
  msg, msgIndex, isLast, onResend, onEdit, onSuggest, onTogglePin, onSaveItem, onRegenerate, suggestions
}) => {
  const [copied, setCopied] = useState(false);
    ...
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full", isUser ? "items-end" : "items-start")}>
      ...
      {!isUser && (
        <div className="flex items-center gap-[28px] mt-1 relative">
          ...
          {/* ✅ Tombol ulanagi sekarang memanggil fungsi regenerasi! */}
