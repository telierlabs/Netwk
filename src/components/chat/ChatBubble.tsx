...
// ─── IMAGE VIEWER fullscreen (halaman proper, bukan modal) ───────────────────
const ImageViewer = ({
  images,
  startIndex,
  onClose,
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 h-[60px] flex-shrink-0">
        ...
        <span className="text-white/70 text-[14px] font-semibold">
        ...
        <button
          onClick={() => {
            ...
            a.download = `cylen-image-${current + 1}.png`;
            a.click();
          }}
...
const ImageGrid = memo(({ images, isUser }: { images: string[]; isUser: boolean }) => {
  ...
  const gridStyle: React.CSSProperties =
    ...
    : count === 3 ? { gridTemplateColumns: '1fr 1fr' }
  ...
  return (
    <>
      <AnimatePresence>
        {viewerOpen && (
          <ImageViewer images={images} startIndex={viewerStart} onClose={() => setViewerOpen(false)} />
        )}
      </AnimatePresence>

      <div className={cn("mb-2", isUser ? "flex justify-end" : "")}>
        <div style={{ display: 'grid', gap: 4, maxWidth: count === 1 ? 240 : 300, width: '100%', ...gridStyle }}>
          {shown.map((src, i) => {
            ...
            return (
              <div
                key={i}
                style={isFirst3 ? { gridRow: '1 / 3' } : undefined}
                className="relative overflow-hidden rounded-xl cursor-pointer border border-white/10 bg-black/10"
                onClick={() => openViewer(i)}
              >
                <div style={{ aspectRatio: count === 1 ? '4/3' : '1/1', width: '100%' }}>
                  <img src={src} alt="" loading="lazy" className="w-full h-full object-cover" />
                </div>
                {isLast && (
                  <div className="absolute inset-0 bg-black/55 flex items-center justify-center">
                    <span className="text-white text-[22px] font-black">+{overflow}</span>
                  </div>
                )}
                {count === 1 && (
...
// ✅ FIX: onClick ada di wrapper terluar → seluruh area bubble bisa diklik
const CollapsibleUserBubble = memo(({ content, onClick }: { content: string; onClick: () => void }) => {
  ...
  const handleToggle = (e: React.MouseEvent) => {
    e.stopPropagation(); // ✅ jangan trigger popup saat klik expand
    setExpanded(v => !v);
  };

  const handleScrollToReply = (e: React.MouseEvent) => {
    e.stopPropagation(); // ✅ jangan trigger popup saat klik reply
    if (!replyText) return;
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      if (el.textContent?.includes(searchSnippet)) {
        ...
        if (bubble) {
          ...
          setTimeout(() => { bubble.style.outline = 'none'; }, 1200);
        }
        break;
      }
    }
  ...
  return (
    // ✅ onClick di sini — seluruh bubble area bisa diklik untuk munculkan popup
    <div
      className="w-full relative flex flex-col transform-gpu cursor-pointer select-none"
      onClick={onClick}
    >
      {replyText && (
        <div
    ...
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start")}>
      ...
      {msg.content && (
        ...
        >
          {isUser ? (
            <>
              {/* ✅ CollapsibleUserBubble — klik area manapun → popup muncul */}
            ...
            // ─── Bubble AI (tidak berubah) ──────────────────────────────
            <div className="cylen-md w-full">
              <style>{`
                .cylen-md { font-size: var(--chat-text-size, 15px); line-height:1.7; color:var(--text); user-select:text; -webkit-user-select:text; }
