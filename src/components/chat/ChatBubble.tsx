...
const MapCard = memo(({ title, url }: { title: string; url: string }) => (
  <div className="flex flex-col gap-0 bg-[var(--cd)] border border-[var(--bd)] rounded-xl overflow-hidden shadow-sm hover:opacity-90 transition-all group my-4 max-w-md transform-gpu">
    ...
    <div className="w-full h-48 bg-[var(--sf)]">
      <iframe width="100%" height="100%" frameBorder="0" style={{ border: 0 }} src={`https://maps.google.com/maps?q=${encodeURIComponent(title)}&t=&z=13&ie=UTF8&iwloc=&output=embed`} allowFullScreen loading="lazy" />
    </div>
    <div className="p-3 bg-[var(--cd)] border-t border-[var(--bd)] flex items-center justify-center">
      <a href={url} target="_blank" rel="noopener noreferrer" className="text-[10px] font-bold uppercase tracking-widest text-[var(--text)] hover:underline">Lihat Detail di Google Maps</a>
    </div>
  </div>
));
...
const ImageGallery = memo(({ src, square }: { src: string; square?: boolean }) => {
  const [isFullscreen, setIsFullscreen] = useState(false);
  return (
    <>
      <div className="relative overflow-hidden cursor-pointer border border-[var(--bd)] shadow-sm transform-gpu" style={square ? { width: 155, height: 155, borderRadius: 12 } : { width: 240, height: 240, borderRadius: 16 }} onClick={() => setIsFullscreen(true)}>
        <img src={src} alt="" loading="lazy" className="w-full h-full" style={{ objectFit: 'cover' }} />
...
const CollapsibleUserBubble = memo(({ content, onClick }: { content: string; onClick: () => void }) => {
  ...
  const handleScrollToReply = (e: React.MouseEvent) => {
    e.stopPropagation(); if (!replyText) return;
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      if (el.textContent?.includes(searchSnippet)) {
        ...
        if (bubble) { bubble.style.transition = 'all 0.3s ease'; bubble.style.outline = '2px solid var(--ac)'; bubble.style.outlineOffset = '2px'; setTimeout(() => { bubble.style.outline = 'none'; }, 1200); }
        break;
      }
    }
  };
  return (
    <div className="w-full relative flex flex-col transform-gpu">
      {replyText && (
        <div onClick={handleScrollToReply} className="mx-2 mt-2 mb-1 bg-[var(--bg)]/40 hover:bg-[var(--bg)]/60 border-l-[3px] border-[var(--ac)] p-2.5 rounded-r-[10px] rounded-bl-[10px] cursor-pointer transition-all active:scale-[0.98]">
      ...
      <div onClick={onClick} className={cn("px-4 cursor-pointer active:opacity-80 transition-opacity pb-3", replyText ? "pt-1" : "pt-3")}>
        <span className="text-[15px] leading-relaxed" style={{ wordBreak: 'break-word' }}>{expanded ? actualContent : actualContent.slice(0, COLLAPSE_THRESHOLD) + (isLong && !expanded ? '...' : '')}</span>
      </div>
      {isLong && (
...
// ── Graph Renderer (kalau ada) ──
const GraphRenderer = memo(({ content }: { content: string }) => {
  try {
    const data = JSON.parse(content);
    if (!data.type || !data.data) return <CodeBlock lang="json" content={content} />;
    const chartData = data.data;
    const h = data.height || 300;
    if (data.type === 'bar') return <div className="my-4 w-full" style={{ height: h }}><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="var(--bd)" /><XAxis dataKey={data.xKey || 'name'} tick={{ fill: 'var(--mu)', fontSize: 12 }} /><YAxis tick={{ fill: 'var(--mu)', fontSize: 12 }} /><Tooltip contentStyle={{ background: 'var(--cd)', border: '1px solid var(--bd)', borderRadius: 12, color: 'var(--text)' }} />{(data.bars || ['value']).map((b: string, i: number) => <Bar key={b} dataKey={b} fill={COLORS[i % COLORS.length]} radius={[6, 6, 0, 0]} />)}</BarChart></ResponsiveContainer></div>;
    if (data.type === 'line') return <div className="my-4 w-full" style={{ height: h }}><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="var(--bd)" /><XAxis dataKey={data.xKey || 'name'} tick={{ fill: 'var(--mu)', fontSize: 12 }} /><YAxis tick={{ fill: 'var(--mu)', fontSize: 12 }} /><Tooltip contentStyle={{ background: 'var(--cd)', border: '1px solid var(--bd)', borderRadius: 12, color: 'var(--text)' }} />{(data.lines || ['value']).map((l: string, i: number) => <Line key={l} type="monotone" dataKey={l} stroke={COLORS[i % COLORS.length]} strokeWidth={2.5} dot={{ fill: COLORS[i % COLORS.length], r: 4 }} />)}</LineChart></ResponsiveContainer></div>;
    if (data.type === 'area') return <div className="my-4 w-full" style={{ height: h }}><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="var(--bd)" /><XAxis dataKey={data.xKey || 'name'} tick={{ fill: 'var(--mu)', fontSize: 12 }} /><YAxis tick={{ fill: 'var(--mu)', fontSize: 12 }} /><Tooltip contentStyle={{ background: 'var(--cd)', border: '1px solid var(--bd)', borderRadius: 12, color: 'var(--text)' }} />{(data.areas || ['value']).map((a: string, i: number) => <Area key={a} type="monotone" dataKey={a} stroke={COLORS[i % COLORS.length]} fill={COLORS[i % COLORS.length] + '33'} strokeWidth={2} />)}</AreaChart></ResponsiveContainer></div>;
    if (data.type === 'pie') return <div className="my-4 w-full" style={{ height: h }}><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={chartData} dataKey={data.valueKey || 'value'} nameKey={data.nameKey || 'name'} cx="50%" cy="50%" outerRadius={Math.min(h / 2 - 20, 120)} label>{chartData.map((_: any, i: number) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}</Pie><Tooltip contentStyle={{ background: 'var(--cd)', border: '1px solid var(--bd)', borderRadius: 12, color: 'var(--text)' }} /></PieChart></ResponsiveContainer></div>;
  } catch {}
...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({
  ...
  // ── Ambil info sumber: domain bersih, logo, judul ──
  const getSourceInfo = (src: { title: string; url: string }) => {
    let displayDomain = 'website';
    try {
      displayDomain = new URL(src.url).hostname.replace(/^www\./, '');
    } catch {}

    const cleanTitle = (src.title && src.title !== 'Referensi' && src.title !== 'Sumber')
      ? src.title
      : displayDomain;

    const fallbackLogo = `https://ui-avatars.com/api/?name=${encodeURIComponent(displayDomain.charAt(0))}&background=random&color=fff&size=32`;
    const logoUrl = `https://www.google.com/s2/favicons?domain=${displayDomain}&sz=64`;

    return { cleanTitle, displayDomain, logoUrl, fallbackLogo };
  };
    ...
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start")}>
      ...
      {msg.content && (
        <motion.div
          ...
          className={cn(
            "msg-bubble rounded-[22px] transform-gpu will-change-transform",
            isUser
        ...
        >
          {isUser ? (
              ...
              <AnimatePresence>
                {showUserMenu && (
                  <>
                    ...
                    >
                      <button onClick={(e) => { e.stopPropagation(); onResend?.(msg.content); setShowUserMenu(false); }} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl hover:bg-[var(--sf)] transition-colors active:scale-[0.98] text-left">
                        <RotateCcw size={16} className="text-[var(--text)]" /><span className="text-[13.5px] font-bold text-[var(--text)]">Ulangi</span>
                      </button>
                      <button onClick={(e) => {
                        e.stopPropagation();
                        ...
                        onEdit?.(cleanEditText); setShowUserMenu(false);
                        ...
                        <Edit2 size={16} className="text-[var(--text)]" /><span className="text-[13.5px] font-bold text-[var(--text)]">Edit</span>
                      </button>
                      <div className="w-full h-[1px] bg-[var(--bd)] my-1" />
                      <button onClick={handleUserMenuCopy} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl hover:bg-[var(--sf)] transition-colors active:scale-[0.98] text-left">
                        {userMenuCopied
                          ? <><Check size={16} className="text-green-500" /><span className="text-[13.5px] font-bold text-green-500">Tersalin</span></>
            ...
            // ── Markdown renderer untuk pesan AI ──
          ...
          {/* ── SUMBER BERITA — hanya tampil di pesan AI, ada sources, bukan user ── */}
          {!isUser && msg.sources && msg.sources.length > 0 && (
            <div className="mt-6 pt-4 border-t border-[var(--bd)] w-full">
              <div className="flex items-center justify-between mb-3 px-1">
                <span className="text-[12px] font-bold uppercase tracking-wider text-[var(--mu)] flex items-center gap-1.5">
                  <List size={14} /> {msg.sources.length} Sumber
                </span>
              </div>

              {/* Scroll horizontal chip sumber */}
              <div className="flex overflow-x-auto pb-2 -mx-1 px-1 gap-2.5" style={{ scrollbarWidth: 'none' }}>
                {msg.sources.map((src, si) => {
                  const { displayDomain, logoUrl, fallbackLogo } = getSourceInfo(src);
                  return (
                    <button
                      key={si}
                      onClick={() => setShowSourcesModal(true)}
                      className="flex items-center gap-2.5 px-3.5 py-2 bg-[var(--bg)] border border-[var(--bd)] rounded-xl hover:bg-[var(--sf)] transition-all shadow-sm flex-shrink-0 active:scale-[0.98]"
                    >
                      <img
                        src={logoUrl}
                        alt=""
                        className="w-5 h-5 rounded-md bg-white object-cover"
                        onError={(e) => { e.currentTarget.src = fallbackLogo; }}
                      />
                      <span className="text-[13px] font-bold text-[var(--text)] whitespace-nowrap">{displayDomain}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <AnimatePresence>
            {showSourcesModal && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-[500] bg-black/40 flex flex-col justify-end sm:justify-center items-center p-0 sm:p-4"
              ...
              >
                <motion.div
                  ...
                  className="bg-[var(--bg)] w-full sm:max-w-lg h-[85vh] sm:h-[600px] rounded-t-[24px] sm:rounded-2xl flex flex-col overflow-hidden shadow-2xl border border-[var(--bd)]"
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="flex items-center justify-between p-4 border-b border-[var(--bd)] bg-[var(--sf)]">
                    <h3 className="font-bold text-[16px] text-[var(--text)] flex items-center gap-2">
                      <Globe size={18} className="text-[var(--ac)]" /> Daftar Sumber Artikel
                    </h3>
                    <button
                      onClick={() => setShowSourcesModal(false)}
                      className="p-2 hover:bg-[var(--bd)] rounded-full transition-colors text-[var(--mu)] hover:text-[var(--text)]"
                    >
                      <X size={20} />
                    </button>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
                    {msg.sources?.map((src, i) => {
                      const { cleanTitle, displayDomain, logoUrl, fallbackLogo } = getSourceInfo(src);
                      return (
                        // ── Link langsung ke artikel — href pakai src.url asli dari GNews ──
                        <a
                          key={i}
                          href={src.url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex gap-4 p-4 border-b border-[var(--bd)] hover:bg-[var(--sf)] transition-colors group"
                        >
                          <div className="text-[12px] font-black text-[var(--mu)] mt-1 w-5 flex-shrink-0">{i + 1}.</div>
                          <div className="flex-1 min-w-0">
                            <h4 className="font-bold text-[15px] text-[var(--text)] leading-snug group-hover:text-[var(--ac)] transition-colors line-clamp-3">
                              {cleanTitle}
                            </h4>
                            <div className="flex items-center gap-2 mt-2">
                              <img
                                className="w-5 h-5 rounded bg-white shadow-sm border border-[var(--bd)] flex-shrink-0"
                                src={logoUrl}
                                onError={(e) => { e.currentTarget.src = fallbackLogo; }}
                                alt=""
                              />
                              <span className="text-[12px] font-semibold text-[var(--mu)] truncate">
                                {displayDomain}
                              </span>
                              <ExternalLink size={11} className="text-[var(--mu)] flex-shrink-0 ml-auto" />
                            </div>
                          </div>
                        </a>
                      );
          ...
          {/* Map cards kalau ada sumber maps */}
          {!isUser && (
            <div className="flex flex-col gap-2 mt-2">
              {msg.sources
                ?.filter(s => s.url.includes('google.com/maps') || s.url.includes('goo.gl/maps'))
                .map((mapSrc, mi) => (
      ...
      {!isUser && (
        <div className="flex items-center gap-[28px] mt-1 relative">
          ...
          <button onClick={() => setLiked(l => l === 'up' ? null : 'up')} title="Suka" className={cn("transition-colors", liked === 'up' ? "text-blue-500" : "text-[var(--mu)] hover:text-[var(--text)]")}><ThumbsUp size={18} /></button>
          <button onClick={() => setLiked(l => l === 'down' ? null : 'down')} title="Tidak Suka" className={cn("transition-colors", liked === 'down' ? "text-red-400" : "text-[var(--mu)] hover:text-[var(--text)]")}><ThumbsDown size={18} /></button>
          <button onClick={() => onRegenerate?.(msgIndex)} title="Ulangi" className="text-[var(--mu)] hover:text-[var(--text)] transition-colors"><RotateCcw size={18} /></button>
          ...
          <button onClick={() => onSaveItem?.({ type: 'teks', title: 'Pesan AI Cylen', preview: msg.content.substring(0, 50) + '...', content: msg.content })} title="Simpan" className="text-[var(--mu)] hover:text-[var(--text)] transition-colors">
            <Bookmark size={18} />
          </button>
        </div>
      )}
...
// ── Memo comparison yang bener — include sources biar re-render pas sumber datang ──
export const ChatBubble = memo(ChatBubbleComponent, (prev, next) => {
  return (
    ...
    JSON.stringify(prev.msg.sources) === JSON.stringify(next.msg.sources) &&
