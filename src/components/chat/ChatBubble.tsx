...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({
  ...
  return (
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start", showUserMenu ? "z-[100]" : "z-10")}>
      ...
      {/* ── ACTION BAR — icon lebih tebal, jarak lebih rapat ── */}
      {!isUser && (
        <div className="flex items-center gap-[16px] mt-1 relative">
          <button
            onClick={handleCopy}
            className={cn("transition-colors", copied ? "text-black" : "text-[var(--mu)] hover:text-[var(--text)]")}
          >
            {copied
          ...
          <button className="text-[var(--mu)] hover:text-[var(--text)] transition-colors">
            <Share2 size={18} strokeWidth={2.5} />
          </button>

          <button
            onClick={() => setLiked(l => l === 'up' ? null : 'up')}
            className={cn("transition-colors", liked === 'up' ? "text-black" : "text-[var(--mu)] hover:text-[var(--text)]")}
          >
          ...
          <button
            ...
            className={cn("transition-colors", liked === 'down' ? "text-black" : "text-[var(--mu)] hover:text-[var(--text)]")}
          >
      ...
      {/* ── SUGGESTIONS — teks tebal, lebih gelap ── */}
        ...
        <div className="flex flex-col gap-[10px] mt-[32px]">
          {suggestions.map((s, i) => (
            ...
            >
              ...
              >
                <path d="M3 1 L3 4" />
                <path d="M3 4 Q3 8 7 8 L15 8" />
                <polyline points="12,5.5 15,8 12,10.5" />
              </svg>

              {/* Teks suggestion — tebal dan lebih gelap */}
              <span
                className="text-[14.5px] group-hover/sug:text-[var(--text)] transition-colors leading-snug"
                style={{ fontWeight: 700, color: 'var(--text)', opacity: 0.65 }}
              >
                {s}
              </span>
            </button>
