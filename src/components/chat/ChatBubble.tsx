...
// ─── 🚀 SMART IMAGE GALLERY (NO CROP GRID, SWIPEABLE, NATIVE FEEL) ───
const SmartImageGallery = memo(({ images }: { images: string[] }) => {
  ...
  return (
    <>
      {/* ─── TAMPILAN DI DALAM CHAT (BUBBLE) ─── */}
      <div className="w-full mb-2 mt-1">
        
        {/* JIKA 1 GAMBAR: Proporsional Penuh, ga dipaksa kotak */}
        {images.length === 1 ? (
          <div 
            onClick={() => openViewer(0)} 
            className="w-fit max-w-full rounded-[20px] overflow-hidden cursor-pointer shadow-sm border border-[var(--bd)]/20 relative bg-[var(--sf)]/20 inline-block"
          >
            <img src={images[0]} alt="Uploaded" className="w-full max-w-full h-auto max-h-[500px] object-contain" />
            <div className="absolute inset-0 bg-black/0 hover:bg-black/10 transition-colors" />
          </div>
        ) : (
          /* JIKA BANYAK GAMBAR: Swipe Ke Samping (Horizontal Scroll), Gak Ada Grid Album Kaku! */
          <div className="flex gap-2 overflow-x-auto pb-2 snap-x" style={{ scrollbarWidth: 'none' }}>
            {images.map((src, i) => (
              <div 
                key={i} 
                onClick={() => openViewer(i)} 
                // h-[220px] konstan, tapi lebarnya (w-auto) ngikutin rasio asli gambarnya.
                className="shrink-0 rounded-[20px] overflow-hidden cursor-pointer shadow-sm border border-[var(--bd)]/20 snap-start bg-[var(--sf)]/20 relative"
              >
                <img src={src} alt={`Uploaded ${i+1}`} className="h-[220px] sm:h-[280px] w-auto min-w-[120px] object-cover" />
                <div className="absolute inset-0 bg-black/0 hover:bg-black/10 transition-colors" />
              </div>
            ))}
          </div>
