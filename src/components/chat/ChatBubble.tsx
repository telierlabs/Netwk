...
import {
  ...
  Edit2, Clock, Globe, List, Trash2, ArrowLeft
} from 'lucide-react';
...
// ─── 🚀 SMART IMAGE GALLERY (VERSI GEDE MAKAN LAYAR) ───
const SmartImageGallery = memo(({ images }: { images: string[] }) => {
  ...
  return (
    <>
      {/* ─── TAMPILAN DI DALAM CHAT ─── */}
      {/* w-[85vw] untuk mobile bikin gambar ngelebar pol ke samping, sm:w-[600px] buat batas Desktop biar ga kebesaran */}
      <div className="w-[85vw] sm:w-[600px] max-w-full mb-2 mt-1 flex flex-col gap-1.5">
        
        {/* JIKA 1 GAMBAR */}
        {images.length === 1 && (
          <div onClick={() => openViewer(0)} className="w-full rounded-2xl overflow-hidden cursor-pointer shadow-sm border border-[var(--bd)]/20 relative group bg-[var(--bg)]">
            <img src={images[0]} alt="Uploaded" className="w-full h-auto max-h-[500px] object-cover" />
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
          </div>
        )}

        {/* JIKA 2 GAMBAR: Berdiri sejajar memanjang ke bawah */}
        {images.length === 2 && (
          <div className="grid grid-cols-2 gap-1.5 rounded-2xl overflow-hidden shadow-sm border border-[var(--bd)]/20 bg-[var(--bg)] w-full h-[250px] sm:h-[350px]">
            {images.map((src, i) => (
              <div key={i} onClick={() => openViewer(i)} className="col-span-1 relative cursor-pointer group">
        ...
        {/* JIKA 3 GAMBAR: 1 Atas, 2 Bawah */}
        {images.length === 3 && (
          <div className="grid grid-cols-2 grid-rows-2 gap-1.5 rounded-2xl overflow-hidden shadow-sm border border-[var(--bd)]/20 bg-[var(--bg)] w-full h-[300px] sm:h-[450px]">
            <div onClick={() => openViewer(0)} className="col-span-2 row-span-1 relative cursor-pointer group">
              <img src={images[0]} alt="Uploaded 1" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
            </div>
            <div onClick={() => openViewer(1)} className="col-span-1 row-span-1 relative cursor-pointer group">
              <img src={images[1]} alt="Uploaded 2" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
            </div>
            <div onClick={() => openViewer(2)} className="col-span-1 row-span-1 relative cursor-pointer group">
              <img src={images[2]} alt="Uploaded 3" className="w-full h-full object-cover" />
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
            </div>
          </div>
        )}

        {/* JIKA 4+ GAMBAR: Grid 2x2 proporsional */}
        {images.length >= 4 && (
          <div className="grid grid-cols-2 grid-rows-2 gap-1.5 rounded-2xl overflow-hidden shadow-sm border border-[var(--bd)]/20 bg-[var(--bg)] w-full h-[300px] sm:h-[450px]">
            {images.slice(0, 4).map((src, i) => (
              <div key={i} onClick={() => openViewer(i)} className="col-span-1 row-span-1 relative cursor-pointer group">
                <img src={src} alt={`Uploaded ${i+1}`} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors" />
                {i === 3 && images.length > 4 && (
      ...
      {/* ─── FULLSCREEN VIEWER (PURE BLACK, NO HEADER, NO INPUT) ─── */}
      <AnimatePresence>
        {viewerOpen && (
          <motion.div 
            initial={{ opacity: 0 }} 
            ...
            className="fixed inset-0 z-[99999] bg-[#000000] flex flex-col items-center justify-center touch-none"
          >
...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({ msg, msgIndex, isLast, onResend, onEdit, onSuggest, onTogglePin, onSaveItem, onRegenerate, onSwipeToReply, suggestions }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent | TouchEvent) => setShowUserMenu(false);
    if (showUserMenu) {
  ...
  return (
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start")}>
      ...
      {((msg.images && msg.images.length > 0) || msg.image) && (
        <SmartImageGallery images={(msg.images && msg.images.length > 0) ? msg.images : [msg.image!]} />
      )}

      {msg.content && (
        <div ref={bubbleRef} className={cn("msg-bubble rounded-[22px] transform-gpu relative", showUserMenu ? "z-[100]" : "z-10", isUser ? "bg-[var(--sf)] border border-[var(--bd)] text-[var(--text)] rounded-tr-[6px] max-w-[85%] shadow-sm" : "bg-transparent text-[var(--text)] px-0 w-full")}>
          {isUser ? (
            <>
              ...
              <AnimatePresence>
                {showUserMenu && (
                  <motion.div onClick={(e) => e.stopPropagation()} initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -10 }} transition={{ duration: 0.15 }} className="absolute z-[200] w-[160px] bg-[var(--bg)] backdrop-blur-xl border border-[var(--bd)] rounded-[16px] shadow-xl flex flex-col p-1.5" style={{ top: userMenuPos.top, right: userMenuPos.right }}>
