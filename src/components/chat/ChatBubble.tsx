...
const PresentationRenderer = ({ content }: { content: string }) => {
  ...
  return (
    <div className="my-6 border-2 border-black rounded-2xl overflow-hidden bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] w-full">
...
const DocumentRenderer = ({ content }: { content: string }) => {
  ...
  return (
    <div className="my-6 bg-white border border-[var(--bd)] rounded-2xl shadow-sm overflow-hidden w-full">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
      ...
      <div className="flex items-center justify-between px-3 h-[60px] bg-[var(--bg)] border-b border-[var(--bd)] shrink-0">
        <div className="flex items-center gap-2">
          <button onClick={() => setIsExpanded(false)} className="p-2 hover:bg-[var(--sf)] rounded-lg text-[var(--mu)] hover:text-[var(--text)] transition-colors flex items-center justify-center">
            <ChevronLeft size={24} />
          </button>
          <span className="font-semibold text-[15px] text-[var(--text)]">Simulasi Visual</span>
        </div>
        <div className="relative flex items-center">
          <button onClick={() => setShowMenu(!showMenu)} className="p-2 hover:bg-[var(--sf)] rounded-lg text-[var(--mu)] hover:text-[var(--text)] transition-colors">
            ...
            {showMenu && (
              <>
                ...
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                  ...
                  <button onClick={handleDownload} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-[var(--text)] hover:bg-[var(--sf)] transition-colors text-left">
      ...
      <div className="flex-1 w-full min-h-0 bg-white relative">
        <iframe srcDoc={content} title="Preview Fullscreen" className="w-full h-full border-none" sandbox="allow-scripts allow-same-origin allow-forms" />
      </div>
    </motion.div>
  );
};
...
const CollapsibleUserBubble = ({ content, onClick }: { content: string; onClick: () => void }) => {
  ...
  if (!isLong) {
    return (
      <div onClick={onClick} className="px-4 py-3 cursor-pointer active:opacity-80 transition-opacity">
        <span className="text-[15px] leading-relaxed" style={{ wordBreak: 'break-word' }}>{content}</span>
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      <div onClick={onClick} className="px-4 pt-3 cursor-pointer active:opacity-80 transition-opacity">
      ...
      <button
        ...
        className="flex items-center gap-1 px-4 pb-2 pt-1 text-[13px] font-semibold opacity-70 hover:opacity-100 transition-opacity"
        style={{ color: 'var(--at)' }}
      >
        {expanded ? (
          <><ChevronUp size={14} /> Ciutkan</>
        ) : (
          <><ChevronDown size={14} /> Lihat selengkapnya</>
        )}
      </button>
    </div>
  );
      ...
      {msg.content && (
        <div
          className={cn(
            ...
            isUser
              ? "bg-[var(--ac)] text-[var(--at)] rounded-tr-none max-w-[78%]"
