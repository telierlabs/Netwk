...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({ msg, msgIndex, isLast, onResend, onEdit, onSuggest, onTogglePin, onSaveItem, onRegenerate, onSwipeToReply, suggestions }) => {
  ...
  // 🔥 Fungsi Ekstrak Tampilan Tanpa Merusak URL Asli
  const getSourceInfo = (src: { title: string; url: string }) => {
    ...
    // Pakai Judul Asli Artikel langsung dari GNews
    const articleTitle = src.title && src.title !== 'Referensi' && src.title !== 'Sumber' ? src.title : `Artikel dari ${displayDomain}`;
    ...
    return { articleTitle, displayDomain, logoUrl, fallbackLogo };
  };
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto p-2" style={{ scrollbarWidth: 'thin' }}>
                    {msg.sources?.map((src, i) => {
                      // 🔥 DI SINI LINK ASLI src.url DIPAKAI. KLIK -> MASUK ARTIKEL 🔥
                      const { articleTitle, displayDomain, logoUrl, fallbackLogo } = getSourceInfo(src);
                      return (
                        <a 
                          key={i} 
                          href={src.url} 
