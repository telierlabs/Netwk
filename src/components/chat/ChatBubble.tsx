...
const MermaidDiagram = memo(({ content }: { content: string }) => {
  ...
  const portalContent = isFullscreen ? (
    <div className="fixed inset-0 z-[999999] bg-[var(--bg)] flex flex-col touch-none select-none overflow-hidden animate-in fade-in zoom-in-95 duration-200">
      {/* Header Atas */}
      <div className="flex items-center justify-between p-4 border-b border-[var(--bd)] shrink-0 bg-[var(--bg)] relative z-20">
  ...
  return (
    <>
      <div 
        ...
        className="my-4 w-full bg-[var(--cd)] border border-[var(--bd)] rounded-2xl p-6 overflow-hidden shadow-sm transform-gpu flex flex-col items-center cursor-pointer hover:border-[var(--text)]/40 transition-all group relative min-h-[200px]"
      >
        {/* CSS INJEKSI OTOMATIS MENYESUAIKAN TEMA CYLEN USER BIAR WARNA JELAS */}
        <div 
          dangerouslySetInnerHTML={{ __html: svgCode }} 
