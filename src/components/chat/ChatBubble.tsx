...
import {
  ...
  Check, MapPin, Navigation, Download,
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex items-center justify-center relative overflow-hidden min-h-0">
        ...
        {total > 1 && (
          <>
            <button onClick={() => setCurrent(c => (c - 1 + total) % total)} className="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 bg-black/40 rounded-full flex items-center justify-center text-white"><ChevronLeft size={22} /></button>
            <button onClick={() => setCurrent(c => (c + 1) % total)} className="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 bg-black/40 rounded-full flex items-center justify-center text-white"><ChevronRight size={22} /></button>
          </>
        )}
      </div>
      {total > 1 && (
        <div className="flex justify-center gap-1.5 py-4 flex-shrink-0">
          {images.map((_, i) => (
            <button key={i} onClick={() => setCurrent(i)} className="w-2 h-2 rounded-full transition-all" style={{ background: i === current ? 'white' : 'rgba(255,255,255,0.35)' }} />
          ))}
        </div>
      )}
    </motion.div>
...
// ─── IMAGE GRID ───────────────────────────────────────────────────────────────
// 1 foto → besar penuh lebar bubble
// 2 foto → 2 kolom sejajar
// 3+ foto → horizontal scroll (geser)
const ImageGrid = memo(({ images, isUser }: { images: string[]; isUser: boolean }) => {
  const [viewerOpen, setViewerOpen] = useState(false);
  const [viewerStart, setViewerStart] = useState(0);
  const count = images.length;
  const openViewer = (i: number) => { setViewerStart(i); setViewerOpen(true); };
  ...
  return (
    <>
      ...
      <div className={cn("mb-2 w-full", isUser ? "flex justify-end" : "")}>
        {count === 1 ? (
          // ── 1 foto: besar, lebar penuh ──────────────────────────────
          <div
            className="relative overflow-hidden rounded-2xl cursor-pointer border border-[var(--bd)] shadow-sm"
            style={{ width: '100%', maxWidth: 320, aspectRatio: '4/3' }}
          ...
          // ── 2 foto: 2 kolom sejajar ──────────────────────────────────
          <div className="grid gap-1" style={{ gridTemplateColumns: '1fr 1fr', maxWidth: 320, width: '100%' }}>
            {images.map((src, i) => (
              <div key={i} className="relative overflow-hidden rounded-xl cursor-pointer border border-[var(--bd)] shadow-sm" style={{ aspectRatio: '1/1' }} onClick={() => openViewer(i)}>
                <img src={src} alt="" loading="lazy" className="w-full h-full object-cover" />
              </div>
            ))}
          </div>
        ) : (
          // ── 3+ foto: horizontal scroll ───────────────────────────────
          <div
            className="flex gap-2 overflow-x-auto pb-1"
            style={{ scrollbarWidth: 'none', maxWidth: '100%', WebkitOverflowScrolling: 'touch' } as any}
          >
            {images.map((src, i) => (
              <div
                key={i}
                className="relative overflow-hidden rounded-xl cursor-pointer border border-[var(--bd)] shadow-sm flex-shrink-0"
                style={{ width: 160, height: 160 }}
                onClick={() => openViewer(i)}
              >
                <img src={src} alt="" loading="lazy" className="w-full h-full object-cover" />
                {/* nomor foto */}
                <div className="absolute bottom-1.5 right-1.5 bg-black/50 rounded-md px-1.5 py-0.5">
                  <span className="text-white text-[10px] font-bold">{i + 1}/{images.length}</span>
                </div>
              </div>
            ))}
          </div>
...
// ─── USER BUBBLE ─────────────────────────────────────────────────────────────
// ✅ TAP BIASA → popup langsung muncul (tanpa long press)
const CollapsibleUserBubble = memo(({ content, onShowMenu }: { content: string; onShowMenu: () => void }) => {
  ...
  const handleScrollToReply = (e: React.MouseEvent) => {
    ...
    for (let i = 0; i < elements.length; i++) {
      ...
      if (el.textContent?.includes(searchSnippet)) {
        ...
        if (bubble) { bubble.style.outline = '2px solid var(--ac)'; bubble.style.outlineOffset = '2px'; setTimeout(() => { bubble.style.outline = 'none'; }, 1200); }
        break;
      }
    }
  };
  ...
  return (
    <div
      className="w-full relative flex flex-col transform-gpu select-none"
      // ✅ onClick langsung → popup muncul saat tap
      onClick={onShowMenu}
      style={{ WebkitUserSelect: 'none', userSelect: 'none', WebkitTouchCallout: 'none' } as any}
    >
      {replyText && (
        <div onClick={handleScrollToReply} className="mx-2 mt-2 mb-1 bg-[var(--bg)]/40 border-l-[3px] border-[var(--ac)] p-2.5 rounded-r-[10px] rounded-bl-[10px] cursor-pointer transition-all active:scale-[0.98]">
...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({
  ...
  const handleUserMenuCopy = (e: React.MouseEvent) => {
    e.stopPropagation();
    let txt = msg.content;
    const m = txt.match(/^\[Membalas pesan: "(.*?)"\]\n\n([\s\S]*)$/);
    if (m) txt = m[2];
  ...
  const getSourceInfo = (src: { title: string; url: string }) => {
    ...
    if (src.url.includes('vertexaisearch') || src.url.includes('googleusercontent') || src.url.includes('google.com/search')) {
      ...
      let g = lastPart.replace(/[^a-z0-9.]/g, '');
      if (g.length > 2 && !g.includes('.')) g += '.com';
      if (g.length > 2) displayDomain = g;
    } else { try { displayDomain = new URL(src.url).hostname.replace(/^www\./, ''); } catch {} }
    return {
      cleanTitle, displayDomain,
      logoUrl: `https://www.google.com/s2/favicons?domain=${displayDomain}&sz=64`,
      fallbackLogo: `https://ui-avatars.com/api/?name=${encodeURIComponent(displayDomain.charAt(0))}&background=random&color=fff&size=32`,
    };
  };
    ...
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start")}>
      ...
      {msg.content && (
        <motion.div
          ...
          dragElastic={0.15}
          onDragEnd={(_, info) => { if (info.offset.x > 60) onSwipeToReply?.(msg); }}
          className={cn(
            "msg-bubble rounded-[22px] transform-gpu will-change-transform",
            isUser
              ? "bg-[var(--sf)] border border-[var(--bd)] text-[var(--text)] rounded-tr-[6px] max-w-[85%] shadow-sm relative"
        ...
        >
          {isUser ? (
            <>
              {/* ✅ TAP → popup langsung */}
              <CollapsibleUserBubble content={msg.content} onShowMenu={() => setShowUserMenu(true)} />
                ...
                {showUserMenu && (
                  <>
                    ...
                    <motion.div
                      ...
                      className="absolute right-0 top-full mt-2 z-[110] w-[165px] bg-[var(--bg)] border border-[var(--bd)] rounded-2xl shadow-2xl overflow-hidden flex flex-col p-1.5"
                      style={{ backdropFilter: 'blur(20px)' }}
                    >
                      <button onClick={(e) => { e.stopPropagation(); onResend?.(msg.content); setShowUserMenu(false); }} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl hover:bg-[var(--sf)] active:bg-[var(--bd)] transition-colors text-left">
                        <RotateCcw size={16} className="text-[var(--text)]" /><span className="text-[13.5px] font-bold text-[var(--text)]">Ulangi</span>
                      </button>
                      <button onClick={(e) => { e.stopPropagation(); let t = msg.content; const m = t.match(/^\[Membalas pesan: "(.*?)"\]\n\n([\s\S]*)$/); if (m) t = m[2]; onEdit?.(t); setShowUserMenu(false); }} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl hover:bg-[var(--sf)] active:bg-[var(--bd)] transition-colors text-left">
                      ...
                      <button onClick={handleUserMenuCopy} className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl hover:bg-[var(--sf)] active:bg-[var(--bd)] transition-colors text-left">
                        {userMenuCopied
            ...
            <div className="cylen-md w-full">
              <style>{`
                /* ✅ Teks AI bisa diseleksi, warna highlight hitam transparan */
                .cylen-md {
          ...
          {!isUser && msg.sources && msg.sources.length > 0 && (
            <div className="mt-6 pt-4 border-t border-[var(--bd)] w-full">
              <span className="text-[12px] font-bold uppercase tracking-wider text-[var(--mu)] flex items-center gap-1.5 mb-3 px-1"><List size={14} /> {msg.sources.length} Sumber</span>
                ...
                {msg.sources.map((src, si) => {
                  ...
                  return (
                    <button key={si} onClick={() => setShowSourcesModal(true)} className="flex items-center gap-2.5 px-3.5 py-2 bg-[var(--bg)] border border-[var(--bd)] rounded-xl hover:bg-[var(--sf)] transition-all shadow-sm flex-shrink-0 active:scale-[0.98]">
                      <img src={logoUrl} alt="" className="w-5 h-5 rounded-md bg-white object-cover" onError={(e) => { e.currentTarget.src = fallbackLogo; }} />
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[500] bg-black/40 flex flex-col justify-end sm:justify-center items-center p-0 sm:p-4">
                <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'tween', duration: 0.25, ease: 'easeOut' }} className="bg-[var(--bg)] w-full sm:max-w-lg h-[85vh] sm:h-[600px] rounded-t-[24px] sm:rounded-2xl flex flex-col overflow-hidden shadow-2xl border border-[var(--bd)]">
                  <div className="flex items-center justify-between p-4 border-b border-[var(--bd)] bg-[var(--sf)]">
                    <h3 className="font-bold text-[16px] text-[var(--text)] flex items-center gap-2"><Globe size={18} className="text-[var(--ac)]" /> Daftar Sumber</h3>
                    <button onClick={() => setShowSourcesModal(false)} className="p-2 hover:bg-[var(--bd)] rounded-full transition-colors text-[var(--mu)]"><X size={20} /></button>
                  </div>
      ...
      {!isUser && (
        <div className="flex items-center gap-[28px] mt-1 relative">
          ...
          <button onClick={() => onSaveItem?.({ type: 'teks', title: 'Pesan AI Cylen', preview: msg.content.substring(0, 50) + '...', content: msg.content })} className="text-[var(--mu)] hover:text-[var(--text)] transition-colors"><Bookmark size={18} /></button>
        </div>
      )}
