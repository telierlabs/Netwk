...
// 👇 KITA GANTI TEMA JADI TERANG (PRISM) BIAR KAYA GROK
...
const CodeBlock = ({ lang, content }: { lang: string; content: string }) => {
  ...
  return (
    <div className="my-4 border border-[var(--bd)] rounded-2xl overflow-hidden bg-[var(--cd)] shadow-sm">
      {/* header bar: clean, no dark backgrounds */}
      ...
      {/* code content — transparent background, relies on card bg */}
      <SyntaxHighlighter
        style={prism}
        language={prismLang}
        PreTag="div"
        customStyle={{
          ...
          fontSize: '13px',
          lineHeight: '1.65',
          background: 'transparent', // Biar nyatu sama warna kartu
          borderRadius: 0,
        }}
        codeTagProps={{
...
const HtmlPreview = ({ content }: { content: string }) => {
  ...
  return (
    <div className="my-3 border border-[var(--bd)] rounded-2xl overflow-hidden shadow-sm bg-[var(--cd)]">
      <div className="flex items-center justify-between px-4 py-2.5 bg-[var(--sf)] border-b border-[var(--bd)]">
        <div className="flex items-center gap-2">
          ...
          <div className="ml-2 px-3 py-1 bg-[var(--bg)] border border-[var(--bd)] rounded-md flex items-center gap-1.5" style={{ minWidth: 120 }}><span style={{ fontSize: 10, color: 'var(--mu)', fontFamily: 'monospace' }}>preview.cylen</span></div>
        </div>
        <div className="flex items-center gap-2">
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--mu)' }}>Live Preview</span>
          <button onClick={() => setIsExpanded(p => !p)} className="p-1 hover:bg-[var(--bd)] rounded transition-colors text-[var(--mu)]"><ExternalLink size={13} /></button>
        </div>
      </div>
      <iframe srcDoc={content} title="Preview" className="w-full border-none bg-white" style={{ height: isExpanded ? 600 : 320 }} sandbox="allow-scripts allow-same-origin" />
    </div>
  );
};
          ...
          {isUser ? (
            ...
            <div className="cylen-md w-full">
              <style>{`
                ...
                /* Hapus background tebel dari inline code, bikin cuma bold dan warna teks aja */
