...
// ─── MERMAID DIAGRAM RENDERER (DENGAN PENANGKAL ERROR) ───
const MermaidDiagram = memo(({ content }: { content: string }) => {
  ...
  useEffect(() => {
    ...
    const renderDiagram = async () => {
      try {
        ...
        // Membersihkan karakter backtick sisa dari markdown
  ...
  // Jika terjadi error render, tampilkan pesan error dan kode mentahnya
  if (errorMsg) {
    return (
      <div className="my-4 w-full bg-[var(--cd)] border border-red-500/30 rounded-2xl p-4 shadow-sm flex flex-col gap-2 transform-gpu">
        <div className="text-[12px] text-red-500 font-bold px-1">Gagal merender diagram:</div>
        <div className="p-3 text-red-400 text-[11px] bg-red-500/10 rounded-xl font-mono overflow-x-auto whitespace-pre-wrap leading-relaxed">
          {errorMsg}
        </div>
        <div className="mt-2 text-[11px] font-bold text-[var(--mu)] px-1">Kode Sumber:</div>
  ...
  return (
    <div className="my-4 w-full bg-[var(--cd)] border border-[var(--bd)] rounded-2xl p-6 overflow-x-auto shadow-sm transform-gpu flex justify-center items-center">
      <div dangerouslySetInnerHTML={{ __html: svgCode }} className="max-w-full [&>svg]:max-w-full [&>svg]:h-auto" />
    </div>
  );
});
