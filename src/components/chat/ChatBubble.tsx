...
//   • FIX (kartu file): early-return `<ActivityBubble/>` DIHAPUS. Dulu saat
//     loading seluruh bubble diganti ActivityBubble, lalu saat selesai diganti
//     lagi dengan bubble biasa + FileLinkElement (kartu persegi panjang) —
//     dua render terpisah, makanya kalimat pembuka hilang & bentuk kartu
//     berubah. Sekarang SATU render dengan urutan tetap:
//        kalimat pembuka → satu kartu persegi → teks penutup
//     `FileActivityBlock` tetap ter-mount di posisi yang sama dari loading
//     sampai selesai, jadi kartunya berubah state, bukan diganti baru.
//   • Teks penutup & action bar ditahan sampai file selesai dibuat.
// ─────────────────────────────────────────────
...
const ChatBubbleComponent: React.FC<ChatBubbleProps> = ({
  ...
  // Lagi ada aktivitas bikin file/gambar untuk pesan terakhir ini?
  const isLiveActivity = !isUser && !!isLast && activityStatus !== 'idle';
  // Blok kalimat pembuka + kartu: tampil selama loading DAN setelah file jadi.
  const showFileBlock = !isUser && (isLiveActivity || !!fileLinkData);

  // ── Loading dots saat streaming tapi belum ada konten ──
  // (dilewati kalau lagi aktivitas file: dots-nya sudah ditangani FileActivityBlock)
  if (!isUser && isStreaming && !isLiveActivity && (!displayContent || displayContent.trim() === '')) {
    return (
      <div
        id={`message-${msgIndex}`}
        style={{
  ...
  return (
    ...
    >
      ...
      {/* Kalimat pembuka + kartu file (satu blok, satu instance loading → selesai) */}
      {showFileBlock && (
        <FileActivityBlock
          activityStatus={isLiveActivity ? activityStatus : 'idle'}
          file={fileLinkData}
        />
      )}
      ...
      {/* Konten teks utama — ditahan selama file masih dibuat (teks penutup
          muncul di bawah kartu setelah kartu selesai) */}
      {displayContent && !isLiveActivity && (
        isUser ? (
          <UserBubble
            msg={msg}
          ...
          <motion.div
            ...
            // Fade halus HANYA untuk pesan yang punya kartu file, supaya teks
            // penutup muncul setelah kartu selesai berubah. Pesan biasa:
            // initial={false} → tidak ada animasi sama sekali (perilaku lama).
            initial={fileLinkData ? { opacity: 0 } : false}
            animate={{ opacity: 1 }}
      ...
      {/* Action bar — HANYA setelah actionsUnlocked & bukan streaming & file sudah jadi */}
      {!isUser && (
        <AnimatePresence>
          {actionsUnlocked && !isStreaming && !isLiveActivity && (
            <motion.div
              key="actions"
      ...
      {/* Suggestions — HANYA setelah actionsUnlocked & bukan streaming & file sudah jadi */}
