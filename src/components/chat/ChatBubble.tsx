...
import {
  ...
  X, Pin, PinOff, Bookmark // 👇 BARU: Nambahin icon Bookmark
} from 'lucide-react';
  ...
  return (
    <div className="my-4 rounded-xl overflow-hidden border border-[var(--bd)] shadow-sm">
      <div className="bg-[#1a1b26] px-4 py-2 border-b border-white/10 flex items-center justify-between">
        ...
        <button onClick={()=>{navigator.clipboard.writeText(content);setCopied(true);setTimeout(()=>setCopied(false),2000);}}
          className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white/20 transition-colors text-[11px] text-white/60 font-medium">
          {copied?<Check size={11} className="text-green-400"/>:<Copy size={11}/>}{copied?'Tersalin':'Salin'}
        </button>
      </div>
      <SyntaxHighlighter style={atomDark} language={lang} PreTag="div" customStyle={{margin:0,padding:'1rem',fontSize:'13px',lineHeight:'1.6'}}>
        {content}
      </SyntaxHighlighter>
    </div>
  );
    ...
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full", isUser ? "items-end" : "items-start")}>
      ...
      {!isUser && (
        <div className="flex items-center gap-[28px] mt-1 relative">
          ...
          {/* 👇 BARU: Tombol Simpan (Bookmark) */}
          <button 
            onClick={() => onSaveItem?.({ type: 'text', content: msg.content })} 
            title="Simpan" 
