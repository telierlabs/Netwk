...
import { auth } from '../../lib/firebase'; // ── FIX: Ambil auth untuk cek kepemilikan pesan ──
...
const CollapsibleUserBubble = memo(({ content, onClick, isMine }: { content: string; onClick: (e: React.MouseEvent) => void, isMine: boolean }) => {
    ...
    <div className="w-full relative flex flex-col transform-gpu">
      {replyText && (
        <div className="mx-2 mt-2 mb-1 bg-[var(--bg)]/40 border-l-[3px] border-[var(--ac)] p-2.5 rounded-r-[10px] rounded-bl-[10px]">
          <span className="text-[11px] font-black text-[var(--ac)] block mb-0.5 uppercase">Membalas</span>
          <span className="text-[12.5px] text-[var(--text)]/80 line-clamp-2 leading-snug">{replyText}</span>
        </div>
      )}
      {/* ── FIX: Cuma bisa diklik (buat edit/ulangi) kalau pesannya milik user sendiri ── */}
      <div onClick={isMine ? onClick : undefined} className={cn("px-4 pb-3 select-none", replyText ? "pt-1" : "pt-3", isMine ? "cursor-pointer active:opacity-80 transition-opacity" : "")}>
...
const GroupChatBubbleComponent: React.FC<GroupChatBubbleProps> = ({
  ...
  // ── FIX: LOGIKA BUBBLE KANAN/KIRI BERDASARKAN ID PENGIRIM ──
  const isMine = msg.senderId === auth.currentUser?.uid;
  const isAI = msg.role === 'assistant';
  const isOtherUser = !isMine && !isAI; // Ini pesan temen lu
  const senderName = msg.senderName || (isMine ? 'Kamu' : 'AI');
  ...
  if (isAI && contentToRender.includes(pptStartTag)) {
    hasPpt = true;
    ...
    if (contentToRender.includes(pptEndTag)) {
      ...
      contentToRender = contentToRender.slice(0, startIndex).trim(); 
    } else {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // ── FIX: Kalau punya kita = Kanan. Kalau temen/AI = Kiri ──
        isMine ? "items-end" : "items-start",
        showUserMenu ? "z-[100]" : "z-10"
      )}
    >
      <div className={cn("flex items-end gap-2 mb-1.5", isMine ? "flex-row-reverse" : "flex-row")}>
          ...
          >
            <motion.div
              ...
              className={cn(
                ...
                isMine || isOtherUser
                  // Desain untuk bubble manusia (Kita & Temen)
                  ? `border border-[var(--bd)] text-[var(--text)] max-w-[85%] shadow-sm ${isMine ? "bg-[var(--sf)] rounded-tr-[6px] ml-auto" : "bg-transparent rounded-tl-[6px] mr-auto"}`
                  // Desain untuk bubble AI
                  : "bg-transparent text-[var(--text)] px-0 w-full"
              )}
            >
              {isMine || isOtherUser ? (
                <>
                  <CollapsibleUserBubble content={msg.content} onClick={openUserMenu} isMine={isMine} />
                  <AnimatePresence>
                    {/* Menu Edit/Salin cuma nongol di pesan punya kita sendiri */}
                    {showUserMenu && isMine && (
                      <motion.div
