...
// ── SPINNER 2 BAGIAN TERBELAH ──
// Dua busur (atas+kanan vs bawah+kiri) berputar berlawanan → efek terbelah
const SplitSpinner = () => (
  <div className="flex items-center gap-2.5">
    <style>{`
      @keyframes gcb-cw  { to { transform: rotate(360deg);  } }
      @keyframes gcb-ccw { to { transform: rotate(-360deg); } }
      .gcb-spin-cw  { animation: gcb-cw  0.85s linear infinite; }
      .gcb-spin-ccw { animation: gcb-ccw 0.85s linear infinite; }
    `}</style>
    <div className="relative w-[16px] h-[16px] shrink-0">
      {/* Busur atas-kanan — searah jarum jam */}
      <div className="gcb-spin-cw absolute inset-0" style={{
        borderRadius: '50%',
      ...
      {/* Busur bawah-kiri — berlawanan jarum jam */}
      <div className="gcb-spin-ccw absolute inset-0" style={{
        borderRadius: '50%',
        border: '2.5px solid transparent',
        borderBottomColor: 'var(--mu)',
        borderLeftColor: 'var(--mu)',
        opacity: 0.55,
      }} />
    </div>
    <span className="text-[10px] font-bold text-[var(--text)]/50 uppercase tracking-widest select-none">
      Mengetik...
    </span>
  </div>
);
...
const GroupChatBubbleComponent: React.FC<GroupChatBubbleProps> = ({
  ...
  const isAILoading = !isUser && isLast && isSending;
  ...
  return (
    ...
    >
      ...
      {/* ══════════════════════════════════════════════════════
          SPINNER — sticky di atas, TIDAK ikut turun bersama
          konten yang sedang streaming ke bawah.

          Kuncinya: spinner berada di LUAR motion.div bubble,
          dan pakai `position: sticky; top: <nilai>` supaya
          tetap terlihat di viewport walau konten tumbuh.
      ══════════════════════════════════════════════════════ */}
      {isAILoading && (
        <div
          style={{
            position: 'sticky',
            top: 10,      // jarak dari tepi atas viewport
            zIndex: 50,
      ...
      {/* ── BUBBLE KONTEN — tumbuh ke bawah secara normal ── */}
      {(msg.content || isAILoading) && (
        <div className="w-full flex">
          ...
          >
            {isUser ? (
              ...
              /* Konten AI — hanya konten teks, tanpa spinner di sini */
              msg.content && (
      ...
      {/* ── TOOLBAR BAWAH AI (hanya muncul setelah selesai) ── */}
