...
const GroupChatBubbleComponent: React.FC<GroupChatBubbleProps> = ({
  ...
  // FUNGSI SWIPE BERLAKU UNTUK KEDUA BUBBLE (AI & USER)
  ...
  return (
    <div id={`group-message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu mt-2 mb-3", isUser ? "items-end" : "items-start", showUserMenu ? "z-[100]" : "z-10")}>
      ...
      {/* ── BUBBLE KONTEN DENGAN MOTION SWIPE ── */}
      {(msg.content || (!isUser && isLast && isSending)) && (
        <div className={cn(
          "msg-bubble rounded-[22px] transform-gpu relative z-10 w-full", 
          isUser ? "flex justify-end" : "flex justify-start"
        )}>
          {/* DRAGGABLE WRAPPER (Bisa digeser untuk User dan AI) */}
          <motion.div 
            drag="x" 
            ...
            className={cn(
               isUser ? "bg-[var(--sf)] border border-[var(--bd)] text-[var(--text)] rounded-tr-[6px] max-w-[85%] shadow-sm" : "bg-transparent text-[var(--text)] px-0 w-full"
            )}
          >
            {isUser ? (
              <>
              ...
              <>
                {/* SPINNER 2 BAGIAN KEPOTONG (SELALU DI ATAS KONTEN) */}
                {!isUser && isLast && isSending && (
                  <div className="flex items-center gap-2.5 mb-3 ml-1 mt-1">
                    <div className="w-[14px] h-[14px] border-[2px] border-[var(--text)] border-t-transparent border-b-transparent rounded-full animate-spin opacity-60" />
                    <span className="text-[10px] font-bold text-[var(--text)]/50 uppercase tracking-widest">Mengetik...</span>
                  </div>
                )}
