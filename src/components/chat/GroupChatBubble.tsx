...
// ════════════════════════════════════════════════
//  SPLIT SPINNER — 2 busur berlawanan, ikut mode
//  Tidak ada bubble, tidak ada warna custom,
//  tidak ada titik. Bersih & elegan.
// ════════════════════════════════════════════════
const SplitSpinner = () => (
  <>
    <style>{`
      @keyframes _gcb_cw  { to { transform: rotate(360deg);  } }
      @keyframes _gcb_ccw { to { transform: rotate(-360deg); } }
      ._gcb_cw  { animation: _gcb_cw  0.75s linear infinite; }
      ._gcb_ccw { animation: _gcb_ccw 0.75s linear infinite; }
    `}</style>
    <div className="relative w-[15px] h-[15px] shrink-0">
      {/* Busur atas-kanan — terang, searah */}
      <div className="_gcb_cw absolute inset-0" style={{
        borderRadius: '50%',
        border: '2px solid transparent',
        borderTopColor:   'var(--text)',
        borderRightColor: 'var(--text)',
      }} />
      {/* Busur bawah-kiri — redup, berlawanan */}
      <div className="_gcb_ccw absolute inset-0" style={{
        borderRadius: '50%',
        border: '2px solid transparent',
...
const GroupChatBubbleComponent: React.FC<GroupChatBubbleProps> = ({
  ...
  // true = AI belum ada konten sama sekali (pure loading)
  const isAILoadingOnly = !isUser && isLast && isSending && !msg.content;
  // true = AI sedang streaming (konten sudah sebagian masuk)
  const isAIStreaming   = !isUser && isLast && isSending && !!msg.content;
  ...
  return (
    ...
    >
      ...
      {/* ════════════════════════════════════════════════
          SPINNER — muncul di bawah nama saat pure loading.
          Fade out begitu konten pertama masuk.
          Tidak ada bubble, tidak ada warna, tidak ada teks.
      ════════════════════════════════════════════════ */}
      <AnimatePresence>
        {isAILoadingOnly && (
          <motion.div
          ...
          >
            ...
            >
              {isUser ? (
                ...
                /* ── KONTEN AI — tumbuh ke bawah, spinner sudah fade out ── */
