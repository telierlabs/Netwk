...
import {
  ...
  Pin, PinOff, Edit2, ChevronDown, ChevronUp
} from 'lucide-react';
...
interface GroupChatBubbleProps {
  ...
  msgIndex: number;
  isLast?: boolean;
  isSending?: boolean; // Tmbahan prop buat deteksi loading
  ...
  return (
    <div id={`group-message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu mt-2 mb-3", isUser ? "items-end" : "items-start", showUserMenu ? "z-[100]" : "z-10")}>
      ...
      <div className={cn("flex items-end gap-2 mb-1.5", isUser ? "flex-row-reverse" : "flex-row")}>
        {!isUser && (
           <div className="w-6 h-6 rounded-full bg-[var(--text)] text-[var(--bg)] flex items-center justify-center font-bold text-[10px] shrink-0 shadow-sm border border-[var(--bd)]/20">
             {senderDisplayName[0]}
           </div>
        )}
      ...
      {/* ── BUBBLE KONTEN ── */}
      {(msg.content || (!isUser && isLast && isSending)) && (
        <div className={cn(
          ...
          {isUser ? (
            <>
              <motion.div drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.1} onDragEnd={handleDragEnd}>
            ...
            <motion.div drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.1} onDragEnd={handleDragEnd} className="w-full">
              
              {/* ── SPINNER 2 BAGIAN KEPOTONG (DI ATAS KONTEN) ── */}
              {!isUser && isLast && isSending && (
                <div className="flex items-center gap-3 mb-2 ml-1">
                  <div className="w-[16px] h-[16px] border-[2.5px] border-[var(--text)] border-t-transparent border-b-transparent rounded-full animate-spin opacity-80" />
                  <span className="text-[11px] font-bold text-[var(--text)]/50 uppercase tracking-widest">Menganalisa...</span>
                </div>
              )}
...
export const GroupChatBubble = memo(GroupChatBubbleComponent, (prev, next) => {
  return prev.msg.content === next.msg.content && prev.msg.pinned === next.msg.pinned && prev.isLast === next.isLast && prev.isSending === next.isSending;
});
