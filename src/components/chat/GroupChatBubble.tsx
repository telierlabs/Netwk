...
const GroupChatBubbleComponent: React.FC<GroupChatBubbleProps> = ({
  ...
  // FUNGSI SWIPE (geser untuk membalas)
  const handleDragEnd = (_e: any, info: PanInfo) => {
    // Kalau ditarik ke kanan lebih dari 55px, picu onSwipeToReply
    if (info.offset.x > 55) {
      onSwipeToReply?.(msg);
    }
  };
    ...
    <div id={`group-message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu mt-2 mb-3", isUser ? "items-end" : "items-start", showUserMenu ? "z-[100]" : "z-10")}>
      ...
      {msg.content && (
        <div className={cn(
          ...
          isUser
            ? "bg-[var(--text)] text-[var(--bg)] rounded-tr-[4px] max-w-[85%] shadow-md ml-auto"
            : "bg-[var(--sf)] border border-[var(--bd)]/40 text-[var(--text)] rounded-tl-[4px] max-w-[90%] shadow-sm mr-auto"
        )}>
          {isUser ? (
            <>
              {/* SWIPE UNTUK BUBBLE USER */}
      ...
      {/* ── TOOLBAR BAWAH AI (DIPOTONG CUMA NYISA 4 ICON) ── */}
      {!isUser && (
        <div className="flex items-center gap-4 mt-2 px-2 relative">
