    ...
    <div id={`message-${msgIndex}`} className={cn("flex flex-col group w-full relative transform-gpu", isUser ? "items-end" : "items-start")}>
      ...
      {msg.content && (
        <motion.div
          ...
          {/* 🔥 FIX LOGIKA LOGO & LINK: Klik = Artikel, Gambar = Domain Asli 🔥 */}
          {msg.sources && msg.sources.length > 0 && (
            <div className="flex flex-wrap gap-2.5 mt-5">
              {msg.sources.map((src, si) => {
                ...
                // Ini trik biar LOGO-nya tetep bener biarpun linknya dari Vertex AI / Google
                ...
                return (
                  // ✅ LINK URL ASLI TETAP DIPAKAI DI SINI BIAR BISA DIKLIK!
                  <a key={si} href={src.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-3.5 py-1.5 bg-[var(--bg)] border border-[var(--bd)] rounded-full text-[12px] font-medium hover:bg-[var(--sf)] transition-all shadow-sm">
                    {/* ✅ LOGONYA PAKAI DOMAIN YANG UDAH DITEBAK (BUKAN GOOGLE/VERTEX LAGI) */}
                    <img 
                      src={`https://www.google.com/s2/favicons?domain=${logoDomain}&sz=32`} 
                      alt="" 
                      className="w-4 h-4 rounded-full bg-white flex-shrink-0 object-cover" 
                      onError={(e) => { e.currentTarget.src = fallbackLogo; }} 
                    />
                    <span className="truncate max-w-[140px] text-[var(--text)]">{displayName}</span>
                  </a>
                );
              })}
            </div>
