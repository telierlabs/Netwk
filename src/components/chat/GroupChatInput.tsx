...
export const GroupChatInput: React.FC<GroupChatInputProps> = ({
  ...
  // ── FIX: STATE BUAT TOAST PERINGATAN WAJIB TAG ──
  ...
  const handleSend = () => {
    ...
    // ── FIX: VALIDASI WAJIB MENGGUNAKAN TAG @ ──
    const hasTag = /@([a-zA-Z0-9_]+)/.test(inputText);
    if (!hasTag) {
      setTagWarning(true);
      setTimeout(() => setTagWarning(false), 2500); // Notif hilang setelah 2.5 detik
  ...
  return (
    <>
      ...
      {/* ── FIX: TAMPILAN NOTIFIKASI WAJIB TAG ── */}
      <AnimatePresence>
        {tagWarning && (
           <motion.div 
             ...
             exit={{ opacity: 0, y: 10 }} 
             className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 px-4 py-2 bg-red-500 text-white text-[13px] font-bold rounded-full shadow-lg z-[200] whitespace-nowrap"
           >
             Kamu wajib menyebut minimal satu AI (contoh: @Gemini)
           </motion.div>
        )}
      </AnimatePresence>
