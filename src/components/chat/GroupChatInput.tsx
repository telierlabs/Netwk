    ...
    <>
      ...
      {lightboxUrl && (
        <div className="fixed inset-0 z-[200] bg-[#0a0a0a]/95 flex items-center justify-center p-4 pointer-events-auto" onClick={() => setLightboxUrl(null)}>
          {lightboxUrl.includes('video') ? <video src={lightboxUrl} controls autoPlay className="max-w-full max-h-full object-contain rounded-xl" /> : <img src={lightboxUrl} alt="" className="max-w-full max-h-full object-contain rounded-xl" />}
      ...
      <footer className="px-2 pt-2 pb-6 relative z-50 w-full pointer-events-auto">
        {showPicker && (
          <>
        ...
        <div className="max-w-4xl mx-auto w-full">
          {isListening ? (
            <div className="flex items-center justify-between w-full h-[64px] rounded-[32px] px-2 relative overflow-hidden bg-[var(--text)]/5 shadow-sm backdrop-blur-md border border-[var(--text)]/10">
              ...
              <button onClick={() => { setVoiceText(''); stopListening(); }} className="relative z-10 w-[46px] h-[46px] rounded-full flex items-center justify-center bg-[var(--bg)] text-[var(--text)] active:scale-95 shadow-sm [-webkit-tap-highlight-color:transparent]"><X size={20} strokeWidth={2.5} /></button>
              ...
              <button onClick={() => { stopListening(); if (voiceText) { setInputText(voiceText); setTimeout(() => handleSend(), 50); } else if (inputText) { handleSend(); } }} className="relative z-10 w-[46px] h-[46px] rounded-full flex items-center justify-center bg-[var(--text)] text-[var(--bg)] active:scale-95 shadow-md [-webkit-tap-highlight-color:transparent]">
            ...
            <div className="relative w-full">
              {/* KOTAK INPUT UTAMA (Elegan mewah kek ChatInput biasa) */}
              <div className="rounded-[32px] flex flex-col relative overflow-hidden bg-[var(--text)]/[0.04] border border-[var(--text)]/10 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md" style={{ WebkitTapHighlightColor: 'transparent' }}>
                ...
                {/* ─── KOTAK REPLY YG MUNCUL PAS DI-SWIPE ─── */}
                <AnimatePresence>
                  {replyingTo && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="px-4 pt-4">
                      <div className="bg-[var(--bg)] rounded-[16px] border-l-[4px] border-[var(--text)] p-3 pr-10 relative shadow-sm">
                        <button onClick={onCancelReply} className="absolute right-2 top-2 p-1.5 text-[var(--mu)] rounded-full active:scale-90 hover:bg-[var(--text)]/5 [-webkit-tap-highlight-color:transparent]"><X size={16} strokeWidth={2.5} /></button>
                ...
                {/* ─── TOMBOL BAWAH (MIC, PAPERCLIP, SEND) ─── */}
                <div className="flex items-center justify-between px-2 pb-2 pt-1 z-[105]">
                  ...
                  <div className="flex items-center gap-1.5 pr-1">
                    {!hasContent && <button onClick={startListening} className="p-2.5 rounded-full text-[var(--text)] bg-[var(--text)]/5 flex-shrink-0 [-webkit-tap-highlight-color:transparent]"><Mic size={20} strokeWidth={2.5} /></button>}
                    {hasContent && <button onClick={handleSend} disabled={isSending} className="w-10 h-10 rounded-full flex items-center justify-center bg-[var(--text)] text-[var(--bg)] scale-105 shadow-md flex-shrink-0 [-webkit-tap-highlight-color:transparent]"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg></button>}
                  </div>
                </div>
              </div>
            </div>
