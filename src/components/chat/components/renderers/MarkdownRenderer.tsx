...
interface StreamingTextProps {
  content: string;
  isStreaming: boolean;
  onComplete?: () => void; // <-- UPDATE: Tambah callback untuk mengunci action bar induk
}
...
export const StreamingText = memo(({ content, isStreaming, onComplete }: StreamingTextProps) => {
  ...
  // Penyetelan Chunking Ideal: 4 karakter per 20ms agar gerakan mengalir mulus, tajam, dan tidak delay kelamaan
  const BATCH = 4; 
  const MS = 20;
  ...
  useEffect(() => {
    ...
    if (!isStreaming) { 
      setRevealedLen(next.length); 
      onComplete?.(); // Trigger selesai jika API langsung beres
      return; 
    }
    ...
    timerRef.current = setInterval(() => {
      setRevealedLen(cur => {
        if (cur >= next.length) { 
          ...
          onComplete?.(); // Trigger selesai ketika teks yang direveal sudah sama dengan panjang text aslinya
          return cur; 
        }
        const nextLen = Math.min(cur + BATCH, next.length);
        if (nextLen === next.length) {
          onComplete?.(); // Trigger selesai tepat di chunk terakhir
        }
        return nextLen;
      });
    }, MS);
    ...
    <>
      ...
      {isStreaming && revealedLen < content.length && (
        <span aria-hidden style={{
          ...
          marginLeft: 4, verticalAlign: 'middle', opacity: 0.6,
