...
const MD_STYLES = `
  ...
  .cylen-chunk-new {
    animation: chunkFadeIn 0.18s ease-out both;
  }
`;
...
// Splits on double-newline so each paragraph/block gets its own fade
...
// ── Streaming Text ─────────────────────────────
// Strategy: render content IMMEDIATELY as it arrives (no typewriter delay),
// but apply a subtle fade-in animation on newly appeared paragraphs.
// This gives the "smooth appearance" feel without any lag.
...
export const StreamingText = memo(({ content, isStreaming, onComplete }: StreamingTextProps) => {
  // Track which paragraph indices have already been "seen" — they won't re-animate
  ...
  // Count how many paragraphs were in previous render
  const paragraphs = splitIntoParagraphs(content);
  const prevParagraphs = splitIntoParagraphs(prevContentRef.current);

  // Any paragraph index >= seenParagraphsRef is "new" this render
  const newFromIndex = seenParagraphsRef.current;

  // After render, update refs
  useEffect(() => {
    prevContentRef.current = content;
    // Mark all current paragraphs as seen (except the last one which may still be growing)
    // We animate the last paragraph every time it changes while streaming
    if (paragraphs.length > 1) {
      seenParagraphsRef.current = paragraphs.length - 1;
    }
  });

  // Signal completion when streaming stops
  useEffect(() => {
    if (!isStreaming && !completedRef.current) {
      completedRef.current = true;
      // Small timeout so the last chunk renders before actions appear
      const t = setTimeout(() => {
  ...
  // Show cursor only while actively streaming
  const showCursor = isStreaming;

  // Build rendered output: paragraphs with selective fade-in
  // We render the FULL content via MarkdownRenderer for correctness,
  // but wrap it in a container that does a gentle fade when content grows.
  return (
    <StreamingContainer
      content={content}
...
const StreamingContainer = memo(({
  content, isStreaming, showCursor, newFromIndex, paragraphCount
}: StreamingContainerProps) => {
  // We split content into "settled" (all but last paragraph) and "active" (last paragraph)
  // Settled: render once, no re-animation
  // Active: re-renders on every chunk with a subtle fade

  const paragraphs = splitIntoParagraphs(content);
  const hasMultiple = paragraphs.length > 1;

  if (!hasMultiple) {
    // Short response — render whole thing with one fade
    return (
  ...
  // Multi-paragraph: settled paragraphs + active last paragraph
  ...
  return (
    <div style={{ userSelect: 'text', WebkitUserSelect: 'text' }}>
      <style>{MD_STYLES}</style>
      {/* Settled paragraphs — rendered once, no animation on re-render */}
      <SettledContent content={settledContent} />
      {/* Active last paragraph — fades in fresh each time it changes while streaming */}
      <ActiveParagraph
...
// ── Settled content — memoized heavily so it never re-renders ──
const SettledContent = memo(({ content }: { content: string }) => (
  <MarkdownRenderer content={content} />
), (prev, next) => prev.content === next.content); // only re-render if content changes

// ── Active paragraph — fades in, updates as chunks arrive ──
