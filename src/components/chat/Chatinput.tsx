...
const ModeSelectorPopup: React.FC<{
  ...
  onUpgradeClick?: () => void;
  above?: boolean;
}> = ({ current, onSelect, onClose, onCloseCanvas, isCanvasActive, onUpgradeClick, above = true }) => (
  <>
      ...
      <footer className={cn("px-2 pb-6 relative z-50 pointer-events-auto w-full", compact ? "pt-0" : "pt-2")}>
        ...
        <div className="max-w-4xl mx-auto w-full">
          {isListening && !showLiveVoice ? (
            /* DYNAMIC SPEECH PILL (BLUR STATE DI TENGAH) */
            ...
            /* STANDARD INPUT MAIN CONTAINER */
            <div className="relative w-full">
              <div className="rounded-full flex flex-col relative overflow-hidden bg-[var(--text)]/[0.04] border border-[var(--text)]/10 shadow-[0_4px_24px_rgba(0,0,0,0.04)] backdrop-blur-md" style={{ WebkitTapHighlightColor: 'transparent' }}>
                ...
                <div className="flex items-center justify-between px-2 pb-2 pt-1">
                  ...
                  {/* RIGHT CONTROLS GROUP (MIC & VOICE SEPARATED PERFECTLY) */}
                  <div className="flex items-center gap-1.5 z-[105] pr-1">
                    {!hasContent && (
                      /* TOMBOL 1: STT MIC BUTTON (DARK BACKGROUND) */
                      <button 
                        onClick={startListening} 
                        className="w-10 h-10 rounded-full flex items-center justify-center text-[var(--text)] bg-[var(--text)]/5 hover:bg-[var(--text)]/10 flex-shrink-0 [-webkit-tap-highlight-color:transparent]"
                      >
                    ...
                    {hasContent ? (
                      ...
                      <button 
                        onClick={handleSend} 
                        disabled={isSending} 
                        className="w-10 h-10 rounded-full flex items-center justify-center bg-[var(--text)] text-[var(--bg)] scale-105 shadow-md flex-shrink-0 [-webkit-tap-highlight-color:transparent]"
                      >
                      ...
                      /* TOMBOL 2: LIVE VOICE MODE WAVE BUTTON (WHITE BACKGROUND) */
                      <button 
                        onClick={() => setShowLiveVoice(true)} 
                        className="w-10 h-10 rounded-full flex items-center justify-center bg-[var(--text)] text-[var(--bg)] shadow-md flex-shrink-0 ml-1 [-webkit-tap-highlight-color:transparent]"
                      >
                        <IconSpeakWave />
                      </button>
                    )}
