...
// ── PERBAIKAN: Fungsi Ngebaca Tanggal Biar Nggak "NAN" ──
const formatSessionDate = (dateString?: string | Date | number) => {
  if (!dateString) return "Baru saja";

  // Kalau formatnya masih string bawaan (contoh: "MINGGU, 24 MEI 2026 PUKUL 12:51")
  if (typeof dateString === 'string') {
    // Kita cek apakah dia punya pola kata 'PUKUL' (format lokal utils.ts lu)
    if (dateString.includes('PUKUL')) {
      return dateString.replace('PUKUL ', ''); // Kita tampilkan langsung aja tanpa nyoba nge-parse jadi objek Date
    }
  }

  // Kalau ternyata udah dalam bentuk timestamp number atau Date API standar
  const d = new Date(dateString);
  if (isNaN(d.getTime())) return "Beberapa saat lalu"; // Kalau beneran rusak, ganti teks biar ga NAN
...
const getSmartTitle = (session: any) => {
  ...
  if (session.title && session.title !== "New Chat" && session.title !== "Chat Baru" && session.title !== "New Conversation") {
    return session.title;
  }
  const firstUserMsg = session.messages?.find((m: any) => m.role === 'user')?.content;
  if (!firstUserMsg) return session.title || "Chat Baru";
...
const SidebarContent: React.FC<{
  ...
  collapsed, chatSessions, activeChatId, customAvatar, customName,
  onSelectChat, onViewProfile, onViewGroup, onViewSaved, onViewSearch, onNewChat,
  onToggleCollapse, onClose, onRequestRename, onPinChat, onRequestDelete
}) => {
    ...
    <div className="flex flex-col h-full relative bg-transparent">
              ...
              <button onClick={() => onSelectChat(session.id)} className={cn("w-full flex items-start gap-2.5 px-3 py-2.5 rounded-xl transition-all active:scale-[0.96] text-left", activeChatId === session.id ? "bg-[var(--sf)]" : "hover:bg-[var(--sf)]")}>
                ...
                <div className="flex-1 min-w-0 pr-6">
                  <div className="flex items-center gap-1.5">
                    <p className={cn("text-[13px] truncate", activeChatId === session.id ? "font-bold text-[var(--text)]" : "font-medium text-[var(--text)]")}>{getSmartTitle(session)}</p>
                    {(session as any).isPinned && <Pin size={10} className="text-[var(--ac)] flex-shrink-0" />}
                  </div>
                  <p className="text-[11px] text-[var(--mu)] truncate mt-0.5">{session.messages[session.messages.length - 1]?.content.slice(0, 35)}...</p>
                  <p className="text-[9px] text-[var(--mu)] opacity-70 mt-1 tracking-wide">{formatSessionDate(session.date || (session as any).updatedAt)}</p>
                </div>
              </button>
              ...
              <AnimatePresence>
                {openMenuId === session.id && (
                  <motion.div initial={{opacity:0, y:-5, scale:0.95}} animate={{opacity:1, y:0, scale:1}} exit={{opacity:0, scale:0.95}} transition={{duration:0.1}} className="absolute right-0 top-9 w-36 bg-[var(--bg)]/95 backdrop-blur-3xl border border-[var(--text)]/10 rounded-xl shadow-2xl z-[100] py-1 overflow-hidden">
                    <button onClick={(e) => { 
                      e.stopPropagation(); 
                      onRequestRename(session.id, getSmartTitle(session));
                      setOpenMenuId(null); 
                    }} className="w-full flex items-center gap-2.5 px-3 py-2 text-[12px] font-semibold text-[var(--text)] hover:bg-[var(--sf)] transition-colors active:scale-95">
                      <Edit2 size={13} /> Ubah Nama
                    </button>
                    
                    <button onClick={(e) => { e.stopPropagation(); onPinChat?.(session.id); setOpenMenuId(null); }} className="w-full flex items-center gap-2.5 px-3 py-2 text-[12px] font-semibold text-[var(--text)] hover:bg-[var(--sf)] transition-colors active:scale-95">
                      <Pin size={13} /> {(session as any).isPinned ? 'Unpin Chat' : 'Pin Chat'}
                    </button>
                    ...
                    <button onClick={(e) => { 
                      ...
                      setOpenMenuId(null); 
                    }} className="w-full flex items-center gap-2.5 px-3 py-2 text-[12px] font-bold text-[var(--text)] opacity-80 hover:opacity-100 hover:bg-[var(--text)]/10 transition-colors active:scale-95">
      ...
      <div className="px-3 pb-4 flex-shrink-0 bg-[var(--sf)] z-10 pt-2 border-t border-[var(--bd)]/50">
        <button onClick={onViewProfile} className={cn("w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[var(--bd)] transition-all active:scale-[0.98]", collapsed && "justify-center px-2")}>
          <div className="w-8 h-8 rounded-full bg-[var(--ac)] flex items-center justify-center flex-shrink-0 shadow-sm overflow-hidden">
            {customAvatar ? <img src={customAvatar} alt="Profile" className="w-full h-full object-cover" /> : <User size={15} className="text-[var(--at)]"/>}
          </div>
          {!collapsed && (
            <div className="flex-1 text-left min-w-0">
              <div className="text-[13px] font-bold text-[var(--text)] truncate">{customName}</div>
              <div className="text-[11px] text-[var(--text)]/50 truncate">Profil & Pengaturan</div>
            </div>
          )}
...
const MobileSidebarContent: React.FC<{
  ...
  chatSessions, activeChatId, customAvatar, customName, onSelectChat, onViewProfile, onViewGroup, onViewSaved, 
  onViewSearch, onClose, onNewChat, onViewTasks, onRequestRename, onPinChat, onRequestDelete, onClearAllChats
}) => {
  const [openMenuId, setOpenMenuId] = useState<string | null>(null);
  const [isHistoryCollapsed, setIsHistoryCollapsed] = useState(false);

  const sortedSessions = [...chatSessions].sort((a, b) => ((b as any).isPinned ? 1 : 0) - ((a as any).isPinned ? 1 : 0));

  return (
    <div className="flex flex-col h-full bg-transparent" onClick={() => setOpenMenuId(null)}>
      <div className="px-6 pt-6 pb-2 flex items-center justify-between border-none">
        <button onClick={() => { onViewProfile(); onClose(); }} className="flex items-center gap-3 active:scale-95 transition-all text-left">
          <div className="w-10 h-10 rounded-full bg-[var(--text)] flex items-center justify-center flex-shrink-0 shadow-lg border border-[var(--text)]/10 overflow-hidden">
            {customAvatar ? <img src={customAvatar} alt="Profile" className="w-full h-full object-cover" /> : <User size={22} className="text-[var(--bg)]" />}
          </div>
          <div className="flex flex-col">
            <span className="text-[18px] font-bold text-[var(--text)] tracking-tight leading-tight">{customName}</span>
          </div>
        </button>
        <button onClick={onClose} className="w-10 h-10 rounded-full bg-[var(--text)]/5 border border-[var(--text)]/10 flex items-center justify-center text-[var(--text)] hover:bg-[var(--text)]/10 active:scale-90 transition-all">
      ...
      <div className="px-5 mb-2 mt-4">
        <button onClick={() => { onViewTasks(); onClose(); }} className="w-full flex items-center gap-3 bg-[var(--text)]/5 text-[var(--text)] rounded-2xl px-5 py-3.5 transition-all active:scale-[0.96] hover:bg-[var(--text)]/10 text-left">
          ...
          <span className="text-[16px] font-bold tracking-wide">Tasks</span>
        </button>
      </div>
      ...
      <div className="px-3 mb-2 flex flex-col gap-1">
        <button onClick={() => { onViewGroup(); onClose(); }} className="flex items-center gap-4 px-4 py-3.5 hover:bg-[var(--text)]/10 rounded-2xl transition-all active:scale-[0.96] text-left">
          ...
          <span className="text-[16px] font-bold text-[var(--text)]">Grup AI</span>
        </button>
        <button onClick={() => { onViewSaved(); onClose(); }} className="flex items-center gap-4 px-4 py-3.5 hover:bg-[var(--text)]/10 rounded-2xl transition-all active:scale-[0.96] text-left">
          ...
          <span className="text-[16px] font-bold text-[var(--text)]">Disimpan</span>
        </button>
      </div>
                  ...
                  >
                    <div className="flex items-center justify-between w-full pr-6">
                      <div className="flex items-center gap-2 overflow-hidden">
                        <span className={cn("text-[16px] truncate max-w-full transition-colors", activeChatId === session.id ? "font-bold text-[var(--text)]" : "font-semibold text-[var(--text)]")}>
                          {getSmartTitle(session)}
                        </span>
                        {(session as any).isPinned && <Pin size={12} className="text-[var(--text)]/60 flex-shrink-0" />}
                      </div>
                    </div>
                    ...
                    <span className="text-[11px] font-semibold tracking-wider uppercase text-[var(--text)]/40 mt-2">
                      {formatSessionDate(session.date || (session as any).updatedAt)}
                    </span>
                  </button>
                    ...
                    {openMenuId === session.id && (
                      ...
                      >
                        <button onClick={(e) => { 
                          ...
                          onRequestRename(session.id, getSmartTitle(session));
                          setOpenMenuId(null); 
                        }} className="w-full flex items-center gap-3 px-4 py-3 text-[14px] font-semibold text-[var(--text)] hover:bg-[var(--text)]/10 transition-colors active:scale-95 flex-shrink-0">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); onPinChat?.(session.id); setOpenMenuId(null); }} className="w-full flex items-center gap-3 px-4 py-3 text-[14px] font-semibold text-[var(--text)] hover:bg-[var(--text)]/10 transition-colors active:scale-95">
                          <Pin size={16} /> {(session as any).isPinned ? 'Unpin Chat' : 'Pin Chat'}
                        </button>
                        ...
                        <button onClick={(e) => { 
                          ...
                          onRequestDelete(session.id);
                          setOpenMenuId(null); 
                        }} className="w-full flex items-center gap-3 px-4 py-3 text-[14px] font-bold text-[var(--text)] opacity-80 hover:opacity-100 hover:bg-[var(--text)]/10 transition-colors active:scale-95">
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [customName, setCustomName] = useState<string>(localStorage.getItem('cylen_user_name') || 'User Premium');

  useEffect(() => {
    // 1. Tarik otomatis dari akun Google yang login
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        setCustomName(user.displayName || 'User Premium');
        setCustomAvatar(user.photoURL || null);
        
        // Simpan ke local storage buat fallback
        if (user.displayName) localStorage.setItem('cylen_user_name', user.displayName);
        if (user.photoURL) localStorage.setItem('cylen_avatar', user.photoURL);
      }
    });

    // 2. Fallback listener kalau user mau ganti avatar/nama manual dari setting
    const handleAvatarChange = () => setCustomAvatar(localStorage.getItem('cylen_avatar'));
    const handleNameChange = () => setCustomName(localStorage.getItem('cylen_user_name') || 'User Premium');

    window.addEventListener('avatar-changed', handleAvatarChange);
    window.addEventListener('user-profile-changed', handleNameChange);
    
    return () => {
      unsubscribe();
      window.removeEventListener('avatar-changed', handleAvatarChange);
      window.removeEventListener('user-profile-changed', handleNameChange);
    };
  }, []);

  const handleRequestRename = (id: string, currentTitle: string) => {
    setRenameChatId(id);
    setRenameInput(currentTitle);
  };

  const handleConfirmRename = () => {
    if (renameChatId && renameInput.trim()) {
      onRenameChat?.(renameChatId, renameInput.trim());
    }
  ...
  const sharedProps = { 
    chatSessions, activeChatId, onSelectChat, onViewProfile, onViewGroup, 
    onViewSaved, onViewSearch, onNewChat, onPinChat, onClearAllChats, 
      ...
      <AnimatePresence>
        {renameChatId && (
          <motion.div
            ...
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[300] flex items-center justify-center bg-black/40 backdrop-blur-md p-4"
          >
            <motion.div
              ...
              className="w-full max-w-[340px] bg-[var(--bg)]/85 backdrop-blur-2xl border border-[var(--text)]/15 rounded-2xl p-6 shadow-2xl flex flex-col gap-4"
            >
              <h3 className="text-[var(--text)] font-bold text-[18px] tracking-tight">Ubah Nama Chat</h3>
              <input
                autoFocus
                value={renameInput}
                onChange={(e) => setRenameInput(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleConfirmRename()}
                className="w-full bg-[var(--text)]/5 border border-[var(--text)]/20 rounded-xl px-4 py-3 text-[14px] text-[var(--text)] outline-none focus:border-[var(--text)]/50 transition-colors"
                placeholder="Masukkan nama chat baru..."
              />
              <div className="flex justify-end gap-3 mt-2">
                <button 
                  onClick={() => setRenameChatId(null)} 
                  className="px-5 py-2.5 text-[14px] font-semibold text-[var(--text)]/70 hover:bg-[var(--text)]/10 rounded-xl transition-colors active:scale-95"
                >
                  Batal
                </button>
                <button 
                  onClick={handleConfirmRename} 
                  className="px-5 py-2.5 text-[14px] font-bold bg-[var(--text)] text-[var(--bg)] rounded-xl hover:opacity-90 transition-opacity shadow-lg active:scale-95"
                >
                  Simpan
                </button>
              </div>
        ...
        {deleteChatId && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-[340px] bg-[var(--bg)]/85 backdrop-blur-2xl border border-[var(--text)]/15 rounded-2xl p-6 shadow-2xl flex flex-col gap-4"
            >
              <h3 className="text-[var(--text)] font-bold text-[18px] tracking-tight">Hapus Obrolan</h3>
              <p className="text-[14px] text-[var(--text)]/70 leading-relaxed">
                Yakin mau hapus chat ini? Obrolan yang sudah dihapus tidak dapat dikembalikan.
              </p>
              <div className="flex justify-end gap-3 mt-2">
                <button 
                ...
                <button 
                  ...
                  className="px-5 py-2.5 text-[14px] font-bold bg-[var(--text)] text-[var(--bg)] rounded-xl hover:opacity-90 transition-colors shadow-lg active:scale-95"
                >
                  Hapus
                </button>
              </div>
