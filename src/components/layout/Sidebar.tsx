...
// 🔥 FIX: Ikon X udah gua masukin biar ga crash lagi!
import { ChevronRight, Search, User, Settings, Bookmark, Users, ChevronsRight, ChevronsLeft, MessageSquare, PenSquare, Clock, MoreHorizontal, Edit2, Pin, Trash2, ListX, X } from 'lucide-react';
...
const MobileSidebarContent: React.FC<{
  ...
  return (
    <div className="flex flex-col h-full bg-transparent" onClick={() => setOpenMenuId(null)}>
      ...
      <div className="p-4 pb-6 flex items-center gap-3 flex-shrink-0 relative z-20 border-t border-[var(--text)]/5">
        <button onClick={() => { onViewSearch(); onClose(); }} className="flex-1 flex items-center gap-3 bg-[var(--text)]/5 hover:bg-[var(--text)]/10 rounded-2xl px-5 py-3.5 transition-all active:scale-[0.96] text-left">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            // 🔥 FIX: Animasi pake curve iOS yang super licin, ga pake spring yang kadang berat
