...
import { ChevronRight, ChevronDown, Search, User, Settings, Bookmark, Users, ChevronsRight, ChevronsLeft, MessageSquare, PenSquare, Clock, Edit2, Pin, Trash2, ListX, X, SquarePen } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-transparent" onClick={() => setOpenMenuId(null)}>
      ...
      <div className="px-6 pt-10 pb-6 flex items-center justify-between border-b border-[var(--text)]/5">
        <button onClick={() => { onViewProfile(); onClose(); }} className="flex items-center gap-4 active:scale-95 transition-all text-left">
          <div className="w-12 h-12 rounded-full bg-[var(--text)] flex items-center justify-center flex-shrink-0 shadow-md border border-[var(--text)]/10"><User size={26} className="text-[var(--bg)]" /></div>
      ...
      <div className="px-5 mb-5 mt-4">
        <button onClick={() => { onViewTasks(); onClose(); }} className="w-full flex items-center gap-3 bg-[var(--text)]/5 text-[var(--text)] rounded-2xl px-5 py-3.5 transition-all active:scale-[0.96] hover:bg-[var(--text)]/10 text-left">
          ...
          <span className="text-[15px] font-bold tracking-wide">Tasks Manager</span>
        </button>
      </div>
      ...
      {/* 🔥 FIX: TOMBOL BAWAH BENER-BENER MELAYANG, GADA BACKGROUND/GARIS */}
      <div className="p-4 pb-8 flex items-center gap-4 flex-shrink-0 relative z-20 bg-transparent"> 
        {/* Kolom pencarian melayang elegan */}
        <button onClick={() => { onViewSearch(); onClose(); }} className="flex-1 flex items-center gap-3 bg-[var(--text)]/5 hover:bg-[var(--text)]/10 rounded-[20px] px-5 py-3.5 transition-all active:scale-[0.96] text-left border border-[var(--text)]/5 shadow-sm">
          <Search size={20} className="text-[var(--text)]/60 flex-shrink-0" />
          <span className="text-[15px] text-[var(--text)]/70 font-bold">Search</span>
        </button>
        {/* Ikon nambah halaman NETRAL TANPA BUBBLE */}
        <button onClick={() => { onNewChat(); onClose(); }} className="p-3 bg-transparent text-[var(--text)] transition-all active:scale-90 hover:opacity-70">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            // 🔥 KACA MOBILE: Background 30%, Blur 3xl biar bener-bener nyaru
