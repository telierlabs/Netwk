...
const SidebarContent: React.FC<{
  ...
  const navItem = (icon: React.ReactNode, label: string, onClick: () => void, tip?: string) => (
    <button
      ...
      className={cn(
        "w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all active:scale-[0.98] text-[var(--mu)] hover:bg-[var(--bd)] hover:text-[var(--text)]",
  ...
  return (
    <div className="flex flex-col h-full relative">
      <div className={cn("flex items-center px-3 py-4 gap-2", collapsed ? "justify-center flex-col" : "justify-between")}>
        ...
        {onToggleCollapse && (
          <button onClick={onToggleCollapse} className="p-1.5 rounded-lg text-[var(--mu)] hover:bg-[var(--bd)] hover:text-[var(--text)] transition-all active:scale-95">
      ...
      <div className="px-3 mb-2">
        <button onClick={onNewChat} className={cn("w-full flex items-center gap-2.5 px-3 py-2.5 bg-[var(--ac)] text-[var(--at)] rounded-xl transition-all active:scale-[0.98] shadow-sm hover:opacity-90", collapsed && "justify-center px-2")}>
      ...
      {!collapsed && (
        <div className="flex-1 overflow-y-auto px-3 flex flex-col gap-1 min-h-0 pb-4" style={{scrollbarWidth:'none'}}>
          <div className="flex items-center gap-2 bg-[var(--sf)] border border-[var(--bd)] rounded-xl px-3 py-2 mb-2 transition-all focus-within:border-[var(--text)]">
          ...
          {filtered.map((session) => (
            <div key={session.id} className="relative group">
              <button onClick={() => onSelectChat(session.id)} className={cn("w-full flex items-start gap-2.5 px-3 py-2.5 rounded-xl transition-all active:scale-[0.98] text-left", activeChatId === session.id ? "bg-[var(--sf)]" : "hover:bg-[var(--sf)]")}>
              ...
              <button 
                ...
                className={cn("absolute right-2 top-2 p-1.5 rounded-lg text-[var(--mu)] hover:text-[var(--text)] hover:bg-[var(--bd)] transition-all active:scale-95", openMenuId === session.id ? "opacity-100" : "opacity-0 group-hover:opacity-100")}
              >
              ...
              <AnimatePresence>
                {openMenuId === session.id && (
                  <motion.div initial={{opacity:0, y:-5, scale:0.95}} animate={{opacity:1, y:0, scale:1}} exit={{opacity:0, scale:0.95}} transition={{duration:0.1}} className="absolute right-0 top-9 w-36 bg-[var(--bg)] border border-[var(--bd)] rounded-xl shadow-xl z-[100] py-1 overflow-hidden">
...
const MobileSidebarContent: React.FC<{
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg)]" onClick={() => setOpenMenuId(null)}>
      ...
      <div className="px-6 pt-10 pb-6 flex items-center justify-between border-b border-[var(--bd)]/30">
        <button onClick={() => { onViewProfile(); onClose(); }} className="flex items-center gap-4 active:scale-95 transition-all text-left">
          <div className="w-12 h-12 rounded-full bg-[var(--text)] flex items-center justify-center flex-shrink-0 shadow-md"><User size={26} className="text-[var(--bg)]" /></div>
          <div className="flex flex-col">
            <span className="text-[20px] font-bold text-[var(--text)] tracking-tight leading-tight">User Premium</span>
            <span className="text-[13px] font-medium text-[var(--mu)] mt-0.5">user@cylen.ai</span>
          </div>
        </button>
        <button onClick={onClose} className="w-11 h-11 rounded-full bg-[var(--sf)] flex items-center justify-center text-[var(--text)] hover:opacity-70 active:scale-90 transition-all">
          <ChevronsLeft size={26} strokeWidth={2.5} />
        </button>
      </div>

      <div className="px-5 mb-5 mt-4">
        <button onClick={() => { onViewTasks(); onClose(); }} className="w-full flex items-center gap-3 bg-[var(--text)] text-[var(--bg)] rounded-2xl px-5 py-4 transition-all active:scale-[0.98] shadow-md hover:opacity-90 text-left">
      ...
      <div className="px-3 mb-3 flex flex-col gap-1">
        <button onClick={() => { onViewGroup(); onClose(); }} className="flex items-center gap-4 px-4 py-3.5 hover:bg-[var(--sf)] rounded-2xl transition-all active:scale-[0.98] text-left">
          <div className="w-9 h-9 rounded-full bg-[var(--sf)] flex items-center justify-center flex-shrink-0"><Users size={18} className="text-[var(--text)]" /></div>
          <span className="text-[15px] font-bold text-[var(--text)]">Grup AI Workspace</span>
        </button>
        <button onClick={() => { onViewSaved(); onClose(); }} className="flex items-center gap-4 px-4 py-3.5 hover:bg-[var(--sf)] rounded-2xl transition-all active:scale-[0.98] text-left">
          <div className="w-9 h-9 rounded-full bg-[var(--sf)] flex items-center justify-center flex-shrink-0"><Bookmark size={18} className="text-[var(--text)]" /></div>
      ...
      <div className="px-3 flex-1 overflow-y-auto min-h-0 pb-4" style={{scrollbarWidth:'none'}}>
        ...
        <div className="flex items-center justify-between px-4 mb-3 mt-4">
          <span className="text-[11px] font-black uppercase tracking-[0.15em] text-[var(--mu)]">Riwayat Chat</span>
          <div className="flex items-center gap-2">
            <button 
              onClick={(e) => { e.stopPropagation(); onClearAllChats?.(); }} 
              className="p-1.5 text-[var(--mu)] hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all active:scale-95"
        ...
        <div className="flex flex-col gap-1">
          {chatSessions.length === 0 && (
            <div className="px-4 py-6 text-center text-[13px] text-[var(--mu)] font-medium bg-[var(--sf)] rounded-2xl mx-2">Belum ada riwayat obrolan.</div>
          )}
          {chatSessions.map((session) => (
            <div key={session.id} className="relative group w-full">
              <button
                onClick={() => { onSelectChat(session.id); onClose(); }}
                className={cn("w-full flex flex-col px-4 py-3.5 rounded-2xl transition-all active:scale-[0.98] text-left relative border border-transparent", activeChatId === session.id ? "bg-[var(--sf)] border-[var(--bd)]" : "hover:bg-[var(--sf)]")}
              >
                ...
                <span className="text-[13px] text-[var(--mu)] mt-1 line-clamp-1">
                  {session.messages[session.messages.length - 1]?.content}
                </span>
                <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--mu)] opacity-70 mt-2">
              ...
              <button 
                ...
                className="absolute right-3 top-3.5 p-1.5 rounded-full text-[var(--mu)] bg-transparent hover:bg-[var(--bd)] hover:text-[var(--text)] transition-all active:scale-90"
              >
              ...
              <AnimatePresence>
                {openMenuId === session.id && (
                  <motion.div 
                    ...
                    className="absolute right-4 top-12 w-[160px] bg-[var(--bg)] border border-[var(--bd)] rounded-[16px] shadow-xl z-[150] py-1.5 overflow-hidden flex flex-col"
                  >
      ...
      <div className="p-4 pb-6 flex items-center gap-3 flex-shrink-0 relative z-20 bg-gradient-to-t from-[var(--bg)] to-transparent">
        <button onClick={() => { onViewSearch(); onClose(); }} className="flex-1 flex items-center gap-3 bg-[var(--sf)] hover:bg-[var(--bd)] rounded-2xl px-5 py-3.5 transition-all active:scale-95 text-left border border-[var(--bd)]/50">
          <Search size={20} className="text-[var(--mu)] flex-shrink-0" />
          <span className="text-[15px] text-[var(--text)] font-bold">Pencarian</span>
        </button>
        <button onClick={() => { onNewChat(); onClose(); }} className="p-3.5 bg-[var(--text)] text-[var(--bg)] rounded-2xl shadow-md transition-all active:scale-90 hover:opacity-90">
          <PenSquare size={22} />
        </button>
      </div>
    </div>
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      <aside className="hidden md:flex flex-col flex-shrink-0 border-r border-[var(--bd)] bg-[var(--bg)] transition-all duration-300 z-30 relative" style={{ width: collapsed ? 64 : 280, minHeight: '100dvh' }}>
      ...
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            {/* Sidebar dengan Hardware Acceleration & Spring Animation */}
            <motion.div 
              initial={{ x: '-100%' }} 
              animate={{ x: 0 }} 
              exit={{ x: '-100%' }} 
              ...
              className="fixed inset-y-0 left-0 w-[85vw] max-w-[340px] bg-[var(--bg)] z-[200] flex flex-col shadow-2xl md:hidden"
            >
