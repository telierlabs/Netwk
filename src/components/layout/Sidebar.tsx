...
const MobileSidebarContent: React.FC<{
  ...
  return (
    <div className="flex flex-col h-full bg-transparent" onClick={() => setOpenMenuId(null)}>
        ...
        <AnimatePresence>
          {!isHistoryCollapsed && (
            <motion.div
              ...
              // OVERFLOW HIDDEN DIHAPUS BIAR TITIK 3 GAK KEPOTONG / NGUMPET
              ...
              {chatSessions.map((session) => (
                <div key={session.id} className="relative group w-full">
                  ...
                  >
                    ...
                    {/* FONT TANGGAL UDAH DIBENERIN BIAR GAK KAKU */}
      ...
      {/* BACKGROUND KOTAK SEARCH DIBAWAH UDAH ILANG, SEKARANG FLOATING + BLUR */}
      <div className="px-6 pb-8 pt-4 flex items-center justify-between flex-shrink-0 relative z-20 backdrop-blur-md bg-transparent border-t border-transparent"> 
        <button onClick={() => { onViewSearch(); onClose(); }} className="flex items-center gap-3 p-2 bg-transparent transition-all active:scale-95 text-left text-[var(--text)]">
          <Search size={24} strokeWidth={2.2} className="flex-shrink-0" />
          <span className="text-[16px] font-bold">Search</span>
        </button>
        
        <div className="flex items-center gap-2">
          {/* ICON GIR MELAYANG */}
          <button onClick={() => { onViewProfile(); onClose(); }} className="p-3 bg-transparent text-[var(--text)] transition-all active:scale-90 hover:opacity-70">
