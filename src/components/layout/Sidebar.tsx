...
// ── MOBILE SIDEBAR CONTENT (DESAIN GROK STYLE) ──
const MobileSidebarContent: React.FC<{
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg)]">
      ...
      {/* 🔥 HEADER & PROFILE (Murni & Gede ala Grok) */}
      <div className="px-5 pt-8 pb-6 flex items-center justify-between">
        ...
        >
          <div className="w-10 h-10 rounded-full bg-[var(--text)] flex items-center justify-center flex-shrink-0">
            <User size={22} className="text-[var(--bg)]" />
          </div>
          <span className="text-[18px] font-bold text-[var(--text)] tracking-tight">User Premium</span>
        </button>
        <button 
          onClick={onClose}
          className="w-8 h-8 rounded-full bg-[var(--sf)] flex items-center justify-center text-[var(--text)] hover:opacity-70 transition-opacity"
        >
          <ChevronsRight size={18} />
        </button>
      </div>

      {/* 🔥 FITUR TAMBAHAN (Grup AI & Tersimpan) - Lebih gede ukurannya */}
      <div className="px-3 mb-4 flex flex-col gap-1">
        <button
          onClick={() => { onViewGroup(); onClose(); }}
          className="flex items-center gap-4 px-4 py-3.5 hover:bg-[var(--sf)] rounded-2xl transition-colors text-left"
        >
      ...
      {/* 🔥 RIWAYAT CHAT (Conversations list polos, gede, dan elegan) */}
        ...
        <div className="flex flex-col gap-1 pb-4">
          ...
          {chatSessions.map((session) => (
            <button
              ...
              className="w-full flex flex-col px-4 py-4 rounded-2xl hover:bg-[var(--sf)] transition-colors text-left relative group"
            >
              <div className="flex items-center justify-between w-full">
                <span className={cn(
                  "text-[16px] truncate max-w-[85%] transition-colors", 
                  activeChatId === session.id ? "font-semibold text-[var(--text)]" : "font-medium text-[var(--text)]"
                )}>
                  {session.title}
                </span>
                <div className="opacity-0 group-hover:opacity-100 transition-opacity text-[var(--mu)]">
      ...
      {/* 🔥 BOTTOM FOOTER (Search bar di bawah ala Grok) */}
      <div className="p-3 flex items-center gap-2 border-t border-[var(--bd)] bg-[var(--bg)] flex-shrink-0">
        <button
          onClick={() => { onViewSearch(); onClose(); }}
          className="flex-1 flex items-center gap-3 bg-[var(--sf)] hover:bg-[var(--bd)] rounded-full px-4 py-3.5 transition-colors text-left"
        >
          <Search size={20} className="text-[var(--mu)] flex-shrink-0" />
          <span className="text-[15px] text-[var(--mu)] font-medium">Search</span>
        </button>

        <button 
          onClick={() => { onViewProfile(); onClose(); }}
          className="p-3.5 bg-transparent hover:bg-[var(--sf)] rounded-full text-[var(--text)] transition-colors flex-shrink-0"
        >
          <Settings size={22} />
        </button>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-y-0 left-0 w-full md:max-w-sm bg-[var(--bg)] z-[200] flex flex-col shadow-2xl md:hidden"
            >
