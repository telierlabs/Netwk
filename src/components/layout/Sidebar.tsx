...
interface SidebarProps {
  ...
  onNewChat: () => void;
  onViewTasks: () => void;
  onRenameChat?: (id: string, newTitle: string) => void; // Update prop
...
const SidebarContent: React.FC<{
  ...
  // Filter & Sort: Pinned chat naik ke atas
                ...
                {openMenuId === session.id && (
                  <motion.div initial={{opacity:0, y:-5, scale:0.95}} animate={{opacity:1, y:0, scale:1}} exit={{opacity:0, scale:0.95}} transition={{duration:0.1}} className="absolute right-0 top-9 w-36 bg-[var(--bg)]/95 backdrop-blur-3xl border border-[var(--text)]/10 rounded-xl shadow-2xl z-[100] py-1 overflow-hidden">
                    ...
                    <button onClick={(e) => { 
                      ...
                      setOpenMenuId(null); 
                    }} className="w-full flex items-center gap-2.5 px-3 py-2 text-[12px] font-bold text-red-500 hover:bg-red-500/10 transition-colors active:scale-95">
...
const MobileSidebarContent: React.FC<{
  ...
  // Filter & Sort untuk Mobile: Pinned chat naik ke atas
                    ...
                    {openMenuId === session.id && (
                      ...
                      >
                        ...
                        <button onClick={(e) => { 
                          ...
                          if(window.confirm('Yakin mau hapus chat ini?')) onDeleteChat?.(session.id); 
                          setOpenMenuId(null); 
                        }} className="w-full flex items-center gap-3 px-4 py-3 text-[14px] font-bold text-[var(--text)] hover:bg-red-500/10 hover:text-red-500 transition-colors active:scale-95">
