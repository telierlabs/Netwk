...
// ── IMPORT LIBRARY (BUKU) SEBAGAI GANTI BOOKMARK ──
import { ChevronRight, ChevronDown, Search, User, Settings, Library, Users, ChevronsRight, ChevronsLeft, MessageSquare, PenSquare, Clock, MoreHorizontal, Edit2, Pin, Trash2, ListX, X, SquarePen } from 'lucide-react';
import { cn } from '../../lib/utils';
import { ChatSession } from '../../types';

// ── IMPORT HOOK PROFILE BARU ──
  ...
  return (
    <div className="flex flex-col h-full relative bg-transparent">
      ...
      <div className="px-2 flex flex-col gap-0.5">
        ...
        {/* ── UPDATE DESKTOP SIDEBAR: Ikon Buku + Teks Perpustakaan ── */}
        {navItem(<Library size={16}/>, 'Perpustakaan', onViewSaved)}
      </div>
    ...
    <div className="flex flex-col h-full bg-transparent" onClick={() => setOpenMenuId(null)}>
      ...
      <div className="px-3 mb-2 flex flex-col gap-1">
        ...
        <button onClick={() => { onViewSaved(); onClose(); }} className="flex items-center gap-4 px-4 py-3.5 hover:bg-[var(--text)]/10 rounded-2xl transition-all active:scale-[0.96] text-left">
          {/* ── UPDATE MOBILE SIDEBAR: Ikon Buku + Teks Perpustakaan ── */}
          <div className="w-9 h-9 rounded-full bg-[var(--text)]/5 flex items-center justify-center flex-shrink-0"><Library size={18} className="text-[var(--text)]" /></div>
          <span className="text-[16px] font-bold text-[var(--text)]">Perpustakaan</span>
        </button>
      </div>
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // ── PANGGIL HOOK DI SINI, GANTIKAN LOCALSTORAGE ──
  const { profile } = useProfile();
  const customAvatar = profile.avatar;
  ...
  const sharedProps = { 
    ...
    onViewSaved, onViewSearch, onNewChat, onPinChat, onClearAllChats, onViewTasks,
