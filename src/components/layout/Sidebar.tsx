...
// ── MOBILE SIDEBAR CONTENT (DESAIN GROK STYLE - REVISI) ──
const MobileSidebarContent: React.FC<{
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg)]">
      ...
      <div className="px-6 pt-10 pb-6 flex items-center justify-between">
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[13px] text-[var(--mu)]">user@cylen.ai</span>
          </div>
        </button>
        <button 
          onClick={onClose}
          className="w-10 h-10 rounded-full bg-[var(--sf)] flex items-center justify-center text-[var(--text)] hover:opacity-70 transition-opacity"
        >
          <ChevronsRight size={24} />
        </button>
      </div>

      {/* 🔥 TASKS MENU (DITAMBAHKAN DI BAWAH PROFIL) */}
      <div className="px-5 mb-5">
        <button className="w-full flex items-center gap-3 bg-[var(--sf)] hover:bg-[var(--bd)] rounded-[20px] px-5 py-4 transition-colors text-left active:scale-[0.98]">
          <Clock size={20} className="text-[var(--text)]" />
          <span className="text-[16px] font-bold text-[var(--text)]">Tasks</span>
        </button>
      </div>

      {/* 🔥 FITUR TAMBAHAN (Grup AI & Tersimpan) */}
      ...
      {/* 🔥 BOTTOM FOOTER (GARIS BORDER DIHAPUS, BERSIH) */}
