...
import { ChevronRight, Search, User, Settings, Bookmark, Users, ChevronsRight } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  onViewProfile, onViewGroup, onViewSaved, onViewSearch
}) => {
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            <div className="flex-1 overflow-y-auto px-4 pt-4 pb-4 flex flex-col gap-5 min-h-0">

              {/* SEARCH + CLOSE */}
              <div className="flex items-center gap-2 bg-[var(--sf)] rounded-2xl px-4 py-3">
                <Search size={15} className="text-[var(--mu)] flex-shrink-0"/>
                <input type="text" placeholder="Cari percakapan..." value={search}
                  onChange={e=>setSearch(e.target.value)}
                  className="flex-1 bg-transparent text-sm outline-none text-[var(--text)] placeholder:text-[var(--mu)]"/>
                <button onClick={onClose} className="flex-shrink-0 text-[var(--mu)] hover:text-[var(--text)] transition-colors">
              ...
              <div>
                ...
                <div className="bg-[var(--sf)] rounded-2xl overflow-hidden">
                  {/* Search Percakapan */}
                  <button onClick={()=>{onViewSearch();onClose();}}
                    ...
                    <span className="flex-1 text-sm font-medium text-left">Cari Percakapan</span>
                    <ChevronRight size={14} className="text-[var(--mu)] opacity-40"/>
                  </button>
                  <button onClick={()=>{onViewGroup();onClose();}}
                    className="w-full flex items-center gap-3 px-4 py-[13px] border-b border-[var(--bd)] hover:bg-[var(--bd)] transition-colors">
                    <div className="w-8 h-8 bg-[var(--bg)] rounded-[10px] flex items-center justify-center flex-shrink-0">
                  ...
                  <button onClick={()=>{onViewSaved();onClose();}}
                    className="w-full flex items-center gap-3 px-4 py-[13px] hover:bg-[var(--bd)] transition-colors">
                    <div className="w-8 h-8 bg-[var(--bg)] rounded-[10px] flex items-center justify-center flex-shrink-0">
              ...
              <div>
                ...
                <div className="flex flex-col">
                  {filtered.map((session) => (
                    <button key={session.id} onClick={()=>{onSelectChat(session.id);onClose();}}
                      className={cn("flex items-start gap-3 px-2 py-3 rounded-xl transition-colors text-left",
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-[var(--mu)] opacity-50 font-mono mt-1 truncate">{session.date}</p>
                      </div>
                    </button>
                  ))}
                </div>
