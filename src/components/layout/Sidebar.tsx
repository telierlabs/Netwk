...
// 🔥 FIX: Ikon titik 3 diganti ChevronDown (V)
import { ChevronRight, ChevronDown, Search, User, Settings, Bookmark, Users, ChevronsRight, ChevronsLeft, MessageSquare, PenSquare, Clock, Edit2, Pin, Trash2, ListX, X } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto px-3 flex flex-col gap-1 min-h-0 pb-4" style={{scrollbarWidth:'none'}}>
          ...
          {filtered.map((session) => (
            <div key={session.id} className="relative group">
              ...
              >
                <ChevronDown size={14} className={cn("transition-transform", openMenuId === session.id ? "rotate-180" : "rotate-0")} />
              </button>
...
const MobileSidebarContent: React.FC<{
  ...
  return (
    <div className="flex flex-col h-full bg-transparent" onClick={() => setOpenMenuId(null)}>
      ...
      <div className="px-6 pt-10 pb-6 flex items-center justify-between border-b border-[var(--text)]/5">
      ...
      <div className="px-5 mb-5 mt-4">
        {/* 🔥 FIX: Bubble item solid di Tasks dibantai jadi transparan elegan */}
        <button onClick={() => { onViewTasks(); onClose(); }} className="w-full flex items-center gap-3 bg-[var(--text)]/5 text-[var(--text)] rounded-2xl px-5 py-4 transition-all active:scale-[0.96] hover:bg-[var(--text)]/10 text-left border border-[var(--text)]/5">
        ...
        <div className="flex items-center justify-between px-4 mb-3 mt-4">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="p-1.5 text-[var(--text)]/50 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all active:scale-90"
        ...
        <div className="flex flex-col gap-1">
          ...
          {chatSessions.map((session) => (
            <div key={session.id} className="relative group w-full">
              ...
              >
                {/* 🔥 FIX: Ikon titik 3 diganti Chevron (V) yang muter kalo diklik */}
      ...
      {/* 🔥 FIX: Bagian bawah melayang tanpa garis, tanpa background norak */}
      <div className="p-4 pb-6 flex items-center gap-3 flex-shrink-0 relative z-20 bg-transparent">
        <button onClick={() => { onViewSearch(); onClose(); }} className="flex-1 flex items-center gap-3 bg-[var(--text)]/5 hover:bg-[var(--text)]/10 rounded-2xl px-5 py-3.5 transition-all active:scale-[0.96] text-left border border-[var(--text)]/5">
          <Search size={20} className="text-[var(--text)]/70 flex-shrink-0" />
          <span className="text-[15px] text-[var(--text)]/80 font-bold">Pencarian</span>
        </button>
        <button onClick={() => { onNewChat(); onClose(); }} className="p-3.5 bg-transparent border border-[var(--text)]/10 text-[var(--text)] rounded-2xl shadow-sm transition-all active:scale-90 hover:bg-[var(--text)]/5">
          <PenSquare size={22} />
        </button>
      </div>
    </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="fixed inset-0 w-full h-[100dvh] bg-[var(--bg)]/70 backdrop-blur-2xl supports-[backdrop-filter]:bg-[var(--bg)]/40 z-[200] flex flex-col md:hidden transform-gpu will-change-transform"
          >
