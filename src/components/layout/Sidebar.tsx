...
interface SidebarProps {
  ...
  onNewChat: () => void;
  onViewTasks: () => void;
  // 🔥 Props baru untuk aksi UI Sidebar
...
// 🔥 Helper untuk format waktu yang rapi (Senin, 10:30, 12 Apr 2026)
        ...
        <div className="flex-1 overflow-y-auto px-3 flex flex-col gap-1 min-h-0 pb-4" style={{scrollbarWidth:'none'}}>
          ...
          {filtered.map((session) => (
            <div key={session.id} className="relative group">
              ...
              {/* 🔥 TOMBOL 3 TITIK (MUNCUL PAS DI-HOVER) */}
              <button 
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg)]" onClick={() => setOpenMenuId(null)}>
      ...
      <div className="px-3 flex-1 overflow-y-auto min-h-0 pb-4" style={{scrollbarWidth:'none'}}>
        ...
        {/* 🔥 HEADER CONVERSATIONS DENGAN TOMBOL HAPUS SEMUA DI TENGAH */}
        <div className="flex items-center justify-between px-4 mb-2 mt-2">
          ...
          <div className="flex items-center gap-3">
            {/* Tombol Clear All / Hapus Semua (Icon V / ListX) */}
            <button 
        ...
        <div className="flex flex-col gap-1">
          ...
          {chatSessions.map((session) => (
            <div key={session.id} className="relative group w-full">
              {/* 🔥 TOMBOL CHAT UTAMA (DENGAN TANGGAL) */}
              <button
              ...
              {/* 🔥 TOMBOL 3 TITIK DI KANAN ATAS ITEM */}
              <button 
                onClick={(e) => { 
                  e.stopPropagation(); 
              ...
              {/* 🔥 POPUP / DROPDOWN MENU KHUSUS MOBILE */}
              <AnimatePresence>
                {openMenuId === session.id && (
                  <motion.div 
      ...
      <div className="p-4 pb-6 flex items-center gap-3 bg-[var(--bg)] flex-shrink-0 relative z-20 border-t border-[var(--bd)] border-opacity-30">
