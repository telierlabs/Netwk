...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header
      className={cn(
        "flex-shrink-0 grid grid-cols-[auto_1fr_auto] items-center px-4 z-50 transition-all duration-300 ease-out outline-none [-webkit-tap-highlight-color:transparent]",
        "bg-[var(--bg)]/40 backdrop-blur-2xl", // Efek kaca transparan menyatu dengan tema
      ...
      <div className="flex flex-col items-center justify-center outline-none overflow-hidden">
        {view === 'chat' && (
          <div className="flex items-center gap-2 max-w-[50vw]">
            ...
            {isTemporary && (
              <span className="text-[10px] font-bold bg-[var(--text)]/10 text-[var(--text)]/70 px-2 py-0.5 rounded-full animate-in fade-in zoom-in duration-200 shrink-0">
                Mode Hantu
              </span>
            )}
          </div>
