...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className={cn(
      "flex-shrink-0 grid grid-cols-[auto_1fr_auto] items-center px-4 z-10 transition-all duration-300 ease-out border-b border-[var(--bd)]/40",
      "bg-gradient-to-b from-[var(--bg)] to-[var(--bg)]/70 backdrop-blur-xl", // 🔥 Ini racikan kaca premiumnya!
      isCanvasActive ? "h-10 opacity-80 py-1" : "py-4"
    )}>
      <button 
        onClick={view === 'chat' ? onMenuClick : onBackClick} 
        className="p-1.5 -ml-1.5 transition-all duration-200 hover:bg-[var(--sf)]/50 rounded-lg active:scale-95 text-[var(--text)]"
      >
        {view === 'chat' ? (
      ...
      <div className="flex items-center gap-2">
        {view === 'group-chat' && (
          <button onClick={onZoomClick} className="p-1.5 transition-all duration-200 hover:bg-[var(--sf)]/50 rounded-lg active:scale-95 text-[var(--text)]">
        ...
        <button 
          ...
          className="p-1.5 -mr-1.5 transition-all duration-200 hover:bg-[var(--sf)]/50 rounded-lg active:scale-95 text-[var(--text)]"
        >
