...
import { ArrowLeft, Video, SquarePen } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className={cn(
      "flex-shrink-0 grid grid-cols-[auto_1fr_auto] items-center px-4 z-20 transition-all duration-300 ease-out", 
      // COPY PASTE SIDEBAR BLUR GROK STYLE: Blur 3xl, background tipis, border subtle.
      "bg-[var(--bg)]/30 backdrop-blur-3xl border-b border-[var(--text)]/[0.03] supports-[backdrop-filter]:bg-transparent", 
      isCanvasActive ? "h-10 opacity-80 py-1" : "py-3" 
    )}>
      <button 
        onClick={view === 'chat' ? onMenuClick : onBackClick} 
        className="p-2 -ml-2 transition-all duration-200 hover:bg-[var(--text)]/10 rounded-xl active:scale-95 text-[var(--text)]"
      >
        {view === 'chat' ? (
          // HAMBURGER GARIS 2 CUSTOM
      ...
      <div className="flex items-center gap-2">
        {view === 'group-chat' && (
          <button onClick={onZoomClick} className="p-2 transition-all duration-200 hover:bg-[var(--text)]/10 rounded-xl active:scale-95 text-[var(--text)]">
            <Video size={22} strokeWidth={2.2} />
          </button>
        )}
        
        <button 
          // LOGIC: Kalo di chat kosong, klik selalu ganti state hantu. Kalo ngga, icon SquarePen berfungsi sebagai new chat.
          onClick={isEmptyChat ? (isTemporary ? onNewChatClick : onNewTempChatClick) : onNewChatClick} 
          className="p-2 -mr-2 transition-all duration-200 hover:bg-[var(--text)]/10 rounded-xl active:scale-95 text-[var(--text)]"
        >
          {isEmptyChat ? (
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={cn("transition-all", isTemporary ? "opacity-100 text-[var(--text)]" : "opacity-50 text-[var(--text)]")}>
              <path d="M9 10h.01"/><path d="M15 10h.01"/>
              <path d="M12 2a8 8 0 0 0-8 8v12l3-3 2.5 2.5L12 19l2.5 2.5L17 19l3 3V10a8 8 0 0 0-8-8z"/>
            </svg>
          ) : (
            // ICON TAMBAH HALAMAN YANG SAMA KAYA DI SIDEBAR
