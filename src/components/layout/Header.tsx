...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className={cn(
      // 🔥 FIX: transition-all diganti biar garis bawah gak kedip item
      "flex-shrink-0 grid grid-cols-[auto_1fr_auto] items-center px-4 z-20 transition-[height,padding,opacity,background-color] duration-300 ease-out outline-none [-webkit-tap-highlight-color:transparent]", 
      ...
      <button 
        ...
        className="p-2 -ml-2 transition-all duration-200 hover:bg-[var(--text)]/10 rounded-xl active:scale-95 text-[var(--text)] outline-none [-webkit-tap-highlight-color:transparent]"
      >
        {view === 'chat' ? (
      ...
      <div className="flex items-center gap-2 outline-none">
        <button 
          ...
          className="p-2 -mr-2 transition-all duration-200 hover:bg-[var(--text)]/10 rounded-xl active:scale-95 text-[var(--text)] outline-none [-webkit-tap-highlight-color:transparent]"
        >
          {isEmptyChat ? (
