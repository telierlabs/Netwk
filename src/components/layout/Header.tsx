  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 outline-none">
        {view === 'chat' && (
          isEmptyChat ? (
            /* Tombol ghost — Tampilan berubah hitam solid kalau aktif */
            <button
              onClick={onToggleGhostMode}
              className={cn(
                "p-2 -mr-2 transition-all duration-200 rounded-xl active:scale-95 outline-none [-webkit-tap-highlight-color:transparent]",
                isTemporary
                  ? "bg-[var(--ac)] text-[var(--at)] shadow-md scale-105" // Item solid pas aktif
                  : "hover:bg-[var(--text)]/10 text-[var(--text)]/50 hover:text-[var(--text)]" // Transparan pas mati
              )}
              title={isTemporary ? "Keluar Mode Hantu" : "Mode Hantu (chat sementara)"}
            >
              {isTemporary ? (
                // SVG Hantu Filled (Solid)
