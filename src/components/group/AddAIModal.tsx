...
const DeepSeekLogo = ({ size = 20 }: { size?: number }) => (
  // DeepSeek official whale/fish mark
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    <path d="M23.748 12.298c-.166-.073-.338-.13-.515-.17a7.878 7.878 0 00-.178-1.524C22.458 7.742 20.07 6 17.396 6c-1.716 0-3.158.53-4.287 1.573a5.8 5.8 0 00-.802.925 5.8 5.8 0 00-.802-.925C10.376 6.53 8.934 6 7.218 6 4.544 6 2.156 7.742 1.57 10.604a7.878 7.878 0 00-.178 1.524 5.79 5.79 0 00-.515.17C.662 12.44.55 12.682.55 12.934c0 .252.111.493.327.651.178.159.417.238.659.218a5.784 5.784 0 001.234 2.285c1.078 1.249 3.08 2.262 4.993 2.262 1.188 0 2.334-.323 3.317-.935l.574-.363.574.363a6.35 6.35 0 003.317.935c1.913 0 3.915-.999 4.993-2.262a5.784 5.784 0 001.234-2.285c.242.02.48-.059.659-.218.216-.158.327-.399.327-.651 0-.252-.111-.494-.289-.636zM9.9 9.715l-.455.285-.455-.285C8.11 9.089 7.217 8.174 7.217 7.12c0-1.232.987-2.23 2.204-2.23 1.217 0 2.204 1 2.204 2.23 0 1.044-.878 1.95-1.725 2.595zm5.344.285l-.455-.285C13.942 9.17 13.05 8.255 13.05 7.21c0-1.232.987-2.23 2.204-2.23 1.217 0 2.204 1 2.204 2.23 0 1.044-.878 1.95-1.725 2.595l-.455.285-.034-.09zm.288 5.467c-1.558 0-2.826-1.305-2.826-2.904 0-1.6 1.268-2.904 2.826-2.904s2.826 1.304 2.826 2.904-1.268 2.904-2.826 2.904z" fill="currentColor" />
  </svg>
);
...
// Perplexity AI — official "squircle-P" logomark
...
export const AI_CONFIG = [
  ...
  { name: 'Claude',        model: 'Claude Sonnet 4',     Logo: ClaudeLogo,     bgColor: '#fdf4f0', iconColor: '#cc785c' },
  ...
  { name: 'Perplexity',    model: 'Sonar Pro',           Logo: PerplexityLogo, bgColor: '#f0f4ff', iconColor: '#20b8cd' },
];
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-0 m-auto z-[201] w-[88%] max-w-[360px] h-fit max-h-[72vh] bg-[var(--bg)] rounded-[22px] shadow-xl border border-[var(--bd)]/15 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="px-5 pt-5 pb-4 flex items-center justify-between shrink-0">
              <div>
                <h3 className="text-[15px] font-semibold text-[var(--text)] tracking-tight">Tambah AI</h3>
                <p className="text-[12px] text-[var(--text)]/40 mt-0.5">Pilih model untuk grup ini</p>
              </div>
              <button
                onClick={onClose}
                className="w-7 h-7 flex items-center justify-center rounded-full bg-[var(--sf)] text-[var(--text)]/50 hover:text-[var(--text)] transition-colors"
              >
            ...
            {/* Divider */}
            <div className="h-px bg-[var(--bd)]/10 mx-5 shrink-0" />
              ...
              {AI_CONFIG.map((ai) => {
                ...
                return (
                  <div
                    key={ai.name}
                    className={cn(
                      'flex items-center gap-3 px-3 py-2.5 rounded-[14px] transition-colors',
                      isAdded
                        ? 'opacity-40 cursor-default'
                        : 'hover:bg-[var(--sf)]/60 cursor-pointer active:scale-[0.99]'
                    )}
                    onClick={() => !isAdded && handleAdd(ai)}
                  >
                    {/* Icon */}
                    <div
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[11.5px] text-[var(--text)]/40 mt-0.5 leading-tight">
                        {ai.model}
                      </div>
                    </div>
                    ...
                    {/* Action */}
                    {isAdded ? (
                      <span className="text-[10.5px] font-semibold text-[var(--text)]/35 uppercase tracking-wider">
                        Aktif
                      </span>
                    ) : (
                      <div className="w-7 h-7 flex items-center justify-center rounded-full bg-[var(--text)]/8 text-[var(--text)]/50 hover:bg-[var(--text)] hover:text-[var(--bg)] transition-all shrink-0">
