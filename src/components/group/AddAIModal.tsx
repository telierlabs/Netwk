...
export const AddAIModal: React.FC<AddAIModalProps> = ({ isOpen, onClose, activeGroupParticipants, onAddParticipant, showToast }) => {
  ...
  return (
    <AnimatePresence>
      {/* Background Dim - Klik di luar buat nutup */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      ...
      {/* Popup Normal di Tengah (Bukan Slide dari Bawah, Gak Bikin Lag) */}
      <motion.div
      ...
      >
        <div className="px-5 pt-5 pb-3 flex items-center justify-between border-b border-[var(--bd)]/15 shrink-0 bg-[var(--bg)]">
          <div>
        ...
        {/* Area Scroll AI */}
        <div className="overflow-y-auto flex-1 px-4 py-3 flex flex-col gap-2 bg-[var(--sf)]/20" style={{ scrollbarWidth: 'none' }}>
          {AI_CONFIG.map((ai) => {
            const isAdded = activeGroupParticipants.some(p => p.name === ai.name);
            const isExpanded = expandedAI === ai.name;

            return (
              <div key={ai.name} className={cn("rounded-2xl border transition-colors bg-[var(--bg)]", isAdded ? "border-[var(--bd)]/15 opacity-50" : isExpanded ? "border-[var(--bd)]/40" : "border-[var(--bd)]/20 hover:border-[var(--bd)]/40")}>
                  ...
                  {isAdded ? (
                    ...
                    <div className="flex items-center gap-1.5">
                      <div className="w-8 h-8 flex items-center justify-center text-[var(--text)]/40">
                        <motion.span animate={{ rotate: isExpanded ? 180 : 0 }} transition={{ duration: 0.15 }} style={{ display: 'flex' }}><ChevronDown size={16} strokeWidth={2} /></motion.span>
                      </div>
                      <button onClick={(e) => {
                        e.stopPropagation();
                  ...
                  {isExpanded && !isAdded && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.15 }} className="overflow-hidden">
                      <div className="px-3 pb-3 pt-1 flex gap-2">
                        <input type="password" placeholder={`API Key ${ai.name}...`} value={apiKeys[ai.name] || ''} onChange={(e) => setApiKeys({ ...apiKeys, [ai.name]: e.target.value })} onClick={(e) => e.stopPropagation()} className="flex-1 bg-[var(--bg)] border border-[var(--bd)]/40 text-[var(--text)] rounded-[12px] px-3 py-2.5 text-[13px] outline-none placeholder:text-[var(--text)]/40 focus:border-[var(--text)]/60" />
          ...
          <div className="mt-2 pt-3 border-t border-[var(--bd)]/15">
            ...
            <div className="flex flex-col gap-2.5 p-3.5 bg-[var(--bg)] rounded-2xl border border-[var(--bd)]/20 shadow-sm">
              <input type="text" placeholder="Nama AI (contoh: Llama Lokal)" value={customName} onChange={(e) => setCustomName(e.target.value)} className="w-full bg-[var(--sf)]/30 border border-[var(--bd)]/30 text-[var(--text)] rounded-[12px] px-3 py-2.5 text-[14px] font-medium outline-none placeholder:text-[var(--text)]/40 focus:border-[var(--text)]/50" />
              <input type="password" placeholder="API Key / Token rahasia..." value={customKey} onChange={(e) => setCustomKey(e.target.value)} className="w-full bg-[var(--sf)]/30 border border-[var(--bd)]/30 text-[var(--text)] rounded-[12px] px-3 py-2.5 text-[13px] outline-none placeholder:text-[var(--text)]/40 focus:border-[var(--text)]/50" />
              <button onClick={() => {
