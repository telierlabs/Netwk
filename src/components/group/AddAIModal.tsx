  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 px-3 py-2 flex flex-col gap-1.5" style={{ scrollbarWidth: 'none' }}>
          {AI_CONFIG.map((ai) => {
            const isAdded = activeGroupParticipants.some(p => p.name === ai.name);
            const isExpanded = expandedAI === ai.name;

            return (
              <div key={ai.name} className={cn("rounded-xl border overflow-hidden transition-colors duration-100", isAdded ? "border-[var(--bd)]/12 opacity-45" : isExpanded ? "border-[var(--bd)]/30 bg-[var(--sf)]/30" : "border-[var(--bd)]/18 hover:border-[var(--bd)]/30")}>
                <div className="flex items-center gap-2.5 px-3 py-2.5">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: ai.bgColor, color: ai.iconColor }}>
                    <ai.Logo size={18} />
                  </div>
                  ...
                  {isAdded ? (
                    <div className="px-2.5 py-0.5 rounded-full bg-[var(--sf)] border border-[var(--bd)]/18 text-[10px] font-semibold text-[var(--text)]/40 uppercase tracking-wide">Aktif</div>
                  ) : (
                    <div className="flex items-center gap-1">
                      <button onClick={() => setExpandedAI(isExpanded ? null : ai.name)} className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-[var(--sf)] text-[var(--text)]/40 transition-colors">
                        <motion.span animate={{ rotate: isExpanded ? 180 : 0 }} transition={{ duration: 0.18, ease: 'easeInOut' }} style={{ display: 'flex' }}><ChevronDown size={14} strokeWidth={2} /></motion.span>
                      </button>
                      <button onClick={() => {
                      ...
                      }} className="w-8 h-8 flex items-center justify-center rounded-full bg-[var(--text)] text-[var(--bg)] active:scale-90 transition-transform shadow-sm"><Plus size={15} strokeWidth={2.5} /></button>
                    </div>
                  )}
                </div>
                  ...
                  {isExpanded && !isAdded && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.18 }} className="overflow-hidden">
                      <div className="px-3 py-2.5 border-t border-[var(--bd)]/12 bg-[var(--sf)]/20 flex gap-2">
                        <input type="password" placeholder={`${ai.name} API Key...`} value={apiKeys[ai.name] || ''} onChange={(e) => setApiKeys({ ...apiKeys, [ai.name]: e.target.value })} className="flex-1 bg-[var(--bg)] border border-[var(--bd)]/22 text-[var(--text)] rounded-xl px-3 py-2 text-[12px] outline-none placeholder:text-[var(--text)]/25 focus:border-[var(--text)]/28" />
                        <button onClick={() => { showToast(`API Key ${ai.name} tersimpan!`); setExpandedAI(null); }} className="px-3 py-2 bg-[var(--text)] text-[var(--bg)] text-[11px] font-semibold rounded-xl flex items-center gap-1 active:scale-95"><Save size={11} /> Simpan</button>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
          ...
          <div className="mt-1.5 pt-3 border-t border-[var(--bd)]/12">
            ...
            <div className="flex flex-col gap-2 p-3 bg-[var(--sf)]/40 rounded-xl border border-[var(--bd)]/12">
              <input type="text" placeholder="Nama AI (contoh: TelierBot)" value={customName} onChange={(e) => setCustomName(e.target.value)} className="w-full bg-[var(--bg)] border border-[var(--bd)]/22 text-[var(--text)] rounded-xl px-3 py-2.5 text-[13px] outline-none placeholder:text-[var(--text)]/25 focus:border-[var(--text)]/28" />
              <input type="password" placeholder="API Key / Token..." value={customKey} onChange={(e) => setCustomKey(e.target.value)} className="w-full bg-[var(--bg)] border border-[var(--bd)]/22 text-[var(--text)] rounded-xl px-3 py-2.5 text-[12px] outline-none placeholder:text-[var(--text)]/25 focus:border-[var(--text)]/28" />
              <button onClick={() => {
              ...
              }} disabled={!customName.trim()} className="w-full py-2.5 bg-[var(--text)] text-[var(--bg)] text-[13px] font-semibold rounded-xl flex items-center justify-center gap-1.5 disabled:opacity-25 active:scale-[0.98] transition-transform">
                <Plus size={14} strokeWidth={2.5} /> Pasang & Simpan
              </button>
            </div>
          </div>
          <div className="h-3" />
