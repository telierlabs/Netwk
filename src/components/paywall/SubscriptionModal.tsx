      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            {/* PILL TOGGLE — kecil */}
            <div className="flex bg-white/[0.04] p-[3px] rounded-full mx-auto w-full max-w-[220px] mb-10 border border-white/10 relative z-20 backdrop-blur-md">
              {TIERS.map((tier) => (
                <button
                  key={tier.id}
            ...
            {/* GLASS CARD — foto full-bleed, tanpa garis/ring warna */}
            <div className="max-w-sm mx-auto relative z-10 mb-10">
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="relative rounded-[28px] overflow-hidden min-h-[360px] shadow-[0_8px_40px_rgba(0,0,0,0.55)]"
                >
                  {/* FOTO TIER — full background, isi seluruh kartu */}
                  <img
                  ...
                  {/* Lapisan kaca blur, tanpa border/garis */}
                  <div className="relative h-full flex flex-col p-6 backdrop-blur-xl bg-white/[0.04]">
                    <div className="mb-6">
                      <div className="text-[17px] font-bold text-white drop-shadow-md">
                        Cylen {currentPlan.name}
                      </div>
                      <div className="text-[12.5px] text-white/70 drop-shadow-md">
                        {currentPlan.title}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                      {currentPlan.features.map((feature, i) => {
                        ...
                        return (
                          ...
                          >
                            <div className="mt-0.5 w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 bg-white/10 backdrop-blur-sm">
                              <Icon size={15} strokeWidth={2} className="text-white" />
                            </div>
                            <span className="text-[14px] font-medium text-white/90 leading-snug pt-1.5 drop-shadow-sm">
                              {feature.text}
                            </span>
                          </motion.div>
                        );
