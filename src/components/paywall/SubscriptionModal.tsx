...
export const SubscriptionModal: React.FC<SubscriptionModalProps> = ({ isOpen, onClose }) => {
  ...
  const [isProcessing, setIsProcessing] = useState(false); // State baru buat loading bayar

  const currentPlan = TIERS.find(t => t.id === activeTier)!;

  // Fungsi baru buat nembak API pembayaran
  const handleUpgrade = async () => {
    try {
      ...
      if (data.redirect_url) {
        // Lempar user ke halaman Midtrans (Ada QRIS, Gopay, dll)
