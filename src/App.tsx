...
// HELPER TITLE LOGIC LANGSUNG DI SINI BIAR GA RIBET IMPORT
const getSmartTitle = (session: any) => {
  ...
  // Ambil 1-4 kata pertama dari chat awal
  ...
  return (
    <div className={cn("app transition-colors duration-300", theme)} style={{ fontFamily: currentFontFamily, height: '100dvh', display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {/* Header Utama disembunyikan untuk halaman yang punya header sendiri (termasuk haptics & report) */}
          ...
          <AnimatePresence mode="wait">
            ...
            {view === 'profile' && (
              <ProfilePage 
                ...
                onViewHaptics={() => setView('haptics')} // <-- KABEL MENU HAPTICS
                onViewReport={() => setView('report')}   // <-- KABEL MENU REPORT
              />
            )}
            
            {/* ─── RENDER HALAMAN BARU DI SINI ─── */}
            {view === 'haptics' && (
              <HapticsPage onBack={() => setView('profile')} />
            )}
            {view === 'report' && (
              <ReportPage onBack={() => setView('profile')} showToast={showToast} />
            )}
            ...
            {view === 'memory' && (
              <MemoryPage memoryItems={memoryItems} onDelete={deleteMemory} onBack={() => setView('profile')} />
            )}
            {view === 'ai-settings' && (
              <AiSettings onBack={() => setView('profile')} showToast={showToast} />
            )}
          </AnimatePresence>
        </div>
      </div>
