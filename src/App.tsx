...
import { subscribeToAuthChanges } from './services/authService'; // IMPORT AUTH LISTENER
...
const getSmartTitle = (session: any) => {
  ...
  const title = firstUserMsg.split(' ').slice(0, 4).join(' ');
  return title;
};
...
export default function App() {
  ...
  // EFFECT UNTUK TEMA TERSINKRONISASI
  useEffect(() => {
  ...
  // 1. TAMPILAN LOADING SAAT CEK FIREBASE AUTH
  if (isAuthLoading) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-[var(--bg)] text-[var(--text)]">
        <div className="animate-pulse text-[12px] font-bold tracking-[0.2em] uppercase">INITIALIZING...</div>
      </div>
    );
  }
  ...
  // 3. JIKA SUDAH LOGIN, TAMPILKAN ANTARMUKA UTAMA
  return (
