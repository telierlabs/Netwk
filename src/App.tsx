...
export default function App() {
  ...
  const { chatSessions, activeChatId, setActiveChatId, messages, isSending, isSearching, sendMessage, createNewChat } = useChat();
  ...
  const handleSendMessage = () => { sendMessage(inputText, attachedImage); setInputText(''); setAttachedImage(null); };
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {view === 'chat' && (
          <ChatPage messages={messages} isSending={isSending} isSearching={isSearching} inputText={inputText} setInputText={setInputText} onSend={handleSendMessage} attachedImage={attachedImage} setAttachedImage={setAttachedImage} />
        )}
        {view === 'profile' && (
          <ProfilePage theme={theme} setTheme={setTheme} font={font} setFont={setFont} showToast={showToast} />
        )}
        {view === 'saved' && (
        ...
        {view === 'group-chat' && activeGroup && (
          <GroupChatPage activeGroup={activeGroup} isSending={isGroupSending} onSendMessage={sendGroupMessage} onAddParticipant={addParticipant} showToast={showToast} />
        )}
      </div>
