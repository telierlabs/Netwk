...
export default function App() {
  ...
  // Firebase auth listener — kalau user login/logout, otomatis update
  useEffect(() => {
  ...
  const handleToggleGhostMode = () => {
    ...
    else { createNewChat(true); showToast('Mode Hantu diaktifkan'); }
    setView('chat');
  };
  ...
  // Loading screen sementara Firebase cek sesi login (max ~1 detik)
  if (isAuthLoading) {
    return (
      <div style={{ display: 'flex', height: '100vh', width: '100vw', alignItems: 'center', justifyContent: 'center', background: '#f2f2f0' }}>
        <div style={{ width: 24, height: 24, border: '2px solid #ccc', borderTopColor: '#111', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />
  ...
  // onLoginSuccess tidak dipakai di sini karena subscribeToAuthChanges
  // otomatis akan set user dan trigger re-render ke main app
  if (!user) {
    return <LoginPage />;
  }

  // Sudah login → antarmuka utama
  return (
    <div className={cn("app transition-colors duration-300", theme)} style={{ fontFamily: currentFontFamily, height: '100dvh', display: 'flex', overflow: 'hidden' }}>
      ...
      <Sidebar
        ...
        onRenameChat={renameChatSession} onPinChat={pinChatSession} onDeleteChat={deleteChatSession} 
      />

      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {view !== 'search' && view !== 'tasks' && view !== 'group-profile' && view !== 'haptics' && view !== 'report' && (
          <Header 
            view={view as any} onMenuClick={() => setIsSidebarOpen(true)} 
            onBackClick={handleBackClick} onNewChatClick={handleNewChat} 
            onToggleGhostMode={handleToggleGhostMode} isCanvasActive={isCanvasActive}
            onZoomClick={() => setIsZoomOpen(true)} isEmptyChat={isEmptyChat}
            isTemporary={isTemporary} activeChatTitle={getSmartTitle(activeChat)} 
          />
        )}
          ...
          <AnimatePresence mode="wait">
            {view === 'chat' && (
              <motion.div key="chat" initial={{opacity:0, scale:0.98}} animate={{opacity:1, scale:1}} exit={{opacity:0}} transition={{ duration: 0.15 }} className="flex-1 flex flex-col overflow-hidden">
                <ChatPage
                  ...
                  onCloseCanvas={() => setIsCanvasActive(false)} isTemporary={isTemporary}
                />
              </motion.div>
            )}
            {view === 'group-list' && (
              <motion.div key="group-list" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} exit={{opacity:0}} transition={{ duration: 0.2 }} className="flex-1 overflow-hidden">
                <GroupListPage groups={groupSessions} onSelectGroup={(id) => { setActiveGroupId(id); setView('group-chat'); }} onCreateGroup={createGroup} onBack={() => setView('chat')} />
              </motion.div>
            )}
            {view === 'group-chat' && activeGroup && (
              <motion.div key="group-chat" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} exit={{opacity:0}} transition={{ duration: 0.2 }} className="flex-1 flex flex-col overflow-hidden">
                <GroupChatPage activeGroup={activeGroup} isSending={isGroupSending} onSendMessage={sendGroupMessage} onAddParticipant={addParticipant} showToast={showToast} onBack={() => setView('group-list')} onNavigateProfile={() => setView('group-profile')} />
              </motion.div>
            )}
            {view === 'group-profile' && activeGroup && (
              <motion.div key="group-profile" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} exit={{opacity:0}} transition={{ duration: 0.2 }} className="flex-1 flex flex-col overflow-hidden">
                <GroupProfilePage activeGroup={activeGroup} showToast={showToast} onBack={() => setView('group-chat')} />
              </motion.div>
            )}
            {view === 'profile' && (
              <ProfilePage 
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed bottom-32 left-1/2 bg-[var(--ac)] text-[var(--at)] px-6 py-3 rounded-full text-[14px] font-bold shadow-[0_10px_40px_rgba(0,0,0,0.2)] z-[100] whitespace-nowrap border border-white/10"
          >
            {toast}
          </motion.div>
        )}
