...
import { PermissionPage } from './pages/PermissionPage'; // TAMBAHAN
import { subscribeToAuthChanges, checkUserPermissions, saveUserPermissions } from './services/authService'; // TAMBAHAN
...
export default function App() {
  ...
  const [needsPermission, setNeedsPermission] = useState(false); // TAMBAHAN STATE
  ...
  // 👇 PERUBAHAN DI SINI: Integrasi Cek Permission Firestore
  useEffect(() => {
    let isMounted = true;
  ...
  // 👇 Langsung render LoginPage kalau user belom login
  if (!user) return <LoginPage />;

  // 👇 Langsung render Halaman Izin kalau belum pernah kasih izin (via Firestore)
  if (needsPermission) {
    return (
