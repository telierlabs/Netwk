...
export default function App() {
  ...
  const {
    ...
    isTemporary,
    renameChatSession, // <-- DESTRUCTURING FUNGSI BARU DARI useChat
    pinChatSession,    // <-- DESTRUCTURING FUNGSI BARU DARI useChat
    deleteChatSession  // <-- DESTRUCTURING FUNGSI BARU DARI useChat
  } = useChat();
  ...
  const activeChat = chatSessions.find(s => s.id === activeChatId); // Cari info session buat dilempar ke header
  ...
  return (
    <div className={cn("app transition-colors duration-300", theme)} style={{ fontFamily: currentFontFamily, height: '100dvh', display: 'flex', overflow: 'hidden' }}>
      ...
      <Sidebar
        ...
        onRenameChat={renameChatSession} // <-- PASSING PROPS KE SIDEBAR
        onPinChat={pinChatSession}       // <-- PASSING PROPS KE SIDEBAR
        onDeleteChat={deleteChatSession} // <-- PASSING PROPS KE SIDEBAR
      />

      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {view !== 'search' && view !== 'tasks' && view !== 'group-profile' && (
          <Header 
            view={view as any} 
