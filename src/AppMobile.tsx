...
export default function AppMobile() {
  ...
  // ── FIX: EKSTRAK joinGroup ──
  const { activeGroup, groupSessions, setActiveGroupId, isSending: isGroupSending, thinkingAI, createGroup, addParticipant, sendGroupMessage, joinGroup } = useGroupChat();
  ...
  // ── LOGIC NANGKEP URL JOIN SETELAH LOGIN ──
  useEffect(() => {
    ...
    if (path.startsWith('/join/')) {
      ...
      // Bersihin URL biar ga join ulang pas direfresh
  ...
  // KALAU BELUM LOGIN, TAMPILKAN LOGIN PAGE (URL TETAP /join/id, JADI SETELAH LOGIN BISA DITANGKAP USEEFFECT DI ATAS)
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {view === 'group-chat' && activeGroup && (
          <GroupChatPage 
            ...
            isSending={isGroupSending} 
            thinkingAI={thinkingAI} 
            onSendMessage={sendGroupMessage} 
            onAddParticipant={addParticipant} 
            showToast={showToast} 
            onBack={() => setView('group-list')} 
            onNavigateProfile={() => setView('group-profile')} 
          />
        )}
