...
const THEME_VARS: Record<string, Record<string, string>> = {
  ...
  't-dark':  { '--bg':'#010101', '--sf':'#161616', '--bd':'rgba(255,255,255,.07)', '--text':'#ffffff', '--mu':'rgba(255,255,255,.35)', '--cd':'#171717', '--ac':'#ffffff', '--at':'#000000', '--ib':'#252525' },
};
...
export default function AppMobile() {
    ...
    const unsubscribe = subscribeToAuthChanges(async (currentUser) => {
      if (currentUser) {
        ...
        try {
          ...
          console.error("Mobile permission check failed:", err);
          if (isMounted) {
  ...
  // JIKA BELUM LOGIN: Tampilkan Landing Page Normal Mobile
  if (!user) {
    return (
      <div style={{
        ...
        <img src="/82374-removebg-preview.png" alt="Cylen" style={{ width: 80, height: 80, marginBottom: 20 }} />
        <h1 style={{ fontSize: '32px', fontWeight: 700, letterSpacing: '-1px', margin: '0 0 8px 0' }}>CYLEN</h1>
        <p style={{ color: '#666', fontSize: '13px', marginBottom: '32px', textAlign: 'center', maxWidth: '280px', lineHeight: 1.5 }}>
          The Minimalist Cyberpunk Ecosystem. Experience pure text storytelling.
        </p>
        <button
    ...
    return (
      <PermissionPage onComplete={async () => {
        try {
          ...
          console.error("Gagal menyimpan konfigurasi izin mobile:", err);
        } finally {
          setNeedsPermission(false);
        }
      }} />
