...
export default function AppMobile() {
  ...
  return (
    <div className={cn('app transition-colors duration-350', theme)} style={{ fontFamily: currentFontFamily, height: '100dvh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <style>{`
        .t-light { --bg:#fff; --sf:#f5f5f5; --bd:rgba(0,0,0,.08); --text:#0a0a0a; --mu:rgba(0,0,0,.38); --cd:#fff; --ac:#0a0a0a; --at:#fff; --ib:#f0f0f0; }
        .t-dark  { --bg:#0d0d0d; --sf:#1c1c1c; --bd:rgba(255,255,255,.07); --text:#f0f0f0; --mu:rgba(255,255,255,.35); --cd:#171717; --ac:#fff; --at:#0d0d0d; --ib:#252525; }
        .t-cream { --bg:#f8f4ed; --sf:#ede8df; --bd:rgba(0,0,0,.07); --text:#2a1f0e; --mu:rgba(42,31,14,.42); --cd:#fff; --ac:#2a1f0e; --at:#f8f4ed; --ib:#e0d8c8; }
      ...
      <Sidebar
        ...
        onSelectChat={(id) => { setActiveChatId(id); setView('chat'); setIsSidebarOpen(false); setIsCanvasActive(false); }} // 🔥 MATIIN CANVAS KALO BUKA CHAT LAMA
      ...
      {view !== 'search' && (
        <Header view={view as any} onMenuClick={() => setIsSidebarOpen(true)} onBackClick={() => { if (view === 'group-chat') setView('group-list'); else if (view === 'memory' || view === 'ai-settings') setView('profile'); else setView('chat'); }} onNewChatClick={handleNewChat} isCanvasActive={isCanvasActive} />
      )}
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {view === 'chat' && (
          <ChatPage
            ...
            onCloseCanvas={() => setIsCanvasActive(false)} // 🔥 INI PENTING! OPER KE DALEM CHATPAGE
          />
        )}
