...
export default function AppMobile() {
  ...
  useEffect(() => {
    // Proses redirect result DULU sebelum subscribe auth state
    // Ini penting agar user yang baru redirect dari Google langsung masuk
    const initAuth = async () => {
      ...
      // Setelah getRedirectResult selesai, baru subscribe auth state
  ...
  if (isAuthLoading) {
    return <div style={{ height: '100dvh', width: '100vw', backgroundColor: '#ffffff' }} />;
  }
  ...
  return (
    <div
      ...
      style={{ fontFamily: currentFontFamily, position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, width: '100vw', height: '100dvh', display: 'flex', flexDirection: 'column', overflow: 'hidden', backgroundColor: 'var(--bg)' }}
    >
      ...
      <Sidebar
        ...
        onSelectChat={(id: string) => { setActiveChatId(id); setView('chat'); setIsSidebarOpen(false); setIsCanvasActive(false); }}
        ...
        onNewChat={handleNewChat} onRenameChat={renameChatSession} onPinChat={pinChatSession} onDeleteChat={deleteChatSession}
      />
      {view !== 'search' && view !== 'tasks' && view !== 'appearance' && view !== 'usage' && view !== 'integrations' && view !== 'security' && view !== 'export' && view !== 'group-profile' && view !== 'saved' && view !== 'haptics' && view !== 'report' && (
        <Header
          view={view as any} onMenuClick={() => setIsSidebarOpen(true)} onBackClick={handleBackClick}
          onNewChatClick={handleNewChat} onToggleGhostMode={handleToggleGhostMode}
          isCanvasActive={isCanvasActive} onZoomClick={() => setIsZoomOpen(true)}
          isEmptyChat={isEmptyChat} isTemporary={isTemporary} activeChatTitle={getSmartTitle(activeChat)}
        />
      )}
      <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {view === 'chat' && (
          <ChatPage
            ...
            webCount={0} postCount={0} activityStatus={activityStatus}
            ...
            onCloseCanvas={() => setIsCanvasActive(false)} onUpgradeClick={() => setIsSubscriptionOpen(true)}
        ...
        {view === 'profile' && (
          <ProfilePage
            ...
            onViewMemory={() => setView('memory')} onViewAiSettings={() => setView('ai-settings')}
            onUpgradeClick={() => setIsSubscriptionOpen(true)} onViewAppearance={() => setView('appearance')}
            onViewUsage={() => setView('usage')} onViewIntegrations={() => setView('integrations')}
            onViewSecurity={() => setView('security')} onViewExport={() => setView('export')}
            onViewHaptics={() => setView('haptics')} onViewReport={() => setView('report')}
          />
        )}
        ...
        {view === 'group-list' && <GroupListPage groups={groupSessions} onSelectGroup={(id: string) => { setActiveGroupId(id); setView('group-chat'); }} onCreateGroup={(title: string) => { const id = createGroup(title); if (id) { setActiveGroupId(id); setView('group-chat'); showToast('Grup berhasil dibuat'); } else showToast('Maksimal 5 grup tercapai'); }} onBack={() => setView('chat')} />}
