...
function applyThemeToDOM(bg: string, isDark: boolean) {
  // 1. Update theme-color (status bar)
  ...
  // 2. Update color-scheme (navigation bar bawah Android)
  ...
  // 3. Set color-scheme di html element juga
  document.documentElement.style.colorScheme = isDark ? 'dark' : 'light';

  // 4. Paksa background di semua level
...
export default function AppMobile() {
  ...
  // Init theme langsung dari OS — TIDAK pakai 't-light' dulu biar gak flash
  ...
  useEffect(() => {
    ...
    // Apply semua CSS vars
    Object.entries(vars).forEach(([key, val]) => root.style.setProperty(key, val));

    // Apply ke DOM (status bar + nav bar + background)
  ...
  return (
    ...
    >
      {/* Force semua ancestor pakai warna yang sama — hapus semua kemungkinan garis */}
      <style>{`
        html, body, #root {
      ...
      {view !== 'search' && view !== 'tasks' && view !== 'appearance' && view !== 'usage' && view !== 'integrations' && view !== 'security' && view !== 'export' && view !== 'group-profile' && view !== 'saved' && (
        <Header
          view={view as any}
