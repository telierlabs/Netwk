...
// Update theme-color meta tag agar status bar OS nyambung dengan app
function setThemeColor(color: string) {
  // Update semua meta theme-color (hapus media query, set langsung)
...
export default function AppMobile() {
  ...
  useEffect(() => {
    ...
    // KUNCI: update theme-color meta agar status bar OS nyambung
    setThemeColor(vars['--bg']);
  }, [theme]);

  // Set theme-color saat pertama load sesuai prefers-color-scheme
  useEffect(() => {
    const isDark = window.matchMedia('(prefers-color-scheme: dark)').matches;
    // Set initial theme berdasarkan OS preference
    if (isDark) {
      setTheme('t-dark');
    } else {
      setTheme('t-light');
