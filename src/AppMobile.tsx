...
import { ZoomOverlay } from './components/chat/ZoomOverlay'; // 🔥 TAMBAHAN ZOOM OVERLAY
...
export default function AppMobile() {
  ...
  const [isZoomOpen, setIsZoomOpen] = useState(false); // 🔥 STATE ZOOM
  ...
  const handleNewChat = () => {
    ...
    setIsZoomOpen(false); // 🔥 TUTUP ZOOM KALO NEW CHAT
  ...
  return (
    <div className={cn('app transition-colors duration-350', theme)} style={{ fontFamily: currentFontFamily, height: '100dvh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {/* 🔥 ZOOM OVERLAY DISINI BIAR NUTUPIN SEMUA */}
      <ZoomOverlay 
        isOpen={isZoomOpen} 
      ...
      {view !== 'search' && (
        <Header 
          ...
          onZoomClick={() => setIsZoomOpen(true)} // 🔥 KONEKIN ZOOM CLICK DI SINI
        />
      )}
