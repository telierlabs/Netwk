...
// ── FUNGSI LOAD BALANCER & PENYADAP PENGATURAN USER ──
function getActiveApiKey(): string {
  ...
  // 2. Tarik semua 10 Key dari Vercel secara aman (Vite-safe)
  const rawKeys = [
  ...
  // 3. Saring biar yang masuk cuma Key yang ada isinya (gak undefined)
  const API_KEYS = rawKeys.filter(key => key && key.trim() !== "");

  // Fallback darurat kalau kosong semua (jaga-jaga)
  if (API_KEYS.length === 0) {
    return ""; 
  }

  // 4. Sistem Muter (Round-Robin) pakai localStorage
  let currentIndex = parseInt(localStorage.getItem('cylen_key_index') || '0');
  
  // Jaga-jaga kalo index ngelebihi jumlah key yang lo masukin di Vercel
  if (currentIndex >= API_KEYS.length) {
    currentIndex = 0;
  }

  // 5. Tentukan key yang dipake buat chat INI
  const activeKey = API_KEYS[currentIndex];

  // 6. Majuin antrian buat chat SELANJUTNYA
...
export class ConnectionError extends Error {
  // Tambahin 'limit' di sini biar bisa nangkep pesan error Limit Kuota dari API
...
function detectAutoMode(messages: { role: string; content: string }[]): 'spark' | 'think' {
  ...
  const thinkKeywords = [
    ...
    // Tambahan keywords biar ayat/surat otomatis masuk mode Think (biar bebas nulis panjang)
...
export async function chatWithGeminiStream(
  ...
  // Bangun Instansi AI baru dengan API Key yang lagi dapet giliran (Load Balancer)
...
5. Langsung balas pesannya tanpa mengulang nama pengirim.`;
    ...
    // ─── AMBIL INSTRUKSI PERSONALIASI DARI USER ───
  ...
  const config: any = {
    ...
    // CATATAN PENTING: Jika SafetySettings ini memicu Error 400 dari Google (biasanya di API Gratisan),
    // hapus seluruh block safetySettings ini di masa depan.
    safetySettings: [
  ...
  try {
    ...
    // Log error asli ke console browser biar ketahuan kalo ngecrash lagi
    console.error("Gemini API Error Detail:", error);
    
    if (!navigator.onLine) throw new ConnectionError('offline', 'No internet connection');
    const msg = error?.message?.toLowerCase() || '';

    // TAMBAHAN: Deteksi kalau Limit API Habis (Error 429) biar UI tau
