...
export async function chatWithGeminiStream(
  ...
  const effectiveMode: 'spark' | 'think' =
    mode === 'auto' ? detectAutoMode(messages) :
    mode === 'think' ? 'think' : 'spark';

  // Model: think pakai thinking mode, spark pakai flash biasa
  ...
  const config: any = {
    systemInstruction: getSystemPrompt(effectiveMode),
    // Think mode: aktifkan thinking budget untuk analisis lebih dalam
    ...(effectiveMode === 'think' && {
