...
function getSystemPrompt(effectiveMode: 'spark' | 'think', memoryContext?: string): string {
  ...
  const memorySection = memoryContext && memoryContext.trim().length > 0
    ? `\n\n---\n\n# MEMORI TENTANG USER — WAJIB DIINGAT\n\nBerikut adalah fakta-fakta yang sudah kamu ketahui tentang user ini dari percakapan sebelumnya. Gunakan informasi ini secara natural dalam percakapan — jangan sebut "berdasarkan memori" atau sejenisnya, cukup tunjukkan bahwa kamu ingat:\n\n${memoryContext}\n\n---`
    : '';
...
export async function extractMemoryFromChat(
  ...
  try {
    ...
    // Ambil 6 pesan terakhir saja biar efisien & hemat token
