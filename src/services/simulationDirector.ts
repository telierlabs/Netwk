...
- Jangan pernah menjawab "topik ini tidak didukung". Terjemahkan konsep apa
  ...
  sebagai koneksi.
- WAJIB MUTLAK — GAYA VISUAL HOLOGRAFIK SAJA: sistem ini adalah HOLOGRAPHIC
  SCIENTIFIC VISUALIZER, BUKAN 3D model realistis/solid seperti game atau
  software CAD. "material.style" HANYA punya 2 pilihan sah: "wireframe"
  (garis rangka bercahaya — DNA helix, jaringan node, kerangka mesin, jalur
  orbit) dan "glow" (permukaan tembus pandang berpendar, opacity rendah
  0.25-0.6 — organ, planet, bodi kendaraan, sel). TIDAK ADA pilihan lain;
  spec dengan style selain itu akan ditolak sistem. Bayangkan hasil akhirnya
  seperti hologram futuristik di atas latar hitam — garis putih/perak
  bercahaya, partikel, transparansi — BUKAN render solid buram warna abu-abu
  polos.
...
- Untuk struktur berulang (DNA, kristal, deret partikel), gunakan entity
  ...
  sphere, style "wireframe" atau "glow") yang diposisikan sepanjang path
  untuk merepresentasikan bagian spesifik (mis. base pair), BUKAN satu
  particleSystem polos tanpa struktur.
...
- Gunakan cross-section/cutaway (via "layers" dengan clipPlane) untuk objek
  yang punya struktur dalam (organ, mesin, planet berlapis) — clipPlane
  tetap berlaku sama di atas material "glow" tembus pandang.
...
- Untuk material "glow", pakai opacity 0.25–0.6 (biar tetap tembus pandang,
  tidak buram) dan emissiveIntensity 0.8–1.8 untuk efek berpendar. Untuk
  "wireframe", opacity boleh lebih tinggi (0.6–0.9) karena garisnya sendiri
  sudah tipis.
...
export interface GenerateSimulationResult {
  ...
  attempts: number;
  /**
   * Alasan gagal yang sebenarnya (pesan error Zod atau error Gemini),
   * cuma terisi kalau usedFallback true. WAJIB ditampilkan ke user secara
   * eksplisit — jangan disembunyikan di balik fallback yang terlihat
   * seperti berhasil.
   */
  errorDetail?: string;
}
