...
const safeNumber = z
  ...
  .refine((n) => Math.abs(n) <= MAX_SAFE_MAGNITUDE, {
    message: `Nilai numerik melebihi batas aman (±${MAX_SAFE_MAGNITUDE})`,
  });

const safePositive = safeNumber.refine((n) => n > 0, {
  message: 'Nilai harus positif',
});

const safeNonNegative = safeNumber.refine((n) => n >= 0, {
...
const MaterialSchema = z.object({
  color: hexColor,
  style: z.enum(['solid', 'wireframe', 'glass', 'glow', 'gradient']),
