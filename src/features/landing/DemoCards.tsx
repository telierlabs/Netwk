...
export const DemoCards: React.FC<DemoCardsProps> = ({ onNavigate }) => {
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '800px', margin: '0 auto' }}>
      ...
      {/* Card 4: Voice (Silver/Abu-abu gelap yang lebih LEBAR) */}
      <div onClick={() => onNavigate('voice')} style={{ background: '#131313', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.04)', textAlign: 'left', overflow: 'hidden', cursor: 'pointer' }}>
        {/* 👇 Gap diperbesar jadi 6px */}
        <div style={{ padding: '40px 24px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '6px', minHeight: '160px' }}>
           {/* 👇 Bar diperbanyak biar ngelebar */}
           {[2, 3, 4, 5, 6, 8, 10, 12, 14, 16, 20, 16, 14, 12, 10, 8, 6, 5, 4, 3, 2].map((h, i) => (
             <div key={i} style={{ 
               width: '6px', /* 👇 Width diperbesar dikit */
               height: `${h * 4}px`, /* 👇 Proporsi tinggi disesuaikan */
               background: 'linear-gradient(to top, #444444, #e0e0e0)', 
               borderRadius: '10px', 
               /* 👇 Delay animasi dibikin lebih halus */
               animation: `wave 1.2s ease-in-out infinite ${i * 0.08}s` 
             }}></div>
           ))}
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 24px', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
