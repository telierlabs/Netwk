...
export interface Material {
  color: string;
  /**
   * HANYA gaya holografik yang sah — arsitektur ini SENGAJA tidak
   * menyediakan opsi solid/opaque/glass/gradient. Ini bukan preferensi AI
   * yang bisa diabaikan; ini batasan struktural DSL. Tidak ada jalur render
   * mesh solid buram sama sekali di sistem ini.
   *
   * 'wireframe' -> garis rangka bercahaya. Cocok untuk struktur/skeleton:
   *   DNA helix, jaringan node, kerangka mesin, jalur orbit.
   * 'glow'      -> permukaan tembus pandang berpendar. Cocok untuk bentuk
   *   bervolume: jantung, sel, planet, bodi kendaraan. SELALU transparan,
   *   tidak pernah opaque/solid.
   */
  style: 'wireframe' | 'glow';
  opacity?: number;
