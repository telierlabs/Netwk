...
export function useSaved() {
  ...
  // 2. Fungsi Simpan dengan AI Auto-Detector (Lebih Pintar)
  const saveItem = async (content: string, chatId?: string) => {
    ...
    // Deteksi cerdas: 
    // 1. Harus ada backticks DAN (ada nama bahasa pemrograman ATAU keyword koding)
    const isCode = content.includes('```') && (content.match(/```[a-zA-Z]+/) || content.includes('function') || content.includes('const') || content.includes('import') || content.includes('return') || content.includes('<div>'));
    
    // 2. Gambar (tapi bukan link biasa)
    const isImage = content.includes('![') || content.includes('[GENERATE_IMAGE');
    
    // 3. Dokumen
    const isDoc = content.includes('[GENERATE_PDF') || content.includes('[GENERATE_DOCS') || content.includes('[GENERATE_EXCEL') || content.includes('[GENERATE_PPT');
    ...
    if (isCode) {
      ...
      type = 'teks'; // Paksa masuk ke teks kalau bukan ketiganya
    }
