...
export function useGroupChat() {
  ...
  // UPDATE: Parameter menerima images
  const sendGroupMessage = async (inputText: string, images?: string[]) => {
    // Validasi ganda (Teks & Gambar)
    if ((!inputText.trim() && (!images || images.length === 0)) || isSending || !activeGroupId) return;

    // UPDATE: Memasukkan images ke dalam message history
    const userMessage: Message = {
      role: 'user',
      content: inputText,
      images: images || [], // Gambar akan tersimpan di sini
      timestamp: formatTimestamp(),
      senderName: 'Kamu'
    };
    ...
    try {
      ...
      for (const ai of aiParticipants) {
        ...
        // ─── MAGIC: ROLEPLAY SYSTEM PROMPT KHUSUS GRUP ───
        const systemPrompt = `Kamu sedang berada di sebuah grup chat bernama "${currentGroup.title}".
Daftar peserta: ${currentGroup.participants.map(p => p.name).join(', ')}.
Ingat namamu adalah ${ai.name}.`;
        ...
        const formattedMessages = conversationHistory.map(m => {
            ...
            return {
                ...
                // Kirim gambar ke AI (jika API support)
        ...
        // UPDATE: Siapkan properti images kosong untuk balasan AI
        // (Berjaga-jaga kalau AI di masa depan bisa membalas dengan gambar)
