...
export function useGroupChat() {
  ...
  const sendGroupMessage = async (inputText: string, images?: string[]) => {
    ...
    try {
      ...
      // ── LOGIKA PARSER (MEMBACA URUTAN TAG SECARA ABSOLUTE) ──
      const mentionRegex = /@([a-zA-Z0-9_]+)/g;
      let match;
      const taggedNames: string[] = [];
      
      // Kumpulin semua nama yang di-tag berurutan
      ...
      if (taggedNames.length > 0) {
        // Susun AI sesuai urutan tag dari array taggedNames
        for (const tagName of taggedNames) {
          ...
          if (foundAI && !respondingAIs.some(r => r.id === foundAI.id)) {
            respondingAIs.push(foundAI); // Hindari AI masuk dua kali berturut-turut
          }
        }
      }

      // Kalau lu ga ngetik @ sama sekali, panggil 1 AI aja (biar API gak boncos nyerang barengan)
      ...
      // ── LOOPING ANTREAN (A KERJA -> BERES -> B KERJA) ──
...
3. DILARANG KERAS memulai jawabanmu dengan namamu sendiri seperti "[${ai.name}]: " atau "${ai.name}: ". Langsung tulis saja isi jawabanmu.`;
        ...
        const formattedMessages = conversationHistory.map(m => {
            ...
            // Kita kasih tau siapa yang nulis histori chat, biar AI paham konteks
        ...
        for await (const chunk of stream) {
          ...
          // ── THE "GUNTING": BERSIHIN TEKS SECARA REAL-TIME JIKA AI LATAH ──
          // Kalau AI bandel nulis "[Gemini]:", kita potong otomatis!
        ...
        // Simpan jawaban final bersih ke memori biar AI selanjutnya bisa baca
