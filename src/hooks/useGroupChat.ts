...
export function useGroupChat() {
  ...
  const [thinkingAI, setThinkingAI] = useState<string | null>(null); // State buat nampilin "GEMINI SEDANG BERPIKIR..."
  ...
  const sendGroupMessage = async (inputText: string, images?: string[]) => {
    ...
    try {
      ...
      // Deteksi siapa yang di-tag (kolaborasi)
      const mentionedAIs = aiParticipants.filter(ai => textLower.includes(`@${ai.name.toLowerCase()}`));
      ...
      for (const ai of respondingAIs) {
        setThinkingAI(ai.name); // Memicu UI "NamaAI SEDANG BERPIKIR..."

        const groupDesc = (currentGroup as any).description ? `Deskripsi grup: ${(currentGroup as any).description}` : '';
        // Bikin AI fokus ngerjain tugasnya sendiri, jangan balasin buat AI lain
        const systemPrompt = `Kamu sedang berada di sebuah grup chat bernama "${currentGroup.title}". ${groupDesc}
Daftar peserta: ${currentGroup.participants.map(p => p.name).join(', ')}. Ingat namamu adalah ${ai.name}.
ATURAN KERAS: Jawab saja bagian yang diarahkan padamu. JANGAN PERNAH membuat naskah percakapan atau menjawab atas nama AI lain. JANGAN tulis namamu di awal jawaban.`;
        ...
        // Nampilin bubble loading milik AI yang lagi ngetik
        ...
        for await (const chunk of stream) {
          ...
          setGroupSessions(prev => prev.map(group => group.id === activeGroupId ? { ...group, messages: group.messages.map((m, idx) => idx === group.messages.length - 1 ? { ...m, content: fullText } : m) } : group));
        }
        
        // Simpan jawaban final ke histori biar AI selanjutnya (kalau ada) bisa baca
        const finalizedAiMessage = { ...aiMessage, content: fullText };
        conversationHistory = [...conversationHistory, finalizedAiMessage];
        
        await new Promise(resolve => setTimeout(resolve, 800)); // Jeda biar estetik
      }
    } catch (error) { 
      console.error("Grup Chat Error:", error); 
    } finally { 
      setIsSending(false); 
      setThinkingAI(null); // Matikan efek loading
    }
  };

  return { activeGroup, groupSessions, activeGroupId, setActiveGroupId, isSending, thinkingAI, createGroup, addParticipant, sendGroupMessage };
}
