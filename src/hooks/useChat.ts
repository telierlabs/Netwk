...
// Bersiin keyword buat GNews, jangan terlalu agresif
function extractSearchKeyword(text: string): string {
  ...
  // Kalau hasil cleaning kosong atau < 3 karakter, pakai sebagian teks asli
  if (keyword.length < 3) {
    // Ambil kata-kata substantif dari teks asli
    const words = text.split(/\s+/).filter(w => w.length > 3 && !/^(apa|ada|tolong|dong|bisa|minta|info|berita|news|terkini|terbaru)\b/i.test(w));
...
export function useChat() {
  ...
  const sendMessage = async (
    ...
    // ── Deklarasi di luar try agar persistent sepanjang fungsi ──
    let injectedContext = '';
    ...
    // ── Step 1: Coba ambil dari GNews dulu ──
    if (isNewsQuery) {
      setIsSearching(true);
      try {
        const searchKeyword = extractSearchKeyword(finalText);
        console.log(`[GNews] Keyword pencarian: "${searchKeyword}"`);

        const articles: NewsArticle[] = await fetchRealtimeNews(searchKeyword);

        if (articles && articles.length > 0) {
          // Validasi ulang URL sebelum simpan — pastikan direct article link
          gnewsSources = articles
            .filter(a => {
              try {
                const u = new URL(a.url);
                return u.pathname.length > 5; // bukan homepage
              } catch { return false; }
            })
            .map(a => ({
              title: a.title,
              url: a.url,       // URL artikel langsung dari GNews
              srcType: 'web'
            }));

          if (gnewsSources.length > 0) {
            injectedContext = `\n\n=== DATA BERITA REAL-TIME ===\n` +
              articles.map((a, i) =>
                `${i+1}. Judul: ${a.title}\nSumber: ${a.source.name}\nURL: ${a.url}\nDeskripsi: ${a.description}`
              ).join('\n\n') +
              `\n\nTugasmu: Rangkum berita di atas. JANGAN sebut "GNews" atau "Berdasarkan data". Tulis seolah kamu tahu sendiri. Ikuti format system prompt.`;

            if (articles[0]?.image) manualImage = articles[0].image;
            isGNewsSuccess = true;
            setWebCount(gnewsSources.length);
            console.log(`[GNews] Sukses! ${gnewsSources.length} sumber valid disiapkan.`);
          }
        }
      } catch (e) {
    ...
    // ── Step 2: Tentukan apakah perlu Gemini search ──
    // KUNCI: kalau GNews udah sukses, JANGAN kasih googleSearch ke Gemini
    ...
    try {
      // Inject GNews context ke pesan terakhir user
      ...
      const stream = await chatWithGeminiStream(
        chatMessages,
        needsGeminiSearch,   // FALSE kalau GNews sukses → Gemini ga pakai googleSearch
        location,
      ...
      // Persistent sources — GNews jadi prioritas utama
      // Pakai ref array biar ga kehapus antar render
      const persistentSources = [...gnewsSources];

      // Init pesan assistant dengan sumber GNews langsung
      ...
      for await (const chunk of stream) {
        ...
        // Handle generated image dari Gemini
        ...
        // ── Grounding chunks dari Gemini ──
        // HANYA diproses kalau GNews GAGAL (biar VertexAI ga nyempil)
        if (!isGNewsSuccess) {
          ...
          if (chunks) {
            ...
            for (const ch of chunks) {
              if (ch.web) {
                ...
                // Ekstrak URL asli dari Google redirect
