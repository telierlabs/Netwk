...
export function useChat() {
  ...
  const sendMessage = async (
    ...
    directText?: string,
    baseMessages?: Message[] // ✅ Tambahan parameter: Buat nyuntik array chat yang udah dibersihin
  ) => {
    ...
    // ✅ Pake baseMessages kalo ada (dari Retry), kalo ga ada pake state messages biasa
  ...
  const retryLastMessage = () => {
    ...
    // Bersihin pesan error dari array
    const cleanedMessages = messages.filter(m => !(m as any).isConnectionError);
    
    // Potong pesan user terakhir (karena mau dikirim ulang)
    const newMessages = cleanedMessages.slice(0, -1);

    // Langsung tembak sendMessage pakai array yang udah bersih (tanpa setTimeout!)
    sendMessage(
      lastUserMsg.content, 
