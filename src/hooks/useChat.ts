...
export function useChat() {
  ...
  // ── 2. HELPER UNTUK SYNCHRONIZE KE FIRESTORE (FIX UNDEFINED ERROR) ──
  const syncSessionToDb = async (session: ChatSession) => {
    if (!auth?.currentUser || session.isTemporary) return;
    try {
      const uid = auth.currentUser.uid;
      // FIX PALING PENTING: Firestore menolak nilai 'undefined'.
      // JSON.parse(JSON.stringify()) akan menghapus semua properti undefined dari messages.
  ...
  // ── 3. FIX: UPDATE JUDUL & TANGGAL OTOMATIS ──
    ...
    setChatSessions(prev => {
      const next = prev.map(session => {
        if (session.id === activeChatId) {
          ...
          // Setiap ada update chat, tanggalnya kita perbarui biar naik ke atas di riwayat
