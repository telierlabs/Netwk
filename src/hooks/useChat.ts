...
export function useChat() {
  ...
  // ── 2. HELPER UNTUK SYNCHRONIZE KE FIRESTORE ──
  ...
  const updateChat = (newMessages: Message[], inputText?: string, shouldSync: boolean = false) => {
    setChatSessions(prev => {
      const next = prev.map(session => session.id === activeChatId ? { ...session, messages: newMessages, title: (session.messages.length === 1 && inputText && !session.isTemporary) ? inputText.slice(0, 30) + (inputText.length > 30 ? '...' : '') : session.title } : session );
      if (shouldSync) {
...
PENTING: Selalu tulis teks penjelasan singkat SETELAH tag penutup selesai!`;
    ...
    try {
      ...
      // ── SYNC HASIL AKHIR AI KE FIRESTORE ──
      setChatSessions(prev => {
        const next = prev.map(session =>
  ...
  const createNewChat = (isGhostMode: boolean = false) => {
    ...
    if (!isGhostMode) syncSessionToDb(newSession); // Langsung buat brankas kosong
    return newId;
  };
