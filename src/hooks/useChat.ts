...
export function useChat() {
  const [chatSessions, setChatSessions] = useState<ChatSession[]>([
    {
      ...
      isTemporary: false // PENTING: Inisialisasi awal
    }
  ]);
  ...
  // DERIVED STATE: Nggak butuh useState tambahan buat isTemporary.
  // Langsung ambil dari session aktif biar selalu SINKRON 100% sama UI
  ...
  const createNewChat = (isGhostMode: boolean = false) => {
    ...
    const newSession: ChatSession = {
      ...
      messages: [],
      date: formatTimestamp(),
      isTemporary: isGhostMode // Murni dikontrol dari sini
    };
    
    // Kalau dia temporary, filter dulu chat temporary sebelumnya (biar g numpuk di sidebar)
    setChatSessions(prev => {
        const filtered = prev.filter(s => !s.isTemporary); // hapus temp lama
  ...
  // createTempChat kita hapus aja, satukan di createNewChat dengan parameter isGhostMode
