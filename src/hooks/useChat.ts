...
// Ekstrak suggestions dari akhir respons AI
// Format: AI nulis di akhir blok ```suggestions ... ```
...
export function useChat() {
  ...
  const sendMessage = async (inputText: string, attachedImage: string | null, attachedImages?: string[]) => {
    ...
    try {
      ...
      for await (const chunk of stream) {
        ...
        // Parse suggestions dari streaming text
  ...
  return { chatSessions, activeChatId, setActiveChatId, messages, isSending, isSearching, searchQuery, sendMessage, createNewChat };
}
