...
export function useChat() {
  ...
  // ── 1. AMBIL RIWAYAT DARI FIRESTORE SAAT LOGIN ──
  useEffect(() => {
    let isMounted = true;
    const fetchChats = async () => {
      ...
      try {
        ...
        if (loaded.length > 0 && isMounted) {
          // Urutkan chat terbaru di atas berdasarkan ID (karena ID = Date.now())
  ...
  // ── 2. HELPER UNTUK SYNCHRONIZE KE FIRESTORE (ANTI GHOST MODE) ──
  const syncSessionToDb = async (session: ChatSession) => {
    // Kalau belum login atau ini adalah MODE HANTU (isTemporary), BATALKAN SIMPAN!
  ...
  // ── 3. FIX: JUDUL CHAT BERUBAH OTOMATIS ──
  const updateChat = (newMessages: Message[], inputText?: string, shouldSync: boolean = false) => {
    setChatSessions(prev => {
      const next = prev.map(session => {
        if (session.id === activeChatId) {
          // Kalau ini chat pertama kali, ganti 'Chat Baru' jadi isi pesan user
      ...
      if (shouldSync) {
        ...
        // Pastikan cuma sync kalau bukan Mode Hantu
  ...
  const sendMessage = async (inputText: string, attachedImage: string | null, attachedImages?: string[], attachedPdfs?: { data: string; name: string }[], directText?: string, baseMessages?: Message[]) => {
    ...
    updateChat(currentMessages, finalText, true); // Sync langsung saat user ngirim pesan
    setIsSending(true);
...
PENTING: Selalu tulis teks penjelasan singkat SETELAH tag penutup selesai!`;
    ...
    try {
      ...
      // ── SYNC HASIL AKHIR AI KE FIRESTORE (Kalau bukan Ghost Mode) ──
      setChatSessions(prev => {
        const next = prev.map(session =>
      ...
      updateChat([...currentMessages, { role: 'assistant', content: '', timestamp: formatTimestamp(), isConnectionError: true, connectionErrorType: errorType } as any], undefined, true);
    } finally {
  ...
  const createNewChat = (isGhostMode: boolean = false) => {
    ...
    // Saat bikin chat baru, hapus semua session Ghost Mode yang sebelumnya kebuka biar sidebar bersih
