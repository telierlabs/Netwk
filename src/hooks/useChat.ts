...
function parseCanvasTags(text: string): { clean: string; canvasNodes?: any[] } {
  ...
  if (match) {
    clean = text.replace(/```json-canvas\n[\s\S]*?```/, '').trim();
    try {
  ...
  const sendMessage = async (
    ...
    try {
      ...
      for await (const chunk of stream) {
        ...
        // Panggil Parser Canvas pas lg streaming (biar JSON-nya ga bocor di layar chat)
        const { clean: streamClean } = parseCanvasTags(clean);
            ...
            ? {
                ...
                messages: session.messages.map((m, idx) =>
                  idx === session.messages.length - 1
                    ? { ...m, content: streamClean, sources: sources.length > 0 ? sources : undefined, image: generatedImage || m.image, suggestions: validSuggestions }
                    : m
                )
              }
            : session
      ...
      // 🔥 FINAL PARSE SETELAH STREAM SELESAI
      ...
      // 🔥 LEMPAR EVENT KE CANVAS KALAU AI NULIS JSON KARTU
