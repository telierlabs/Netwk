...
function hideTagsWhileStreaming(text: string): string {
  ...
  t = t.replace(/\[GENERATE_(IMAGE|PDF|DOCS|EXCEL|PPT):[\s\S]*?(\[\/GENERATE_(PDF|DOCS|EXCEL|PPT)\]|\]|$)/g, '');
  return t.trim();
}
...
export function useChat() {
  ...
  // STATE STATUS MENDUKUNG SEMUA FORMAT BARU
  const [activityStatus, setActivityStatus] = useState<'idle' | 'image' | 'pdf' | 'docs' | 'excel' | 'ppt'>('idle');
  ...
  const sendMessage = async (inputText: string, attachedImage: string | null, attachedImages?: string[], attachedPdfs?: { data: string; name: string }[], directText?: string, baseMessages?: Message[]) => {
    ...
    // ── INSTRUKSI GENERATOR MULTI-FORMAT ──
...
PENTING: Selalu tulis teks penjelasan singkat SETELAH tag penutup selesai!`;
    ...
    try {
      ...
      let currentActivity: 'idle' | 'image' | 'pdf' | 'docs' | 'excel' | 'ppt' = 'idle';
      ...
      for await (const chunk of stream) {
        ...
        if (currentActivity === 'idle') {
          if (fullText.includes('[GENERATE_PDF:')) { currentActivity = 'pdf'; setActivityStatus('pdf'); }
      ...
      for (const m of matchers) {
        ...
        if (found) {
          ...
          try {
            ...
            // Await untuk async function (DOCX & PPT)
            const dataUrl = await m.generator(title, content);
            generatedFileInfo = { name: title, dataUrl, type: m.type };
