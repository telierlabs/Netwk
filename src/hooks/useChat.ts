...
export function useChat() {
  ...
  const sendMessage = async (
    ...
    if (isNewsQuery) {
      setIsSearching(true);
      try {
        // 🔥 FIX: Pembersih keyword biar GNews gampang nemu!
        ...
        if (chunks) {
          const newSources = chunks.map(ch => {
            if (ch.web) {
              ...
              // 🔥 HACKING VERTEX AI URL 🔥
              if (url.includes('vertexaisearch') || url.includes('googleusercontent')) {
                 const titleParts = title.split(/[-|]/);
                 // Ambil judul aslinya (biasanya ada di bagian pertama sebelum tanda | atau -)
                 const realTitle = titleParts[0].trim();
                 
                 // KITA UBAH URL-NYA JADI PENCARIAN GOOGLE BERDASARKAN JUDUL
                 // Ini satu-satunya cara biar gak nge-blank putih atau lari ke beranda
