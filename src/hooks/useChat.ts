    ...
    if (isNewsQuery) {
      ...
      try {
        ...
        if (articles && articles.length > 0) {
          ...
          manualSources = articles.map(a => ({
            title: a.title, 
            url: a.url, // GNEWS direct link aslinya dipertahankan
            srcType: 'web'
          }));
        ...
        if (chunks && !isGNewsSuccess) {
          const newSources = chunks.map(ch => {
            if (ch.web) {
              ...
              // LOGIKA BENER: Kalau ini link redirect Google, ekstrak link artikel aslinya
              ...
              // Kalau halamannya murni hasil pencarian (bukan artikel), BUANG.
