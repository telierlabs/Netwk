  ...
  const sendMessage = async (
    ...
    if (isNewsQuery) {
      ...
      try {
        ...
        if (searchKeyword === '') searchKeyword = 'indonesia'; 
        ...
        if (articles && articles.length > 0) {
          ...
          // 🔥 MURNI GNEWS, TANPA DI POTONG SAMA SEKALI
          manualSources = articles.map((a: any) => ({
            title: a.title, // Judul asli artikel
            url: a.url,     // Link asli artikel
            srcType: 'web'
          }));
    ...
    // 🔥 KALAU USER NANYA BERITA, MATIKAN VERTEX GOOGLE SEARCH SECARA MUTLAK
      ...
      for await (const chunk of stream) {
        ...
        if (chunks) {
          const newSources = chunks.map(ch => {
            if (ch.web) {
              // Vertex AI Link tetep murni buat pertanyaan di luar berita
              return { title: ch.web.title || 'Referensi', url: ch.web.uri || '#', srcType: 'web' };
            }
