...
const generateProfessionalPDF = (title: string, content: string) => {
  ...
  doc.setFontSize(11);
  // doc.setLineHeightFactor(1.5); // Kalau Vercel error gara-gara ini, hapus aja baris ini.
  ...
  lines.forEach((line: string) => {
    ...
    if (cursorY > pageHeight - margin) {
      ...
      // Kasih nomor halaman di halaman sebelumnya (Opsional tapi keren)
      const pageCount = doc.internal.pages.length - 1;
      doc.setFontSize(9);
      doc.text(`Halaman ${pageCount}`, pageWidth - margin, pageHeight - 10, { align: 'right' });
      doc.setFontSize(11); // Balikin ke size normal
    }
...
export function useChat() {
  ...
  const sendMessage = async (
    ...
    // ── INSTRUKSI RAHASIA KE AI BUAT FITUR GAMBAR & PDF DEWA ──
...
Jika user meminta dibuatkan file PDF (surat, makalah, laporan, rangkuman, dll), kamu harus buatkan materi yang SANGAT LENGKAP DAN DETAIL (bisa berlembar-lembar panjang). Gunakan pemisah '---' untuk menandai bagian baru atau pergantian bab.
WAJIB bungkus isinya dengan format persis seperti ini:
...
[/GENERATE_PDF]`;
    ...
    try {
      ...
      if (pdfMatch) {
        ...
        try {
          await new Promise(r => setTimeout(r, 1500)); // Loading delay UX
      ...
      // Kalau ada PDF, tempelkan syntax
      if (generatedPdfInfo) {
