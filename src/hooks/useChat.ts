...
export function useChat() {
  ...
  const sendMessage = async (
    ...
    try {
        ...
        if (chunks) {
          ...
          newSources.forEach(s => {
            if (!existingUrls.has(s.url)) {
              ...
              // X/Twitter domain → postCount, sisanya webCount
            ...
            ? {
                ...
                messages: session.messages.map((m, idx) =>
                  idx === session.messages.length - 1
                    ? { ...m, content: clean, sources: sources.length > 0 ? sources : undefined, image: generatedImage || m.image, suggestions: validSuggestions }
                    : m
                )
              }
            : session
      ...
      setChatSessions(prev => prev.map(session =>
        session.id === activeChatId
          ? {
              ...session,
              messages: session.messages.map((m, idx) =>
                idx === session.messages.length - 1 ? { ...m, suggestions: validFinalSuggestions } : m
              )
            }
          : session
      ));
      ...
      // ✅ Pas gagal, kita push error card ke chat list
  ...
  const retryLastMessage = () => {
    ...
    // 2. Filter out error card sebelum ngirim ulang
    // Kita filter semua pesan asisten yang isConnectionError = true
    const cleanedMessages = messages.filter(m => !(m as any).isConnectionError);
    
    // 3. Kita hapus juga pesan user yang terakhir itu (karena fungsi sendMessage akan nge-push lagi)
    const newMessages = cleanedMessages.slice(0, -1);

    // 4. Update state chat list jadi tanpa error card
    setChatSessions(prev => prev.map(session =>
      session.id === activeChatId ? { ...session, messages: newMessages } : session
    ));

    // 5. Eksekusi ulang pesan
    setTimeout(() => { 
