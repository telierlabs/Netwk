...
export function useChat() {
  ...
  const sendMessage = async (
    ...
    // 🚀 INTERCEPTOR GNEWS: Cek apakah user nanya berita
    let injectedContext = '';
    ...
    if (isNewsQuery) {
      setIsSearching(true);
      try {
        const articles = await fetchRealtimeNews(finalText);
        if (articles && articles.length > 0) {
          // Suapin datanya ke Gemini secara paksa
          injectedContext = `\n\n=== DATA BERITA REAL-TIME ===\n` + 
            articles.map((a: any, i: number) => `${i+1}. Judul: ${a.title}\nSumber: ${a.source.name}\nDeskripsi: ${a.description}`).join('\n\n') +
            `\n\nTugasmu: Rangkum berita di atas dengan gaya bahasamu. WAJIB patuhi format yang ada di system prompt (Garis pemisah, dll). JANGAN SEBUTKAN "Berdasarkan data GNews".`;
          
          // Ambil sumber asli biar logonya bagus
          manualSources = articles.map((a: any) => ({ title: a.source.name, url: a.url, srcType: 'web' }));
          
          // Ambil gambar pertama dari berita biar ditampilin visualnya!
          if (articles[0].image) {
    ...
    // Kalau udah pakai GNews, MATIKAN fitur Google Search bawaan Gemini biar gak bentrok sama Vertex AI!
    const needsSearch = !isNewsQuery && /^(apa|cari|jelaskan|siapa|bagaimana|kenapa|mengapa|kapan|cara|tolong|how|what|who|why|when|where|harga|cuaca|jadwal|lokasi|tempat|restoran|dimana|near|location|place|restaurant)/i.test(finalText);
    ...
    try {
      // Masukin konteks berita curian kita ke pesan terakhir user (biar AI baca)
      ...
      let fullText       = '';
      // Inject sumber berita dan gambar manual ke dalam State AI!
      let sources: { title: string; url: string }[] = [...manualSources];
      ...
      for await (const chunk of stream) {
        ...
        if (chunks) {
          const newSources = chunks.map(ch => {
            if (ch.web)  return { title: ch.web.title  || 'Sumber', url: ch.web.uri  || '#', srcType: 'web' };
