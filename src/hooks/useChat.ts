  ...
  const sendMessage = async (
    ...
    if (isNewsQuery) {
      ...
      try {
        ...
        if (searchKeyword === '') searchKeyword = 'indonesia OR nasional'; 
        ...
        if (articles && articles.length > 0) {
          ...
          manualSources = articles.map(a => ({
            title: a.title, 
            url: a.url, // INI LINK DIRECT GNEWS (LANGSUNG ARTIKEL)
            srcType: 'web'
          }));
    ...
    try {
      ...
      for await (const chunk of stream) {
        ...
        // PERBAIKAN FATAL 1: BIKIN KONDISI HANYA JALAN JIKA BUKAN BERITA GNEWS
        if (chunks && !isGNewsSuccess) {
          const newSources = chunks.map(ch => {
            if (ch.web) {
              const uri = ch.web.uri || '#';
              
              // PERBAIKAN FATAL 2: FILTER PEMBUNUH LINK VERTEX & GOOGLE SEARCH
              // Biar yang nampil di Cylen bener-bener link jurnalis murni!
              if (uri.includes('vertexaisearch') || uri.includes('google.com/search')) {
                return null;
              }

              return { title: ch.web.title || 'Referensi', url: uri, srcType: 'web' };
            }
