...
// ── MEMORY HELPERS (FIXED: SEKARANG PAKE FIRESTORE) ──
...
export function useChat() {
  ...
  const sendMessage = async (inputText: string, attachedImage: string | null, attachedImages?: string[], attachedPdfs?: { data: string; name: string }[], directText?: string, baseMessages?: Message[]) => {
    ...
    updateChat(currentMessages, finalText, true);
    setIsSending(true);
...
PENTING: Selalu tulis teks penjelasan singkat SETELAH tag penutup selesai!`;
    ...
    try {
      ...
      // ── AMBIL MEMORY DARI FIRESTORE SEBELUM NGE-STREAM ──
      const memoryContextString = await buildMemoryContext();
      
      const stream = await chatWithGeminiStream(chatMessages, needsSearch, location, images.length > 0 ? images : undefined, attachedPdfs, chatMode, memoryContextString);
      ...
      // ── TRIGGER SIMPAN MEMORY KE FIRESTORE ──
