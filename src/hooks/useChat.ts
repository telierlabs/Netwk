...
// ── IMPORT LIBRARY DOKUMEN ──
import { jsPDF } from 'jspdf'; 
import { Document, Packer, Paragraph, TextRun, HeadingLevel, AlignmentType } from 'docx';
import * as XLSX from 'xlsx';
import PptxGenJS from 'pptxgenjs';

// ─── 1. ENGINE PDF (Standar) ───
...
const generateVisualEbookPDF = async (title: string, content: string, coverImage?: string | null) => {
  ...
  // Kalau AI nge-generate gambar, tempel di tengah cover!
  ...
  doc.setTextColor(210, 210, 210); // Warna Off-white biar enak dibaca
  doc.setFontSize(12);
  ...
  lines.forEach((line: string) => {
    // Kalau mentok bawah, bikin halaman baru + background hitam
    if (cursorY > pageHeight - 20) {
      ...
      // Balikin font ke normal buat ngetik lanjutannya
      doc.setFontSize(12);
    ...
    if (line.includes('---')) {
        ...
        doc.text(line, margin, cursorY);
        cursorY += 7; // Line height yang lega
    }
  });
...
function hideTagsWhileStreaming(text: string): string {
  ...
  // Tambahin EBOOK di dalam regex pencarian biar disembunyiin!
...
function loadMemoryItems(): { id: string; text: string; timestamp: string; rawDate: number }[] {
  try { const saved = localStorage.getItem('cylen_memory_items'); if (saved) return JSON.parse(saved); } catch (e) {} return [];
}
function saveMemoryItems(items: any[]) {
  try { localStorage.setItem('cylen_memory_items', JSON.stringify(items)); } catch (e) {}
}
function formatMemoryTimestamp(): string {
  const now = new Date(); const DAYS = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab']; const MONTHS = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Ags', 'Sep', 'Okt', 'Nov', 'Des'];
...
function buildMemoryContext(): string {
  const items = loadMemoryItems(); return items.length === 0 ? '' : items.map(item => `- ${item.text}`).join('\n');
}
async function autoSaveMemory(messages: { role: string; content: string }[]): Promise<void> {
  try {
    const existing = loadMemoryItems();
    const newFacts = await extractMemoryFromChat(messages, existing.map(i => i.text));
    if (newFacts.length === 0) return;
    const newItems = newFacts.map(text => ({ id: `mem-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, text, timestamp: formatMemoryTimestamp(), rawDate: Date.now() }));
    saveMemoryItems([...newItems, ...existing]);
  } catch (e) {}
}
...
export function useChat() {
  ...
  // STATE EBOOK MASUK
  const [activityStatus, setActivityStatus] = useState<'idle' | 'image' | 'pdf' | 'docs' | 'excel' | 'ppt' | 'ebook'>('idle');
  ...
  const updateChat = (newMessages: Message[], inputText?: string) => {
    setChatSessions(prev => prev.map(session => session.id === activeChatId ? { ...session, messages: newMessages, title: (session.messages.length === 1 && inputText && !session.isTemporary) ? inputText.slice(0, 30) + (inputText.length > 30 ? '...' : '') : session.title } : session ));
  };

  const togglePin = (msgIndex: number) => {
    setChatSessions(prev => prev.map(session => session.id === activeChatId ? { ...session, messages: session.messages.map((m, i) => i === msgIndex ? { ...m, pinned: !m.pinned } : m) } : session ));
  };
  ...
  const sendMessage = async (inputText: string, attachedImage: string | null, attachedImages?: string[], attachedPdfs?: { data: string; name: string }[], directText?: string, baseMessages?: Message[]) => {
    ...
    // ── INSTRUKSI GENERATOR DENGAN EBOOK ──
...
PENTING: Selalu tulis teks penjelasan singkat SETELAH tag penutup selesai!`;
    ...
    if (needsSearch && !isSearching) { setIsSearching(true); setWebCount(0); setPostCount(0); }
    ...
    try {
      ...
      const stream = await chatWithGeminiStream(chatMessages, needsSearch, location, images.length > 0 ? images : undefined, attachedPdfs, chatMode, buildMemoryContext());
      ...
      let currentActivity: 'idle' | 'image' | 'pdf' | 'docs' | 'excel' | 'ppt' | 'ebook' = 'idle';

      // ── BACA STREAMING & DETEKSI SEMUA TAG ──
      for await (const chunk of stream) {
        const c = chunk as GenerateContentResponse;
        fullText += c.text || '';

        if (currentActivity === 'idle') {
          // Deteksi dini Animasi
          if (fullText.includes('[GENERATE_EBOOK:')) { currentActivity = 'ebook'; setActivityStatus('ebook'); }
          else if (fullText.includes('[GENERATE_PDF:')) { currentActivity = 'pdf'; setActivityStatus('pdf'); }
          else if (fullText.includes('[GENERATE_DOCS:')) { currentActivity = 'docs'; setActivityStatus('docs'); }
          else if (fullText.includes('[GENERATE_EXCEL:')) { currentActivity = 'excel'; setActivityStatus('excel'); }
          else if (fullText.includes('[GENERATE_PPT:')) { currentActivity = 'ppt'; setActivityStatus('ppt'); }
          else if (fullText.includes('[GENERATE_IMAGE:')) { currentActivity = 'image'; setActivityStatus('image'); }
        }
        ...
        if (chunks) {
          ...
          setWebCount(localWebCount); setPostCount(localPostCount);
        }
        ...
        setChatSessions(prev => prev.map(session =>
          session.id === activeChatId ? { ...session, messages: session.messages.map((m, idx) => idx === session.messages.length - 1 ? { ...m, content: noSugg, sources: sources.length > 0 ? sources : undefined, suggestions: validSuggestions } : m ) } : session
        ));
      }

      // ── EKSEKUSI PEMBUATAN FILE DI LATAR BELAKANG ──
      ...
      // 1. Prioritas Gambar (Biar bisa dilempar jadi Cover Ebook)
      ...
      // 2. Format Dokumen (Termasuk Ebook yang butuh lemparan finalGeneratedImage)
      const matchers = [
      ...
      for (const m of matchers) {
        ...
        if (found) {
          ...
          try {
            await new Promise(r => setTimeout(r, 1000)); // Jeda UX
            const title = found[1].trim();
            const content = found[2].trim();
            // Panggil generator yang cocok!
          ...
          break; // Cuma bikin 1 file per response
        }
      }
      ...
      setChatSessions(prev => prev.map(session =>
        session.id === activeChatId ? { ...session, messages: session.messages.map((m, idx) => idx === session.messages.length - 1 ? { ...m, content: finalCleanText, suggestions: validFinalSuggestions, image: finalGeneratedImage } : m ) } : session
      ));
      ...
      setIsSending(false); setIsSearching(false); setActivityStatus('idle'); setWebCount(0); setPostCount(0);
    }
  };
  ...
  const createNewChat = (isGhostMode: boolean = false) => {
    ...
    setActiveChatId(newId); return newId;
  };

  const renameChatSession = (id: string, newTitle: string) => setChatSessions(prev => prev.map(session => session.id === id ? { ...session, title: newTitle } : session));
  const pinChatSession = (id: string) => setChatSessions(prev => prev.map(session => session.id === id ? { ...session, isPinned: !(session as any).isPinned } : session));
  const deleteChatSession = (id: string) => setChatSessions(prev => {
    const filtered = prev.filter(session => session.id !== id);
    if (filtered.length === 0) { const newId = Date.now().toString(); setActiveChatId(newId); return [{ id: newId, title: 'Chat Baru', messages: [], date: formatTimestamp(), isTemporary: false }]; }
    if (id === activeChatId) setActiveChatId(filtered[0].id); return filtered;
  });
