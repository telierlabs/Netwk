...
export function useChat() {
  ...
  const sendMessage = async (
    ...
    let injectedContext = '';
    // INI DIA. KITA SIMPAN MANUAL SOURCES DI SINI BIAR GAK KETIMPA.
    let persistentSources: { title: string; url: string; srcType: string }[] = []; 
    let manualImage = '';
    ...
    if (isNewsQuery) {
      setIsSearching(true);
      try {
        let searchKeyword = finalText.toLowerCase().replace(/(apa|ada|tolong|cariin|cari|info|berita|news|terkini|terbaru|hari ini|sekarang|dong|lebih|detail|lanjut|tentang|berikan|jelaskan|update)/gi, '').trim();
        searchKeyword = searchKeyword.replace(/[^a-z0-9\s]/gi, ' ').replace(/\s+/g, ' ').trim();
        
        if (searchKeyword === '' || searchKeyword.length < 3) searchKeyword = 'indonesia OR nasional'; 
        ...
        if (articles && articles.length > 0) {
          injectedContext = `\n\n=== DATA BERITA REAL-TIME ===\n` + 
            ...
            `\n\nTugasmu: Rangkum berita di atas dengan gaya bahasamu yang futuristik dan tegas. WAJIB patuhi format yang ada di system prompt. JANGAN SEBUTKAN "Berdasarkan data GNews".`;
          
          persistentSources = articles.map(a => ({
            title: a.title, 
            url: a.url, // GNEWS direct link
            srcType: 'web'
          }));
    ...
    try {
      ...
      let localWebCount  = persistentSources.length;
      ...
      // INITIALIZE ASSISTANT MESSAGE WITH INITIAL SOURCES
      ...
      for await (const chunk of stream) {
            ...
            if (ch.web) {
              ...
              if (uri.includes('google.com/url')) {
                try {
                  ...
                  const extractedUrl = urlObj.searchParams.get('url') || urlObj.searchParams.get('q');
                  if (extractedUrl) uri = extractedUrl;
                } catch(e) {}
              }

              if (uri.includes('google.com/search') || uri.includes('vertexaisearch')) {
                return null;
              }

              return { title, url: uri, srcType: 'web' };
            }
        ...
        // KITA UPDATE STATE PAKE PERSISTENT SOURCES
        setChatSessions(prev => prev.map(session =>
          session.id === activeChatId
            ? {
                ...session,
                messages: session.messages.map((m, idx) =>
                  idx === session.messages.length - 1
                    ? { ...m, content: noSugg, sources: persistentSources.length > 0 ? [...persistentSources] : undefined, image: generatedImage || m.image, suggestions: validSuggestions }
                    : m
                )
              }
            : session
      ...
      // FINAL STATE UPDATE DENGAN SUMBER YANG PASTI ADA
      setChatSessions(prev => prev.map(session =>
        session.id === activeChatId
          ? {
              ...session,
