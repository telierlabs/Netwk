...
// ── Helper: Load memory dari localStorage ──
...
// ── Helper: Save memory ke localStorage ──
...
// ── Helper: Format timestamp untuk memory ──
...
// ── Helper: Bangun memory context string untuk system prompt ──
...
// ── Auto-save memory setelah AI selesai reply (background, silent) ──
async function autoSaveMemory(
  ...
  try {
    ...
    // Prepend baru ke existing, simpan
...
export function useChat() {
  ...
  const sendMessage = async (
    ...
    // ─── PENAMBAHAN TRACKING DARI CLAUDE ───
    trackMessageSent();
      ...
      try {
        ...
        if (articles && articles.length > 0) {
          ...
          manualSources = articles.map((a: any) => {
            return { title: a.title, url: a.url, srcType: 'web' };
          });
    ...
    // ── Ambil memory context untuk diinject ke system prompt ──
    ...
    try {
          ...
          const newSources = chunks.map(ch => {
            if (ch.web) {
              ...
              if (url.includes('vertexaisearch') || url.includes('googleusercontent')) {
                 ...
                 url = `https://www.google.com/search?q=${encodeURIComponent(realTitle)}`;
              }
      ...
      // ── AUTO MEMORY: Jalankan diam-diam di background setelah reply selesai ──
      // Hanya jalankan kalau bukan mode temporary (ghost/private)
      if (!isTemporary) {
        const finalMessages = [
          ...currentMessages.map(m => ({ role: m.role, content: m.content })),
          { role: 'assistant' as const, content: finalCleanText }
        ];
        // Fire and forget — tidak nunggu, tidak ganggu UX
  ...
  const pinChatSession = (id: string) => {
    setChatSessions(prev => prev.map(session =>
      session.id === id ? { ...session, isPinned: !(session as any).isPinned } : session
    ));
  };
  ...
  const deleteChatSession = (id: string) => {
    setChatSessions(prev => {
      ...
      // Kalau semua chat dihapus, bikin default kosong
      ...
      // Kalau chat yang aktif dihapus, pindah ke chat teratas
      if (id === activeChatId) {
