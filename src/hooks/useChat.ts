...
export function useChat() {
  ...
  // ✅ Fungsi ini sekarang pintar! Bisa me-retry error ATAU me-regenerate pesan AI berdasarkan index.
  const retryLastMessage = (targetIndex?: number) => {
    // Kalau dipencet di error card, index kosong (ambil ujung). Kalau dipencet di ikon AI, ambil index pesan itu.
    const aiIndex = targetIndex !== undefined ? targetIndex : messages.length - 1;
    
    // Cari pesan user terakhir tepat sebelum pesan AI ini
    let userIndex = -1;
    ...
    if (userIndex === -1) return; // Jaga-jaga kalau gak ada pesan user sama sekali

    const lastUserMsg = messages[userIndex];
    
    // Potong array sampai SEBELUM pesan user tersebut, karena fungsi sendMessage akan masukkin ulang.
