      ...
      try {
        ...
        if (articles && articles.length > 0) {
          ...
          manualSources = articles.map((a: any) => {
            // BIARIN URL ASLINYA JANGAN DIUBAH!
            return { title: a.source.name, url: a.url, srcType: 'web' };
          });
        ...
        if (chunks) {
          const newSources = chunks.map(ch => {
            if (ch.web) {
              ...
              let url = ch.web.uri || '#'; // BIARIN URL TRACKING VERTEX-NYA! JANGAN DIPOTONG!
              
              if (url.includes('vertexaisearch') || url.includes('googleusercontent') || url.includes('google.com')) {
                 const titleParts = title.split(/[-|]/);
                 // Cuma bersihin nama medianya doang buat ditampilin
