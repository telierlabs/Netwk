...
// TAmbahan import icon buat menu titik tiga
import { Search, User, Settings, Bookmark, Users, ChevronsLeft, ChevronsRight, PenSquare, ArrowLeft, Clock, MoreHorizontal, Edit2, Pin, Trash2 } from 'lucide-react';
...
const DesktopSidebar: React.FC<any> = ({ collapsed, onToggle, view, onViewProfile, onViewGroup, onViewSaved, onViewSearch, onViewTasks, onNewChat, chatSessions, activeChatId, onSelectChat, onRenameChat, onPinChat, onDeleteChat }) => {
  ...
  // Sorting: Pinned di atas, sisanya diurut
  const filtered = chatSessions
    .filter((s:any) => (getSmartTitle(s) || '').toLowerCase().includes(search.toLowerCase()))
              ...
              <button onClick={() => onSelectChat(session.id)} className={cn('w-full flex items-start gap-2.5 px-3 py-2.5 rounded-xl transition-colors text-left', activeChatId === session.id ? 'bg-[var(--sf)]' : 'hover:bg-[var(--sf)]')}>
                ...
                <div className="flex-1 min-w-0 pr-6">
                  <div className="flex items-center gap-1.5">
                    ...
                    {session.isTemporary && (
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="var(--mu)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, opacity: 0.5 }}>
              ...
              <AnimatePresence>
                {openMenuId === session.id && (
                  <motion.div initial={{opacity:0, y:-5, scale:0.95}} animate={{opacity:1, y:0, scale:1}} exit={{opacity:0, scale:0.95}} transition={{duration:0.1}} className="absolute right-0 top-9 w-36 bg-[var(--bg)] border border-[var(--bd)] rounded-xl shadow-2xl z-[100] py-1 overflow-hidden">
                    <button onClick={(e) => { 
                      e.stopPropagation(); 
                      const newTitle = prompt('Masukkan nama chat baru:', getSmartTitle(session));
                      if (newTitle && newTitle.trim()) onRenameChat?.(session.id, newTitle.trim());
                      setOpenMenuId(null); 
                    }} className="w-full flex items-center gap-2.5 px-3 py-2 text-[12px] font-medium text-[var(--text)] hover:bg-[var(--sf)] transition-colors">
                      <Edit2 size={13} /> Ubah Nama
                    </button>
                    
                    <button onClick={(e) => { e.stopPropagation(); onPinChat?.(session.id); setOpenMenuId(null); }} className="w-full flex items-center gap-2.5 px-3 py-2 text-[12px] font-medium text-[var(--text)] hover:bg-[var(--sf)] transition-colors">
                      <Pin size={13} /> {session.isPinned ? 'Unpin Chat' : 'Pin Chat'}
                    </button>
                    ...
                    <button onClick={(e) => { 
                      e.stopPropagation(); 
                      if(window.confirm('Yakin mau hapus chat ini?')) onDeleteChat?.(session.id); 
                      setOpenMenuId(null); 
                    }} className="w-full flex items-center gap-2.5 px-3 py-2 text-[12px] font-medium text-red-500 hover:bg-red-500/10 transition-colors">
...
export default function AppDesktop() {
  ...
  const {
    ...
    retryLastMessage,
    chatMode, setChatMode, isTemporary,
    renameChatSession, pinChatSession, deleteChatSession // <-- DITAMBAHIN
  } = useChat();
  ...
  return (
    <div className={cn('app transition-colors duration-350', theme)} style={{ fontFamily: currentFontFamily, height: '100dvh', display: 'flex', overflow: 'hidden' }}>
      ...
      <DesktopSidebar
        ...
        onSelectChat={(id: string) => { setActiveChatId(id); setView('chat'); setIsCanvasActive(false); }}
        ...
        onRenameChat={renameChatSession} // <-- DITAMBAHIN
        onPinChat={pinChatSession}       // <-- DITAMBAHIN
        onDeleteChat={deleteChatSession} // <-- DITAMBAHIN
      />
      
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
        
        {/* Header Desktop - Ditampilkan selain chat, dll */}
        {view !== 'chat' && view !== 'search' && view !== 'tasks' && view !== 'appearance' && view !== 'usage' && view !== 'integrations' && view !== 'security' && view !== 'export' && view !== 'group-profile' && view !== 'saved' && (
          <header className={cn("flex items-center justify-between px-6 border-b transition-all duration-500 ease-in-out flex-shrink-0", isCanvasActive ? "h-10 opacity-60 border-[var(--bd)]" : "py-4 border-[var(--bd)]")}>
            <div className="flex items-center gap-3">
              ...
              >
                <ArrowLeft size={isCanvasActive ? 14 : 18}/>
              </button>
              <div className="flex items-center gap-2">
                <h1 className={cn("fontsemibold text-[var(--text)] transition-all", isCanvasActive ? "text-[13px]" : "text-[15px]")}>
                  {view === 'profile' && 'Profil & Pengaturan'}
                  {view === 'group-list' && 'Grup AI'}
                  {view === 'group-chat' && activeGroup?.title}
                  {view === 'memory' && 'Memori'}
                  {view === 'ai-settings' && 'Pengaturan AI'}
                </h1>
              </div>
            </div>
          </header>
        ...
        {/* Chat view — header ghost mode di atas chat Desktop */}
        {view === 'chat' && isEmptyChat && (
          <div className={cn(
            "flex-shrink-0 flex items-center justify-end px-4 py-2 border-b border-[var(--bd)]/40 transition-all",
            isTemporary ? "bg-[var(--text)]/[0.02]" : ""
          )}>
            <div className="flex items-center gap-3">
              <button
                onClick={() => {
                ...
                className="p-2 rounded-xl transition-all active:scale-95 text-[var(--text)]/40 hover:bg-[var(--text)]/10 hover:text-[var(--text)] outline-none"
              >
                {isTemporary ? (
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="text-[var(--text)]">
                    <path d="M12 2a8 8 0 0 0-8 8v12l3-3 2.5 2.5L12 19l2.5 2.5L17 19l3 3V10a8 8 0 0 0-8-8z"/>
        ...
        {/* Cylen di pojok kiri atas CHAT, pakai TITLE otomatis kalo udah ada pesan */}
        {view === 'chat' && !isEmptyChat && (
          <div className="absolute top-4 left-6 z-10 hidden md:block">
             <span className="font-bold text-[var(--text)] opacity-50 tracking-tight text-[18px]">
               {getSmartTitle(activeChat)}
             </span>
          </div>
        )}
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {view === 'group-list' && <GroupListPage groups={groupSessions} onSelectGroup={(id: string) => { setActiveGroupId(id); setView('group-chat'); }} onCreateGroup={(title: string) => { const id = createGroup(title); if (id) { setActiveGroupId(id); setView('group-chat'); showToast('Grup dibuat'); } }} onBack={() => setView('chat')} />}
          {view === 'group-chat' && activeGroup && <GroupChatPage activeGroup={activeGroup} isSending={isGroupSending} onSendMessage={sendGroupMessage} onAddParticipant={addParticipant} showToast={showToast} onBack={() => setView('group-list')} onNavigateProfile={() => setView('group-profile')} />}
          {view === 'group-profile' && activeGroup && <GroupProfilePage activeGroup={activeGroup} showToast={showToast} onBack={() => setView('group-chat')} />}
