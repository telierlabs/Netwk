...
import { GroupProfilePage } from './pages/GroupProfilePage'; // <-- IMPORT BARU
...
import { Search, User, Settings, Bookmark, Users, ChevronsLeft, ChevronsRight, PenSquare, ArrowLeft, Video, Clock } from 'lucide-react';
  ...
  return (
    <div className={cn('app transition-colors duration-350', theme)} style={{ fontFamily: currentFontFamily, height: '100dvh', display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
        ...
        {view !== 'chat' && view !== 'search' && view !== 'tasks' && view !== 'appearance' && view !== 'usage' && view !== 'integrations' && view !== 'security' && view !== 'export' && view !== 'group-profile' && (
        ...
        {view === 'chat' && isEmptyChat && (
          <div className={cn(
            ...
            <div className="flex items-center gap-3">
              {isTemporary && (
                <span className="text-[12px] font-semibold text-[var(--text)]/40 flex items-center gap-1.5">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M9 10h.01"/><path d="M15 10h.01"/>
                    <path d="M12 2a8 8 0 0 0-8 8v12l3-3 2.5 2.5L12 19l2.5 2.5L17 19l3 3V10a8 8 0 0 0-8-8z"/>
                  </svg>
                  Mode Hantu — tidak tersimpan
                </span>
              )}
              {/* Tombol ghost desktop - NORMAL */}
              <button
                onClick={() => {
                  if (isTemporary) {
                    createNewChat(false);
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {view === 'group-list' && <GroupListPage groups={groupSessions} onSelectGroup={(id) => { setActiveGroupId(id); setView('group-chat'); }} onCreateGroup={(title) => { const id = createGroup(title); if (id) { setActiveGroupId(id); setView('group-chat'); showToast('Grup dibuat'); } }} onBack={() => setView('chat')} />}
