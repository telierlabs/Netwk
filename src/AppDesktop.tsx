...
const DesktopSidebar: React.FC<{
  ...
  return (
    <aside className="flex flex-col flex-shrink-0 border-r border-[var(--bd)] transition-all duration-200" style={{ width: collapsed ? 64 : 240, height: '100dvh', backgroundColor: 'var(--bg)' }}>
      ...
      <div className="px-2 flex flex-col"><NavBtn icon={<Search size={17}/>} label="Cari" onClick={onViewSearch} active={view==='search'}/><NavBtn icon={<Users size={17}/>} label="Grup AI" onClick={onViewGroup} active={view==='group-list'||view==='group-chat'}/><NavBtn icon={<Bookmark size={17}/>} label="Tersimpan" onClick={onViewSaved} active={view==='saved'}/></div>
  ...
  return (
    <div className={cn('app transition-colors duration-350', theme)} style={{ fontFamily: currentFontFamily, height: '100dvh', display: 'flex', overflow: 'hidden' }}>
      <DesktopSidebar
        ...
        onSelectChat={(id) => { setActiveChatId(id); setView('chat'); setIsCanvasActive(false); }} // 🔥 MATIIN CANVAS
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
        {view !== 'chat' && view !== 'search' && (
          <header className={cn("flex items-center gap-3 px-6 border-b transition-all duration-500 ease-in-out flex-shrink-0", isCanvasActive ? "h-10 opacity-60 border-[var(--bd)]" : "py-4 border-[var(--bd)]")}>
            ...
            <h1 className={cn("font-semibold text-[var(--text)] transition-all", isCanvasActive ? "text-[13px]" : "text-[15px]")}>{view === 'profile' && 'Profil & Pengaturan'}{view === 'saved' && 'Tersimpan'}</h1>
          </header>
        )}
