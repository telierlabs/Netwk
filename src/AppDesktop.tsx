...
const DesktopSidebar: React.FC<{
  ...
  const NavBtn = ({ icon, label, onClick, active }: any) => (<button onClick={onClick} title={label} className={cn('w-full flex items-center justify-center p-3 rounded-xl transition-colors my-0.5', !collapsed && 'justify-start gap-3 px-3', active ? 'bg-[var(--bd)] text-[var(--text)]' : 'text-[var(--mu)] hover:bg-[var(--bd)] hover:text-[var(--text)]')}><div className="flex-shrink-0">{icon}</div>{!collapsed && <span className="text-[13.5px] font-medium">{label}</span>}</button>);
  ...
  return (
    <aside className="flex flex-col flex-shrink-0 border-r border-[var(--bd)] transition-all duration-200" style={{ width: collapsed ? 64 : 240, height: '100dvh', backgroundColor: 'var(--bg)' }}>
      <div className={cn('flex items-center py-4 px-2', collapsed ? 'flex-col gap-3' : 'px-3 gap-2 justify-between')}><button onClick={onNewChat} title="Cylen" className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 hover:opacity-70 transition-opacity"><img src="/82374-removebg-preview.png" alt="Cylen" style={{ width: 38, height: 38, objectFit: 'contain' }} /></button>{!collapsed && <span className="font-bold text-[15px] tracking-tight text-[var(--text)] flex-1">Cylen</span>}<button onClick={onToggle} className="p-1.5 rounded-lg text-[var(--mu)] hover:bg-[var(--bd)] hover:text-[var(--text)] transition-colors flex-shrink-0" title={collapsed ? 'Expand' : 'Collapse'}>{collapsed ? <ChevronsRight size={17}/> : <ChevronsLeft size={17}/>}</button></div>
          ...
          <header className={cn("flex items-center justify-between px-6 border-b transition-all duration-500 ease-in-out flex-shrink-0", isCanvasActive ? "h-10 opacity-60 border-[var(--bd)]" : "py-4 border-[var(--bd)]")}>
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                {isTemporary && view === 'chat' && <span className="text-[10px] font-bold bg-[var(--text)] text-[var(--bg)] px-2 py-0.5 rounded-full">Mode Hantu</span>}
              </div>
            </div>
            {view === 'group-chat' && (
