...
export default function AppDesktop() {
  ...
  // Sync state lokal setiap ganti chat dari sidebar
  useEffect(() => {
    if (currentSession) {
  ...
  const isTemporary = localGhostMode; // PAKAI LOCAL STATE
        ...
        {view === 'chat' && isEmptyChat && (
          <div className={cn(
            ...
            <div className="flex items-center gap-3">
              ...
              {/* Tombol ghost desktop */}
              <button
                onClick={() => {
                  const newState = !localGhostMode;
                  setLocalGhostMode(newState);
                  createNewChat(newState);
                }}
                title={isTemporary ? "Keluar Mode Hantu" : "Mode Hantu (chat sementara)"}
                className={cn(
                  "p-2 rounded-xl transition-all active:scale-95",
                  isTemporary
                    ? "bg-[var(--ac)] text-[var(--at)] shadow-md"
