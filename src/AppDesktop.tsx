...
const DesktopSidebar: React.FC<{
  collapsed: boolean; onToggle: () => void; view: View | string;
  onViewProfile: () => void; onViewGroup: () => void; onViewSaved: () => void;
  onViewSearch: () => void; onViewTasks: () => void; onNewChat: () => void;
  chatSessions: any[]; activeChatId: string; onSelectChat: (id: string) => void;
            ...
            <button key={session.id} onClick={() => onSelectChat(session.id)} className={cn('w-full flex items-start gap-2.5 px-3 py-2.5 rounded-xl transition-colors text-left', activeChatId === session.id ? 'bg-[var(--sf)]' : 'hover:bg-[var(--sf)]')}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  {/* Ghost badge di sidebar item kalau sesi itu temporary */}
                  {session.isTemporary && (
...
const DesktopChatView: React.FC<any> = (props) => {
  ...
  if (isEmpty) {
    // KONDISI PENTING: Kalau lagi Mode Hantu, render ChatPage murni biar SVG Ghost muncul
    if (props.isTemporary) {
      return <ChatPage {...props} />;
    }

    // Kalau chat biasa, render Cylen Welcome Mode
    return (
...
export default function AppDesktop() {
  ...
  const isTemporary = currentSession?.isTemporary ?? false;
  ...
  return (
    <div className={cn('app transition-colors duration-350', theme)} style={{ fontFamily: currentFontFamily, height: '100dvh', display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
        ...
        {/* Desktop header — hanya untuk non-chat views */}
        {view !== 'chat' && view !== 'search' && view !== 'tasks' && view !== 'appearance' && view !== 'usage' && view !== 'integrations' && view !== 'security' && view !== 'export' && (
          <header className={cn("flex items-center justify-between px-6 border-b transition-all duration-500 ease-in-out flex-shrink-0", isCanvasActive ? "h-10 opacity-60 border-[var(--bd)]" : "py-4 border-[var(--bd)]")}>
            ...
            {view === 'group-chat' && (
              <button onClick={() => setIsZoomOpen(true)} className="p-2 bg-[var(--sf)] hover:bg-[var(--bd)] rounded-xl text-[var(--text)] transition-all flex items-center gap-2 px-4">
                <Video size={18} />
        ...
        {/* Chat view — header ghost mode di atas chat */}
              ...
              <button
                ...
                className={cn(
                  ...
                  isTemporary
                    ...
                    : "text-[var(--text)]/40 hover:bg-[var(--text)]/10 hover:text-[var(--text)]"
                )}
              >
