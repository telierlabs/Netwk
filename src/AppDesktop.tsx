  ...
  return (
    ...
    >
      <div className={cn('flex items-center py-4 px-2', collapsed ? 'flex-col gap-3' : 'px-3 gap-2 justify-between')}>
        <button onClick={onNewChat} title="Cylen AI" className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 hover:opacity-70 transition-opacity">
          ...
          {filtered.map(session => (
            <button key={session.id} onClick={() => onSelectChat(session.id)} className={cn('w-full flex items-start gap-2.5 px-3 py-2.5 rounded-xl transition-colors text-left', activeChatId === session.id ? 'bg-[var(--sf)]' : 'hover:bg-[var(--sf)]')}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-[var(--mu)] truncate mt-0.5">{session.messages[session.messages.length-1]?.content.slice(0,40)}...</p>
              </div>
            </button>
          ))}
        </div>
...
export default function AppDesktop() {
  ...
  const handleSendMessage = (images?: string[], pdfs?: { data: string; name: string }[], directText?: string) => {
    ...
    if (msgLower.includes('ganti') || msgLower.includes('ubah') || msgLower.includes('jadikan')) {
      if (msgLower.includes('gelap') || msgLower.includes('hitam') || msgLower.includes('item') || msgLower.includes('dark')) { setTheme('t-dark'); showToast('Tema Gelap diaktifkan!'); } 
      else if (msgLower.includes('terang') || msgLower.includes('putih') || msgLower.includes('light')) { setTheme('t-light'); showToast('Tema Terang diaktifkan!'); } 
      else if (msgLower.includes('hijau') || msgLower.includes('hutan') || msgLower.includes('forest')) { setTheme('t-forest'); showToast('Tema Forest diaktifkan!'); } 
      else if (msgLower.includes('ungu') || msgLower.includes('malam') || msgLower.includes('dusk')) { setTheme('t-dusk'); showToast('Tema Dusk diaktifkan!'); }
      else if (msgLower.includes('krim') || msgLower.includes('cream') || msgLower.includes('kuning')) { setTheme('t-cream'); showToast('Tema Cream diaktifkan!'); }
      else if (msgLower.includes('abu') || msgLower.includes('slate')) { setTheme('t-slate'); showToast('Tema Slate diaktifkan!'); }

      if (msgLower.includes('font') || msgLower.includes('huruf')) {
        if (msgLower.includes('modern') || msgLower.includes('biasa')) { setFont('Modern'); showToast('Font Modern diterapkan!'); } 
        else if (msgLower.includes('koding') || msgLower.includes('hacker') || msgLower.includes('mono')) { setFont('Mono'); showToast('Font Hacker diterapkan!'); } 
        else if (msgLower.includes('buku') || msgLower.includes('klasik') || msgLower.includes('serif')) { setFont('Serif'); showToast('Font Klasik diterapkan!'); } 
        else if (msgLower.includes('robot') || msgLower.includes('techy')) { setFont('Techy'); showToast('Font Robot diterapkan!'); }
      }
    }
  ...
  const handleNewChat = () => { createNewChat(); setView('chat'); showToast('Memulai chat baru'); };
  ...
  const chatProps = {
    ...
    isCanvasActive, // 🔥 PROPS DI OPER KE CHATPAGE
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
        {view !== 'chat' && view !== 'search' && (
          {/* 🔥 TRANSISI HEADER DESKTOP MENYUSUT */}
          <header className={cn(
            "flex items-center gap-3 px-6 border-b transition-all duration-500 ease-in-out flex-shrink-0",
            isCanvasActive ? "h-10 opacity-60 border-[var(--bd)]" : "py-4 border-[var(--bd)]"
          )}>
            <button
              onClick={() => {
            ...
            <h1 className={cn("font-semibold text-[var(--text)] transition-all", isCanvasActive ? "text-[13px]" : "text-[15px]")}>
              ...
              {view === 'ai-settings' && 'Personalisasi Cylen'}
      ...
      <AnimatePresence>
        {toast && (
          <motion.div initial={{opacity:0,y:20,x:'-50%'}} animate={{opacity:1,y:0,x:'-50%'}} exit={{opacity:0,y:20,x:'-50%'}} className="fixed bottom-8 left-1/2 bg-[var(--ac)] text-[var(--at)] px-6 py-2.5 rounded-full text-sm font-medium shadow-2xl z-[100] whitespace-nowrap">
            {toast}
          </motion.div>
        )}
      </AnimatePresence>
