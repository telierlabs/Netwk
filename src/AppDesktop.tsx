...
export default function AppDesktop() {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToAuthChanges((currentUser) => {
      console.log("Status User Desktop:", currentUser ? "Login Sukses" : "Belum Login");
      setUser(currentUser);
  ...
  const chatProps = { messages, isSending, isSearching, activityStatus, webCount: 0, postCount: 0, inputText, setInputText, onSend: handleSendMessage, attachedImage, setAttachedImage, onTogglePin: togglePin, pinnedMessages, onSaveItem: (item: any) => { saveItem({ ...item, chatId: activeChatId }); showToast('Pesan berhasil disimpan!'); }, onRetry: retryLastMessage, mode: chatMode, onModeChange: setChatMode, isCanvasActive, onOpenCanvas: () => setIsCanvasActive(true), onCloseCanvas: () => setIsCanvasActive(false), onUpgradeClick: () => setIsSubscriptionOpen(true), isTemporary };
