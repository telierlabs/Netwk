...
import { subscribeToAuthChanges, checkUserPermissions, saveUserPermissions } from './services/authService';
...
const THEME_VARS: Record<string, Record<string, string>> = {
  ...
  't-dark':  { '--bg':'#010101', '--sf':'#161616', '--bd':'rgba(255,255,255,.07)', '--text':'#ffffff', '--mu':'rgba(255,255,255,.35)', '--cd':'#111111', '--ac':'#ffffff', '--at':'#000000', '--ib':'#252525' },
};
...
export default function AppDesktop() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToAuthChanges(async (currentUser) => {
      if (currentUser) {
        ...
        try {
          const hasPerms = await checkUserPermissions(currentUser.uid);
          if (isMounted) {
            setNeedsPermission(!hasPerms);
            setIsAuthLoading(false);
          }
        } catch (err) {
          console.error("Desktop permission check failed:", err);
          if (isMounted) {
  ...
  if (isAuthLoading || (user && isRouting)) {
    return <div style={{ height: '100dvh', width: '100vw', backgroundColor: '#010101' }} />;
  }

  // JIKA BELUM LOGIN: Tampilkan Landing Page Normal Desktop
  if (!user) {
    return (
      <div style={{
        ...
        <img src="/82374-removebg-preview.png" alt="Cylen" style={{ width: 100, height: 100, marginBottom: 24 }} />
        <h1 style={{ fontSize: '48px', fontWeight: 700, letterSpacing: '-1.5px', margin: '0 0 12px 0' }}>CYLEN</h1>
        <p style={{ color: '#666', fontSize: '15px', marginBottom: '40px', textAlign: 'center', maxWidth: '400px' }}>
          The Minimalist Cyberpunk Ecosystem. Experience AI built with ultra scannability and pure focus.
        </p>
        <button
  ...
  if (needsPermission) {
    return (
      <PermissionPage onComplete={async () => {
        try {
          await saveUserPermissions(user.uid);
        } catch (err) {
          console.error("Gagal menyimpan konfigurasi izin:", err);
        } finally {
          setNeedsPermission(false);
        }
      }} />
