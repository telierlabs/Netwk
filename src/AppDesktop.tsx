...
// ── SIDEBAR GROK-STYLE (icon only, collapsed by default di desktop) ──
    ...
    >
      ...
      <div className={cn('flex items-center py-4 px-2', collapsed ? 'flex-col gap-3' : 'px-3 gap-2 justify-between')}>
        <button
          onClick={() => { onNewChat(); }}
          title="Cylen AI"
          className="w-9 h-9 bg-[var(--ac)] rounded-xl flex items-center justify-center flex-shrink-0 hover:opacity-80 transition-opacity"
        >
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="var(--at)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
...
// ── GROK-STYLE CENTERED WELCOME + INPUT ──
const DesktopChatView: React.FC<{
  ...
  onSend: (imgs?: string[], pdfs?: any[]) => void;
  ...
  if (isEmpty) {
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-4" style={{ minHeight: 0 }}>
        ...
        >
          <div className="w-16 h-16 bg-[var(--ac)] rounded-2xl flex items-center justify-center shadow-lg">
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--at)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/>
            </svg>
          </div>
          <span className="font-bold text-[var(--text)]" style={{ fontSize: '2.6rem', letterSpacing: '-0.02em', lineHeight: 1 }}>
            Cylen
          </span>
        </div>

        {/* Input lebar kaya Grok */}
        <div style={{ width: '100%', maxWidth: 780, animation: 'fadeUp 0.4s ease forwards' }}>
          <ChatPage
            {...props}
            desktopWelcomeMode
          />
  ...
  // ── CHAT AKTIF: input di bawah, normal ──
  return (
    <ChatPage {...props} />
  );
};
...
export default function AppDesktop() {
  ...
  const [collapsed, setCollapsed] = useState(true); // default collapsed kaya Grok
  const [theme, setTheme] = useState<Theme>('t-dark'); // default dark kaya Grok
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
        ...
        {/* Topbar minimal — hanya untuk halaman non-chat */}
