      ...
      <div className={cn('px-2 pb-4 flex-shrink-0', collapsed && 'flex justify-center')}>
        <button onClick={onViewProfile} title="Profil & Pengaturan" className={cn('flex items-center gap-3 p-3 rounded-xl text-[var(--mu)] hover:bg-[var(--bd)] hover:text-[var(--text)] transition-colors', collapsed ? 'justify-center w-10 h-10' : 'w-full px-3')}>
          <div className="w-7 h-7 rounded-full bg-[var(--ac)] flex items-center justify-center flex-shrink-0">
            <UserIcon size={13} className="text-[var(--at)]"/>
          </div>
          {!collapsed && (
            <>
...
export default function AppDesktop() {
  ...
  const chatProps = {
    messages, isSending, isSearching, activityStatus,
  ...
  if (isAuthLoading) {
    return (
      <div style={{
        ...
        <div style={{ fontSize: 12, fontWeight: 'bold', letterSpacing: '0.2em', textTransform: 'uppercase', opacity: 0.5 }}>
          INITIALIZING...
        </div>
      </div>
    );
  ...
  // 3. SUDAH LOGIN → TAMPILKAN APP UTAMA
  return (
