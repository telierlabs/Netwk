...
import { Search, User, Settings, Bookmark, Users, ChevronsLeft, ChevronsRight, PenSquare, ArrowLeft, Clock } from 'lucide-react';
...
const DesktopSidebar: React.FC<any> = ({ collapsed, onToggle, view, onViewProfile, onViewGroup, onViewSaved, onViewSearch, onViewTasks, onNewChat, chatSessions, activeChatId, onSelectChat }) => {
  const [search, setSearch] = useState('');
  const filtered = chatSessions.filter((s:any) => s.title.toLowerCase().includes(search.toLowerCase()));
      ...
      {!collapsed && (
        <div className="flex-1 overflow-y-auto px-3 flex flex-col gap-1 min-h-0" style={{scrollbarWidth:'none'}}>
          ...
          {filtered.map((session:any) => (
            <button key={session.id} onClick={() => onSelectChat(session.id)} className={cn('w-full flex items-start gap-2.5 px-3 py-2.5 rounded-xl transition-colors text-left', activeChatId === session.id ? 'bg-[var(--sf)]' : 'hover:bg-[var(--sf)]')}>
              <div className={cn('w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0', activeChatId === session.id ? 'bg-[var(--ac)]' : 'bg-[var(--bd)]')}/>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <p className="text-[13px] font-medium text-[var(--text)] truncate flex-1">{session.title}</p>
                  {session.isTemporary && (
