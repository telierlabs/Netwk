...
import { HapticsPage } from './pages/HapticsPage'; // <-- IMPORT BARU
import { ReportPage } from './pages/ReportPage';   // <-- IMPORT BARU

import { Theme, Font, View } from './types';
import { FONTS } from './constants';
import { Search, User, Settings, Bookmark, Users, ChevronsLeft, ChevronsRight, PenSquare, Clock, MoreHorizontal, Edit2, Pin, Trash2 } from 'lucide-react';
    ...
    <div className={cn('app transition-colors duration-350', theme)} style={{ fontFamily: currentFontFamily, height: '100dvh', display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
        ...
        {view === 'chat' && !isEmptyChat && (
          <div className="absolute top-4 left-6 z-10 hidden md:block pointer-events-none">
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {view === 'chat' && isEmptyChat && (
            <div className={cn(
              "flex-shrink-0 flex items-center justify-end px-4 py-2 transition-all border-b border-transparent z-10",
              isTemporary ? "bg-[var(--text)]/[0.02]" : ""
            )}>
              <button
                onClick={() => {
                  if (isTemporary) createNewChat(false);
          ...
          {view === 'profile' && (
            <ProfilePage 
              theme={theme} setTheme={setTheme} font={font} setFont={setFont} showToast={showToast} 
              onViewMemory={() => setView('memory')} 
              onViewAiSettings={() => setView('ai-settings')} 
              onUpgradeClick={() => setIsSubscriptionOpen(true)} 
              onViewAppearance={() => setView('appearance')} 
              ...
              onViewHaptics={() => setView('haptics')} // <-- ROUTING BARU
              onViewReport={() => setView('report')}   // <-- ROUTING BARU
            />
          )}
