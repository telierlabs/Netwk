...
export default function AppNative() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setErrorMsg(err?.message || err?.code || 'Gagal login. Coba lagi.');
    } finally {
  ...
  // ✅ Pengecekan login diaktifkan kembali (hapus `false &&`)
  if (!user) {
    return (
      <div style={{
        ...
        <h1 style={{ color: '#fff', fontSize: '32px', fontWeight: 600, marginBottom: '48px', letterSpacing: '-0.02em' }}>
          Cylen
        </h1>
        <button
          onClick={handleLogin}
          ...
          style={{
            ...
            background: '#fff', color: '#000', border: 'none', fontSize: '15px',
            fontWeight: 600, cursor: isLoggingIn ? 'not-allowed' : 'pointer'
          }}
        >
          {isLoggingIn ? 'Memuat...' : 'Masuk dengan Google'}
        </button>
        {errorMsg && <p style={{ color: '#ef4444', fontSize: '13px', marginTop: '20px' }}>{errorMsg}</p>}
      </div>
    );
  }
