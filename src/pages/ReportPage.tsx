  ...
  return (
    <main className="flex-1 bg-[var(--bg)] relative z-[100] flex flex-col min-h-screen">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-[var(--bg)] sticky top-0 z-10">
        <button onClick={onBack} className="p-2 -ml-2 rounded-full hover:bg-[var(--sf)] text-[var(--text)] transition-colors active:scale-95 [-webkit-tap-highlight-color:transparent]">
          <X size={24} strokeWidth={2.5} />
        </button>
        <h1 className="text-[16px] font-bold text-[var(--text)] tracking-tight flex-1 text-center pr-2">Report a Problem</h1>
        <button 
          onClick={handleSubmit} 
          className="bg-[var(--text)] text-[var(--bg)] px-5 py-2 rounded-full text-[14px] font-bold shadow-sm hover:scale-105 active:scale-95 transition-transform [-webkit-tap-highlight-color:transparent]"
        >
          Submit
        </button>
      </div>
      ...
      <div className="max-w-2xl w-full mx-auto pb-10 pt-4 px-4 flex flex-col gap-4">
        ...
        {/* TEXTAREA (GENERAL FEEDBACK DIHAPUS, LANGSUNG INPUT TEKS) */}
            ...
            {images.map((src, idx) => (
              <div key={idx} className="relative flex-shrink-0 w-20 h-20 rounded-[16px] overflow-hidden border border-[var(--bd)]/50 bg-[var(--sf)]">
                ...
                <button 
                  ...
                  className="absolute top-1 right-1 w-6 h-6 bg-black/60 backdrop-blur-md text-white rounded-full flex items-center justify-center hover:bg-black/80 transition-colors active:scale-90"
                >
