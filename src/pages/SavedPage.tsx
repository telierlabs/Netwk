...
export const SavedPage: React.FC<SavedPageProps> = ({ savedItems, onDelete }) => {
        ...
        {filtered.length === 0 ? (
          ...
          groups.map(group => (
            <div key={group.label}>
              ...
              {(activeTab === 'semua' || activeTab === 'gambar') && group.items.some(i => i.type === 'gambar') && (
                <div className="grid grid-cols-2 gap-2 mb-2">
                  {group.items.filter(i => i.type === 'gambar').map(item => (
                    <div key={item.id} className="bg-[var(--cd)] rounded-2xl overflow-hidden relative" style={{ border: '1px solid var(--bd)' }}>
                      <button
                        onClick={() => onDelete(item.id)}
                        className="absolute top-2 right-2 w-8 h-8 rounded-lg flex items-center justify-center z-10 text-white"
              ...
              {/* Non-gambar cards */}
              {group.items.filter(i => i.type !== 'gambar').map(item => (
                <div key={item.id} className="flex items-start gap-3 bg-[var(--cd)] rounded-2xl p-4 mb-2" style={{ border: '1px solid var(--bd)' }}>
