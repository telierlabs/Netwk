              ...
              {(activeTab === 'semua' || activeTab === 'gambar') && group.items.some(i => i.type === 'gambar') && (
                <div className="grid grid-cols-2 gap-3 mb-3">
                  {group.items.filter(i => i.type === 'gambar').map(item => (
                    ...
                    >
                      {/* BIKIN TOMBOL SAMPAH SELALU KELIHATAN (opacity-100) */}
                      <button
                        onClick={(e) => { e.stopPropagation(); onDelete(item.id); }}
                        className="absolute top-2 right-2 w-8 h-8 rounded-full flex items-center justify-center z-10 text-white bg-black/40 backdrop-blur-md hover:bg-red-500 transition-colors opacity-100"
                      >
                        <TrashIcon />
                      </button>
              ...
              <div className="flex flex-col gap-2">
                {group.items.filter(i => i.type !== 'gambar').map(item => (
                  ...
                  >
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); onDelete(item.id); }}
                      className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 text-[var(--mu)] bg-transparent hover:bg-red-500/10 hover:text-red-500 transition-colors opacity-100"
                    >
                      <TrashIcon />
                    </button>
                  </div>
