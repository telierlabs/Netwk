...
import { ArrowLeft } from 'lucide-react'; // <-- Import tambahan buat Icon Back
...
interface SavedPageProps {
  ...
  onBack?: () => void; // <-- Prop tambahan buat fungsi tombol back
}
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0, backgroundColor: 'var(--bg)' }}>
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-[var(--bd)]/15 sticky top-0 z-20 bg-[var(--bg)]">
        <div className="flex items-center gap-1">
          ...
          <h1 className="text-[17px] font-semibold text-[var(--text)]">Disimpan</h1>
        </div>
      </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  {group.items.filter(i => i.type === 'gambar').map(item => (
                    <div 
                      ...
                      className="bg-[var(--sf)]/30 rounded-[20px] overflow-hidden relative cursor-pointer hover:opacity-90 transition-opacity border border-[var(--bd)]/15 shadow-sm group" 
                    >
                      <button
                        onClick={(e) => { e.stopPropagation(); onDelete(item.id); }}
                        className="absolute top-2 right-2 w-8 h-8 rounded-full flex items-center justify-center z-10 text-white bg-black/40 backdrop-blur-md hover:bg-red-500 transition-colors opacity-0 group-hover:opacity-100"
                      >
                        <TrashIcon />
                      </button>
              ...
              <div className="flex flex-col gap-2">
                {group.items.filter(i => i.type !== 'gambar').map(item => (
                  <div 
                    key={item.id} 
                    onClick={() => item.chatId && onOpenChat?.(item.chatId)}
                    className="flex items-start gap-3.5 bg-[var(--sf)]/30 rounded-[20px] p-4 cursor-pointer hover:bg-[var(--sf)]/60 transition-colors border border-[var(--bd)]/15 shadow-sm group" 
                  >
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); onDelete(item.id); }}
                      className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 text-[var(--mu)] bg-transparent hover:bg-red-500/10 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"
                    >
                      <TrashIcon />
                    </button>
                  </div>
