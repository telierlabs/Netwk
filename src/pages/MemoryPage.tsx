...
interface MemoryPageProps {
  ...
  loading?: boolean; // Tambah properti loading dari hook
}

export const MemoryPage: React.FC<MemoryPageProps> = ({ memoryItems, onDelete, loading = false }) => {
  return (
    <div className="flex-1 flex flex-col bg-[var(--bg)] overflow-hidden" style={{ minHeight: 0 }}>
      ...
      <div className="px-6 py-8 flex flex-col items-center justify-center text-center flex-shrink-0">
        ...
        <p className="text-[13px] text-[var(--mu)] mt-2 max-w-[260px] leading-relaxed">
          Konteks dan preferensi yang dipelajari Cylen dari percakapanmu di Cloud.
        </p>
      </div>
            ...
            <div className="flex flex-col">
              {loading ? (
                <div className="px-5 py-12 text-center flex flex-col items-center">
                  <div className="w-8 h-8 border-4 border-[var(--mu)] border-t-[var(--text)] rounded-full animate-spin mb-4"></div>
                  <span className="text-[13px] font-medium text-[var(--mu)]">Menarik ingatan dari Cloud...</span>
                </div>
