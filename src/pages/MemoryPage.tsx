...
// 👇 Ikon Bulan Sabit (Moon) biar senada sama gambar no 6
const MoonIcon = () => (
...
// 👇 Ikon Tempat Sampah (Trash) buat tombol hapus (di gambar no 6 kaga ada, tapi kita butuh biar user bisa ngapus memorinya)
const TrashIcon = () => (
...
export const MemoryPage: React.FC<MemoryPageProps> = ({ memoryItems, onDelete }) => {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
      ...
      {/* ── MOCKUP DESIGN CARD (Tiru Plek-Ketiplek Gambar No 6) ── */}
      <div className="px-4 py-6 flex-1 overflow-y-auto min-h-0">
        <div className="bg-white border border-[var(--bd)] rounded-2xl shadow-sm overflow-hidden mb-6">
          {/* CARD HEADER */}
          <div className="px-5 py-4 border-b border-[var(--bd)] flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-[var(--sf)] flex items-center justify-center">
                <MoonIcon />
              </div>
              <div className="flex flex-col">
                <span className="text-sm font-semibold text-[var(--text)]">Cylen Ingat</span>
                <span className="text-xs text-[var(--mu)]">Konteks dari percakapan sebelumnya</span>
              </div>
            </div>
            {/* BLACK BADGE (Notifikasi hitam bulet angka '4') 👇 Tiru gambar no 6 */}
            <div className="w-8 h-8 rounded-full bg-black text-white flex items-center justify-center font-mono font-bold text-xs shadow-lg">
              {memoryItems.length}
            </div>
          </div>

          {/* CARD BODY (LIST ITEMS) Pake titik-titik 👇 Tiru gambar no 6 */}
          <div className="py-2">
              ...
              memoryItems.map((item, index) => (
                <div key={item.id} className="px-5 py-4 group border-b border-[var(--bd)] last:border-b-0">
                  <div className="flex items-start gap-4">
                    {/* 👇 Titik bulet berwarna aksen (aksen dari gambar no 6 gua ganti aksen app lu biar senada) */}
                    <span className="text-[var(--ac)] text-lg mt-0.5">•</span>
                    ...
                    {/* 👇 Tombol Hapus (Mininalist Trash Icon), nongol cuma pas di-hover */}
                    <button
                      onClick={() => onDelete(item.id)}
                      className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-[var(--mu)] bg-[var(--sf)] hover:bg-red-100 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100 transition-opacity"
                      title="Hapus memori"
                    >
                      <TrashIcon />
                    </button>
