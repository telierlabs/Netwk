...
// 👇 Import 6 halaman detail yang baru dibuat
...
import { PricingDetail } from '../features/landing/PricingDetail'; // <-- Import Pricing

export const LoginPage: React.FC<{ onLoginSuccess?: () => void }> = ({ onLoginSuccess }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  
  // 👇 State buat nyimpen lagi buka halaman apa (tambahin 'pricing')
  const [activePage, setActivePage] = useState<'home' | 'chat' | 'build' | 'imagine' | 'voice' | 'docs' | 'pricing'>('home');
  ...
  if (activePage === 'pricing') return <PricingDetail onBack={() => setActivePage('home')} onLogin={handleLogin} />; // <-- Route Pricing
