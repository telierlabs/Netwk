...
type PageKey = 'home' | 'chat' | 'build' | 'imagine' | 'voice' | 'docs' | 'pricing' | 'about' | 'privacy' | 'terms' | 'news-detail' | 'contact';

// Halaman yang punya URL publik sendiri (bisa diakses/refresh langsung).
// Cuma privacy & terms yang di-sync ke address bar, sisanya tetap state-only.
const PAGE_TO_PATH: Partial<Record<PageKey, string>> = {
...
export const LoginPage: React.FC<{ onLoginSuccess?: () => void }> = ({ onLoginSuccess }) => {
  ...
  // Sync activePage -> address bar (cuma untuk halaman yang punya URL publik)
  useEffect(() => {
    const targetPath = PAGE_TO_PATH[activePage] || '/';
    if (window.location.pathname !== targetPath) {
      window.history.pushState({ activePage }, '', targetPath);
    }
  }, [activePage]);

  // Dengerin tombol back/forward browser
  useEffect(() => {
    const onPopState = () => setActivePage(getInitialPage());
