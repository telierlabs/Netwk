...
import { LegalDetail } from '../features/landing/LegalDetail'; // <-- Import halaman Legal

export const LoginPage: React.FC<{ onLoginSuccess?: () => void }> = ({ onLoginSuccess }) => {
  const [isLoading, setIsLoading] = useState(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  
  // 👇 State ditambahin 'about', 'privacy', dan 'terms'
  const [activePage, setActivePage] = useState<'home' | 'chat' | 'build' | 'imagine' | 'voice' | 'docs' | 'pricing' | 'about' | 'privacy' | 'terms'>('home');
