...
const STORAGE_KEY = 'cylen_personality_settings';
...
function loadSettings(): PersonalitySettings {
  try {
    const saved = localStorage.getItem(STORAGE_KEY);
    if (saved) return JSON.parse(saved);
  } catch (e) {}
  return { tone: 'Default', warmth: 2, formality: 2, enthusiasm: 2, length: 2, customPrompt: '' };
}
...
export function buildPersonalityPrompt(settings: PersonalitySettings): string {
  ...
  if (settings.tone !== 'Default' && toneMap[settings.tone]) {
    parts.push(`**Nada Dasar:** ${toneMap[settings.tone]}`);
  }

  // Kehangatan
  const warmthMap: Record<number, string> = {
    1: 'Bersikap netral dan tidak terlalu hangat — lebih ke fakta dan efisiensi.',
    2: '',
    3: 'Tunjukkan empati yang dalam, gunakan kata-kata yang menyentuh dan peduli.',
  };
  if (warmthMap[settings.warmth]) parts.push(`**Kehangatan:** ${warmthMap[settings.warmth]}`);

  // Gaya Bahasa
  const formalityMap: Record<number, string> = {
    1: 'Gunakan bahasa gaul dan santai — boleh pakai singkatan, slang Indonesia yang relevan.',
    2: '',
    3: 'Gunakan bahasa Indonesia yang baku dan formal sesuai KBBI.',
  };
  ...
  const enthusiasmMap: Record<number, string> = {
    1: 'Respons dengan tenang dan datar — minim emoji, minim ekspresi berlebihan.',
    2: '',
    3: 'Tunjukkan antusiasme tinggi! Gunakan emoji secara ekspresif dan energik.',
  };
  ...
  const lengthMap: Record<number, string> = {
    1: 'Selalu jawab sesingkat mungkin — maksimal 3-4 kalimat kecuali ada kebutuhan teknis.',
    2: '',
    3: 'Berikan jawaban yang sangat lengkap dan detail — jangan ragu untuk panjang.',
  };
  if (lengthMap[settings.length]) parts.push(`**Panjang Jawaban:** ${lengthMap[settings.length]}`);

  // Custom prompt user
  if (settings.customPrompt.trim()) {
    parts.push(`**Instruksi Tambahan dari User:** ${settings.customPrompt.trim()}`);
  }
...
export const AiSettings = ({ onBack }: { onBack: () => void }) => {
  ...
  const handleSave = () => {
    ...
    // Simpan settings lengkap
    localStorage.setItem(STORAGE_KEY, JSON.stringify(settings));

    // Bangun & simpan ke cylen_temp_prompt yang dibaca geminiService
