...
export const ProfilePage: React.FC<ProfilePageProps> = ({
  ...
  const { profile, updateProfile } = useProfile(); // ── PANGGIL HOOK DI SINI
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = async (event) => { 
        ...
        // Langsung tembak ke Firestore biar nggak repot nunggu tombol save
        await updateProfile({ avatar: result });
        setShowAvatarMenu(false); 
        setShowFullAvatar(false); 
        showToast('Foto profil diperbarui di Cloud');
      };
  ...
  const handleSaveAvatar = async () => {
    if (previewImg) {
      await updateProfile({ avatar: previewImg });
      setPreviewImg(null); 
      showToast('Foto profil diperbarui di Cloud');
    }
  };
  ...
  const handleDeleteAvatar = async () => {
    ...
    setShowAvatarMenu(false); 
    setShowFullAvatar(false); 
    showToast('Foto profil dihapus dari Cloud');
  };
  ...
  const handleSaveLanguage = async (code: string, name: string) => {
    ...
    localStorage.setItem('cylen_lang', code); // Opsional buat jaga-jaga
    window.dispatchEvent(new CustomEvent('language-changed', { detail: { lang: code } }));
    setShowLangPopup(false); 
    showToast(`Bahasa diubah ke ${name}`);
  };

  // ── VARIABEL TAMPILAN MURNI DARI FIRESTORE ──
  const displayName = profile.name;
  const displayAvatar = previewImg || profile.avatar; // Preview kalau baru ganti, else dari db
  const activeLangData = LANGUAGES.find(l => l.code === profile.lang) || LANGUAGES[0];
      ...
      <div className="max-w-2xl mx-auto pb-6">
        ...
        <div className="bg-[var(--sf)] rounded-[24px] p-4 flex items-center gap-4 mx-4 mt-2 border border-[var(--bd)]/50">
          <div className="relative group shrink-0">
            ...
            <button onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click(); }} className="absolute -bottom-1 -right-1 w-7 h-7 bg-[var(--text)] text-[var(--bg)] rounded-full flex items-center justify-center shadow-md border-[2.5px] border-[var(--sf)] active:scale-95 transition-transform"><Camera size={13} strokeWidth={2.5} /></button>
          </div>
          <div className="flex-1 min-w-0 flex flex-col justify-center">
             {!showEditName ? (
               ...
               <div className="flex items-center gap-2 mb-1">
                 <input autoFocus value={editNameInput} onChange={(e) => setEditNameInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSaveName()} className="bg-[var(--bg)] border border-[var(--bd)] rounded px-2 py-1 text-[16px] text-[var(--text)] w-full outline-none" />
                 <button onClick={handleSaveName}><Check size={20} className="text-blue-500" /></button>
               </div>
             )}
             <div className="text-[14px] text-[var(--mu)] truncate mt-0.5">{user?.email || 'Guest User'}</div>
        ...
        {/* REPORT A PROBLEM MISAH SENDIRI (TANPA PANAH >) */}
        <CardGroup>
          <CardItem icon={<IconReport />} title="Report a Problem" onClick={onViewReport} border={false} right={<div />} />
        </CardGroup>
        
        <div className="h-4"></div> {/* SPACE DIKIT BIAR RAPI */}

        {/* SIGN OUT MISAH SENDIRI */}
        <CardGroup>
          ...
          <>
            ...
            <motion.div {...popupAnimation} className="fixed bottom-0 left-0 right-0 bg-[var(--bg)] rounded-t-[32px] z-[201] p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.2)] border-t border-[var(--bd)]/10 max-w-2xl mx-auto">
              ...
              <div className="flex flex-col gap-2 max-h-[50vh] overflow-y-auto pb-4" style={{ scrollbarWidth: 'none' }}>
                {LANGUAGES.map((l) => (
                  <button key={l.code} onClick={() => handleSaveLanguage(l.code, l.name)} className={cn("w-full flex items-center justify-between px-5 py-4 rounded-2xl transition-all active:scale-[0.98]", profile.lang === l.code ? "bg-[var(--text)] text-[var(--bg)] shadow-md" : "bg-[var(--sf)] text-[var(--text)] hover:bg-[var(--bd)]/50 border border-[var(--bd)]/20")}>
                    ...
                    {profile.lang === l.code && <Check size={20} strokeWidth={3} />}
                  </button>
                ))}
              </div>
