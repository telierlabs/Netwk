...
const LANGUAGES = [
  ...
  { code: 'jv', name: 'Basa Jawa', flag: 'ꦮꦺ' },
  { code: 'su', name: 'Basa Sunda', flag: '᮱' },
];
  ...
  return (
    <main className="flex-1 overflow-y-auto bg-[var(--bg)] relative z-50" style={{ minHeight: 0 }}>
      ...
      <AnimatePresence>
        {showLangPopup && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowLangPopup(false)} className="fixed inset-0 bg-black/60 z-[200] backdrop-blur-sm" />
            <motion.div initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 100 }} transition={{ type: "spring", bounce: 0, duration: 0.3 }} className="fixed bottom-0 left-0 right-0 bg-[var(--bg)] rounded-t-[32px] z-[201] p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.2)] border-t border-[var(--bd)]/10 max-w-2xl mx-auto">
