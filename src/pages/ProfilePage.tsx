...
interface ProfilePageProps {
  ...
  onViewMemory?: () => void; // 👇 BARU: Fungsi buat buka halaman Memori
}
      ...
      <div className="max-w-2xl mx-auto px-4">
        ...
        <div className="bg-[var(--cd)] border border-[var(--bd)] rounded-2xl flex items-center gap-4 p-5 mt-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="font-bold text-base truncate">{user?.displayName || 'Kamu'}</div>
            ...
            <div className="px-4 pb-4 flex flex-col gap-2">
              {FONTS.map((f) => (
                <button key={f.id} onClick={() => { setFont(f.id); showToast(`Font ${f.name} diterapkan`); }}
                  ...
                  <div className="text-xl w-8 flex-shrink-0 font-semibold" style={{ fontFamily: f.family }}>Ag</div>
        ...
        <div className="flex flex-col gap-2 mt-4">
          {user && (
            <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-[14px] bg-[var(--cd)] border border-[var(--bd)] rounded-2xl hover:bg-red-50 transition-colors">
              <div className="w-8 h-8 bg-red-100 rounded-[10px] flex items-center justify-center flex-shrink-0"><LogOut size={16} className="text-red-500" /></div>
          ...
          <button onClick={() => { if(confirm('Hapus akun permanen?')) showToast('Permintaan dikirim'); }}
            className="w-full flex items-center gap-3 px-4 py-[14px] bg-[var(--cd)] border border-[var(--bd)] rounded-2xl hover:bg-red-50 transition-colors">
            <div className="w-8 h-8 bg-red-100 rounded-[10px] flex items-center justify-center flex-shrink-0"><X size={16} className="text-red-500" /></div>
        ...
        <div className="flex flex-col items-center gap-2 mt-10 mb-4">
          ...
          <span className="text-sm font-bold">Cylen AI</span>
