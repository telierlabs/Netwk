...
// ── KOMPONEN LOADING DINAMIS BERDASARKAN NAMA AI ──
const CylenDynamicLoading: React.FC<{ name: string }> = ({ name }) => {
  return (
    <div className="w-full flex items-center justify-start gap-4 py-4 pl-1 select-none pointer-events-none">
      ...
      <span style={{ fontFamily: 'monospace', fontSize: '12px', letterSpacing: '0.02em', color: 'var(--text)', opacity: 0.5, fontWeight: 600 }}>
        {name.toUpperCase()} SEDANG BERPIKIR...
      </span>
    </div>
  );
};
...
interface GroupChatPageProps {
  ...
  thinkingAI?: string | null; // ── Nangkep state dari App.tsx ──
}

export const GroupChatPage: React.FC<GroupChatPageProps> = ({
  activeGroup, isSending, onSendMessage, onAddParticipant, showToast, onBack, onNavigateProfile, thinkingAI
}) => {
  ...
  }, [activeGroup.messages, isSending, thinkingAI]);
  ...
  // ── LOGIKA LOADING (TAMPIL HANYA JIKA AI BELUM MUNCULIN TEKS DI BUBBLE) ──
  ...
  const showLoading = isSending && !lastAiMsgHasContent && thinkingAI;
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-[var(--bg)] relative">
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-6" style={{ scrollbarWidth: 'none' }}>
        <div className="max-w-2xl mx-auto flex flex-col gap-5 pb-4">
          ...
          {/* ── MUNCULIN EFEK LOADING JIKA AI SEDANG BERPIKIR ── */}
          {showLoading && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full flex flex-col items-start px-2 mt-2">
              <CylenDynamicLoading name={thinkingAI} />
            </motion.div>
          )}
