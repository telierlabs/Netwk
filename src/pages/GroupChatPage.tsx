...
import { 
  Users, Plus, Mail, Share2, Bot, X, UserPlus, MessageSquare, Sparkles,
  Video, ChevronDown, ChevronUp, Save
} from 'lucide-react';
...
import { ZoomOverlay } from '../components/chat/ZoomOverlay'; // 🔥 Udah diimport nih
...
export const GroupChatPage: React.FC<GroupChatPageProps> = ({
  ...
  const [showZoom, setShowZoom] = useState(false); // 🔥 State buat ngebuka ZoomOverlay
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[var(--bg)] relative outline-none">
      ...
      <div className="px-4 py-3 flex items-center justify-between border-b border-[var(--bd)]/30 outline-none bg-[var(--bg)]">
        
        <div className="flex items-center gap-3">
          <div 
            onClick={onNavigateProfile}
            className="w-10 h-10 rounded-full bg-[var(--sf)] border border-[var(--bd)] overflow-hidden flex items-center justify-center cursor-pointer hover:opacity-80 active:scale-95 transition-all shadow-sm shrink-0 outline-none [-webkit-tap-highlight-color:transparent]"
          >
        ...
        <div className="flex items-center gap-1.5 outline-none">
          <button 
            ...
            className="p-2 text-[var(--text)]/80 hover:text-[var(--text)] hover:bg-[var(--sf)] rounded-full transition-all active:scale-95 outline-none [-webkit-tap-highlight-color:transparent]"
            title="Tambah Anggota"
          >
            <UserPlus size={20} strokeWidth={2.5} />
          </button>

          {/* 🔥 Klik icon ini buat buka Overlay Zoom */}
          <button 
          ...
          <button 
            ...
            className="flex items-center justify-center px-4 py-2 ml-1 bg-[var(--text)] text-[var(--bg)] rounded-full text-[14px] font-black shadow-md hover:scale-105 active:scale-95 transition-all outline-none [-webkit-tap-highlight-color:transparent]"
          >
            + AI
          </button>
        </div>
          ...
          {isSending && (
            <div className="flex flex-col items-start gap-2 outline-none">
              <div className="flex items-center gap-2 px-1 outline-none">
                ...
                <span className="text-[10px] font-bold text-[var(--text)] uppercase tracking-widest animate-pulse">AI sedang berdiskusi...</span>
              </div>
      ...
      {/* 🔥 Render Komponen ZoomOverlay di sini */}
      <ZoomOverlay 
        isOpen={showZoom} 
      ...
      <AnimatePresence>
        {showInviteModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 outline-none">
            ...
            <motion.div {...smoothAnimation} className="relative w-full max-w-md bg-[var(--bg)]/95 backdrop-blur-2xl rounded-[32px] shadow-2xl border border-[var(--bd)]/50 overflow-hidden transform-gpu outline-none">
              <div className="p-6 border-b border-[var(--bd)]/50 flex items-center justify-between outline-none">
                <h3 className="text-[18px] font-bold tracking-tight text-[var(--text)] outline-none">Info Grup & Undangan</h3>
                <button onClick={() => setShowInviteModal(false)} className="p-2 hover:bg-[var(--sf)] text-[var(--text)] rounded-full transition-colors active:scale-95 outline-none [-webkit-tap-highlight-color:transparent]"><X size={20} /></button>
              </div>
        ...
        {showAIModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 outline-none">
            ...
            <motion.div {...smoothAnimation} className="relative w-full max-w-md bg-[var(--bg)]/95 backdrop-blur-2xl rounded-[32px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.5)] border border-[var(--text)]/10 overflow-hidden flex flex-col max-h-[85vh] transform-gpu outline-none">
              ...
              <div className="p-5 border-b border-[var(--bd)]/50 flex items-center justify-between bg-[var(--bg)]/50 sticky top-0 z-10 outline-none">
                <div className="flex items-center gap-3 outline-none">
                  ...
                  <div className="outline-none">
                    <h3 className="text-[17px] font-black text-[var(--text)] tracking-tight outline-none">Tambah AI ke Grup</h3>
                    <p className="text-[10px] font-bold text-[var(--text)]/50 uppercase tracking-widest outline-none">Pilih Model AI</p>
                  </div>
                </div>
                <button onClick={() => setShowAIModal(false)} className="p-2 hover:bg-[var(--sf)] text-[var(--text)] rounded-full transition-colors active:scale-90 outline-none [-webkit-tap-highlight-color:transparent]"><X size={22} strokeWidth={2.5} /></button>
              </div>
                          ...
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden bg-[var(--sf)]/30 border-t border-[var(--bd)]/50 outline-none">
                            <div className="p-3 flex items-center gap-2 outline-none">
                              <input 
                                ...
                                className="flex-1 bg-[var(--bg)] border border-[var(--bd)] text-[var(--text)] rounded-[12px] px-3 py-2 text-[13px] outline-none focus:border-[var(--text)]/50 transition-colors placeholder:text-[var(--text)]/30 [-webkit-tap-highlight-color:transparent]"
                              />
                              <button 
                ...
                <div className="mt-2 pt-4 border-t border-[var(--bd)]/50 outline-none">
                  ...
                  <div className="flex flex-col gap-2 p-3 bg-[var(--sf)]/50 rounded-[20px] border border-[var(--bd)] outline-none">
                    <input 
                      ...
                      className="w-full bg-[var(--bg)] border border-[var(--bd)] text-[var(--text)] rounded-[12px] px-3 py-2.5 text-[14px] font-semibold outline-none focus:border-[var(--text)]/50 transition-colors [-webkit-tap-highlight-color:transparent]"
                    />
                    <input 
                      type="password" 
                      ...
                      className="w-full bg-[var(--bg)] border border-[var(--bd)] text-[var(--text)] rounded-[12px] px-3 py-2.5 text-[13px] outline-none focus:border-[var(--text)]/50 transition-colors [-webkit-tap-highlight-color:transparent]"
                    />
                    <button 
