...
interface GroupChatPageProps {
  ...
  isSending: boolean;
  // UPDATE PENTING: Tambahkan images sebagai array string opsional
...
export const GroupChatPage: React.FC<GroupChatPageProps> = ({
  ...
  // --- LOGIKA MENDENGARKAN PERUBAHAN FOTO GRUP ---
  ...
  // UPDATE PENTING: Jangan blokir gambar lagi! Lempar ke onSendMessage
  const handleSend = (images?: string[], pdfs?: { data: string; name: string }[]) => {
    if (!inputText.trim() && (!images || images.length === 0)) return;
    
    // Sekarang gambar yang lu pilih dari penjepit kertas bakal dikirim!
    onSendMessage(inputText, images);
    setInputText('');
  };
      ...
      <div className="px-2 py-2 flex items-center justify-between border-b border-[var(--bd)]/20 bg-[var(--bg)] z-10">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 cursor-pointer hover:opacity-80 transition-opacity flex-1 min-w-0 py-1" onClick={onNavigateProfile}>
            {/* FOTO GRUP SYNC */}
            <div className="w-10 h-10 bg-[var(--text)] rounded-full flex items-center justify-center text-[var(--bg)] font-bold shrink-0 overflow-hidden">
              {groupAvatar ? (
