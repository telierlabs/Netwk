...
export const GroupChatPage: React.FC<GroupChatPageProps> = ({
  ...
  // ── STATE BALAS PESAN (PENTING BUAT SWIPE) ──
  ...
  const handleSend = (images?: string[], pdfs?: { data: string; name: string }[]) => {
    ...
    // Kalau ada pesan yang di-reply, tempel di pesannya
    if (replyingTo) {
    ...
    setInputText('');
    setReplyingTo(null); // Tutup kotak reply
  };
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[var(--bg)] relative">
      ...
      {/* ─── HEADER (Tanpa Garis Abu-abu di bawahnya) ─── */}
            ...
            <div key={i} className={cn("flex flex-col", msg.role === 'user' ? "items-end" : "items-start")}>
              {msg.role === 'system' ? (
                ...
                <GroupChatBubble 
                  ...
                  isSending={isSending} 
                  onSwipeToReply={(m) => setReplyingTo(m)} // Kawat pemicu pop-up balasan
                />
              )}
            </div>
          ))}
