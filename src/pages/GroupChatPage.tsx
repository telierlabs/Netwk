...
// ─── ACCURATE AI BRAND LOGOS ────────────────────────────────────────
const ChatGPTLogo = ({ size = 22 }: { size?: number }) => (
...
const GeminiLogo = ({ size = 22 }: { size?: number }) => (
...
const ClaudeLogo = ({ size = 22 }: { size?: number }) => (
...
const DeepSeekLogo = ({ size = 22 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const MetaLogo = ({ size = 22 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const GrokLogo = ({ size = 22 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export const GroupChatPage: React.FC<GroupChatPageProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-[var(--bg)] relative">
      ...
      {/* ─── HEADER: panah kembali + nama grup ─── */}
      <div className="px-4 py-3 flex items-center justify-between border-b border-[var(--bd)]/20 bg-[var(--bg)]">
        <div className="flex items-center gap-2">
          <button
            onClick={onBack}
            className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-[var(--sf)] text-[var(--text)] transition-colors active:scale-95"
          >
        ...
        {showInviteModal && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 left-0 right-0 z-[201] mx-auto w-full max-w-sm sm:relative sm:bottom-auto sm:inset-0 sm:m-auto sm:absolute bg-[var(--bg)] rounded-t-[28px] sm:rounded-[24px] shadow-2xl border border-[var(--bd)]/20 overflow-hidden"
            >
              <div className="w-10 h-1 bg-[var(--bd)]/25 rounded-full mx-auto mt-3 mb-1 sm:hidden" />
              <div className="px-5 pt-3 pb-2 flex items-center justify-between">
                <h3 className="text-[16px] font-semibold text-[var(--text)]">Info Grup</h3>
              ...
              <div className="px-5 pb-8 flex flex-col gap-4">
                <div className="flex items-center justify-between px-4 py-3.5 bg-[var(--sf)] rounded-2xl">
                  <span className="text-[14px] text-[var(--text)]/60">Kapasitas Member</span>
                  <span className="text-[14px] font-semibold text-[var(--text)]">{userCount} / 10</span>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    ...
                    >
                      <div className="w-14 h-14 rounded-2xl bg-[var(--sf)] flex items-center justify-center text-[var(--text)]/60 group-hover:bg-[var(--text)] group-hover:text-[var(--bg)] transition-all active:scale-95">
                        {opt.icon}
                      </div>
        ...
        {showAIModal && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 left-0 right-0 z-[201] mx-auto w-full max-w-sm bg-[var(--bg)] rounded-t-[28px] shadow-2xl border border-[var(--bd)]/20 flex flex-col overflow-hidden"
              style={{ maxHeight: '92vh' }}
            >
              {/* drag pill */}
              <div className="w-10 h-1 bg-[var(--bd)]/25 rounded-full mx-auto mt-3 mb-0 shrink-0" />

              {/* Header */}
              <div className="px-5 pt-3 pb-3 flex items-center justify-between border-b border-[var(--bd)]/15 shrink-0">
                <div>
                  <h3 className="text-[16px] font-semibold text-[var(--text)]">Tambah AI ke Grup</h3>
                  <p className="text-[12px] text-[var(--text)]/40 mt-0.5">Pilih model AI</p>
                </div>
                <button
                  onClick={() => setShowAIModal(false)}
                  className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[var(--sf)] text-[var(--text)]/50 transition-colors active:scale-90"
                >
                  <X size={18} strokeWidth={2} />
                </button>
              </div>
              ...
              <div className="overflow-y-auto flex-1 px-4 py-3 flex flex-col gap-2" style={{ scrollbarWidth: 'none' }}>
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        isAdded
                          ? "border-[var(--bd)]/15 opacity-50"
                          : isExpanded
                          ? "border-[var(--bd)]/35 bg-[var(--sf)]/20"
                          : "border-[var(--bd)]/20 hover:border-[var(--bd)]/35"
                      )}
                    >
                      <div className="flex items-center gap-3 px-4 py-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-[14px] font-semibold text-[var(--text)]">{ai.name}</div>
                          <div className="text-[12px] text-[var(--text)]/45 mt-0.5">{ai.model}</div>
                        </div>

                        {/* Action buttons */}
                        {isAdded ? (
                          <div className="px-3 py-1 rounded-full bg-[var(--sf)] border border-[var(--bd)]/20 text-[11px] font-semibold text-[var(--text)]/45 uppercase tracking-wide">
                            Aktif
                          </div>
                        ) : (
                          <div className="flex items-center gap-1.5">
                            <button
                              ...
                              className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[var(--sf)] text-[var(--text)]/40 transition-colors"
                            >
                              <motion.span
                            ...
                            <button
                              ...
                              className="w-9 h-9 flex items-center justify-center rounded-full bg-[var(--text)] text-[var(--bg)] active:scale-90 transition-transform shadow-sm"
                            >
                      ...
                      <AnimatePresence>
                        {isExpanded && !isAdded && (
                          ...
                          >
                            <div className="px-4 py-3 border-t border-[var(--bd)]/15 bg-[var(--sf)]/20 flex gap-2">
                              <input
                                type="password"
                                placeholder={`${ai.name} API Key...`}
                                value={apiKeys[ai.name] || ''}
                                onChange={(e) => setApiKeys({ ...apiKeys, [ai.name]: e.target.value })}
                                className="flex-1 bg-[var(--bg)] border border-[var(--bd)]/25 text-[var(--text)] rounded-xl px-3 py-2.5 text-[13px] outline-none placeholder:text-[var(--text)]/25 focus:border-[var(--text)]/30 transition-colors"
                              />
                              <button
                                onClick={() => { showToast(`API Key ${ai.name} tersimpan!`); setExpandedAI(null); }}
                                className="px-3.5 py-2 bg-[var(--text)] text-[var(--bg)] text-[12px] font-semibold rounded-xl flex items-center gap-1.5 active:scale-95 transition-transform"
                              >
                                <Save size={13} />
                                Simpan
                              </button>
                ...
                {/* Custom AI */}
                <div className="mt-2 pt-4 border-t border-[var(--bd)]/15">
                  <div className="flex items-center gap-2 mb-3 px-1">
                    <Sparkles size={13} className="text-[var(--text)]/35" />
                    <span className="text-[11px] font-semibold text-[var(--text)]/35 uppercase tracking-widest">Custom AI</span>
                  </div>
                  <div className="flex flex-col gap-2.5 p-4 bg-[var(--sf)]/40 rounded-2xl border border-[var(--bd)]/15">
                    <input
                      type="text"
                      placeholder="Nama AI (contoh: TelierBot)"
                      value={customName}
                      onChange={(e) => setCustomName(e.target.value)}
                      className="w-full bg-[var(--bg)] border border-[var(--bd)]/25 text-[var(--text)] rounded-xl px-4 py-3 text-[14px] outline-none placeholder:text-[var(--text)]/25 focus:border-[var(--text)]/30 transition-colors"
                    />
                    <input
                      type="password"
                      ...
                      className="w-full bg-[var(--bg)] border border-[var(--bd)]/25 text-[var(--text)] rounded-xl px-4 py-3 text-[13px] outline-none placeholder:text-[var(--text)]/25 focus:border-[var(--text)]/30 transition-colors"
                    />
                    <button
                      onClick={() => {
                      ...
                      className="w-full py-3 bg-[var(--text)] text-[var(--bg)] text-[14px] font-semibold rounded-xl flex items-center justify-center gap-2 disabled:opacity-25 active:scale-[0.98] transition-transform"
                    >
                      <Plus size={16} strokeWidth={2.5} />
                      Pasang & Simpan
                    </button>
