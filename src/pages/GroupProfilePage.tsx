...
interface GroupProfilePageProps {
  ...
  onLeaveGroup: (id: string) => Promise<{success: boolean, message?: string}>; // ── FIX: Tambah properti ini ──
}

export const GroupProfilePage: React.FC<GroupProfilePageProps> = ({
  activeGroup, onBack, showToast, onLeaveGroup
}) => {
  ...
  const handleLeaveGroupBtn = async () => {
    if (window.confirm('Yakin ingin keluar dari grup ini?')) {
      ...
      if (res.success) {
        ...
        // Navigasi baliknya bakal di-handle di App.tsx
      } else {
          ...
          <motion.div key="main" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.15 }} className="flex-1 flex flex-col h-full overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
            ...
            <div className="pt-5 flex flex-col pb-8">
              ...
              <div className="mt-4 px-5">
                <button onClick={handleLeaveGroupBtn} className="w-full flex items-center gap-3 px-4 py-3.5 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-2xl font-bold transition-colors active:scale-[0.98] justify-center">
