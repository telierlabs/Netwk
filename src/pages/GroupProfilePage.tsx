...
export const GroupProfilePage: React.FC<GroupProfilePageProps> = ({
  ...
  // Listener untuk sinkronisasi nama/avatar user
  useEffect(() => {
      ...
      <AnimatePresence mode="wait">
        ...
        {view === 'main' && (
          <motion.div key="main" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.15 }} className="flex-1 flex flex-col h-full overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
            ...
            {/* ─── DAFTAR ANGGOTA (DENGAN NAMA USER CUSTOM) ─── */}
              ...
              <AnimatePresence mode="wait">
                {activeTab === 'user' ? (
                  <motion.div key="user" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.15 }} className="flex flex-col">
                    {users.map(u => {
                      // CEK APAKAH INI AKUN KAMU, JIKA IYA TAMPILKAN NAMA/FOTO CUSTOM
                      const isMe = u.id === 'user-1' || u.name === 'Kamu';
                      const finalName = isMe ? userName : u.name;
                      const finalAvatar = isMe ? userAvatar : null;
                      ...
                      return (
                        <div key={u.id} className="flex items-center gap-4 px-5 py-3 hover:bg-[var(--sf)]/50 transition-colors cursor-pointer">
                          <div className="w-12 h-12 bg-[var(--text)] rounded-full flex items-center justify-center text-[var(--bg)] shrink-0 overflow-hidden">
                            {finalAvatar ? <img src={finalAvatar} alt="User" className="w-full h-full object-cover" /> : <User size={22} />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <p className="text-[15px] font-bold text-[var(--text)] truncate">{finalName}</p>
                              {isMe && <span className="px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-[var(--ac)] text-[var(--at)]">Admin</span>}
                            </div>
        ...
        {showEditInfo && (
          ...
          >
            <motion.div 
              ...
              className="w-full max-w-[340px] bg-[var(--bg)] rounded-3xl p-6 shadow-2xl border border-[var(--bd)]/10"
            >
              ...
              <div className="flex flex-col gap-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full mt-1.5 bg-[var(--sf)] px-4 py-3.5 rounded-2xl outline-none text-[15px] font-bold text-[var(--text)] border border-[var(--bd)]/20 focus:border-[var(--text)]/30 transition-colors" 
                  />
                </div>
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full mt-1.5 bg-[var(--sf)] px-4 py-3.5 rounded-2xl outline-none text-[14px] text-[var(--text)] resize-none h-28 border border-[var(--bd)]/20 focus:border-[var(--text)]/30 transition-colors font-medium leading-relaxed" 
                    placeholder="Tambahkan penjelasan tentang grup ini..." 
                  />
                </div>
              </div>
