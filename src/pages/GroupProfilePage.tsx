...
import { ArrowLeft, MessageSquare, Share2, UserPlus, Image as ImageIcon, LogOut, User, Bot, ChevronRight, Download, Camera, Trash2, X, Check } from 'lucide-react';
...
export const GroupProfilePage: React.FC<GroupProfilePageProps> = ({
  ...
  const handleSaveAvatar = () => {
    if (previewImg) {
      ...
      setPreviewImg(null);
      showToast('Foto grup berhasil diperbarui');
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-[var(--bg)] overflow-hidden relative z-50">
      ...
      <AnimatePresence mode="wait">
        ...
        {/* ================= VIEW: MAIN PROFILE ================= */}
        {view === 'main' && (
          <motion.div key="main" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} className="flex-1 flex flex-col h-full overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
            ...
            <div className="flex flex-col items-center pt-2 pb-8 border-b border-[var(--bd)]/10">
              <div className="relative group cursor-pointer mb-5" onClick={() => setShowMenu(true)}>
                <div className="w-32 h-32 bg-[var(--text)] rounded-full flex items-center justify-center text-[var(--bg)] text-5xl font-bold shadow-lg overflow-hidden border-4 border-[var(--bg)]">
                  {groupAvatar ? (
                ...
                <div className="absolute bottom-1 right-1 w-9 h-9 bg-[var(--text)] text-[var(--bg)] border-2 border-[var(--bg)] rounded-full flex items-center justify-center shadow-md transition-transform group-hover:scale-110">
        ...
        {/* ================= VIEW: GALLERY GRID ================= */}
        {view === 'gallery' && (
          <motion.div key="gallery" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.2 }} className="flex-1 flex flex-col bg-[var(--bg)] absolute inset-0 z-30">
        ...
        {/* ================= VIEW: DETAIL FOTO ================= */}
        {view === 'detail' && (
          <motion.div key="detail" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.2 }} className="flex-1 flex flex-col bg-black absolute inset-0 z-40">
      ...
      <AnimatePresence>
        {showMenu && (
          <>
            ...
            <motion.div initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 100 }} transition={{ type: "spring", bounce: 0, duration: 0.3 }} className="fixed bottom-0 left-0 right-0 bg-[var(--bg)] rounded-t-[24px] z-[201] p-5 shadow-2xl">
      ...
      <AnimatePresence>
        {previewImg && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="fixed inset-0 bg-black z-[300] flex flex-col">
