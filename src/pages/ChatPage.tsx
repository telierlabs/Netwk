...
// UPDATE: Animasi DNA Helix Murni (2 Titik Solid Berputar Kanan-Kiri Secara Horizontal)
const DnaHelixLoading = () => (
  <div className="flex items-center py-5 pl-4">
    <style>{`
      @keyframes helix-horizontal-1 {
        0%, 100% { transform: translateX(0px) scale(1); opacity: 1; z-index: 2; }
        25% { transform: translateX(20px) scale(0.7); opacity: 0.4; }
        50% { transform: translateX(0px) scale(0.5); opacity: 0.2; z-index: 1; }
        75% { transform: translateX(-20px) scale(0.7); opacity: 0.4; }
      }
      @keyframes helix-horizontal-2 {
        0%, 100% { transform: translateX(0px) scale(0.5); opacity: 0.2; z-index: 1; }
        25% { transform: translateX(-20px) scale(0.7); opacity: 0.4; }
        50% { transform: translateX(0px) scale(1); opacity: 1; z-index: 2; }
        75% { transform: translateX(20px) scale(0.7); opacity: 0.4; }
      }
    `}</style>
    <div style={{ position: 'relative', width: '60px', height: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      {/* Titik Elemen 1 */}
      <div style={{ position: 'absolute', width: '8px', height: '8px', borderRadius: '50%', background: 'var(--text)', animation: 'helix-horizontal-1 1.4s ease-in-out infinite' }} />
      {/* Titik Elemen 2 */}
      <div style={{ position: 'absolute', width: '8px', height: '8px', borderRadius: '50%', background: 'var(--text)', animation: 'helix-horizontal-2 1.4s ease-in-out infinite' }} />
    </div>
  </div>
);
            ...
            {messages.map((msg, i) => {
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  {/* LOADING DENGAN ANIMASI DNA HELIX SPIN HORIZONTAL */}
                  {showLoadingHere && (
