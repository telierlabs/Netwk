...
// Hook animasi angka naik smooth (fake counter)
function useAnimatedCount(target: number, duration = 800) {
  ...
  useEffect(() => {
    if (target === 0) { setDisplay(0); prevTarget.current = 0; return; }
...
// 2. Loading Web Search — 2 baris selalu tampil, besar seperti Grok
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ animation: 'pulseIcon 1.4s ease-in-out infinite', display: 'flex', alignItems: 'center' }}>
          <Search size={20} strokeWidth={2.5} color="var(--mu)" />
        </div>
        <span style={{ fontSize: 15, color: 'var(--mu)', fontWeight: 600 }}>
          Searching the web
        </span>
        {animatedWeb > 0 && (
          <span style={{
