...
const NormalLoading = ({ text }: { text: string }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
    ...
    `}</style>
    {/* Spinner dibikin lebih gede (20px) dan tebel */}
    <div style={{
      width: 20, height: 20,
...
const WebSearchLoading = ({ query, sourceCount }: { query: string, sourceCount: number }) => {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, width: '100%', marginTop: 8 }}>
      ...
      {/* Baris Atas */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        {/* Kontainer fix 32px biar senter sama logo TelierNews */}
      ...
      {/* Baris Bawah */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        {/* Kontainer fix 32px juga biar lurus sempurna ke bawah */}
        ...
        <span style={{
          fontSize: 12, color: 'var(--mu)', fontFamily: 'monospace', fontWeight: 600,
          whiteSpace: 'nowrap',
          overflow: 'hidden',
...
// ... (Sisa komponen sama: ConnectionErrorCard & EmptyState)
const ConnectionErrorCard = ({ type, onRetry }: { type?: string; onRetry?: () => void }) => {
  ...
  return (
    <div className="w-full rounded-2xl border border-[var(--bd)] bg-[var(--cd)] p-5 flex flex-col gap-3">
      ...
      <p className="text-sm text-[var(--mu)] leading-relaxed">{isOffline ? 'No internet connection. Please check your network and try again.' : 'Connection failed. Check network connection and try again.'}</p>
      {onRetry && (
...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  // Ambil jumlah sumber daya yang ditarik AI saat ini
