...
const EmptyState = () => <div className="flex-1" />;
...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  // ─── DETEKSI STREAMING ───
  // "streaming" = isSending masih true DAN pesan AI terakhir sudah ada isinya
  // Loading spinner ditampilkan HANYA selama belum ada konten AI sama sekali
  const lastMsg = messages[messages.length - 1];
  const lastAiMsgHasContent =
    lastMsg && lastMsg.role === 'assistant' && lastMsg.content && lastMsg.content.length > 0;

  // showLoading: spinner muter — tampil saat kirim tapi AI belum balas sama sekali
  const showLoading = isSending && !lastAiMsgHasContent;

  // isStreamingLastMsg: konten AI sedang tumbuh (streaming)
  const isStreamingLastMsg = isSending && !!lastAiMsgHasContent;
        ...
        {isEmptyChat ? (
          ...
          <div className="max-w-3xl mx-auto flex flex-col gap-10 w-full pb-[160px]">
            {messages.map((msg, i) => {
              ...
              // Loading spinner hanya tampil setelah pesan user TERAKHIR,
              // dan hanya selama AI belum mulai balas (showLoading)
              const showLoadingHere = showLoading && i === lastUserIndex;
              // isStreaming hanya untuk pesan AI terakhir yang sedang tumbuh
              const isThisStreaming = isStreamingLastMsg && i === messages.length - 1 && msg.role === 'assistant';
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  {/* Loading spinner — hanya muncul sebelum ada konten AI */}
                  {showLoadingHere && (
            ...
            {/* Fallback: loading sebelum pesan user pertama masuk */}
