...
// Fake counter: naik terus dari 0 sampai target, lalu lanjut naik pelan sambil nunggu
function useFakeCounter(active: boolean, realTarget: number, maxFake: number = 30) {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      let next: number;
      if (current < target) {
        // Kejar target real dengan lompat +1 sampai +3
        next = Math.min(current + Math.ceil(Math.random() * 3), target);
      } else if (current < maxFake) {
        // Lewat target real, tetap naik pelan buat kesan "masih loading"
        next = current + 1;
      } else {
        // Udah maxFake, berhenti
        next = current;
      }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontSize: 16, color: 'var(--mu)', fontWeight: 600 }}>
          Searching the web
        </span>
        <span style={{ fontSize: 16, color: 'var(--mu)', fontWeight: 700, opacity: 0.75, minWidth: 24 }}>
          {fakeWeb} results
        </span>
      </div>

      {/* Baris 2 — Reading posts, slide in */}
      {showRow2 && (
        <div style={{
          display: 'flex',
          alignItems: 'center',
