...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative">
      ...
      {/* 🔥 TOMBOL PANAH KE BAWAH (Udah Gede, Posisi Naik, Bentuk ^ Kebalik Bersih) */}
      <div 
        className={`absolute bottom-32 right-5 md:right-8 z-[60] transition-all duration-300 ${showScrollButton ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-90 pointer-events-none'}`}
      >
        <button 
          onClick={scrollToBottom}
          className="w-12 h-12 flex items-center justify-center bg-[var(--ac)] text-[var(--at)] rounded-full shadow-2xl hover:scale-110 active:scale-95 transition-transform"
        >
          {/* Ikon ^ kebalik murni, nggak pakai gagang/batang */}
