...
/* ── Empty State ── */
const EmptyState = ({ onPrompt }: { onPrompt: (text: string) => void }) => {
  const starters = [
    { icon: '✍️', text: 'Bantu gua nulis sesuatu' },
    { icon: '🔍', text: 'Cariin info terbaru' },
    { icon: '💡', text: 'Kasih gua ide kreatif' },
    { icon: '💻', text: 'Bantu debug kode gua' },
  ];
  return (
    <div className="flex flex-col items-center justify-center flex-1 px-6 py-12 gap-6">
      {/* floating moon */}
      <div className="flex flex-col items-center gap-3">
        <div className="w-14 h-14 bg-[var(--ac)] rounded-full flex items-center justify-center"
          style={{ animation: 'float 3s ease-in-out infinite' }}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--at)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/>
          </svg>
        </div>
        <style>{`@keyframes float { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-8px)} }`}</style>
        <h2 className="text-xl font-bold text-[var(--text)]">Halo, saya Cylen</h2>
        <p className="text-sm text-[var(--mu)] text-center">Mau ngobrol apa hari ini?</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 w-full max-w-sm">
        {starters.map((s, i) => (
          <button key={i} onClick={() => onPrompt(s.text)}
            className="flex flex-col gap-2 p-4 bg-[var(--sf)] border border-[var(--bd)] rounded-2xl text-left hover:bg-[var(--bd)] active:scale-95 transition-all">
            <span className="text-2xl">{s.icon}</span>
            <span className="text-[13px] font-medium text-[var(--text)] leading-snug">{s.text}</span>
          </button>
        ))}
      </div>
    </div>
...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  // hanya pesan AI pertama (welcome) = empty state
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      {pinnedMessages.length > 0 && (
        <div className="px-4 pt-2 flex-shrink-0">
          <div className="max-w-3xl mx-auto">
            <div className="flex items-center gap-3 bg-[var(--sf)] border border-[var(--bd)] rounded-xl px-4 py-3">
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 flex flex-col">
        {isEmptyChat && !isSending ? (
          <EmptyState onPrompt={(text) => { setInputText(text); setTimeout(() => onSend(), 0); }} />
        ) : (
