...
const GhostEmptyState = () => (
  <div className="flex-1 flex flex-col items-center justify-center pointer-events-none select-none" style={{ paddingBottom: '20vh' }}>
    ...
    <div className="flex flex-col items-center gap-3 text-center mt-6" style={{ animation: 'ghost-in 0.4s 0.12s ease-out both', opacity: 0 }}>
      ...
      <span style={{ fontSize: 14, color: 'var(--text)', opacity: 0.4, lineHeight: 1.5, maxWidth: 260 }}>Chat ini tidak akan muncul di riwayat and akan dihapus sepenuhnya</span>
    </div>
  </div>
);
...
// UPDATE: MASTER KINETIC ENGINE CORE — Menampung 2 tipe animasi (Helix & Neutrino) dengan Text Contextual Generator
const CylenDynamicLoading: React.FC<{ isSearching: boolean; userPrompt: string }> = ({ isSearching, userPrompt }) => {
  // Logic Penentu Teks Status Berdasarkan Keyword Input User
  const getStatusText = () => {
    const p = userPrompt.toLowerCase();
    if (isSearching) return 'Mencari informasi di jaringan...';
    if (p.includes('buatkan kode') || p.includes('bikin kode') || p.includes('script') || p.includes('coding')) return 'Menyusun kode kompilasi...';
    if (p.includes('analisis') || p.includes('cek kode') || p.includes('benerin')) return 'Menganalisis struktur data...';
    if (p.includes('buatkan') || p.includes('tulis') || p.includes('bikin')) return 'Membangun arsitektur konten...';
    return 'Cylen sedang memproses instruksi...';
  };

  return (
    <div className="flex items-center gap-5 py-5 pl-4 select-none pointer-events-none">
      <style>{`
        /* 1. DNA HELIX CONFIG */
        @keyframes helix-h-1 {
          0%, 100% { transform: translateX(0px) scale(1); opacity: 1; z-index: 2; }
          25% { transform: translateX(18px) scale(0.7); opacity: 0.4; }
          50% { transform: translateX(0px) scale(0.5); opacity: 0.15; z-index: 1; }
          75% { transform: translateX(-18px) scale(0.7); opacity: 0.4; }
        }
        @keyframes helix-h-2 {
        ...
        /* 2. NEUTRINO OSCILLATION CONFIG */
        @keyframes neu-rotate-engine { 100% { transform: rotate(360deg); } }
        @keyframes neu-scale-engine { 0% { transform: scale(0.3); opacity: 0.2; } 100% { transform: scale(1.3); opacity: 1; } }
      `}</style>

      {isSearching ? (
        /* JIKA AKTIF SEARCHING -> KELUARKAN 07 // NEUTRINO OSCILLATION (3 TITIK SOLID) */
        <div style={{ position: 'relative', width: '24px', height: '24px', animation: 'neu-rotate-engine 2.5s linear infinite', flexShrink: 0 }}>
          <div style={{ position: 'absolute', top: 0, left: '9px', width: '6px', height: '6px', borderRadius: '50%', background: 'var(--text)', animation: 'neu-scale-engine 1.2s ease-in-out infinite alternate' }} />
          <div style={{ position: 'absolute', bottom: 0, left: 0, width: '6px', height: '6px', borderRadius: '50%', background: 'var(--text)', animation: 'neu-scale-engine 1.2s ease-in-out infinite alternate', animationDelay: '-0.4s' }} />
          <div style={{ position: 'absolute', bottom: 0, right: 0, width: '6px', height: '6px', borderRadius: '50%', background: 'var(--text)', animation: 'neu-scale-engine 1.2s ease-in-out infinite alternate', animationDelay: '-0.8s' }} />
        </div>
      ) : (
        /* JIKA CHAT NORMAL -> KELUARKAN DNA HELIX (2 TITIK SOLID HORIZONTAL) */
        <div style={{ position: 'relative', width: '48px', height: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <div style={{ position: 'absolute', width: '7.5px', height: '7.5px', borderRadius: '50%', background: 'var(--text)', animation: 'helix-h-1 1.3s ease-in-out infinite' }} />
          <div style={{ position: 'absolute', width: '7.5px', height: '7.5px', borderRadius: '50%', background: 'var(--text)', animation: 'helix-h-2 1.3s ease-in-out infinite' }} />
        </div>
      )}

      {/* CONTEXTUAL TEXT GENERATOR */}
      <span style={{ fontFamily: 'monospace', fontSize: '12px', letterSpacing: '0.02em', color: 'var(--text)', opacity: 0.5, fontWeight: 500 }}>
        {getStatusText()}
      </span>
    </div>
  );
...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  // State untuk mengunci string input terakhir milik user agar teks loading tidak hilang saat input dikosongkan
  ...
  const handleSendWrapper = (images?: string[], pdfs?: { data: string; name: string }[], directText?: string) => {
    ...
    // Kunci teks prompt user ke dalam state sebelum input text dibersihkan
    if (finalPrompt.trim() !== '') {
            ...
            {messages.map((msg, i) => {
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  {/* LOADING DENGAN MATRIX DYNAMIC ENGINE */}
                  {showLoadingHere && (
