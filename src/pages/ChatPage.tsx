...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  // ✅ STATE BUAT NYIMPEN PESAN YG LAGI DI-REPLY (SWIPE)
  ...
  // ✅ KETIKA MENGIRIM PESAN (LOGIKA POTONG TEKS REPLY ADA DI SINI)
  const handleSendWrapper = (images?: string[], pdfs?: { data: string; name: string }[], directText?: string) => {
    let finalPrompt = directText || inputText;

    // Kalau ada pesan reply, kasih konteks ke dalam prompt AI
    if (replyingToMsg && finalPrompt.trim() !== '') {
      // ✅ FIX POTONGAN TEKS: Ambil max 80 karakter pertama dari pesan AI biar chat gak bengkak
      const cuplikanPesan = replyingToMsg.content.length > 80 
        ? replyingToMsg.content.substring(0, 80) + "..." 
        : replyingToMsg.content;

      finalPrompt = `[Membalas pesan: "${cuplikanPesan}"]\n\n${finalPrompt}`;
      setReplyingToMsg(null); // Tutup box reply setelah kekirim
    }

    if (!directText) {
       setInputText(''); // Bersihin input text normal
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative bg-[var(--bg)]">
                  ...
                  <div id={`message-${i}`} className="w-full">
                    {isConnectionError ? (
                      ...
                      <ChatBubble
                        ...
                        onSwipeToReply={(messageToReply) => setReplyingToMsg(messageToReply)} // ✅ TANGKAP SWIPE DARI BUBBLE
                      />
                    )}
                  </div>
      ...
      <div className="absolute bottom-0 left-0 w-full z-[100] pointer-events-none">
        <ChatInput
          inputText={inputText} setInputText={setInputText}
          onSend={(imgs, pdfs) => handleSendWrapper(imgs, pdfs)} // ✅ Pake fungsi wrapper kita
          isSending={isSending} attachedImage={attachedImage} setAttachedImage={setAttachedImage}
          mode={mode} onModeChange={onModeChange}
          isCanvasActive={isCanvasActive} onOpenCanvas={onOpenCanvas} onCloseCanvas={onCloseCanvas}
          onUpgradeClick={onUpgradeClick}
          replyingTo={replyingToMsg} // ✅ Kirim pesan reply ke input
          onCancelReply={() => setReplyingToMsg(null)} // ✅ Fungsi batal reply
        />
      </div>
    </div>
  );
