...
/* ── Web Search Loading (Grok Style - TelierNews Integration) ── */
const WebSearchLoading = ({ query }: { query: string }) => {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%', marginTop: 8 }}>
      ...
      {/* 1. Loading Atas: Status Cylen */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ width: 14, height: 14, border: '2px solid var(--bd)', borderTopColor: 'var(--text)', borderRadius: '50%', animation: 'spinSlow 0.8s linear infinite' }} />
        <span style={{ fontSize: 13, color: 'var(--mu)', fontWeight: 500 }}>
          {query ? `Menganalisis: "${query}"...` : 'Mengumpulkan konteks...'}
        </span>
      </div>

      {/* 2. Loading Bawah: TelierNews Node */}
      <div style={{ 
        ...
        <div style={{ position: 'relative', width: 36, height: 36, flexShrink: 0 }}>
          {/* Logo TelierNews dari folder public */}
          <img 
            src="/IMG_20260425_143520.png" 
            alt="TelierNews" 
            style={{ width: '100%', height: '100%', objectFit: 'contain', animation: 'spinSlow 3s linear infinite' }}
          />
          {/* Efek Glow Futuristik */}
          <div style={{ position: 'absolute', inset: -5, background: 'var(--text)', opacity: 0.05, filter: 'blur(8px)', borderRadius: '50%', animation: 'pulseGlow 2s ease-in-out infinite' }} />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            {/* Indikator Online */}
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#10b981', animation: 'pulseGlow 1.5s infinite' }} />
            <span style={{ fontSize: 10, color: 'var(--mu)', textTransform: 'uppercase', letterSpacing: '0.05em', fontFamily: 'monospace' }}>
              Menarik Data Aktual...
            </span>
          </div>
        </div>
