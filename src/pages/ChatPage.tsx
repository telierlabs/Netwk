...
const NormalLoading = () => (
  <div className="flex items-center py-2">
    <style>{`
      @keyframes spin-minimal { 100% { transform: rotate(360deg); } }
    `}</style>
    <svg
      width="20" height="20" viewBox="0 0 24 24" fill="none"
      stroke="var(--text)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
      style={{ animation: 'spin-minimal 0.8s linear infinite', opacity: 0.7 }}
    >
...
/* ─── Ghost Mode Empty State ─── */
const GhostEmptyState = () => (
  <div className="flex flex-col items-center justify-center flex-1 gap-5 px-8 select-none pointer-events-none">
    <style>{`
    ...
    {/* Ghost icon besar */}
    <div style={{ animation: 'ghost-float 3s ease-in-out infinite, ghost-fade-in 0.5s ease-out both' }}>
      <svg
    ...
    <div
      ...
      style={{ animation: 'ghost-fade-in 0.5s 0.1s ease-out both', opacity: 0 }}
    >
      <span className="text-[18px] font-bold text-[var(--text)] opacity-60 tracking-tight">
        Mode Hantu Aktif
      </span>
      <span className="text-[14px] text-[var(--text)] opacity-35 leading-relaxed max-w-[260px]">
        Chat ini tidak akan disimpan ke riwayat. Semua percakapan hilang saat kamu keluar.
      </span>
    </div>

    {/* Badge kecil */}
    <div
      className="flex items-center gap-2 px-4 py-2 rounded-full border border-[var(--text)]/10 bg-[var(--text)]/5"
      style={{ animation: 'ghost-fade-in 0.5s 0.2s ease-out both', opacity: 0 }}
    >
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--text)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: 0.5 }}>
        <path d="M9 10h.01"/><path d="M15 10h.01"/>
        <path d="M12 2a8 8 0 0 0-8 8v12l3-3 2.5 2.5L12 19l2.5 2.5L17 19l3 3V10a8 8 0 0 0-8-8z"/>
      </svg>
      <span className="text-[12px] font-semibold text-[var(--text)] opacity-40">Tidak tersimpan</span>
    </div>
  </div>
);
...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  const checkScroll = () => {
    if (scrollContainerRef.current) {
      ...
      if (distanceToBottom > 150) setAutoScroll(false);
      else setAutoScroll(true);
    }
  };

  useEffect(() => {
    if (autoScroll && !isSending) {
  ...
  const handleSendWrapper = (images?: string[], pdfs?: { data: string; name: string }[], directText?: string) => {
    ...
    if (replyingToMsg && finalPrompt.trim() !== '') {
      ...
      const cuplikanPesan = rawText.length > 60 ? rawText.substring(0, 60) + '...' : rawText;
      finalPrompt = `[Membalas pesan: "${cuplikanPesan.replace(/"/g, "'")}"]\n\n${finalPrompt}`;
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative bg-[var(--bg)]">
      ...
      >
        {/* Empty state: ghost mode atau normal */}
                  ...
                  <div id={`message-${i}`} className="w-full">
                    {isConnectionError ? (
                      ...
                      <ChatBubble
                        ...
                        onSwipeToReply={(messageToReply) => setReplyingToMsg(messageToReply)}
                      />
                    )}
                  </div>
      ...
      <div className={`absolute bottom-40 right-6 md:right-10 z-[60] transition-all duration-400 ease-out ${
