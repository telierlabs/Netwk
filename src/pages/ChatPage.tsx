...
/* ─── GHOST EMPTY STATE — MUNCUL DI TENGAH LAYAR CHTPAGE ─── */
...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  const checkScroll = () => {
    if (scrollContainerRef.current) {
      ...
      // FIX: Only show button if there are messages AND scrolled up
      const hasContent = messages.length > 0 || isSending;
      setShowScrollButton(hasContent && distanceToBottom >= 200);
      setAutoScroll(distanceToBottom <= 150);
    }
  };

  useEffect(() => {
    if (autoScroll && !isSending) chatEndRef.current?.scrollIntoView({ behavior: 'auto' });
  }, [messages, isSending]);
  ...
  // Empty = belum ada pesan dari user sama sekali
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative bg-[var(--bg)]">
      ...
      >
        {isEmptyChat ? (
          /* JIKA MODE HANTU AKTIF = MUNCUL HANTU GEDE DI TENGAH. KALAU NGGAK = POLOS */
                ...
                <React.Fragment key={i}>
                  <div id={`message-${i}`} className="w-full">
                    {isConnectionError
                      ...
                      : <ChatBubble msg={msg} msgIndex={i} isLast={i===messages.length-1} suggestions={msg.suggestions} onSuggest={handleSuggest} onResend={(content)=>{setInputText(content);setTimeout(()=>handleSendWrapper(),0);}} onEdit={(content)=>setInputText(content)} onTogglePin={onTogglePin} onSaveItem={onSaveItem} onRegenerate={onRetry} onSwipeToReply={(m)=>setReplyingToMsg(m)} />
                    }
                  </div>
                  {showLoadingHere && (
                    <div className="w-full flex flex-col items-start -mt-4 mb-2">
                      {isSearching ? <WebSearchLoading active={isSearching} webCount={webCount} postCount={postCount} /> : <NormalLoading />}
                    </div>
                  )}
                </React.Fragment>
              );
