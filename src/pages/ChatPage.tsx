...
import { Pin, Search, Palette, FileText } from 'lucide-react'; // <-- Tambah Palette & FileText
...
interface ChatPageProps {
  ...
  webCount: number;
  postCount: number;
  activityStatus?: 'idle' | 'image' | 'pdf'; // <-- PROP BARU
  inputText: string;
...
// ── KOMPONEN BARU BUAT ANIMASI GAMBAR / PDF ──
const ActivityLoading = ({ status }: { status: 'image' | 'pdf' }) => {
  const isPdf = status === 'pdf';
  const Icon = isPdf ? FileText : Palette;
  const text = isPdf
    ? "Sedang menyusun dokumen PDF..."
    : "Sedang melukis gambarmu...";

  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 12, width: '100%', padding: '2px 0' }}>
      <style>{`
        @keyframes pulseIconGen { 0%, 100% { opacity: 0.45; transform: scale(0.95); } 50% { opacity: 1; transform: scale(1.05); } }
        @keyframes shimmerText { 0% { background-position: -200% 0; } 100% { background-position: 200% 0; } }
      `}</style>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ animation: 'pulseIconGen 1.5s ease-in-out infinite', display: 'flex', alignItems: 'center', flexShrink: 0, color: 'var(--text)' }}>
...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  activityStatus = 'idle', // <-- AMBIL STATUS DARI PROPS
  ...
  // Modifikasi: Loading tetap tampil kalau activityStatus lagi jalan, meskipun pesan udah masuk
            ...
            {messages.map((msg, i) => {
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  {/* LOGIKA BARU BUAT NAMPILIN ANIMASI BERDASARKAN STATUS */}
                  {showLoadingHere && (
                    <div className="w-full flex flex-col items-start -mt-4 mb-2">
                      {activityStatus !== 'idle'
                        ? <ActivityLoading status={activityStatus} />
                        : isSearching
