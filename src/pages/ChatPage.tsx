...
import { Pin, ChevronDown } from 'lucide-react'; // 🔥 Icon Panah
  ...
  return (
    <div className="w-full rounded-2xl border border-[var(--bd)] bg-[var(--cd)] p-5 flex flex-col gap-3">
      <div className="flex items-center gap-2">
        {isLimit ? <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="#ef4444" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg> : <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="var(--text)" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v4m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/></svg>}
      ...
      {!isLimit && onRetry && <button onClick={onRetry} className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-[var(--ac)] text-[var(--at)] rounded-xl font-semibold text-[15px] hover:opacity-90 active:scale-95 transition-all"><svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg> Retry</button>}
    </div>
  );
};
...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  isCanvasActive = false, onOpenCanvas, onCloseCanvas 
}) => {
  const chatEndRef = useRef<HTMLDivElement>(null);
  const scrollContainerRef = useRef<HTMLDivElement>(null); // 🔥 Wadah buat deteksi scroll
  
  const [showScrollButton, setShowScrollButton] = useState(false);

  // 🔥 Fungsi buat cek posisi: Kalau nggak di bawah, munculin tombol panah
  const checkScroll = () => {
    if (scrollContainerRef.current) {
      const { scrollHeight, scrollTop, clientHeight } = scrollContainerRef.current;
      // Toleransi 100px dari bawah
      const isBottom = scrollHeight - scrollTop - clientHeight < 100;
      setShowScrollButton(!isBottom);
    }
  };

  // Cek posisi tiap ada pesan baru atau status ngirim berubah
  useEffect(() => {
    checkScroll();
    // Kalau user lagi ngetik, auto scroll ke bawah aja
    if (isSending) {
  ...
  if (desktopWelcomeMode) {
    return (
      <div style={{ width: '100%' }}>
        <ChatInput inputText={inputText} setInputText={setInputText} onSend={(imgs, pdfs) => onSend(imgs, pdfs)} isSending={isSending} attachedImage={attachedImage} setAttachedImage={setAttachedImage} compact mode={mode} onModeChange={onModeChange} isCanvasActive={isCanvasActive} onOpenCanvas={onOpenCanvas} onCloseCanvas={onCloseCanvas} />
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative">
      ...
      {/* AREA CANVAS */}
      {isCanvasActive && (
        <div className="w-full h-[55vh] md:h-[65vh] flex-shrink-0 relative z-0 transition-all duration-500 bg-[var(--bg)]">
          <CylenCanvas />
        </div>
      )}

      {pinnedMessages.length > 0 && (
        <div className="px-4 pt-2 flex-shrink-0 relative z-10 bg-[var(--bg)]">
      ...
      <main 
        ...
        onScroll={checkScroll} // 🔥 Pantau terus pas user scroll
        className="flex-1 overflow-y-auto p-4 md:p-8 flex flex-col relative z-10 bg-[var(--bg)] scroll-smooth"
      >
      ...
      {/* 🔥 TOMBOL PANAH KE BAWAH (ChevronDown) */}
      <div 
        className={`absolute bottom-24 right-6 md:right-10 z-[60] transition-all duration-300 ${showScrollButton ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-90 pointer-events-none'}`}
      >
        <button 
          onClick={scrollToBottom}
          className="w-10 h-10 flex items-center justify-center bg-[var(--ac)] text-[var(--at)] rounded-full shadow-xl hover:scale-110 active:scale-95 transition-transform"
        >
      ...
      <div className="relative z-50">
        <ChatInput inputText={inputText} setInputText={setInputText} onSend={(imgs, pdfs) => onSend(imgs, pdfs)} isSending={isSending} attachedImage={attachedImage} setAttachedImage={setAttachedImage} mode={mode} onModeChange={onModeChange} isCanvasActive={isCanvasActive} onOpenCanvas={onOpenCanvas} onCloseCanvas={onCloseCanvas} />
      </div>
    </div>
  );
};
