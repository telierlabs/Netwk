...
// PERUBAHAN:
//   • Auto-scroll TIDAK aktif saat isStreaming (teks sedang muncul)
//   • Auto-scroll hanya aktif saat: kirim pesan baru, loading awal,
//     atau user sudah scroll ke bawah sendiri
//   • Saat streaming selesai (isSending jadi false), baru auto-scroll
//     ke bawah sekali
// ─────────────────────────────────────────────
...
const GhostEmptyState = () => (
  <div
    className="flex-1 flex flex-col items-center justify-center pointer-events-none select-none"
  ...
  >
    ...
    <div
      ...
      style={{ animation: 'ghost-in 0.4s 0.12s ease-out both', opacity: 0 }}
    >
      <span style={{ fontSize: 24, fontWeight: 700, color: 'var(--text)', opacity: 0.85, letterSpacing: '-0.01em' }}>
        Private Chat
      </span>
      <span style={{ fontSize: 14, color: 'var(--text)', opacity: 0.4, lineHeight: 1.5, maxWidth: 260 }}>
        Chat ini tidak akan muncul di riwayat dan akan dihapus sepenuhnya
      </span>
    </div>
  </div>
);
...
const EmptyState = () => (
  <div
    className="flex-1 flex flex-col items-center justify-center select-none pointer-events-none relative overflow-hidden"
  ...
  >
    ...
    <div
      ...
      style={{ animation: 'text-fade-in-premium 0.8s cubic-bezier(0.16, 1, 0.3, 1) both' }}
    >
      {/* Orbit particles — sama seperti sebelumnya */}
      ...
      {isSearching ? (
        <div
          style={{
            ...
            width: '20px',
            height: '20px',
            animation: 'neu-rot-engine 2.2s linear infinite',
            flexShrink: 0,
          }}
        >
...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  // ── Auto-scroll logic ──────────────────────
  // autoScrollEnabled: true hanya saat user kirim pesan baru (bukan saat streaming isi)
  ...
  const checkScroll = () => {
    ...
    // Jika user scroll ke bawah sendiri (<= 150px dari bawah), aktifkan auto-scroll
    if (distanceToBottom <= 150) autoScrollEnabled.current = true;
    else if (distanceToBottom > 300) autoScrollEnabled.current = false;
  };

  // Saat pesan baru dikirim (isSending baru jadi true), scroll ke bawah sekali
  useEffect(() => {
    if (isSending && !prevIsSending.current) {
      autoScrollEnabled.current = true;
      chatEndRef.current?.scrollIntoView({ behavior: 'smooth' });
    }
    // Saat selesai (isSending jadi false), scroll sekali ke bawah
    if (!isSending && prevIsSending.current) {
      setTimeout(() => {
        chatEndRef.current?.scrollIntoView({ behavior: 'smooth' });
      }, 100);
  ...
  // Jangan auto-scroll saat isi pesan streaming bertambah
  // (hanya scroll jika autoScrollEnabled masih true dari kirim pesan tadi)
  useEffect(() => {
    ...
    // Scroll hanya jika auto scroll masih aktif dan tidak sedang streaming
  ...
  const handleSendWrapper = (
    ...
    if (!directText) {
      setInputText('');
      onSend(images, pdfs, finalPrompt);
    } else {
      onSend(images, pdfs, finalPrompt);
    }
  };
  ...
  const lastAiMsgHasContent =
    lastMsg && lastMsg.role === 'assistant' && lastMsg.content && lastMsg.content.length > 0;
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative bg-[var(--bg)]">
          ...
          <div className="max-w-3xl mx-auto">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[var(--mu)] mb-0.5">
                  Pesan Disematkan
                </p>
                  ...
                  <div id={`message-${i}`} className="w-full">
                    {isConnectionError ? (
                      ...
                      <ChatBubble
                        ...
                        activityStatus={
                          isThisLastMessage && isSending ? activityStatus : 'idle'
                        }
                        suggestions={msg.suggestions}
                        onSuggest={handleSuggest}
                        onResend={(content) => {
                          setInputText(content);
                          setTimeout(() => handleSendWrapper(), 0);
                        }}
      ...
      <div
        className={`absolute bottom-40 right-6 md:right-10 z-[60] transition-all duration-400 ease-out ${
          showScrollButton
