...
/* ── Web Search loading — nampilin query nyata ── */
const WebSearchLoading = ({ query }: { query: string }) => {
  // Generate 3 baris sumber berdasarkan query
  const sources = [
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%' }}>
      ...
      {sources.map((row, i) => (
        <div key={i} style={{
          ...
          opacity: 0,
          animation: `wsAppear 0.35s ease forwards`,
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={{
              ...
              <span style={{
                ...
                background: 'linear-gradient(90deg, transparent 0%, var(--bg) 50%, transparent 100%)',
                animation: `wsShimmer 2s ease-in-out infinite`,
                animationDelay: `${i * 0.3}s`,
                pointerEvents: 'none',
              }} />
            </span>
            {/* garis pendek di bawah teks */}
            <div style={{
        ...
        <div className="max-w-3xl mx-auto flex flex-col gap-8">
          ...
          {isSending && (
            <div className="flex flex-col items-start w-full">
              {/* moon icon + neural pulse sejajar */}
              ...
              {/* web search loading di bawah */}
              {isSearching ? <WebSearchLoading query={searchQuery} /> : <NeuralPulse />}
            </div>
          )}
