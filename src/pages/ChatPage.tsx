...
// ── LOADING HELIX 2-TITIK — POLOS, TANPA TEKS ──
// Catatan: sebelumnya komponen ini juga menampilkan teks "Mencari..." dan
// jumlah hasil pencarian saat isSearching true. Teks tersebut sudah dihapus
// sesuai permintaan — sekarang cuma animasi 2 titik helix, tidak pernah
// menampilkan teks apapun. Source boxes (kotak kecil hasil pencarian) tetap
// dipertahankan karena itu elemen visual, bukan teks.
const CylenDynamicLoading: React.FC<{
  ...
  return (
    <div className="flex flex-col gap-2 py-3 pl-1 select-none pointer-events-none">
      ...
      {/* Baris 1: helix saja, tanpa teks apapun */}
      ...
      {/* Baris 2: source boxes muncul satu-satu (tetap dipertahankan, bukan teks) */}
...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  // 🔧 FIX: tinggi spacer bawah, dihitung dinamis tiap kali user kirim pesan
  // baru, supaya selalu ada "ruang kosong" yang cukup buat men-scroll pesan
  // user itu sampai ke bagian ATAS viewport (kayak ChatGPT/Claude), bukan
  // cuma sampai ke bawah container kalau kontennya belum cukup tinggi.
  ...
  useEffect(() => {
    // Begitu pesan baru mulai dikirim (isSending baru jadi true): scroll
    // pesan USER TERAKHIR ke bagian ATAS viewport (block: 'start'), bukan
    // scroll ke elemen paling bawah (chatEndRef). Itu yang sebelumnya bikin
    // layar kerasa "nyangkut" di bawah alih-alih pesan user naik ke atas
    // kayak di ChatGPT/Claude.
    if (isSending && !prevIsSending.current) {
      autoScrollEnabled.current = true;

      // Kasih spacer setinggi viewport container, biar walau balasan AI
      // belum selesai/belum setinggi itu, tetap ada ruang buat pesan user
      // beneran nyampe ke atas layar (bukan mentok di scrollHeight yang
      // masih pendek).
      const containerHeight = scrollContainerRef.current?.clientHeight || 0;
      setBottomSpacer(containerHeight);

      requestAnimationFrame(() => {
        const el = document.getElementById(`message-${lastUserIndex}`);
        el?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      });
    }

    if (!isSending && prevIsSending.current) {
      // Balasan AI selesai: matiin spacer pelan-pelan & biarin posisi scroll
      // apa adanya (gak dipaksa balik ke bawah), biar user yang udah baca
      // dari atas gak ke-drag paksa ke bawah.
      setTimeout(() => setBottomSpacer(0), 300);
    }

    prevIsSending.current = isSending;
  }, [isSending, lastUserIndex]);
      ...
      <main ref={scrollContainerRef} onScroll={checkScroll} className="flex-1 overflow-y-auto p-4 md:px-8 pt-6 flex flex-col relative z-10 scroll-smooth" style={{ scrollbarWidth: 'none' }}>
        {isEmptyChat ? (
          ...
          <div className="max-w-3xl mx-auto flex flex-col gap-10 w-full pb-[160px]">
            ...
            {/* 🔧 FIX: spacer dinamis — kasih "napas" ekstra di bawah pesan
                terakhir SAAT sedang mengirim, supaya browser punya cukup
                ruang scroll buat naikin pesan user ke bagian atas viewport.
                Tanpa ini, kalau balasan AI belum panjang, scrollIntoView
                'start' gak akan sanggup naik penuh karena scrollHeight
                total container masih lebih pendek dari yang dibutuhkan. */}
            <div style={{ height: bottomSpacer, flexShrink: 0, transition: 'height 0.25s ease' }} />
