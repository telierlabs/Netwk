...
export const ChatPage: React.FC<ChatPageProps> = ({
  ...
  const [autoScroll, setAutoScroll]             = useState(true); // ✅ State untuk pintar ngatur scroll

  // ✅ Deteksi jarak scroll user
  const checkScroll = () => {
    if (scrollContainerRef.current) {
      ...
      // ✅ Kalau user nge-scroll ke atas lebih dari 150px, matikan fitur paksa scroll!
  ...
  // ✅ Animasi scroll pas AI lagi ngetik tiap dapet teks baru
  useEffect(() => {
    if (autoScroll) {
      // Pake 'auto' bukan 'smooth' biar pas lagi asik streaming ngga macet-macet nyendat
      chatEndRef.current?.scrollIntoView({ behavior: 'auto' });
    }
  }, [messages]);

  // ✅ Paksa nendang ke bawah (smooth) TAPI HANYA saat user baru mencet tombol KIRIM
  useEffect(() => {
    if (isSending) {
      setAutoScroll(true);
  ...
  const scrollToBottom = () => {
    setAutoScroll(true); // ✅ Nyalain lagi auto scroll kalau klik tombol panah bawah
    chatEndRef.current?.scrollIntoView({ behavior: 'smooth' });
  };
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative bg-[var(--bg)]">
      ...
      <main
        ...
        onScroll={checkScroll} // ✅ Deteksi scroll jalan terus di sini
