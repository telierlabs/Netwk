...
// Hook: animasikan angka dari nilai lama ke nilai baru
...
// 2. Loading Web Search — rata kiri, counter naik, logo T slide in
const WebSearchLoading = ({ webCount, postCount }: { webCount: number; postCount: number }) => {
  ...
  // Baris 2 (Reading posts) muncul slide-in dari atas saat postCount > 0
  const [showPosts, setShowPosts] = useState(false);
  useEffect(() => {
    if (postCount > 0 && !showPosts) setShowPosts(true);
  }, [postCount]);
  ...
  return (
    <div style={{
      ...
      {/* Baris 2 — Reading posts, slide in dari atas saat muncul */}
      {showPosts && (
        <div style={{
          display: 'flex',
          alignItems: 'center',
          ...
          <div style={{ width: 15, height: 15, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.55 }}>
            <img
