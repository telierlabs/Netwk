  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative bg-[var(--bg)]">
      ...
      <main
        ...
        className="flex-1 overflow-y-auto p-4 md:px-8 pt-6 flex flex-col relative z-10 scroll-smooth transform-gpu"
            ...
            {messages.map((msg, i) => {
              ...
              return (
                <React.Fragment key={i}>
                  <div id={`message-${i}`} className="w-full" style={{ contentVisibility: 'auto', containIntrinsicSize: '1px 200px' }}>
                    {isConnectionError ? (
      ...
      {/*
        ✅ FIX UTAMA: Hapus pointer-events-none dari wrapper ini.
        Dulu: pointer-events-none → semua touch di dalam (picker, backdrop, input) diblokir di mobile.
        ChatInput punya pointer-events-auto di footer-nya, tapi picker popup (absolute bottom-full)
        dan backdrop (fixed inset-0) mewarisi pointer-events-none dari parent ini.
        Desktop tidak terpengaruh karena mouse event berbeda dengan touch event di Android.
      */}
