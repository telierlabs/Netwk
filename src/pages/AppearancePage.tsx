...
import { useProfile } from '../hooks/useProfile'; // <-- Panggil hook Profile
...
export const AppearancePage = ({ onBack, showToast }: any) => {
  ...
  // State lokal buat slider biar smooth (gak nunggu loading internet)
  const [localTextSize, setLocalTextSize] = useState<number>(profile.textSize || 16);
  ...
  // 1. Terapkan Tema (Sinkron ke Firestore)
  const applyTheme = async (id: string) => {
    // Terapin ke layar detik itu juga biar instan
  ...
  // 2. Terapkan Font Size (Sinkron ke Firestore pake jeda)
  useEffect(() => {
    document.documentElement.style.fontSize = `${(localTextSize / 16) * 100}%`;
    const timeoutId = setTimeout(() => {
      // Simpan ke Firestore cuma kalau slider udah berhenti di-drag selama 300ms
    ...
    <div className="flex flex-col h-full bg-[var(--bg)] overflow-y-auto pb-12" style={{ minHeight: 0, scrollbarWidth: 'none' }}>
      ...
      <div className="px-4 mt-6">
        <div className="flex gap-2 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
          {UI_THEMES.map((t) => {
            const isActive = profile.theme === t.id; // <-- Cek langsung dari profile
            const Icon = t.icon;
            return (
      ...
      <div className="px-4 mt-8 mb-8">
        ...
        <div className="flex flex-col gap-2">
          {FONTS.map(f => (
            <button key={f.id} onClick={() => applyFont(f.name)} className={cn("flex items-center gap-4 p-4 rounded-3xl transition-all text-left border", profile.font === f.name ? "bg-[var(--text)] text-[var(--bg)] border-transparent shadow-md" : "bg-[var(--sf)] text-[var(--text)] border-[var(--bd)]/20 hover:border-[var(--text)]/20")}>
              ...
              {profile.font === f.name && <Check size={18} />}
            </button>
          ))}
        </div>
      </div>
