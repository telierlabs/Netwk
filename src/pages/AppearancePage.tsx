...
export const AppearancePage = ({ onBack, showToast }: any) => {
  ...
  // State lokal buat slider biar licin
  const [localTextSize, setLocalTextSize] = useState<number>(16);

  // Sync state lokal dengan Firestore pas pertama load
  useEffect(() => {
    if (profile.textSize) {
  ...
  // ── 1. LOGIKA APPLY TEMA (DIPERBAIKI) ──
  const applyTheme = async (id: string) => {
    // A. Hitung variabel warna yang bener
    ...
    // B. Tembak langsung ke root document (HTML) biar se-aplikasi ganti warna
    const root = document.documentElement;
    Object.entries(varsToApply).forEach(([key, val]) => {
      root.style.setProperty(key, val);
    });
    
    // C. Timpa class di root, ini ngasih sinyal ke seluruh komponen (termasuk App.tsx)
    root.className = id === 'system' ? '' : id;
    
    // D. Ganti warna header meta Chrome/Safari di HP
    ...
    // F. Fallback buat komponen luar yang masih baca localStorage (kalau ada)
