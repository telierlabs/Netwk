...
export const HapticsPage: React.FC<HapticsPageProps> = ({ onBack }) => {
  ...
  // State lokal biar UI responnya instan
  const [haptics, setHaptics] = useState({ main: true, buttons: true, ai: true });

  // Tarik data dari Cloud
  useEffect(() => {
    if (profile.haptics) {
      setHaptics(profile.haptics);
    }
  }, [profile.haptics]);

  // Fungsi utilitas getar
  ...
  const updateHaptics = async (key: 'main' | 'buttons' | 'ai', value: boolean) => {
    const newHaptics = { ...haptics, [key]: value };
    setHaptics(newHaptics); // Update UI
    
    // Kalau dinyalain, kasih efek getar sedikit sebagai feedback
    if (value && navigator.vibrate) {
      triggerVibrate(30); 
    }

    await updateProfile({ haptics: newHaptics }); // Simpan ke Cloud
  };
  ...
  return (
    <main className="flex-1 overflow-y-auto bg-[var(--bg)] relative z-[100] flex flex-col min-h-screen">
      ...
      <div className="flex items-center gap-4 px-4 py-3 border-b border-[var(--bd)]/20 bg-[var(--bg)] sticky top-0 z-10">
        <button 
          onClick={() => {
            if (haptics.main && haptics.buttons) triggerVibrate(20);
            onBack();
          }} 
          className="p-2 -ml-2 rounded-full hover:bg-[var(--sf)] text-[var(--text)] transition-colors active:scale-95"
        >
      ...
      <div className="max-w-2xl w-full mx-auto pb-10 pt-4 px-4">
        ...
        <div className="bg-[var(--sf)] rounded-[24px] overflow-hidden border border-[var(--bd)]/50 mb-6">
          <div className="w-full flex items-center justify-between px-5 py-[18px]">
            ...
            <Toggle 
              checked={haptics.main} 
              onChange={() => updateHaptics('main', !haptics.main)} 
            />
          </div>
        </div>
          ...
          <div className="animate-in fade-in slide-in-from-top-2 duration-300">
            ...
            <div className="bg-[var(--sf)] rounded-[24px] flex flex-col overflow-hidden border border-[var(--bd)]/50">
              <div className="w-full flex items-center justify-between px-5 py-[18px] border-b border-[var(--bd)]">
                ...
                <Toggle 
                  checked={haptics.buttons} 
                  onChange={() => updateHaptics('buttons', !haptics.buttons)} 
                />
              </div>
              <div className="w-full flex items-center justify-between px-5 py-[18px]">
                ...
                <Toggle 
                  checked={haptics.ai} 
                  onChange={() => updateHaptics('ai', !haptics.ai)} 
                />
              </div>
            </div>
          </div>
